# TODO: Translation updated at 2024-03-16 21:54

translate de strings:

    # game/screens.rpy:134
    old "Yes"
    new "Ja"

    # game/screens.rpy:135
    old "No"
    new "Nein"

    # game/screens.rpy:167
    old "Return"
    new "Zurück"

    # game/screens.rpy:169
    old "History"
    new "Archiv"

    # game/screens.rpy:171
    old "Skip"
    new "Überspringen"

    # game/screens.rpy:173
    old "Auto"
    new "Auto"

    # game/screens.rpy:175
    old "Options"
    new "Optionen"

    # game/screens.rpy:177
    old "Accessibility"
    new "Barrierefreiheit"

    # game/screens.rpy:179
    old "Saves"
    new "Spielstände"

    # game/screens.rpy:182
    old "Mods"
    new "Modifikationen"

    # game/screens.rpy:184
    old "Main menu"
    new "Hauptmenü"

    # game/screens.rpy:187
    old "Quit"
    new "Beenden"

    # game/screens.rpy:189
    old "Playtime"
    new "Spielzeit"

    # game/screens.rpy:195
    old "Current scene"
    new "Aktuelle Szene"

    # game/screens.rpy:195
    old "Current track"
    new "Aktuelles Musikstück"

    # game/screens.rpy:210
    old "Start"
    new "Start"

    # game/screens.rpy:214
    old "Extras"
    new "Extras"

    # game/screens.rpy:259
    old "Adult content is disabled."
    new "Inhalte für Erwachsene sind deaktiviert."

    # game/screens.rpy:261
    old "Adult content is enabled."
    new "Inhalte für Erwachsene sind aktiviert."

    # game/screens.rpy:263
    old "Disable adult content"
    new "Nur jugendfreie Inhalte zeigen"

    # game/screens.rpy:266
    old "Fullscreen mode"
    new "Vollbildmodus"

    # game/screens.rpy:268
    old "Skip unread text"
    new "Ungelesene Texte überspringen"

    # game/screens.rpy:270
    old "Skip after choices"
    new "Überspringen nach Entscheidungen"

    # game/screens.rpy:273
    old "Parallax"
    new "Parallax"

    # game/screens.rpy:276
    old "Discord activity"
    new "Discord Aktivität"

    # game/screens.rpy:279
    old "Hardware cursor"
    new "Hardware-Cursor"

    # game/screens.rpy:286
    old "Blinking arrow"
    new "Blinkender Pfeil"

    # game/screens.rpy:296
    old "Text speed"
    new "Textgeschwindigkeit"

    # game/screens.rpy:305
    old "Auto mode delay"
    new "Auto-Modus Verzögerung"

    # game/screens.rpy:315
    old "Music volume"
    new "Musiklautstärke"

    # game/screens.rpy:325
    old "SFX volume"
    new "SFX Lautstärke"

    # game/screens.rpy:329
    old "Test"
    new "Test"

    # game/screens.rpy:333
    old "Language selection..."
    new "Sprachauswahl..."

    # game/screens.rpy:359
    old "Options > Language"
    new "Optionen > Sprache"

    # game/screens.rpy:370
    old "English"
    new "Englisch"

    # game/screens.rpy:371
    old "Russian"
    new "Russisch"

    # game/screens.rpy:372
    old "French"
    new "Französisch"

    # game/screens.rpy:373
    old "Spanish"
    new "Spanisch"

    # game/screens.rpy:374
    old "Deutsch"
    new "Deutsch"

    # game/screens.rpy:375
    old "Portuguese"
    new "Portugiesisch"

    # game/screens.rpy:376
    old "Japanese"
    new "Japanisch"

    # game/screens.rpy:439
    old "Are you sure you want to\nload this save?"
    new "Sind Sie sicher, dass Sie\ndiesen Spielstand laden möchten?"

    # game/screens.rpy:455
    old "Scene"
    new "Szene"

    # game/screens.rpy:467
    old "Are you sure you want to\ndelete this save?"
    new "Sind Sie sicher, dass Sie\ndiesen Spielstand löschen möchten?"

    # game/screens.rpy:478
    old "Save"
    new "Speichern"

    # game/screens.rpy:530
    old "Jukebox"
    new "Jukebox

    # game/screens.rpy:551
    old "Gallery"
    new "Galerie"

    # game/screens.rpy:572
    old "Library"
    new "Bücherei"

    # game/screens.rpy:593
    old "Cinema"
    new "Kino"

    # game/screens.rpy:647
    old "Extras > Jukebox"
    new "Extras > Jukebox"

    # game/screens.rpy:651
    old "Now playing"
    new "Gerade spielt"

    # game/screens.rpy:685
    old "Vol."
    new "Vol."

    # game/screens.rpy:687
    old "Stop"
    new "Stopp"

    # game/screens.rpy:715
    old "Extras > Gallery"
    new "Extras > Galerie"

    # game/screens.rpy:762
    old "Page:"
    new "Seite:"

    # game/screens.rpy:796
    old "Extras > Library"
    new "Extras > Bücherei"

    # game/screens.rpy:881
    old "Extras > Cinema"
    new "Extras > Kino"

    # game/screens.rpy:1024
    old "Fonts"
    new "Schriftarten"

    # game/screens.rpy:1031
    old "Default"
    new "Standard"

    # game/screens.rpy:1037
    old "High contrast"
    new "Hoher Kontrast"

    # game/screens.rpy:1040
    old "Self-voicing"
    new "Sprachausgabe"

    # game/screens.rpy:1047
    old "Enable"
    new "Aktivieren"

    # game/screens.rpy:1057
    old "Volume drop"
    new "Dialogverstärkung"

    # game/screens.rpy:1059
    old "Colorblind Filter"
    new "Filter für Farbenblinde"

    # game/screens.rpy:1153
    old "You must be 18+ to see adult content within this game.\nDo you meet these requirements?"
    new "Sie müssen mindestens 18 Jahre alt sein,\num Inhalte für Erwachsene in diesem Spiel zu sehen.\nErfüllen Sie diese Anforderungen?"

    # game/screens.rpy:1154
    old "Note: answering \"No\" does NOT prevent you from playing the game."
    new "Hinweis: Wenn Sie mit \"Nein\" antworten, werden Sie NICHT daran gehindert, das Spiel zu spielen."

    # game/screens.rpy:1155
    old "This question will not be shown again after you start playing."
    new "Diese Frage wird nach Spielbeginn nicht mehr angezeigt."

# TODO: Translation updated at 2024-12-18 04:28

translate de strings:

    # game/screens.rpy:376
    old "Simplified Chinese"
    new "Chinesisch (vereinfacht)"

    # game/screens.rpy:1061
    old "Mental health"
    new "Mentale Gesundheit"

    # game/screens.rpy:1068
    old "Skip hurtful adult scenes"
    new "Überspringe psychisch belastende Inhalte"

