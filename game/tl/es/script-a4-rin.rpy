#Adapted and fixed by Dr.Simp/Adaptado y corregido por Dr.Simp

# game/script-a4-rin.rpy:12
translate es a4_rin_illusions_for_people_7f31fb74:

    # "I make it in time for class, though not in time for breakfast."
    "Llego a clase a tiempo, aunque no a tiempo para el desayuno."

# game/script-a4-rin.rpy:14
translate es a4_rin_illusions_for_people_76acec4f:

    # "The classroom is bathing in the gentle light of the sun."
    "El salón de clases está bañado en la suave luz del sol."

# game/script-a4-rin.rpy:16
translate es a4_rin_illusions_for_people_84b35954:

    # "This means that it's going to be intolerably hot in the afternoon. For now, though, it's pleasant."
    "Esto significa que va a hacer un calor insoportable en la tarde. Sin embargo, por ahora, es agradable."

# game/script-a4-rin.rpy:18
translate es a4_rin_illusions_for_people_ef36b290:

    # "I look at Misha and Shizune's animated discussion about whatever, Hanako staring out of the classroom window, Mutou stumbling into the classroom four minutes late and with no recollection of what he's supposed to be teaching today."
    "Veo la animada discusión de Misha y Shizune sobre cualquier cosa, Hanako mirando fuera de la ventana del salón, Mutou tropezando al entrar cuatro minutos tarde y sin recordar qué se supone que debería enseñar hoy."

# game/script-a4-rin.rpy:20
translate es a4_rin_illusions_for_people_bb7f4611:

    # "I could never imagine dropping out of school just like that, even if it's only for a few weeks."
    "No podría imaginar abandonar la escuela así como así, aun si es solo por unas semanas."

# game/script-a4-rin.rpy:22
translate es a4_rin_illusions_for_people_bac92b0e:

    # "On the other hand, Rin doesn't seem to have a problem with the idea, or going through with it."
    "Por otro lado, Rin parece no tener problema con la idea, o seguirla."

# game/script-a4-rin.rpy:24
translate es a4_rin_illusions_for_people_5caa598f:

    # "Then again, somehow I got caught along in her insane isolation, even if we ended up hurting each other."
    "Pero si lo piensas, de algún modo me vi envuelto en su aislamiento insensato, aun si terminamos lastimándonos el uno al otro."

# game/script-a4-rin.rpy:26
translate es a4_rin_illusions_for_people_9a496fce:

    # "Or did we? Maybe only I got hurt."
    "¿O acaso fue así? Tal vez solo yo fui lastimado."

# game/script-a4-rin.rpy:31
translate es a4_rin_illusions_for_people_3a907645:

    # "It takes me until late in the afternoon to realize that today is Monday. The art club meets today."
    "Me toma hasta bien entrada la tarde para darme cuenta de que hoy es lunes. El club de arte se reúne este día."

# game/script-a4-rin.rpy:33
translate es a4_rin_illusions_for_people_a416797a:

    # "Not just that. Due to the exams, this will be the last art club meeting before summer vacation."
    "No solo eso. Debido a los exámenes, esta será la última reunión del club antes de las vacaciones de verano."

# game/script-a4-rin.rpy:36
translate es a4_rin_illusions_for_people_c7a2ba03:

    # "I have no real business going there…"
    "En realidad no tengo ningún propósito de ir…"

# game/script-a4-rin.rpy:38
translate es a4_rin_illusions_for_people_692d23cb:

    # "But I want to talk with the teacher."
    "Pero quiero hablar con el maestro."

# game/script-a4-rin.rpy:43
translate es a4_rin_illusions_for_people_e0072604:

    # "So, I end up loitering awkwardly in front of the art room, waiting for the meeting to end."
    "Por tanto, termino holgazaneando como un tipo raro frente al salón de arte, esperando a que la reunión termine."

# game/script-a4-rin.rpy:45
translate es a4_rin_illusions_for_people_cd742d22:

    # no "That's it for this trimester, everyone!"
    no "¡Eso es todo por el trimestre, muchachos!"

# game/script-a4-rin.rpy:47
translate es a4_rin_illusions_for_people_834b7f71:

    # "His voice is loud enough to be heard through the door and way too enthusiastic for it to be genuine."
    "Su voz suena con fuerza suficiente para ser escuchada a través de la puerta y con demasiado entusiasmo para ser genuina."

# game/script-a4-rin.rpy:49
translate es a4_rin_illusions_for_people_2e218c04:

    # no "The next meeting is after summer vacations, on the Monday of the first week of next term."
    no "La siguiente reunión será después de las vacaciones de verano, el lunes de la primera semana del siguiente trimestre."

# game/script-a4-rin.rpy:51
translate es a4_rin_illusions_for_people_931a477c:

    # no "I hope to see everyone there again!"
    no "¡Espero volver a verlos a todos entonces!"

# game/script-a4-rin.rpy:53
translate es a4_rin_illusions_for_people_2d4e2ee4:

    # no "Have a nice vacation!"
    no "¡Que tengan buenas vacaciones!"

# game/script-a4-rin.rpy:61
translate es a4_rin_illusions_for_people_65ba730a:

    # "There's a confused answering chorus of voices, and the door to the classroom opens, releasing a flow of students."
    "Hay un coro de voces confuso como respuesta, y la puerta al salón se abre, dejando salir un flujo de estudiantes."

# game/script-a4-rin.rpy:63
translate es a4_rin_illusions_for_people_4ee3fa5a:

    # "I wait for everyone else to leave, so that I can talk to Nomiya alone. It's almost dinnertime, so I don't have to wait too long."
    "Espero a que todos los demás se vayan, para poder hablar con Nomiya a solas. Casi es la hora de la cena, así que no tengo que esperar mucho."

# game/script-a4-rin.rpy:70
translate es a4_rin_illusions_for_people_ad927562:

    # "Without Rin, it feels pretty pointless to go there, but I want to talk with the teacher."
    "Sin Rin se siente como si no tuviera sentido ir ahí, pero quiero hablar con el maestro."

# game/script-a4-rin.rpy:75
translate es a4_rin_illusions_for_people_be2393a2:

    # "The meeting itself isn't noteworthy, just as my skills with water colors are not worth mentioning."
    "La reunión en sí no es digna de mencionar, al igual que mi habilidad con la acuarela no es digna de mencionarse."

# game/script-a4-rin.rpy:77
translate es a4_rin_illusions_for_people_4fe8a008:

    # "Nomiya tries to encourage and advise me without sounding too condescending, but he's not doing a very good job of it."
    "Nomiya trata de alentarme y aconsejarme sin sonar demasiado condescendiente, pero no está haciendo un trabajo muy bueno en ello."

# game/script-a4-rin.rpy:79
translate es a4_rin_illusions_for_people_46374257:

    # "If nothing else, joining the art club has taught me that I like art. It would be nice if I could actually try and make some art in the art club, though."
    "Si nada más, unirme al club de arte me ha enseñado que me gusta el arte. Aunque sería agradable si pudiera realmente intentar hacer un poco de arte en el club."

# game/script-a4-rin.rpy:81
translate es a4_rin_illusions_for_people_f0c94bcf:

    # "After the fruits of everyone's labor have been piled into a neat stack on the teacher's desk, he clears his throat to give a little speech."
    "Después de que los frutos de la labor de todos han sido rejuntados en una pila ordenada sobre el escritorio del maestro, él aclara la garganta para dar un pequeño discurso."

# game/script-a4-rin.rpy:86
translate es a4_rin_illusions_for_people_cd742d22_1:

    # no "That's it for this trimester, everyone!"
    no "¡Eso es todo por este trimestre, muchachos!"

# game/script-a4-rin.rpy:88
translate es a4_rin_illusions_for_people_ad24b41e:

    # "His voice is pretty loud and way too enthusiastic for it to be genuine."
    "Su voz suena bastante fuerte y con demasiado entusiasmo para ser genuina."

# game/script-a4-rin.rpy:93
translate es a4_rin_illusions_for_people_2e218c04_1:

    # no "The next meeting is after summer vacations, on the Monday of the first week of next term."
    no "La siguiente reunión será después de las vacaciones, el lunes de la primera semana del siguiente trimestre."

# game/script-a4-rin.rpy:95
translate es a4_rin_illusions_for_people_931a477c_1:

    # no "I hope to see everyone there again!"
    no "¡Espero volver a ver a todos entonces!"

# game/script-a4-rin.rpy:100
translate es a4_rin_illusions_for_people_2d4e2ee4_1:

    # no "Have a nice vacation!"
    no "¡Que tengan buenas vacaciones!"

# game/script-a4-rin.rpy:107
translate es a4_rin_illusions_for_people_8ad95814:

    # "Everyone wishes him a nice vacation back as they file out the door."
    "Todos le desean de vuelta unas buenas vacaciones mientras salen en fila por la puerta."

# game/script-a4-rin.rpy:109
translate es a4_rin_illusions_for_people_6f112a37:

    # "I stay behind, waiting until the two of us are alone. It's almost dinnertime, so I don't have to wait long."
    "Yo me quedo atrás, esperando hasta que los dos nos quedamos solos. Es casi la hora de la cena, así que no tengo que esperar mucho."

# game/script-a4-rin.rpy:111
translate es a4_rin_illusions_for_people_980d9bf6:

    # "Nomiya is looking through the paintings, some of which are actually pretty nice."
    "Nomiya está viendo las pinturas, algunas de las cuales son bastante buenas, de hecho."

# game/script-a4-rin.rpy:113
translate es a4_rin_illusions_for_people_fb54616f:

    # "Rin might outclass everyone else in the art club, but she isn't the only one with talent."
    "Puede que Rin supere a todos los demás en el club de arte, pero no es la única con talento."

# game/script-a4-rin.rpy:115
translate es a4_rin_illusions_for_people_54219272:

    # hi "Excuse me, teacher…"
    hi "Disculpe, maestro…"

# game/script-a4-rin.rpy:122
translate es a4_rin_illusions_for_people_54034757:

    # no "Hmm? What is it, Nakai?"
    no "¿Hmm? ¿Qué sucede, Nakai?"

# game/script-a4-rin.rpy:124
translate es a4_rin_illusions_for_people_3aab9384:

    # "He raises his eyebrows questioningly, smiling widely."
    "Levanta sus cejas a modo de pregunta, sonriendo ampliamente."

# game/script-a4-rin.rpy:126
translate es a4_rin_illusions_for_people_31f4c323:

    # hi "It's about Rin…"
    hi "Es sobre Rin…"

# game/script-a4-rin.rpy:131
translate es a4_rin_illusions_for_people_36329094:

    # no "Oh? Is something wrong with Tezuka?"
    no "¿Oh? ¿Hay algo malo con Tezuka?"

# game/script-a4-rin.rpy:133
translate es a4_rin_illusions_for_people_fc0a34fe:

    # hi "No, but…"
    hi "No, pero…"

# game/script-a4-rin.rpy:135
translate es a4_rin_illusions_for_people_d38f6202:

    # "I hesitate for a split second, not certain how to say what I want to say, giving Nomiya enough time to start blabbering by himself."
    "Dudo por una fracción de segundo, inseguro de cómo decir lo que quiero decir, dándole suficiente tiempo a Nomiya de empezar a parlotear por cuenta propia."

# game/script-a4-rin.rpy:140
translate es a4_rin_illusions_for_people_e4bf3b74:

    # no "I saw her a few days ago when I was passing by at Sae's gallery."
    no "La vi hace unos días cuando estaba pasando por la galería de Sae."

# game/script-a4-rin.rpy:142
translate es a4_rin_illusions_for_people_337601cf:

    # no "She said she'd get one or two more paintings done for the exhibition."
    no "Ella dijo que tendría hechas una o dos pinturas más para la exhibición."

# game/script-a4-rin.rpy:147
translate es a4_rin_illusions_for_people_8b9867f4:

    # no "I was quite pleased, she's a surprisingly hard worker. I'd always thought she was a bit lazy, doing what she wants instead of the assignments…"
    no "Estaba muy complacido, ella es una chica que trabaja sorprendentemente duro. Siempre había creído que era un poco floja, haciendo lo que quisiera en lugar de sus tareas…"

# game/script-a4-rin.rpy:149
translate es a4_rin_illusions_for_people_9f26bea0:

    # "He seems to notice my anxiety and realizes he is digressing, shutting up before finishing the thought."
    "Él parece notar mi ansiedad y se percata de que está divagando, guardando silencio antes de terminar su idea."

# game/script-a4-rin.rpy:154
translate es a4_rin_illusions_for_people_6257930d:

    # no "Ah, but you had something to talk about. What is it?"
    no "Ah, pero si tenías algo de qué hablar. ¿De qué se trata?"

# game/script-a4-rin.rpy:156
translate es a4_rin_illusions_for_people_38c55f9b:

    # hi "I don't know… she feels detached from everything, as if she can't think of anything but the exhibition."
    hi "No lo sé… ella se siente desconectada de todo, como si no pudiera pensar en nada más que la exhibición."

# game/script-a4-rin.rpy:161
translate es a4_rin_illusions_for_people_9f0b7322:

    # no "Well, isn't that good? She is focused on her painting, as she should be."
    no "Bueno, ¿no es eso bueno? Está concentrada en sus pinturas, como debería ser."

# game/script-a4-rin.rpy:163
translate es a4_rin_illusions_for_people_6a3888e3:

    # hi "Yeah, but this is different. It's like she's obsessed. I went to see her, and…"
    hi "Sí, pero esto es diferente. Es como si estuviese obsesionada. Fui a verla, y…"

# game/script-a4-rin.rpy:168
translate es a4_rin_illusions_for_people_ced88658:

    # no "Have you been bothering her?"
    no "¿La has estado molestando?"

# game/script-a4-rin.rpy:170
translate es a4_rin_illusions_for_people_58bc905f:

    # "He cuts in before I finish saying what I meant to say, instantly looking quite irritated."
    "Me interrumpe antes de que pueda terminar lo que quería decir, súbitamente viéndose muy irritado."

# game/script-a4-rin.rpy:172
translate es a4_rin_illusions_for_people_76d971b7:

    # hi "No… I don't… think so."
    hi "No… yo… no creo."

# game/script-a4-rin.rpy:174
translate es a4_rin_illusions_for_people_c9025062:

    # hi "I'm just concerned because she's stopped coming to school completely. She feels strange, too."
    hi "Solo estoy preocupado porque ha dejado de venir a la escuela por completo. Ella se siente extraña, también."

# game/script-a4-rin.rpy:176
translate es a4_rin_illusions_for_people_ea4d8105:

    # hi "Stranger than usual, at the very least."
    hi "Más extraña de lo usual, por lo menos."

# game/script-a4-rin.rpy:181
translate es a4_rin_illusions_for_people_2be6fe65:

    # no "Humbug! This is much more important for her than some lousy math class, or physics, or whatever."
    no "¡Disparates! Esto es mucho más importante para ella que una inmunda clase de Matemáticas, o Física, o lo que sea."

# game/script-a4-rin.rpy:183
translate es a4_rin_illusions_for_people_49df6569:

    # no "This is exactly why this school is so flexible, to give every student a chance to fulfill themselves."
    no "Esta es la razón exacta del porqué esta escuela es tan flexible, para darle al estudiante toda oportunidad de realizarse a sí mismo."

# game/script-a4-rin.rpy:188
translate es a4_rin_illusions_for_people_07931ba5:

    # no "Tezuka is a painter, so she should paint, no? And have an exhibition. That's what artists do. She should be allowed to concentrate on that, not these other frivolous classes. She should be encouraged."
    no "Tezuka es una pintora, así que debería pintar, ¿no? Y tener una exhibición. Eso es lo que los artistas hacen. A ella se le debería de permitir concentrarse en eso, no estas otras clases frívolas. Ella debería de ser alentada."

# game/script-a4-rin.rpy:190
translate es a4_rin_illusions_for_people_1f5efcfb:

    # no "If you think about it, it's really quite obvious."
    no "Si piensas en ello, es realmente bastante obvio."

# game/script-a4-rin.rpy:192
translate es a4_rin_illusions_for_people_46db08e4:

    # "His counterarguments are not very convincing, but I'm having a hard time trying to make any kind of rebuttal."
    "Sus contestaciones no son muy convincentes, pero estoy teniendo dificultades tratando de pensar en algún tipo de réplica."

# game/script-a4-rin.rpy:194
translate es a4_rin_illusions_for_people_3c6f0e91:

    # "My grudging silence is interpreted as assent, and Nomiya turns to shuffle the stack of turned-in assignments on his desk like a deck of cards."
    "Mi resentido silencio es interpretado como consentimiento, y Nomiya vuelve a barajar la pila de tareas entregadas sobre su escritorio como si se tratara de un mazo de cartas."

# game/script-a4-rin.rpy:199
translate es a4_rin_illusions_for_people_4fd197c4:

    # no "I have to say, while we're talking about Tezuka's exhibition…"
    no "Tengo que decir, ya que estamos hablando de la exhibición de Tezuka…"

# game/script-a4-rin.rpy:201
translate es a4_rin_illusions_for_people_7f74f1ac:

    # no "I'm very excited to see how it turns out."
    no "Estoy muy emocionado por ver cómo resultará."

# game/script-a4-rin.rpy:206
translate es a4_rin_illusions_for_people_d0e69e36:

    # no "She's still so young, yet has such wonderful skill, and style!"
    no "Ella sigue siendo tan joven, y aun así tiene una habilidad maravillosa, ¡y estilo!"

# game/script-a4-rin.rpy:208
translate es a4_rin_illusions_for_people_95a73d61:

    # "He's talking to the air, to relax the mood that got a bit too negative."
    "Le habla al aire, para relajar el ambiente que se tornó tan negativo."

# game/script-a4-rin.rpy:213
translate es a4_rin_illusions_for_people_6bbc934c:

    # no "I take it that you will be attending?"
    no "¿Me imagino que asistirás?"

# game/script-a4-rin.rpy:215
translate es a4_rin_illusions_for_people_028d71b4:

    # hi "Yeah, I guess so."
    hi "Sí, supongo que sí."

# game/script-a4-rin.rpy:220
translate es a4_rin_illusions_for_people_5e0f21ac:

    # no "Well, we'll meet there next."
    no "Bien, a la próxima nos veremos allá."

# game/script-a4-rin.rpy:227
translate es a4_rin_illusions_for_people_d991f2c6:

    # "I take that as my cue to leave. And I do, although I'm not happy about it."
    "Considero eso como mi oportunidad de irme. Y la tomo, aunque no estoy contento de ello."

# game/script-a4-rin.rpy:229
translate es a4_rin_illusions_for_people_ca80d7e4:

    # "My message didn't get through, to say the least."
    "Mi mensaje no llegó, por no decir más."

# game/script-a4-rin.rpy:240
translate es a4_rin_demused_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:249
translate es a4_rin_demused_ff53f23f:

    # n "{vspace=270}The day after that, all the missed opportunities and things I should have said come crashing down on me. There's nothing left to do afterwards but brood."
    n "{vspace=270}Al siguiente día, todas las oportunidades perdidas y cosas que debería de haber dicho me caen encima. No resta nada por hacer después de ello, además de lamentarse."

# game/script-a4-rin.rpy:251
translate es a4_rin_demused_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:258
translate es a4_rin_demused_9afb296f:

    # n "{vspace=270}Second day. I begin to feel anxious. I start doubting my doubt and it feels stupid, especially since I still can't think about anything else than Rin."
    n "{vspace=270}Segundo día. Empiezo a sentirme con ansias. Comienzo a dudar de mis dudas y se siente estúpido, especialmente porque no puedo pensar en nada más que en Rin."

# game/script-a4-rin.rpy:260
translate es a4_rin_demused_d41d8cd9_2:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:267
translate es a4_rin_demused_e59a7803:

    # n "{vspace=270}Third day. Japanese exam, {b}and{/b} world history exam. Great. The thing I hate most about her is that she can make me feel this awful even though I should be focusing on entirely different stuff right now."
    n "{vspace=270}Tercer día, examen de japonés, {b}y{/b} examen de Historia Universal. Genial. Lo que más odio de ella es que puede hacerme sentir así de horrible aunque debiera de concentrarme en cosas completamente diferentes en este momento."

# game/script-a4-rin.rpy:269
translate es a4_rin_demused_d41d8cd9_3:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:276
translate es a4_rin_demused_7bd0d1d0:

    # n "{vspace=270}Fourth day. Math exam. We have a math exam. It goes how it goes. I don't care."
    n "{vspace=270}Cuarto día. Examen de Matemáticas. Tenemos un examen de Matemáticas. Va como va. No me importa."

# game/script-a4-rin.rpy:278
translate es a4_rin_demused_d41d8cd9_4:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:285
translate es a4_rin_demused_c189afa1:

    # n "{vspace=270}Fifth day. Nomiya asks me again if I will attend the exhibition opening. I can't say no to him even though I seriously want to. I just don't want to discuss with him anything Rin-related so it's just better to take the path of least resistance."
    n "{vspace=270}Quinto día. Nomiya me pregunta de nuevo si asistiré a la apertura de la exhibición. No puedo decirle que no aunque realmente quiera hacerlo. Simplemente no quiero discutir con él nada relacionado con Rin así que es mejor tomar el camino menos difícil."

# game/script-a4-rin.rpy:287
translate es a4_rin_demused_d41d8cd9_5:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:296
translate es a4_rin_demused_784a49fa:

    # "On the sixth day, the day before the exhibition opening, I find Rin standing in the hallway in front of my room when I return to the dorms after dinner."
    "En el sexto día, el día antes de la apertura de la exhibición, encuentro a Rin de pie en el pasillo frente a mi habitación cuando regreso a los dormitorios después de cenar."

# game/script-a4-rin.rpy:300
translate es a4_rin_demused_31192155:

    # hi "What are you doing here?"
    hi "¿Qué estás haciendo aquí?"

# game/script-a4-rin.rpy:302
translate es a4_rin_demused_6b0080ca:

    # "My tone is angrier than I intended. I'm a little disappointed that I was unable to restrain myself, but it can't be helped."
    "Mi voz suena más enojada de lo que quería. Estoy un poco decepcionado de no haber podido contenerme a mí mismo, pero no hay más remedio."

# game/script-a4-rin.rpy:304
translate es a4_rin_demused_7b1bc929:

    # "Rin just stands there, like she just happened to coincidentally be standing around here where she has no business being. The way she reacts so coolly to everything annoys me now."
    "Rin solo está ahí de pie, como si acabara de pararse por mera coincidencia donde no tiene nada que ver. La manera tan fría en que reacciona a todo me molesta."

# game/script-a4-rin.rpy:306
translate es a4_rin_demused_76bb851c:

    # "This is not good. It's been six days, and the sight of her has me boiling. She hasn't even opened her mouth yet."
    "Esto no está bien. Han sido seis días, y el solo verla me hace arder. Ni siquiera ha abierto la boca todavía."

# game/script-a4-rin.rpy:311
translate es a4_rin_demused_574f5625:

    # rin "Finished painting."
    rin "Terminé de pintar."

# game/script-a4-rin.rpy:313
translate es a4_rin_demused_0339551f:

    # hi "Shouldn't you be at the gallery? Preparing?"
    hi "¿No deberías de estar en la galería? ¿Preparándote?"

# game/script-a4-rin.rpy:318
translate es a4_rin_demused_32e621dd:

    # rin "They said no."
    rin "Dijeron que no."

# game/script-a4-rin.rpy:320
translate es a4_rin_demused_59f480b4:

    # "I guess the gallery owner does that part then, getting the paintings framed, hung on the walls and whatnot."
    "Entonces supongo que la galerista hace esa parte, hacer enmarcar las pinturas, colgarlas en las paredes y demás."

# game/script-a4-rin.rpy:322
translate es a4_rin_demused_72b47445:

    # hi "So, why are you here?"
    hi "Así que, ¿por qué estás aquí?"

# game/script-a4-rin.rpy:327
translate es a4_rin_demused_94a52e1f:

    # rin "Felt like it."
    rin "Se me ocurrió."

# game/script-a4-rin.rpy:329
translate es a4_rin_demused_6bdaaf03:

    # "This same old stupid pattern emerges again; me asking her questions to which she replies with answers that don't answer anything, because it's the only other way we can converse."
    "Este mismo patrón estúpido emerge de nuevo; yo haciéndole preguntas a las que ella contesta con algo que no responde nada, porque es la única manera en la que podemos conversar."

# game/script-a4-rin.rpy:331
translate es a4_rin_demused_663541e5:

    # "Apart from me listening to her blabbering about whatever, which isn't really a conversation."
    "Aparte de escuchar su balbuceo sobre cualquier cosa, que realmente no es una conversación."

# game/script-a4-rin.rpy:333
translate es a4_rin_demused_794ad679:

    # "Is this a play? Are there some unseen roles that we have unknowingly set ourselves into, dictating the rules of engagement whenever we see each other, inevitably leading to us hurting each other?"
    "¿Es esto una representación teatral? ¿Acaso hay roles invisibles en los que sin darnos cuenta nos hemos convertido, dictando las reglas del juego cada vez que nos vemos, inevitablemente llevándonos a lastimarnos el uno al otro?"

# game/script-a4-rin.rpy:335
translate es a4_rin_demused_d4744e37:

    # "Her nonchalant answers accompanied by even more nonchalant shrugs leave me none the wiser. I guess I should be happy that the exhibition preparations are complete."
    "Sus respuestas indiferentes acompañadas por ese encogerse de hombros de manera todavía más indiferente me dejan confundido. Supongo que debería de estar contento de que las preparaciones para la exhibición están completas."

# game/script-a4-rin.rpy:342
translate es a4_rin_demused_eb7842ab:

    # "When I walk into my room, I hear her footsteps following me in."
    "Cuando entro a mi cuarto, escucho sus pisadas seguirme hacia dentro."

# game/script-a4-rin.rpy:344
translate es a4_rin_demused_e50c3695:

    # "I didn't invite her in. I won't ask her to leave."
    "No la invité a entrar. No le pediré que se vaya."

# game/script-a4-rin.rpy:361
translate es a4_rin_demused_51c8ea4c:

    # "She claims my bed without asking permission, making me wish that I had taken the time to make it before I left in the morning, then stands up again as though she sat on hot coals."
    "Reclama mi cama sin pedirme permiso, haciéndome desear que haya tenido el tiempo de tenderla antes de salir esta mañana, luego se pone de pie otra vez como si se hubiera sentado en brasas ardientes."

# game/script-a4-rin.rpy:365
translate es a4_rin_demused_76d044ed:

    # "I half-lean against the single corner of my desktop that isn't cluttered with stuff, to rest my legs at least a little bit."
    "Me medio recargo sobre el único rincón de mi escritorio que no está lleno de cosas amontonadas, para descansar mis piernas por lo menos un poco."

# game/script-a4-rin.rpy:370
translate es a4_rin_demused_9fc28e0d:

    # "Rin spends a few moments glancing curiously around my room. It makes me realize that she's never seen it before."
    "Rin pasa unos momentos echando un vistazo con curiosidad por mi habitación. Hace que me dé cuenta de que nunca ha estado aquí antes."

# game/script-a4-rin.rpy:372
translate es a4_rin_demused_ad9059a3:

    # "For a moment, she actually looks like she's concentrating. Trying to get everything. This must be the eye for detail that makes her an artist."
    "Por un momento, ella realmente parece estarse concentrando. Tratando de absorberlo todo. Este debe ser el ojo para el detalle que la hace una artista."

# game/script-a4-rin.rpy:377
translate es a4_rin_demused_15d65c2d:

    # "Since the room is small, she quickly runs out of things to look at, but nothing else transpires, allowing the uncomfortable silence to take over the atmosphere."
    "Ya que la habitación es pequeña, ella pronto se queda sin cosas que ver, pero nada más ocurre, permitiendo al silencio incómodo apoderarse del ambiente."

# game/script-a4-rin.rpy:379
translate es a4_rin_demused_449f3709:

    # "The mood is chilly to say the least, and both of us are on guard, waiting for the other to make the first move."
    "El humor es frío por no decir más, y ambos estamos en guardia, esperando a que el otro haga el primer movimiento."

# game/script-a4-rin.rpy:381
translate es a4_rin_demused_88ca76fd:

    # "Of course, Rin could play this game forever. So it has to be me."
    "Por supuesto, Rin podría jugar este juego por siempre. Así que tengo que ser yo."

# game/script-a4-rin.rpy:383
translate es a4_rin_demused_c85221a4:

    # hi "So…"
    hi "Y bien…"

# game/script-a4-rin.rpy:385
translate es a4_rin_demused_2a535c94:

    # "I give up because she'd never try to open conversation, and because it seems that she wants to say something, and I want to get it over with."
    "Me doy por vencido porque ella nunca trataría de iniciar una conversación, y porque parece que ella quiere hacer algo, y quiero terminar con esto."

# game/script-a4-rin.rpy:387
translate es a4_rin_demused_d7cffadc:

    # "Why else would she be here if she didn't want to talk?"
    "¿Por qué estaría aquí si no es porque quiere hablar?"

# game/script-a4-rin.rpy:389
translate es a4_rin_demused_14925818:

    # "I don't know what to say myself. I want to be angry, but I can't bring myself to yell at her or anything."
    "Yo no sé qué decir. Quiero estar enojado, pero no me atrevo a gritarle o algo parecido."

# game/script-a4-rin.rpy:391
translate es a4_rin_demused_b908b896:

    # "My voice catches her attention, and she tries to search for words as well, but it seems that she is not entirely certain as to why she's here either."
    "Mi voz capta su atención, y ella también trata de buscar palabras que decir, pero parece que tampoco está del todo segura del porqué está aquí."

# game/script-a4-rin.rpy:401
translate es a4_rin_demused_e3b745d1:

    # "And so, Rin simply takes a few steps to close the distance between us and rises on the tips of her toes to even out the height difference…"
    "Y así, Rin simplemente toma unos cuantos pasos para cerrar la distancia entre nosotros y se para de puntillas para igualar la diferencia de altura."

# game/script-a4-rin.rpy:406
translate es a4_rin_demused_4cf20e4a:

    # centered "'It was a bad idea.'"
    centered "“Fue una mala idea.”"

# game/script-a4-rin.rpy:408
translate es a4_rin_demused_b2968db4:

    # centered "'Maybe you should forget about it, and I will too.'"
    centered "“Quizás deberías olvidarlo, y yo lo haré también.”"

# game/script-a4-rin.rpy:410
translate es a4_rin_demused_90729675:

    # "It's a reflex, and almost as an afterthought, the words 'no,' 'yes' and 'maybe' simultaneously surface inside my mind."
    "Es un reflejo, y casi como una idea tardía, las palabras “no,” “sí” y “tal vez” emergen al mismo tiempo dentro de mi mente."

# game/script-a4-rin.rpy:412
translate es a4_rin_demused_d38d5f1d:

    # "My hand is between her lips and mine, a wall that I raised to guard against… something."
    "Mi mano se encuentra entre sus labios y mis labios, un muro que he levantado para protegerme de… algo."

# game/script-a4-rin.rpy:414
translate es a4_rin_demused_2bc22592:

    # "Her breath feels warm against my fingers. The scent of her skin lingers about, the mysterious indescribable sensation that captures me and draws my eyes deep into hers."
    "Su aliento se siente cálido en mis dedos. El aroma de su piel permanece en el aire, la sensación indescriptible y misteriosa que me captura lleva mis ojos a la profundidad de sus ojos."

# game/script-a4-rin.rpy:421
translate es a4_rin_demused_fb68f57c:

    # "The look in her eyes is surprised, quizzical as to why the impertinent hand prevented her advances."
    "La mirada en sus ojos es de sorpresa, inquisitiva en cuanto al porqué de la mano impertinente que previene sus insinuaciones."

# game/script-a4-rin.rpy:423
translate es a4_rin_demused_03c5f631:

    # "Her eyes are really big and glistening with moisture, and staring right into my own with a soft gaze that I'm having a hard time to match."
    "Sus ojos son realmente grandes y relucen con humedad, y contemplando directamente hacia los míos con una mirada suave a la que estoy teniendo dificultad para igualar."

# game/script-a4-rin.rpy:425
translate es a4_rin_demused_03b9a4e4:

    # "Rin's half-open mouth makes her look even more confused, although the sensual way her lips are arching is signaling something completely different."
    "La boca a medio abrir de Rin la hace verse todavía más confundida, aunque el sensual arco formado por sus labios señale algo completamente diferente."

# game/script-a4-rin.rpy:430
translate es a4_rin_demused_ce0164c3:

    # rin "Please."
    rin "Por favor."

# game/script-a4-rin.rpy:435
translate es a4_rin_demused_1f1699db:

    # rin "I need you."
    rin "Te necesito."

# game/script-a4-rin.rpy:437
translate es a4_rin_demused_d927605a:

    # "The words come from her throat as a coarse whisper meant only for me, bypassing her tongue and teeth without giving them any chance to interrupt."
    "Las palabras desde su garganta son tan ásperas como un susurro destinado solo para mí, circunvalando su lengua y dientes sin darles oportunidad de interrumpir."

# game/script-a4-rin.rpy:443
translate es a4_rin_demused_acecb949:

    # "They sober me in an instant, and I clumsily flinch back to get a little bit of distance between us, painfully scraping against my desk in the process."
    "Me espabilan en un instante, y torpemente retrocedo para tener un poco más de distancia entre nosotros, raspándome dolorosamente contra el escritorio en el proceso."

# game/script-a4-rin.rpy:445
translate es a4_rin_demused_c77bf8b9:

    # "Maybe it's her choice of words, maybe the way she says it, but something in it puts me off."
    "Tal vez sea su elección de palabras, tal vez el modo en que las dijo, pero algo en ellas me repele."

# game/script-a4-rin.rpy:447
translate es a4_rin_demused_9cc620ed:

    # "Something is wrong, something is terribly wrong again."
    "Algo está mal, está horriblemente mal otra vez."

# game/script-a4-rin.rpy:449
translate es a4_rin_demused_25827a5e:

    # hi "Need me for what?"
    hi "¿Necesitarme para qué?"

# game/script-a4-rin.rpy:451
translate es a4_rin_demused_e9163df2:

    # "All the unpleasant feelings emerge again, and I feel my heartbeat suddenly increasing at least tenfold."
    "Todos los sentimientos desagradables brotan nuevamente, y siento mis latidos repentinamente acelerarse por lo menos diez veces."

# game/script-a4-rin.rpy:456
translate es a4_rin_demused_e17041e2:

    # "Rin's eyes go out of focus and back again as her body relaxes from its tensed state, and she stands upright again."
    "Los ojos de Rin pierden foco y retroceden mientras su cuerpo se relaja de su estado de tensión, y se para derecha de nuevo."

# game/script-a4-rin.rpy:461
translate es a4_rin_demused_0bc155f7:

    # rin "I don't think I was thinking about anything. Why do you draw patterns in that dust on your night table?"
    rin "No creo que haya estado pensando en algo. ¿Por qué dibujas patrones en el polvo sobre tu mesa de noche?"

# game/script-a4-rin.rpy:466
translate es a4_rin_demused_8c166848:

    # rin "There is a word for that kind of thing but I can't remember…"
    rin "Hay una palabra para ese tipo de cosas pero no la puedo recordar…"

# game/script-a4-rin.rpy:468
translate es a4_rin_demused_c2ef6a90:

    # "Her remark almost throws me off track and I glance over her shoulder at the small table next to my bed, but I can't see anything from this distance."
    "Su comentario casi me hace perder la concentración y echo un vistazo sobre su hombro a la mesita enseguida de mi cama, pero no puedo ver nada a esta distancia."

# game/script-a4-rin.rpy:470
translate es a4_rin_demused_456d4f56:

    # "So she needs me for nothing specific?"
    "¿Así que no me necesita para nada en especial?"

# game/script-a4-rin.rpy:472
translate es a4_rin_demused_deea301b:

    # "Just happened to come by because she thought I'd be glad to see her after she shut me out, no complaints accepted, for a week."
    "Tan solo pasa que vino para acá porque pensó que estaría contento de verla después de que me apartara, sin aceptar quejas, por una semana."

# game/script-a4-rin.rpy:474
translate es a4_rin_demused_a00b83cc:

    # "Completely altruistic motives?"
    "¿Motivos completamente altruistas?"

# game/script-a4-rin.rpy:476
translate es a4_rin_demused_329b80b2:

    # "Felt like it?"
    "¿Se le ocurrió?"

# game/script-a4-rin.rpy:478
translate es a4_rin_demused_909db814:

    # hi "Bullshit. I can answer myself."
    hi "Tonterías. Puedo responder por mí mismo."

# game/script-a4-rin.rpy:480
translate es a4_rin_demused_9fc58eff:

    # hi "To play mind games with whenever you want, to kiss whenever you want, to ignore whenever you want, to fulfill your whims whenever you want?"
    hi "¿Para jugar con mi mente cuando te dé la gana, besar cuando te dé la gana, ignorar cuando te dé la gana, satisfacer tus caprichos cuando te dé la gana?"

# game/script-a4-rin.rpy:482
translate es a4_rin_demused_f97a24e8:

    # hi "Is that it? What you need me for?"
    hi "¿Es eso? ¿Para lo que me necesitas?"

# game/script-a4-rin.rpy:484
translate es a4_rin_demused_0cfbf71f:

    # "My voice is sounding very angry again, even to myself."
    "Mi voz suena muy enojada otra vez, incluso para mí mismo."

# game/script-a4-rin.rpy:486
translate es a4_rin_demused_809d2f18:

    # extend " Good."
    extend " Bien."

# game/script-a4-rin.rpy:491
translate es a4_rin_demused_a1c3fdcc:

    # "Rin too finally catches the mood and her curious expression changes instantly to something more uncharacteristic."
    "Rin también entiende el ambiente finalmente y su expresión de curiosidad cambia inmediatamente a algo más atípico."

# game/script-a4-rin.rpy:496
translate es a4_rin_demused_13f95aea:

    # rin "No—"
    rin "No—"

# game/script-a4-rin.rpy:498
translate es a4_rin_demused_e282de84:

    # "She leaves it at that, her eyes restlessly wandering around, searching the room as if the words she tries to find were written in the tapestries of my walls."
    "Ella lo deja en eso, sus ojos divagando alrededor sin descanso, buscando en el cuarto como si las palabras que intenta encontrar estuviesen escritas en el empapelado de mis paredes."

# game/script-a4-rin.rpy:500
translate es a4_rin_demused_a461d02c:

    # hi "Then what?"
    hi "¿Entonces qué?"

# game/script-a4-rin.rpy:507
translate es a4_rin_demused_9af4a2f2:

    # rin "I needed to paint."
    rin "Necesitaba pintar."

# game/script-a4-rin.rpy:509
translate es a4_rin_demused_ad9b967e:

    # "Paint."
    "Pintar."

# game/script-a4-rin.rpy:511
translate es a4_rin_demused_21700daa:

    # "Of course. That's what artists do."
    "Claro. Eso es lo que hacen los artistas."

# game/script-a4-rin.rpy:513
translate es a4_rin_demused_06be40c7:

    # "The words reverberate through my being, beating in my blood over the piercing whistle of my anger."
    "Las palabras reverberan a través de mi ser, latiendo en mi sangre por sobre el silbido ensordecedor de mi ira."

# game/script-a4-rin.rpy:517
translate es a4_rin_demused_01704145:

    # hi "Don't give me that, Rin! I'm not some damn muse of yours, free to play with for the sake of painting!"
    hi "¡No me vengas con eso, Rin! ¡No soy alguna maldita musa tuya, libre de ser abusada por el bien de la pintura!"

# game/script-a4-rin.rpy:519
translate es a4_rin_demused_5489b14d:

    # hi "I am not some medium for whatever you aspire to, I am me!"
    hi "No soy algún medio para lo que sea que tú aspires, ¡yo soy yo!"

# game/script-a4-rin.rpy:521
translate es a4_rin_demused_26a3fc06:

    # hi "So what if I don't know anything about my future?"
    hi "¿Y qué si no sé nada de mi futuro?"

# game/script-a4-rin.rpy:523
translate es a4_rin_demused_da898656:

    # hi "There's things I want, and things I care about! Even I can dream of things other than nightmares!"
    hi "¡Hay cosas que quiero, y cosas que me importan! ¡Incluso yo puedo soñar con algo más que pesadillas!"

# game/script-a4-rin.rpy:525
translate es a4_rin_demused_18b372de:

    # "I'm yelling, but I'm way past the point of caring about things like that."
    "Estoy gritando, pero estoy más allá del punto en el que me importan cosas como esa."

# game/script-a4-rin.rpy:530
translate es a4_rin_demused_6368f21f:

    # "Rin looks down at her toes and wiggles them a little melancholically while she takes in my outburst without saying anything to defend herself."
    "Rin mira abajo hacia sus dedos y los mueve un poco con melancolía mientras recibe mi estallido sin decir nada para defenderse a sí misma."

# game/script-a4-rin.rpy:532
translate es a4_rin_demused_3478eebe:

    # "Only after I have finished does she try to respond somehow."
    "Solo hasta que yo termino ella intenta responder de algún modo."

# game/script-a4-rin.rpy:537
translate es a4_rin_demused_e5d99da7:

    # rin "I can't do anything else. Or I can do all sorts of things, but I… can't… do."
    rin "No puedo hacer nada más. O puedo hacer todo tipo de cosas, pero… no puedo… hacer."

# game/script-a4-rin.rpy:542
translate es a4_rin_demused_3ef381c4:

    # rin "It's the only thing I sort of do properly. Most of the time."
    rin "Es la única cosa que hago más o menos bien. La mayoría del tiempo."

# game/script-a4-rin.rpy:544
translate es a4_rin_demused_ffcb8c64:

    # "I understand completely. Art first, everything else second, or thousandth."
    "Lo entiendo completamente. El arte es primero, todo lo demás está en segundo lugar, o en el milésimo."

# game/script-a4-rin.rpy:546
translate es a4_rin_demused_c2142b2f:

    # hi "What about me? Am I nothing? When I was interested in art, did that make you feel like I was a little interesting, for a little while?"
    hi "¿Qué hay de mí? ¿No soy nada? Cuando estaba interesado en el arte, ¿acaso eso te hizo sentir como si yo fuese un poquito interesante, por un corto momento?"

# game/script-a4-rin.rpy:548
translate es a4_rin_demused_e245d512:

    # hi "Tell me. I really want to know. Did you ever think about my perspective, or is it just all you?"
    hi "Dime. Realmente quiero saberlo. ¿Alguna vez pensaste en mi perspectiva, o acaso todo se trata de ti?"

# game/script-a4-rin.rpy:550
translate es a4_rin_demused_a3f931d8:

    # "The words rise like bile in my throat."
    "Las palabras se alzan como hiel en mi garganta."

# game/script-a4-rin.rpy:555
translate es a4_rin_demused_e34a9b74:

    # "She looks alarmed. And also completely uncomprehending, as if she just doesn't understand what I'm angry about."
    "Ella se ve alarmada. Y también completamente desconcertada, como si simplemente no comprendiera por qué estoy enojado."

# game/script-a4-rin.rpy:557
translate es a4_rin_demused_10996ab4:

    # "I can't believe she could be so stupid."
    "No puedo creer que ella sea tan estúpida."

# game/script-a4-rin.rpy:562
translate es a4_rin_demused_d0190ebc:

    # rin "I didn't want to—?"
    rin "¿Yo no quise—?"

# game/script-a4-rin.rpy:564
translate es a4_rin_demused_9fbe4b85:

    # "This time it's Rin who interrupts herself in midsentence."
    "Esta vez es Rin quien se interrumpe a sí misma a media palabra."

# game/script-a4-rin.rpy:569
translate es a4_rin_demused_5f8cd9ff:

    # rin "Don't you understand? I can't."
    rin "¿No entiendes? No puedo."

# game/script-a4-rin.rpy:571
translate es a4_rin_demused_b046783b:

    # hi "Can't what?"
    hi "¿No puedes qué?"

# game/script-a4-rin.rpy:573
translate es a4_rin_demused_bf421548:

    # hi "You never explain! How am I supposed to understand anything if you never say anything?"
    hi "¡Nunca te explicas! ¿Cómo se supone que vaya a entender algo si nunca dices nada?"

# game/script-a4-rin.rpy:575
translate es a4_rin_demused_fbab08c2:

    # hi "Why don't you ever talk?"
    hi "¿Por qué nunca hablas?"

# game/script-a4-rin.rpy:577
translate es a4_rin_demused_e360754c:

    # hi "Say something!"
    hi "¡Di algo!"

# game/script-a4-rin.rpy:579
translate es a4_rin_demused_0c8eefa8:

    # "But she doesn't."
    "Pero no lo hace."

# game/script-a4-rin.rpy:581
translate es a4_rin_demused_2dc193f7:

    # "Venting my anger at her feels satisfying. It feels wrong to take so much satisfaction in it, but I can't stop."
    "Descargar mi ira sobre ella se siente agradable y sentirme satisfecho con ello se siente terrible, pero no puedo detenerme."

# game/script-a4-rin.rpy:586
translate es a4_rin_demused_3348afb5:

    # "Not wanting to face my anger head-on, Rin turns around to steadfastly look out of my window even though there is nothing to look at."
    "No queriendo encontrar mi cólera de frente, Rin se vuelve para mirar fijamente fuera de mi ventana aunque no haya nada que ver allá."

# game/script-a4-rin.rpy:588
translate es a4_rin_demused_74f1b510:

    # "The worst of my ire gone, I shut up as I can't be bothered to keep on yelling at the back of her head, so silence finally returns."
    "Lo peor de mi ira se ha ido, me callo ya que no puedo tomarme la molestia de continuar gritándole a la nuca de Rin, así que el silencio finalmente regresa."

# game/script-a4-rin.rpy:590
translate es a4_rin_demused_1a0101c7:

    # "I try to discern some hints of her reaction through my adrenaline-distorted vision."
    "Trato de discernir algún indicio de su reacción a través de mi visión distorsionada por la adrenalina."

# game/script-a4-rin.rpy:592
translate es a4_rin_demused_57528871:

    # "My feedback was not the best kind, but I hope Rin got the clue that she just can't ignore everything else whenever she feels like it."
    "Mi crítica no fue del mejor tipo, pero espero que Rin tenga una idea clara de que no puede simplemente ignorar todo lo demás cuando sea que le dé la gana."

# game/script-a4-rin.rpy:594
translate es a4_rin_demused_c5280ef1:

    # "I'd hate it if she didn't. She never ever listens to anything, she's so unaffected by the world around her."
    "Odiaría que no fuese así. Ella nunca pero nunca escucha algo, el mundo a su alrededor jamás la altera."

# game/script-a4-rin.rpy:596
translate es a4_rin_demused_159f59d9:

    # "Not this time, it seems."
    "No esta vez, parece ser."

# game/script-a4-rin.rpy:598
translate es a4_rin_demused_1371e84c:

    # "Her body is shaking like from holding back tears, but I already know that Rin is not crying."
    "Su cuerpo tiembla como si estuviese conteniendo el llanto, pero yo ya sé que Rin no está llorando."

# game/script-a4-rin.rpy:600
translate es a4_rin_demused_4c154e58:

    # "Her indifference made me so furious. Now that it's gone, I'm at a loss. I wonder…"
    "Su indiferencia me puso furioso. Ahora que se ha ido, me quedo sin palabras. Me pregunto…"

# game/script-a4-rin.rpy:602
translate es a4_rin_demused_4966d1e6:

    # "Did I go too far?"
    "¿Fui demasiado lejos?"

# game/script-a4-rin.rpy:604
translate es a4_rin_demused_ca81d4a9:

    # hi "Look, I—"
    hi "Escucha, yo—"

# game/script-a4-rin.rpy:609
translate es a4_rin_demused_1149e5d6:

    # rin "Go away."
    rin "Vete."

# game/script-a4-rin.rpy:611
translate es a4_rin_demused_e6915669:

    # rin "Go away, Hisao."
    rin "Vete, Hisao."

# game/script-a4-rin.rpy:613
translate es a4_rin_demused_958e171d:

    # "Her voice is tiny and tired as she says this, but I hear the words clear as day."
    "Su voz suena tan diminuta y cansada cuando dice esto, pero escucho las palabras tan claras como el día."

# game/script-a4-rin.rpy:615
translate es a4_rin_demused_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:617
translate es a4_rin_demused_e0f03b86:

    # "What is there to say any more?"
    "¿Qué más queda por decir?"

# game/script-a4-rin.rpy:619
translate es a4_rin_demused_7f393299:

    # hi "This is my room."
    hi "Esta es mi habitación."

# game/script-a4-rin.rpy:621
translate es a4_rin_demused_7ec7dab5:

    # "The blunt, hollow remark is a fitting conclusion for this unpleasant discussion that became an even more unpleasant and very one-sided yelling match."
    "El comentario, hueco y directo, es una conclusión apropiada para esta desagradable discusión convertida en un todavía más desagradable y desigual concurso de gritos."

# game/script-a4-rin.rpy:626
translate es a4_rin_demused_2cbb4a3b:

    # "After a moment of collecting herself Rin just gives up, I can see it from the way she slumps her shoulders, and walks out."
    "Después de un momento para recobrar la calma Rin se da por vencida, puedo verlo por el modo en el que baja sus hombros, y camina hacia afuera."

# game/script-a4-rin.rpy:631
translate es a4_rin_demused_af16ff33:

    # "Even though she deliberately looks to the other direction, I can see how she's biting the corner of her lip so hard it might start bleeding if she won't stop."
    "Aunque ella deliberadamente vea en otra dirección, puedo ver que se muerde la comisura del labio tan fuerte que podría empezar a sangrar si no se detiene."

# game/script-a4-rin.rpy:633
translate es a4_rin_demused_81325e86:

    # "As she makes her exit, I realize that she left the door open when she came in and my yelling must've echoed around the dorm hallways."
    "Cuando hace su salida, me percato de que ella dejó la puerta abierta al entrar y mis gritos deben haber hecho eco por los pasillos del dormitorio."

# game/script-a4-rin.rpy:635
translate es a4_rin_demused_64b0036f:

    # "I sigh. Now that she's gone, I am left alone with my guilt."
    "Suspiro. Ahora que se ha ido, me quedo solo con mi culpa."

# game/script-a4-rin.rpy:637
translate es a4_rin_demused_91dd6dfe:

    # "As the thumping in my chest slowly subdues, anxiety replaces it."
    "A medida que el golpeteo en mi pecho se tranquiliza poco a poco, la ansiedad lo reemplaza."

# game/script-a4-rin.rpy:639
translate es a4_rin_demused_07372464:

    # "Somehow, I feel that none of this would've ever happened if not for me."
    "De algún modo siento que nada de esto habría pasado de no ser por mí."

# game/script-a4-rin.rpy:641
translate es a4_rin_demused_be3e51ff:

    # "No matter how infuriating, unbearable and outrageous Rin is, she is not the Rin I thought I knew."
    "No importa cuán exasperante, insoportable y abusiva sea Rin, ella no es la Rin que pensé que conocía."

# game/script-a4-rin.rpy:643
translate es a4_rin_demused_17f8e076:

    # "The Rin that I expected Rin to be."
    "La Rin que esperaba que fuera Rin."

# game/script-a4-rin.rpy:645
translate es a4_rin_demused_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:647
translate es a4_rin_demused_87b374e0:

    # "Was it me who caused all this by talking Rin into taking her chances with the exhibition?"
    "¿Acaso fui yo quien causó todo esto al convencer a Rin de probar su suerte con la exhibición?"

# game/script-a4-rin.rpy:649
translate es a4_rin_demused_2ec4fcb3:

    # "Am I directly responsible for Rin becoming like she has been for the past weeks?"
    "¿Soy directamente responsable de que Rin se convirtiera en lo que ha sido las últimas semanas?"

# game/script-a4-rin.rpy:651
translate es a4_rin_demused_daa1faec:

    # "I can't think of any other explanation for her weird behavior than the exhibition and all the things that came along with it."
    "No puedo pensar en alguna explicación para su extraño comportamiento, además de la exhibición y todas las cosas que vinieron junto con ella."

# game/script-a4-rin.rpy:653
translate es a4_rin_demused_c3e57bc0:

    # "Maybe it was the only way that could have brought us closer, but all it did was separate us further away from each other, and now beyond the reach of either of us."
    "Tal vez era el único camino que podría habernos unido, pero todo lo que hizo fue separarnos todavía más el uno del otro, y ahora más allá del alcance de cualquiera de los dos."

# game/script-a4-rin.rpy:658
translate es a4_rin_demused_6e0c50f7:

    # "I bang my head hard against the wall."
    "Golpeo con fuerza mi cabeza contra la pared."

# game/script-a4-rin.rpy:665
translate es a4_rin_demused_498da18b:

    # "Twice, to make sure it hurts."
    "Dos veces, para asegurarme de que duele."

# game/script-a4-rin.rpy:682
translate es a4_rin_the_scene_e9ec2627:

    # "A headache is relentlessly thumping against the back of my head as I push open the door to the 22nd Corner."
    "Una jaqueca retumba contra mi nuca sin descanso cuando empujo para abrir la puerta a la 22a Esquina."

# game/script-a4-rin.rpy:684
translate es a4_rin_the_scene_4ec2eeed:

    # "Apart from that, I'm perfectly calm."
    "Además de eso, estoy perfectamente tranquilo."

# game/script-a4-rin.rpy:692
translate es a4_rin_the_scene_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:691
translate es a4_rin_the_scene_69ee39bd:

    # n "{vspace=60}After venting on Rin all that anger that I had bottled inside, it felt like a great weight had been lifted from my heart."
    n "{vspace=60}Después de descargar sobre Rin todo el enojo que tenía reprimido, se sintió como si un gran peso se hubiese levantado de mi corazón."

# game/script-a4-rin.rpy:693
translate es a4_rin_the_scene_18b060d2:

    # n "The tension that had grasped my mind for the past few weeks faded away without leaving even a shadow behind."
    n "La tensión que se había aferrado a mi mente durante las semanas anteriores se desvaneció sin dejar siquiera una sombra detrás."

# game/script-a4-rin.rpy:695
translate es a4_rin_the_scene_35b9b761:

    # n "In this nearly Zen-like state of enlightenment I realized that perhaps it was a bad idea to yell at her like that."
    n "En este estado cercano a la iluminación Zen me percato de que tal vez fue una mala idea gritarle de esa manera."

# game/script-a4-rin.rpy:697
translate es a4_rin_the_scene_a6e98ad6:

    # n "{vspace=30}I really meant it, but what good does blowing up like that do? Nothing."
    n "{vspace=30}Hablaba muy en serio pero, ¿qué bien hace explotar así? Ninguno."

# game/script-a4-rin.rpy:699
translate es a4_rin_the_scene_de0cf754:

    # n "I am not like that. I don't normally yell at people. I don't know why I did yesterday."
    n "Yo no soy así. Yo no le grito a la gente normalmente. No sé por qué lo hice ayer."

# game/script-a4-rin.rpy:701
translate es a4_rin_the_scene_3256da36:

    # n "So I keep feeling really guilty about it and wanting to take my words back."
    n "Así que me sigo sintiendo bastante culpable al respecto y sigo queriendo retractarme."

# game/script-a4-rin.rpy:703
translate es a4_rin_the_scene_5d9486bc:

    # n "{vspace=60}Rin is probably upset too. Even more than my own behavior, her reaction shocked me."
    n "{vspace=60}Rin probablemente también está molesta. Aun más que mi propio comportamiento, su reacción me conmocionó."

# game/script-a4-rin.rpy:705
translate es a4_rin_the_scene_18a0c250:

    # nvl clear
    # n "{vspace=30}I've always thought of her as unchanging, detached from her surroundings so that seeing my yelling get her so upset felt… out of place."
    nvl clear
    n "{vspace=30}Siempre había pensado en ella como alguien invariable, distante de sus alrededores así que ver que mis gritos la hayan molestado tanto se sintió… fuera de lugar."

# game/script-a4-rin.rpy:709
translate es a4_rin_the_scene_23cfa8b2:

    # n "{vspace=30}I wonder if she understands how I feel?"
    n "{vspace=30}¿Me pregunto si entiende cómo me siento?"

# game/script-a4-rin.rpy:711
translate es a4_rin_the_scene_a416026e:

    # n "In Rin's world everything seems to be so absolute and subjective… absolutely subjective, as if she was completely unable to see things from other points of view than her own."
    n "En el mundo de Rin todo parece ser tan absoluto y subjetivo… absolutamente subjetivo, como si ella fuese completamente incapaz de ver las cosas desde un punto de vista diferente al suyo."

# game/script-a4-rin.rpy:713
translate es a4_rin_the_scene_492227c4:

    # n "But ultimately, is anyone able to do it? Maybe objectivity and altruism are just illusions for people who like to think of themselves as compassionate."
    n "Pero después de todo, ¿hay alguien capaz de hacerlo? Quizás el objetivismo y el altruismo son solo ilusiones para la gente que gusta de pensar en sí misma como compasiva."

# game/script-a4-rin.rpy:715
translate es a4_rin_the_scene_84f03e94:

    # n "Just like art is an illusion for people who think reality is merely a veil for something greater."
    n "Justo como el arte es una ilusión para la gente que piensa que la realidad es meramente un velo para algo más grandioso."

# game/script-a4-rin.rpy:717
translate es a4_rin_the_scene_c9f1153e:

    # n "Even when you stop thinking that the world revolves around you or start thinking outside of the mythical box, you are just inside another, bigger box that you can't escape."
    n "Aun cuando dejas de pensar que el mundo da vueltas alrededor de ti o comienzas a pensar en algo distinto al mítico común, tan solo estás dentro de otro común más grande del que no puedes escapar."

# game/script-a4-rin.rpy:719
translate es a4_rin_the_scene_c70301ec:

    # n "{vspace=30}Maybe that, ultimately, makes her like the rest of us."
    n "{vspace=30}Tal vez eso, a fin de cuentas, la hace como el resto de nosotros."

# game/script-a4-rin.rpy:727
translate es a4_rin_the_scene_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:736
translate es a4_rin_the_scene_d25001ff:

    # "I step through the door to find a gallery full of illusioned people."
    "Camino por la puerta para encontrarme con la galería llena de gente ilusionada."

# game/script-a4-rin.rpy:738
translate es a4_rin_the_scene_3da58e1f:

    # "Despite Sae's remarks during my earlier visits I always thought it was very spacious, but now when it's crowded like this it looks positively cramped."
    "A pesar de las observaciones de Sae durante mis primeras visitas yo siempre pensé que era muy espaciosa, pero ahora que está así de llena de gente, definitivamente se ve pequeña."

# game/script-a4-rin.rpy:743
translate es a4_rin_the_scene_cc50cec4:

    # "I immediately notice Sae standing in the middle of a lively discussion, busily chattering with some old gentlemen."
    "Inmediatamente noto a Sae de pie en medio de una animada discusión, ocupada charlando con algún caballero mayor."

# game/script-a4-rin.rpy:745
translate es a4_rin_the_scene_c361493f:

    # "She's actually pretty tall and kind of cool-looking, so she stands out in the crowd."
    "Ella de hecho es bastante alta y se ve algo espectacular, así que sobresale de la multitud."

# game/script-a4-rin.rpy:747
translate es a4_rin_the_scene_a9ce38bc:

    # "There are a few dozen wine glasses laid on the tables along the back wall, filled with burgundy liquid. A vast majority of the guests are sipping from their own glasses."
    "Hay unas docenas de copas de vino colocadas sobre las mesas a lo largo de la pared del fondo, llenas de líquido color borgoña. Una vasta mayoría de los invitados bebe de sus copas."

# game/script-a4-rin.rpy:749
translate es a4_rin_the_scene_7cd827ac:

    # "The socialites and art connoisseurs are mingling happily, exchanging mild opinions about Rin's art which seems to be a secondary object of interest for most."
    "Los miembros de la alta sociedad y los conocedores de arte se mezclan con alegría intercambiando leves comentarios sobre el arte de Rin, el cual parece ser un objeto de interés secundario para la mayoría."

# game/script-a4-rin.rpy:751
translate es a4_rin_the_scene_d46da2da:

    # "I feel distanced, excluded from the other people here."
    "Me siento distanciado, excluido del resto de la gente aquí."

# game/script-a4-rin.rpy:753
translate es a4_rin_the_scene_73ce016d:

    # "I can't claim even at a stretch to be a social chameleon, so this situation is quite unnerving."
    "No puedo afirmar, incluso si exagerara, ser un camaleón social, así que esta situación es bastante inquietante."

# game/script-a4-rin.rpy:755
translate es a4_rin_the_scene_6f96359c:

    # "Since I don't blend into the crowd at all, I just fake that I do, trying to look as cool and smooth as I can."
    "Ya que no me mezclo con la multitud en absoluto, solo finjo que lo hago, tratando de verme tan indiferente y seguro como puedo."

# game/script-a4-rin.rpy:757
translate es a4_rin_the_scene_6d98206a:

    # "I wonder how Rin is handling all this. If it was me, I would be quite freaked out."
    "Me pregunto cómo estará manejando Rin todo esto. Si fuera yo, estaría aterrado."

# game/script-a4-rin.rpy:765
translate es a4_rin_the_scene_cdc85e8e:

    # "Throwing the anxiety aside, I try to carefully navigate through the crowd, stealing peeks at the framed paintings now hanging on the walls."
    "Haciendo la ansiedad a un lado, trato de navegar con cuidado a través de la muchedumbre, robándole miradas a las pinturas enmarcadas ahora colgando sobre las paredes."

# game/script-a4-rin.rpy:774
translate es a4_rin_the_scene_e37f01d4:

    # "Rin's exhibition takes about half of the gallery's wall space. Some paintings are less familiar than others, but I recognize most of them."
    "La exhibición de Rin toma alrededor de la mitad del espacio en las paredes de la galería. Algunas de las pinturas me resultan menos familiares que otras, pero reconozco la mayoría de ellas."

# game/script-a4-rin.rpy:776
translate es a4_rin_the_scene_8051b9a2:

    # "Some I've seen being created at the club meetings after all, or remember from the time when Rin was choosing her portfolio."
    "Algunas las he visto siendo creadas en las reuniones del club después de todo, o las recuerdo de las veces en las que Rin estaba escogiéndolas para su portafolio."

# game/script-a4-rin.rpy:778
translate es a4_rin_the_scene_5c9641bc:

    # "I note that a couple of the unfinished paintings are framed and on the wall as well. Maybe that's what they call coincidental art?"
    "Noto que un par de pinturas sin terminar están enmarcadas y en la pared, también. ¿Tal vez eso es lo que llaman arte fortuito?"

# game/script-a4-rin.rpy:780
translate es a4_rin_the_scene_afe8fadc:

    # "Even Rin's failures, if you can call them that, became exhibits of her skill. Quite paradoxical."
    "Incluso los fracasos de Rin, si puedes llamarlos así, se han convertido en una pieza de exhibición de su habilidad. Bastante paradójico."

# game/script-a4-rin.rpy:788
translate es a4_rin_the_scene_36595355:

    # "She herself is nowhere to be seen, which is strange because even though it's crowded, the gallery {b}is{/b} pretty small."
    "Ella no se deja ver por ninguna parte, lo cual es extraño porque aun si está repleto de gente, la galería {b}es{/b} bastante pequeña."

# game/script-a4-rin.rpy:790
translate es a4_rin_the_scene_78ea3f71:

    # "It's fine, sort of. I don't know how to face her after yesterday. Maybe I shouldn't have even come."
    "Está bien, más o menos. No sé qué cara ponerle después de lo de ayer. Tal vez no debería ni haber venido."

# game/script-a4-rin.rpy:792
translate es a4_rin_the_scene_b7b167a0:

    # "But I promised various people, Rin included, that I would, so…"
    "Pero se lo prometí a muchas personas, incluida Rin, que vendría, así que…"

# game/script-a4-rin.rpy:794
translate es a4_rin_the_scene_43093a63:

    # "Damn, it sounds like I do the things I do because some kind of instinctual properness compels me to, not because it would be sensible (or not)."
    "Maldición, suena como si hiciera las cosas que hago a causa de algún tipo de honorabilidad instintiva que me obliga, no porque fuera sensato (o no)."

# game/script-a4-rin.rpy:801
translate es a4_rin_the_scene_993ea1ac:

    # "I sneak closer to Sae to wait for a lull in the conversation so I can chat her up too."
    "Me escabullo más cerca de Sae para esperar por un descanso en la conversación y así también poder charlar con ella."

# game/script-a4-rin.rpy:803
translate es a4_rin_the_scene_0ca65725:

    # "Even though her voice is almost completely buried under the general background noise, I hear bits and pieces of her talking about Rin."
    "Aunque su voz esté casi completamente sumergida debajo del ruido de fondo generalizado, escucho pedacitos de ella hablando sobre Rin."

# game/script-a4-rin.rpy:805
translate es a4_rin_the_scene_3022a3f0:

    # sa "Yes, she is a high schooler at a local school… even though she's graduating next year I'm sure various art schools would be interested in…"
    sa "Sí, ella es una estudiante de preparatoria en una escuela local… aunque se graduará el siguiente año estoy segura de que varias escuelas de arte estarán interesadas en…"

# game/script-a4-rin.rpy:807
translate es a4_rin_the_scene_5c97a6ee:

    # sa "…I thought it'd be interesting to have an exhibition of someone who is still in early stages of development…"
    sa "… Pensé que sería interesante tener una exhibición de alguien que todavía está en las fases iniciales de desarrollo…"

# game/script-a4-rin.rpy:809
translate es a4_rin_the_scene_206f5e36:

    # "It's so strange, it's like Rin is some kind of mini-celebrity even though this is nothing but a small exhibition opening at a small art gallery of a small town."
    "Es tan extraño, es como si Rin fuese algún tipo de minicelebridad aunque esto no sea más que una apertura de una pequeña exhibición en una pequeña galería de arte en una pequeña ciudad."

# game/script-a4-rin.rpy:811
translate es a4_rin_the_scene_07764980:

    # sa "In fact, there is a friend of mine from…"
    sa "De hecho, hay un amigo mío de…"

# game/script-a4-rin.rpy:816
translate es a4_rin_the_scene_40554ac7:

    # mystery "It's Hisao!"
    mystery "¡Es Hisao!"

# game/script-a4-rin.rpy:818
translate es a4_rin_the_scene_de462b46:

    # "My eavesdropping is interrupted by a familiar voice and a familiar slap to the back. I don't need to guess the source of either, even without turning around."
    "Dejo de escuchar a escondidas al ser interrumpido por una voz familiar y una palmada familiar en la espalda. No necesito adivinar de quién viene, aun si no me doy vuelta."

# game/script-a4-rin.rpy:820
translate es a4_rin_the_scene_2b3d4056:

    # hi "Hi Emi."
    hi "Hola Emi."

# game/script-a4-rin.rpy:837
translate es a4_rin_the_scene_29e755bd:

    # emi "Hi! Are you like, a representative of the art club or something? I don't see anyone else from the school here…"
    emi "¡Hola! ¿Eres como, un representante del club de arte o algo así? No veo a nadie más de la escuela aquí…"

# game/script-a4-rin.rpy:839
translate es a4_rin_the_scene_a594f5be:

    # hi "Umm… I don't know, really. I guess I am if that's the case."
    hi "Ahh… No lo sé, en realidad. Supongo que lo soy si ese es el caso."

# game/script-a4-rin.rpy:841
translate es a4_rin_the_scene_0583026f:

    # hi "What about you?"
    hi "¿Qué hay de ti?"

# game/script-a4-rin.rpy:846
translate es a4_rin_the_scene_de967264:

    # emi "What about me?"
    emi "¿Qué hay de mí?"

# game/script-a4-rin.rpy:848
translate es a4_rin_the_scene_f5a21f62:

    # hi "Err…"
    hi "Ehh…"

# game/script-a4-rin.rpy:853
translate es a4_rin_the_scene_517d1d91:

    # emi "You didn't think I'm interested in art? Is that it, Hisao?"
    emi "¿No pensaste que estaría interesada en el arte? ¿Es eso, Hisao?"

# game/script-a4-rin.rpy:855
translate es a4_rin_the_scene_15f22183:

    # hi "No, that's not what I… well, maybe a little, if you put it that way."
    hi "No, eso no es lo que yo… bueno, tal vez un poco, si lo pones de esa manera."

# game/script-a4-rin.rpy:857
translate es a4_rin_the_scene_a1c1b2d5:

    # hi "I mean, even though you hang out with Rin I've never heard you talk about art with her so…"
    hi "Quiero decir, aun si te juntas con Rin nunca te he escuchado hablar de arte con ella así que…"

# game/script-a4-rin.rpy:862
translate es a4_rin_the_scene_83589a0d:

    # "Emi huffs and looks around her, looking discontented."
    "Emi resopla y mira a su alrededor, viéndose descontenta."

# game/script-a4-rin.rpy:867
translate es a4_rin_the_scene_3e44e17a:

    # emi "It's true, I don't get it at all, but she came to my track meet so I thought it's only fair to return the favor."
    emi "Es verdad, no lo entiendo para nada, pero vino a mi competencia de atletismo así que solo pensé que es justo regresar el favor."

# game/script-a4-rin.rpy:872
translate es a4_rin_the_scene_06111ba7:

    # "She leans closer, trying to look confidential but only managing to look conspiring."
    "Se inclina hacia mí, tratando de parecer confidente pero solo logrando verse conspirativa."

# game/script-a4-rin.rpy:874
translate es a4_rin_the_scene_8221bfd4:

    # emi "Do you {b}get{/b} art?"
    emi "¿Tú {b}entiendes{/b} el arte?"

# game/script-a4-rin.rpy:876
translate es a4_rin_the_scene_2c690564:

    # hi "No. No, I don't."
    hi "No. No lo entiendo."

# game/script-a4-rin.rpy:878
translate es a4_rin_the_scene_689872df:

    # hi "At all."
    hi "En absoluto."

# game/script-a4-rin.rpy:883
translate es a4_rin_the_scene_cce5fcc8:

    # "My emphasizing headshake draws a giggle and a cheery headshake of her own out of Emi."
    "El sacudir mi cabeza como énfasis extrae una risilla y un alegre meneo de cabeza de Emi."

# game/script-a4-rin.rpy:888
translate es a4_rin_the_scene_5b34a6d0:

    # emi "Me neither!"
    emi "¡Yo tampoco!"

# game/script-a4-rin.rpy:893
translate es a4_rin_the_scene_b195e96f:

    # emi "Hey, let's go talk with Rin! I bet you haven't yet, because I haven't either."
    emi "Oye, ¡vamos a hablar con Rin! Apuesto a que todavía no has ido, porque yo tampoco he ido."

# game/script-a4-rin.rpy:898
translate es a4_rin_the_scene_269bc9d0:

    # emi "Come on!"
    emi "¡Vamos!"

# game/script-a4-rin.rpy:914
translate es a4_rin_the_scene_a32ed3ae:

    # "Before she has a chance to forcefully drag me to Rin, Nomiya appears behind her with Rin in his tow."
    "Antes de que tenga una oportunidad de arrastrarme a la fuerza hacia Rin, Nomiya aparece detrás de ella con Rin a sus espaldas."

# game/script-a4-rin.rpy:916
translate es a4_rin_the_scene_9160c98c:

    # "She's not dressed for the occasion, instead opting for the usual school uniform and unkempt hair."
    "Ella no está vestida para la ocasión, en su lugar optando por el uniforme escolar usual y cabello desarreglado."

# game/script-a4-rin.rpy:918
translate es a4_rin_the_scene_f236e970:

    # "Maybe her natural look is what suits her the best."
    "Quizás su apariencia natural es lo que queda mejor."

# game/script-a4-rin.rpy:923
translate es a4_rin_the_scene_03739b69:

    # emi "Hello, teacher! Hi, Rin!"
    emi "¡Hola, maestro! ¡Qué tal, Rin!"

# game/script-a4-rin.rpy:925
translate es a4_rin_the_scene_c73fdc78:

    # "Unfazed, Emi greets the teacher cheerfully, causing him to turn around and look down confusedly."
    "Sin inmutarse, Emi saluda al maestro con alegría, ocasionando que él se tenga que dar vuelta y mirar hacia abajo confundido."

# game/script-a4-rin.rpy:930
translate es a4_rin_the_scene_e5a20256:

    # no "Who are you?"
    no "¿Quién eres?"

# game/script-a4-rin.rpy:935
translate es a4_rin_the_scene_34fed68d:

    # emi "I'm Emi, from school, class 3-4. Don't you remember?"
    emi "Soy Emi, de la escuela, grupo 3-4. ¿No me recuerda?"

# game/script-a4-rin.rpy:937
translate es a4_rin_the_scene_89bce785:

    # "She looks positively shocked at the prospect that there could be a person who doesn't know her."
    "Se ve bastante asombrada ante la posibilidad de que pueda haber alguien que no la conoce."

# game/script-a4-rin.rpy:942
translate es a4_rin_the_scene_11ee8254:

    # no "Oh, sorry. You are in the same class as Tezuka is, right?"
    no "Oh, disculpa. Estás en el mismo grupo que Tezuka, ¿verdad?"

# game/script-a4-rin.rpy:947
translate es a4_rin_the_scene_b286e85a:

    # emi "Yeah!"
    emi "¡Sí!"

# game/script-a4-rin.rpy:952
translate es a4_rin_the_scene_1daed64e:

    # no "You'll have to pardon me, I have trouble remembering students who don't take art."
    no "Tendrás que perdonarme, tengo problema recordando estudiantes que no cursan arte."

# game/script-a4-rin.rpy:957
translate es a4_rin_the_scene_f8e4b68f:

    # emi "Don't mind, don't mind!"
    emi "¡No hay problema, no hay problema!"

# game/script-a4-rin.rpy:962
translate es a4_rin_the_scene_f9024bee:

    # emi "Hi Rin!"
    emi "¡Qué tal Rin!"

# game/script-a4-rin.rpy:967
translate es a4_rin_the_scene_811c5f41:

    # rin "Hello."
    rin "Hola."

# game/script-a4-rin.rpy:972
translate es a4_rin_the_scene_9cda1dad:

    # emi "Congratulations for your super cool art thing! I'm sure you'll be a big hit!"
    emi "¡Felicidades por tu cosa de arte supergenial! ¡Estoy segura de que serás un gran éxito!"

# game/script-a4-rin.rpy:974
translate es a4_rin_the_scene_aed72305:

    # "She throws her arms into the air for boisterous emphasis, almost hitting me in the face."
    "Ella lanza sus manos al aire para un escandaloso énfasis, casi golpeándome en la cara."

# game/script-a4-rin.rpy:979
translate es a4_rin_the_scene_a21e593c:

    # emi "And look, Hisao came too!"
    emi "¡Y mira, Hisao también vino!"

# game/script-a4-rin.rpy:984
translate es a4_rin_the_scene_5f559886:

    # "Rin doesn't look at me, nor does she greet me."
    "Rin no me mira, tampoco me saluda."

# game/script-a4-rin.rpy:986
translate es a4_rin_the_scene_503fd49a:

    # hi "Congratulations, Rin."
    hi "Felicidades, Rin."

# game/script-a4-rin.rpy:988
translate es a4_rin_the_scene_08fc470e:

    # "She keeps averting her gaze, pointedly looking at her sandals."
    "Ella sigue desviando su mirada, mirando fijamente a sus sandalias."

# game/script-a4-rin.rpy:993
translate es a4_rin_the_scene_649d4c9b:

    # "Oblivious to the tension between us and ignorant of what happened yesterday, Emi keeps on blabbering about this and that to an unresponsive Rin."
    "Inconsciente de la tensión entre nosotros e ignorante de lo sucedido ayer, Emi sigue parloteando sobre esto y aquello a una Rin indiferente."

# game/script-a4-rin.rpy:995
translate es a4_rin_the_scene_e9294ae9:

    # "I guess she's used to not getting much out of her at times."
    "Supongo que está acostumbrada a no observar mucho en ella de cuando en cuando."

# game/script-a4-rin.rpy:1017
translate es a4_rin_the_scene_e99d6ce1:

    # "Before long, Nomiya and Sae turn to Rin, introducing her."
    "En poco tiempo, Nomiya y Sae se vuelven hacia Rin, presentándola."

# game/script-a4-rin.rpy:1019
translate es a4_rin_the_scene_9aa74b97:

    # "Expecting it, I catch the second of confusion when the guests see her arms."
    "Esperando eso, capto el segundo de confusión cuando los invitados ven sus brazos."

# game/script-a4-rin.rpy:1024
translate es a4_rin_the_scene_6d88c598:

    # "Sae is luckily on the ball and briefly explains about our school."
    "Sae por suerte tiene eso en mente y brevemente explica nuestra escuela."

# game/script-a4-rin.rpy:1026
translate es a4_rin_the_scene_f47b727b:

    # "Doubtful faces quickly change to curious."
    "Los rostros dudosos rápidamente cambian a curiosos."

# game/script-a4-rin.rpy:1028
translate es a4_rin_the_scene_5f14c8e3:

    # "Man" "Would you mind telling us something about your art?"
    "Hombre" "¿Te importaría decirnos algo sobre tu arte?"

# game/script-a4-rin.rpy:1030
translate es a4_rin_the_scene_68f9a3e4:

    # "Man" "I thought the development is quite easily noticeable, what do you yourself think of the differences between the older and more current works?"
    "Hombre" "Pienso que el desarrollo es bastante fácil de notar, ¿qué piensas tú misma de las diferencias entre tus viejas y tus más recientes obras?"

# game/script-a4-rin.rpy:1032
translate es a4_rin_the_scene_98907ffb:

    # "Man" "It's quite rare for someone so young to dabble into abstraction."
    "Hombre" "Es bastante raro para alguien tan joven incursionar en lo abstracto."

# game/script-a4-rin.rpy:1034
translate es a4_rin_the_scene_4e19473e:

    # "Woman" "It would've been interesting to see how you work!"
    "Mujer" "¡Sería interesante ver cómo trabajas!"

# game/script-a4-rin.rpy:1036
translate es a4_rin_the_scene_4834f385:

    # "Man" "Oh, definitely! I assume you use your feet? Must've been a great trouble to learn it, you should be proud."
    "Hombre" "¡Oh, definitivamente! ¿Asumo que usas tus pies? Debe haber sido un gran problema aprenderlo, deberías de estar orgullosa."

# game/script-a4-rin.rpy:1041
translate es a4_rin_the_scene_7c641666:

    # rin "I… ummm…"
    rin "Yo… ehhh…"

# game/script-a4-rin.rpy:1045
translate es a4_rin_the_scene_888dbb6e:

    # "Man" "Will you be pursuing a career as an artist after school?"
    "Hombre" "¿Seguirás una carrera como artista después de la escuela?"

# game/script-a4-rin.rpy:1047
translate es a4_rin_the_scene_1598f9c5:

    # "She is bombarded with so many questions she can't even hope to answer all of them."
    "Rin es bombardeada con tantas preguntas que no puede ni esperar a responderlas todas."

# game/script-a4-rin.rpy:1049
translate es a4_rin_the_scene_08ee072a:

    # "Maybe that's for the best, Rin tends to talk nonsense more than occasionally."
    "Quizás eso sea para lo mejor, Rin tiende a decir tonterías más que ocasionalmente."

# game/script-a4-rin.rpy:1051
translate es a4_rin_the_scene_4da16c57:

    # "Man" "So where do you get your ideas?"
    "Hombre" "Y bien, ¿de dónde obtienes tus ideas?"

# game/script-a4-rin.rpy:1056
translate es a4_rin_the_scene_673dbc28:

    # rin "That's the fourth, I mean fifth worst…"
    rin "Esa es la cuarta, quiero decir quinta peor…"

# game/script-a4-rin.rpy:1058
translate es a4_rin_the_scene_ed5867c3:

    # "Rin keeps stumbling with her words, looking more and more vexed by the expectant inquiries."
    "Rin sigue tropezándose con sus palabras, viéndose más y más irritada por las ansiosas preguntas."

# game/script-a4-rin.rpy:1063
translate es a4_rin_the_scene_b7f8fd67:

    # rin "Ah…"
    rin "Ah…"

# game/script-a4-rin.rpy:1065
translate es a4_rin_the_scene_cecd7896:

    # "Everyone is waiting for her to say something, but she looks like a cat got her tongue."
    "Todos esperan que diga algo, pero parece que el ratón le comió la lengua."

# game/script-a4-rin.rpy:1067
translate es a4_rin_the_scene_b75b20f1:

    # "Each question piling up just adds to her distress."
    "Cada pregunta que se va amontonando solo se suma a su angustia."

# game/script-a4-rin.rpy:1072
translate es a4_rin_the_scene_a86cd36d:

    # "I fail to hear the question that is the proverbial one too many."
    "No puedo escuchar la pregunta que es la gota que derrama el proverbial vaso."

# game/script-a4-rin.rpy:1074
translate es a4_rin_the_scene_225297ac:

    # "It's like a motor stalling."
    "Es como un motor que se para."

# game/script-a4-rin.rpy:1095
translate es a4_rin_the_scene_7fd81128:

    # "Rin just freezes for a long, long second until she falls on her knees, hitting the floor ungracefully like a sack of potatoes."
    "Rin simplemente se congela por un largo, largo segundo hasta que cae sobre sus rodillas, golpeando el suelo sin gracia como un saco de papas."

# game/script-a4-rin.rpy:1097
translate es a4_rin_the_scene_be781a3f:

    # "Woman" "Are you all right?"
    "Mujer" "¿Te encuentras bien?"

# game/script-a4-rin.rpy:1099
translate es a4_rin_the_scene_48efa8e6:

    # rin "I don't know…"
    rin "No lo sé…"

# game/script-a4-rin.rpy:1101
translate es a4_rin_the_scene_96e070f8:

    # no "Tezuka? What's wrong, girl?"
    no "¿Tezuka? ¿Qué sucede, mi niña?"

# game/script-a4-rin.rpy:1103
translate es a4_rin_the_scene_eab25463:

    # rin "I don't know what's wrong…"
    rin "No sé qué está mal…"

# game/script-a4-rin.rpy:1105
translate es a4_rin_the_scene_f080eb7f:

    # "A terrible silence falls upon the people gathered around Rin."
    "Un terrible silencio cae sobre la gente reunida en torno a Rin."

# game/script-a4-rin.rpy:1107
translate es a4_rin_the_scene_2a09f94f:

    # "Everyone is petrified, not knowing how to react to her sudden… seizure, or something."
    "Todos están hechos piedra, sin saber cómo reaccionar a su súbito… ataque, o algo."

# game/script-a4-rin.rpy:1109
translate es a4_rin_the_scene_2e170a2d:

    # "She breathes with deep, trembling gasps as if she was running out of air, staring ahead of herself with hollow eyes."
    "Ella respira con jadeos profundos y temblorosos como si se le estuviera acabando el aire, mirando al frente con ojos vacíos."

# game/script-a4-rin.rpy:1132
translate es a4_rin_the_scene_3b57b7b5:

    # "Seeing that nobody does anything, I force myself to step to Rin and lift her up from the floor, letting her lean against me to keep standing."
    "Al ver que nadie hace nada, atravieso la muchedumbre a la fuerza para ir hacia Rin y la levanto del suelo, permitiendo que se recargue en mí para mantenerse en pie."

# game/script-a4-rin.rpy:1134
translate es a4_rin_the_scene_b605b41d:

    # hi "Would you like some fresh air? OK, let's go outside for a bit."
    hi "¿Te gustaría un poco de aire fresco? Está bien, vamos hacia afuera un poco."

# game/script-a4-rin.rpy:1136
translate es a4_rin_the_scene_a01b4bc5:

    # "I don't even wait for her to answer before grasping her shoulder and pulling her past the stunned-looking Nomiya, Sae, Emi and guests."
    "Ni siquiera espero a que me responda antes de sujetarla del hombro y jalarla frente a Nomiya, Sae, Emi y los invitados con miradas estupefactas."

# game/script-a4-rin.rpy:1138
translate es a4_rin_the_scene_5ae85c24:

    # hi "Excuse us."
    hi "Discúlpenos."

# game/script-a4-rin.rpy:1147
translate es a4_rin_the_scene_47ff5bf3:

    # "The cool evening breeze hits my face at the door."
    "La fría brisa de la noche me alcanza en el rostro en la puerta."

# game/script-a4-rin.rpy:1152
translate es a4_rin_the_scene_769ae004:

    # "I let go of Rin and she leans against the stone wall, trying to catch her breath."
    "Dejo ir a Rin y se recarga sobre la pared de piedra, intentando recuperar el aire."

# game/script-a4-rin.rpy:1154
translate es a4_rin_the_scene_dd0872f3:

    # hi "Are you all right?"
    hi "¿Te encuentras bien?"

# game/script-a4-rin.rpy:1159
translate es a4_rin_the_scene_5d06bfbe:

    # rin "I couldn't say anything…"
    rin "No podía decir nada…"

# game/script-a4-rin.rpy:1161
translate es a4_rin_the_scene_b64b1686:

    # "Rin is still not looking at me, so I look away too."
    "Rin sigue sin verme, así que aparto mi mirada."

# game/script-a4-rin.rpy:1165
translate es a4_rin_the_scene_16b8f65c:

    # "The streetlights and colored neon signs twist my vision into a blur of near-blindness, forcing me to look back."
    "El alumbrado público y las señales de colores neón distorsionan mi visión en una mancha casi enceguecida, obligándome a mirar a otro lado."

# game/script-a4-rin.rpy:1167
translate es a4_rin_the_scene_1a60359f:

    # "At least she talks, even if she's not directing her words to me."
    "Por lo menos habla, aun si no está dirigiendo sus palabras hacia mí."

# game/script-a4-rin.rpy:1169
translate es a4_rin_the_scene_2ff399c8:

    # hi "What did you want to say?"
    hi "¿Qué querías decir?"

# game/script-a4-rin.rpy:1171
translate es a4_rin_the_scene_78de87e9:

    # "Maybe both of us can imagine that we are talking to an invisible friend."
    "Tal vez ambos podemos imaginar que estamos hablando con un amigo invisible."

# game/script-a4-rin.rpy:1176
translate es a4_rin_the_scene_4e7ade8d:

    # rin "I don't know."
    rin "No lo sé."

# game/script-a4-rin.rpy:1181
translate es a4_rin_the_scene_a1c846a2:

    # rin "Something that would have meant something."
    rin "Algo que significara algo."

# game/script-a4-rin.rpy:1183
translate es a4_rin_the_scene_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:1185
translate es a4_rin_the_scene_7a537113:

    # "The silence lasts for a long time."
    "El silencio dura por un largo rato."

# game/script-a4-rin.rpy:1187
translate es a4_rin_the_scene_4eef4039:

    # "I don't feel comfortable being alone with Rin. I am not good at imagining things that don't exist, do… or that things that exist, don't."
    "No me siento cómodo estando a solas con Rin. No soy bueno imaginando que existen cosas que no lo hacen… o que no existen cosas que sí lo hacen."

# game/script-a4-rin.rpy:1189
translate es a4_rin_the_scene_28aa5a0b:

    # hi "We should go back in."
    hi "Deberíamos volver adentro."

# game/script-a4-rin.rpy:1191
translate es a4_rin_the_scene_4b5021f3:

    # hi "The guests Sae invited are in there, they probably want to meet you and talk with you."
    hi "La gente que Sae invitó está ahí dentro, probablemente ellos quieran conocerte y hablar contigo."

# game/script-a4-rin.rpy:1193
translate es a4_rin_the_scene_ccc3e9d3:

    # hi "You know, ask you questions and stuff. About those paintings you worked so hard for."
    hi "Tú sabes, hacerte preguntas y cosas así. Sobre esas pinturas para las que trabajaste tan duro."

# game/script-a4-rin.rpy:1198
translate es a4_rin_the_scene_3304ddd5:

    # rin "I don't want them to ask me questions like that. I can never say the right things."
    rin "No quiero que me hagan preguntas así. Nunca puedo decir las cosas correctas."

# game/script-a4-rin.rpy:1200
translate es a4_rin_the_scene_75fa03c0:

    # hi "What do you want then?"
    hi "¿Entonces qué quieres?"

# game/script-a4-rin.rpy:1202
translate es a4_rin_the_scene_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:1211
translate es a4_rin_the_scene_ef00f851:

    # rin "That someone wouldn't have to ask questions from me." nointeract
    rin "Que alguien no tuviera que hacerme preguntas." nointeract

# game/script-a4-rin.rpy:1254
translate es a4_rin_wavelength_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1235
translate es a4_rin_wavelength_d72a268b:

    # n "{vspace=60}Vacation, huh?"
    n "{vspace=60}Vacaciones, ¿eh?"

# game/script-a4-rin.rpy:1237
translate es a4_rin_wavelength_329304f8:

    # n "Some people will stay at the school even over the holidays, some will go back to their families."
    n "Algunos se quedarán en la escuela aun durante las vacaciones, otros volverán con sus familias."

# game/script-a4-rin.rpy:1239
translate es a4_rin_wavelength_8784e275:

    # n "I probably should make the trip back home and report to my parents that I'm alive and well."
    n "Probablemente debería de hacer el viaje de regreso a casa y reportarle a mis padres que estoy vivo y sano."

# game/script-a4-rin.rpy:1241
translate es a4_rin_wavelength_a9ed5746:

    # n "{vspace=30}Not much to do at the school anyway, I suppose."
    n "{vspace=30}No hay mucho que hacer en la escuela de todos modos, supongo."

# game/script-a4-rin.rpy:1243
translate es a4_rin_wavelength_53975cd8:

    # n "Next trimester will be stressful. Everyone will have to seriously start thinking about what to do after graduation."
    n "El siguiente trimestre será estresante. Todos tendrán que pensar seriamente en qué hacer después de graduarse."

# game/script-a4-rin.rpy:1245
translate es a4_rin_wavelength_dc5d6e2d:

    # n "{vspace=60}Including me…"
    n "{vspace=60}Incluyéndome a mí…"

# game/script-a4-rin.rpy:1271
translate es a4_rin_wavelength_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1255
translate es a4_rin_wavelength_ee4c3c2d:

    # "A look at my doodles convinces me to stop trying to salvage them. It's a mess of lifeless lines, a waste of paper if it wasn't the flip side of my exam."
    "Una mirada a mis garabatos me convence de dejar de tratar de salvaguardarlos. Es un desastre de líneas sin vida, un desperdicio de papel si no fuese la parte de atrás de mi examen."

# game/script-a4-rin.rpy:1281
translate es a4_rin_wavelength_d41d8cd9_2:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1262
translate es a4_rin_wavelength_7ea49f73:

    # n "{vspace=90}Maybe it's because I didn't really set out to draw anything in particular."
    n "{vspace=90}Tal vez porque no me propuse dibujar algo en particular."

# game/script-a4-rin.rpy:1264
translate es a4_rin_wavelength_3a273586:

    # n "I just wanted to kill some time, so the drawing became exactly like I am."
    n "Tan solo quería matar un poco de tiempo, así que el dibujo se convirtió en exactamente lo que soy."

# game/script-a4-rin.rpy:1266
translate es a4_rin_wavelength_fb104aa5:

    # n "Without a direction to go to."
    n "Sin una dirección adónde ir."

# game/script-a4-rin.rpy:1268
translate es a4_rin_wavelength_bdb42e04:

    # n "{vspace=60}It'd be easier if I had some special talent, like Rin."
    n "{vspace=60}Sería más fácil si tuviera algún talento especial, como Rin."

# game/script-a4-rin.rpy:1270
translate es a4_rin_wavelength_e6acf50c:

    # n "She has it easy."
    n "Lo tiene fácil."

# game/script-a4-rin.rpy:1272
translate es a4_rin_wavelength_103f5a47:

    # n "It makes me kind of jealous."
    n "Me pone un poco celoso."

# game/script-a4-rin.rpy:1274
translate es a4_rin_wavelength_f33578dd:

    # n "It pisses me off that she herself can't seem to be happy about it."
    n "Me da rabia que ella misma parece no poder estar contenta de ello."

# game/script-a4-rin.rpy:1300
translate es a4_rin_wavelength_d41d8cd9_3:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1285
translate es a4_rin_wavelength_0e057b53:

    # mu "Aaaand… time!"
    mu "Yyyy… ¡tiempo!"

# game/script-a4-rin.rpy:1287
translate es a4_rin_wavelength_e5356afe:

    # "Mutou's call for the end of the exam draws groans of displeasure from half the class."
    "El llamado de Mutou para el fin del examen extrae gemidos de descontento de la mitad del grupo."

# game/script-a4-rin.rpy:1289
translate es a4_rin_wavelength_3b0137b6:

    # "I don't blame them, the exam was kinda tricky."
    "No los culpo, el examen era algo difícil."

# game/script-a4-rin.rpy:1291
translate es a4_rin_wavelength_d8faab67:

    # "Mutou expects a lot from our class, even though he's not strict at all. I guess he'd like all of us to become scientists."
    "Mutou espera mucho de mi grupo, aun si no es para nada estricto. Supongo que le gustaría que todos nosotros fuésemos científicos."

# game/script-a4-rin.rpy:1296
translate es a4_rin_wavelength_9586a0ba:

    # mu "Put down your pencils and turn in your papers please."
    mu "Dejen sus lápices y entreguen sus hojas, por favor."

# game/script-a4-rin.rpy:1298
translate es a4_rin_wavelength_1457f049:

    # "The biggest groan comes from the desk to my side."
    "El quejido más grande viene del banco a mi lado."

# game/script-a4-rin.rpy:1313
translate es a4_rin_wavelength_eb0752fd:

    # "Misha's despair is almost tangible."
    "La desesperanza de Misha es casi tangible."

# game/script-a4-rin.rpy:1315
translate es a4_rin_wavelength_a623fe97:

    # "The dark aura of lost hope emanating from her seat makes me simultaneously frightened of and sympathetic to her."
    "La oscura aura de ilusiones perdidas emanando de su asiento me hace sentir simultáneamente miedo y compasión por ella."

# game/script-a4-rin.rpy:1320
translate es a4_rin_wavelength_7a348718:

    # mu "Now then, there should be homeroom before you are free, but I only have a few announcements to make so this should be over quickly…"
    mu "Y bien, debería haber una sesión de aula antes de que estén libres, pero solo tengo unos anuncios que hacer así que esto debería de acabar rápido…"

# game/script-a4-rin.rpy:1322
translate es a4_rin_wavelength_91fd800a:

    # "His announcements are never important, so I listen to him only with one ear."
    "Sus anuncios nunca son importantes, así que solo lo escucho con un oído."

# game/script-a4-rin.rpy:1324
translate es a4_rin_wavelength_cb89b0f0:

    # "Misha seems to be too down in the dumps to even pretend attentiveness."
    "Misha parece estar demasiado deprimida como para siquiera pretender prestar atención."

# game/script-a4-rin.rpy:1326
translate es a4_rin_wavelength_239b2154:

    # "She slumps her head against the desktop, looking stricken."
    "Deja caer su cabeza contra el escritorio, viéndose afligida."

# game/script-a4-rin.rpy:1328
translate es a4_rin_wavelength_273b3860:

    # hi "Cheer up, Misha."
    hi "Alégrate, Misha."

# game/script-a4-rin.rpy:1330
translate es a4_rin_wavelength_5729ea2e:

    # hi "It's vacation! Don't worry about the test."
    hi "¡Son vacaciones! No te preocupes por el examen."

# game/script-a4-rin.rpy:1335
translate es a4_rin_wavelength_f5ce562e:

    # mi "Thanks, Hicchan."
    mi "Gracias, Hicchan."

# game/script-a4-rin.rpy:1337
translate es a4_rin_wavelength_b6fb34fa:

    # "Her frown becomes a small smile, and a sparkle of excitement lights in her eyes."
    "Su mohín se convierte en una pequeña sonrisa, y una chispa de entusiasmo ilumina sus ojos."

# game/script-a4-rin.rpy:1342
translate es a4_rin_wavelength_6d2eb729:

    # mi "What're you going to do over your summer vacation, Hicchan?"
    mi "¿Qué vas a hacer en tus vacaciones de verano, Hicchan?"

# game/script-a4-rin.rpy:1347
translate es a4_rin_wavelength_a16a5be9:

    # mi "I'm going to Shicchan's place, they have this awesome and super cool mansion! I'm so excited~!"
    mi "Yo voy a ir a la casa de Shicchan, ¡tienen esta mansión asombrosa y supergenial! ¡Estoy tan emocionada~!"

# game/script-a4-rin.rpy:1352
translate es a4_rin_wavelength_6b4afc3d:

    # mi "I'm sure it'll be the bestest summer vacation ever~!"
    mi "¡Estoy segura de que serán las más mejores vacaciones de todas~!"

# game/script-a4-rin.rpy:1354
translate es a4_rin_wavelength_a1a6f5e9:

    # "She seems to have forgotten all about her misery in a few seconds and bounces up and down on her seat as if to pump up her excitement."
    "Parece haber olvidado todo sobre su miseria en unos cuantos segundos y rebota de arriba a abajo en su asiento como si inflara su emoción."

# game/script-a4-rin.rpy:1356
translate es a4_rin_wavelength_b725356d:

    # hi "I don't really have any plans, I guess…"
    hi "En realidad no tengo ningún plan, supongo…"

# game/script-a4-rin.rpy:1361
translate es a4_rin_wavelength_afb19d07:

    # mi "Is that so~? Maybe you should—"
    mi "¿Es en serio~? Tal vez deberías—"

# game/script-a4-rin.rpy:1366
translate es a4_rin_wavelength_4c5bd624:

    # "A finger tapping her shoulder steals Misha's attention away from me."
    "Un dedo golpeteando su hombro roba la atención de Misha en mí."

# game/script-a4-rin.rpy:1371
translate es a4_rin_wavelength_9e779258:

    # "Shizune points to Mutou, who is expectantly looking back at the two of them."
    "Shizune apunta a Mutou, quien está mirándolas a las dos con expectación."

# game/script-a4-rin.rpy:1376
translate es a4_rin_wavelength_b24d56ea:

    # mi "Oops! Sorry, Shicchan, I didn't notice teacher finished already, ehehe~."
    mi "¡Ups! Perdón, Shicchan, no me di cuenta de que el maestro ya había terminado, ejeje~."

# game/script-a4-rin.rpy:1378
translate es a4_rin_wavelength_6c3c9bf1:

    # "She clears her throat and takes a deep breath…"
    "Ella aclara su garganta y toma aire con profundidad…"

# game/script-a4-rin.rpy:1384
translate es a4_rin_wavelength_745cb1ff:

    # mi "Stand!"
    mi "¡De pie!"

# game/script-a4-rin.rpy:1386
translate es a4_rin_wavelength_6fe3f787:

    # "I stand up with everyone."
    "Me pongo de pie con todos los demás."

# game/script-a4-rin.rpy:1411
translate es a4_rin_wavelength_d41d8cd9_4:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1393
translate es a4_rin_wavelength_6cc7d4ef:

    # n "{vspace=60}Since I came here, I've always wondered about something."
    n "{vspace=60}Desde que vine aquí, siempre me he preguntado algo."

# game/script-a4-rin.rpy:1395
translate es a4_rin_wavelength_e7fa89db:

    # n "What do the wheelchair-bound students think about this daily tradition, being unable to do it 'properly?'"
    n "¿Qué piensan los estudiantes en sillas de ruedas sobre esta tradición, siendo incapaces de seguirla “apropiadamente”?"

# game/script-a4-rin.rpy:1397
translate es a4_rin_wavelength_26d00bbf:

    # n "Is it a faux pas to keep to this tradition in a place that bypasses many others for convenience?"
    n "¿Es un tropiezo mantener esta tradición en un lugar que evita muchas otras por conveniencia?"

# game/script-a4-rin.rpy:1399
translate es a4_rin_wavelength_19bacfc2:

    # n "Even though I never asked anyone, during these short weeks here I've come to the conclusion that they definitely are not insulted."
    n "Aunque nunca le pregunté a nadie, durante estas cortas semanas aquí he llegado a la conclusión de que ellos definitivamente no se sienten insultados."

# game/script-a4-rin.rpy:1401
translate es a4_rin_wavelength_c4785b31:

    # n "They understand."
    n "Ellos entienden."

# game/script-a4-rin.rpy:1403
translate es a4_rin_wavelength_ef0629f8:

    # n "That's what I like about this school. Nobody is too uptight about anything, everyone is so… considerate and understanding of each other."
    n "Eso es lo que me gusta de esta escuela. Nadie es demasiado estirado sobre algo, todos son tan… considerados y comprensibles con los demás."

# game/script-a4-rin.rpy:1407
translate es a4_rin_wavelength_bd48d49c:

    # n "{vspace=60}I wish the whole world could be like this."
    n "{vspace=60}Desearía que el mundo entero fuera así."

# game/script-a4-rin.rpy:1432
translate es a4_rin_wavelength_d41d8cd9_5:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1417
translate es a4_rin_wavelength_7b28a49b:

    # mi "Booooow!"
    mi "¡Reevereeenciaaa!"

# game/script-a4-rin.rpy:1425
translate es a4_rin_wavelength_6b23919b:

    # "I turn the page slowly, listening to the rustling sound the paper makes when my fingers grasp it."
    "Le doy vuelta a la página con lentitud, escuchando el crujir que el papel hace cuando mis dedos la sujetan."

# game/script-a4-rin.rpy:1450
translate es a4_rin_wavelength_d41d8cd9_6:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1432
translate es a4_rin_wavelength_86da4edd:

    # n "{vspace=30}I'm restless."
    n "{vspace=30}Me siento inquieto."

# game/script-a4-rin.rpy:1434
translate es a4_rin_wavelength_675b692c:

    # n "It's the summer vacation."
    n "Son las vacaciones de verano."

# game/script-a4-rin.rpy:1436
translate es a4_rin_wavelength_411e7189:

    # n "No class, no homework, no art club meetings. Just free time to spend however I want."
    n "No hay clases, ni tareas, ni reuniones del club de arte. Solo tiempo libre para pasar como me dé la gana."

# game/script-a4-rin.rpy:1438
translate es a4_rin_wavelength_d7334c86:

    # n "It doesn't feel like anything."
    n "No se siente como nada."

# game/script-a4-rin.rpy:1440
translate es a4_rin_wavelength_04741731:

    # n "I tried to cheer up Misha, but I'm not feeling too cheery myself."
    n "Traté de animar a Misha, pero yo mismo no me siento muy animado."

# game/script-a4-rin.rpy:1442
translate es a4_rin_wavelength_bc409793:

    # n "To be honest, the free time is intimidating. It reminds me of the hospital and the long, meaningless days that had to be filled somehow."
    n "Para ser honesto, el tiempo libre es intimidante. Me recuerda al hospital y los largos días sin sentido que tenían que ser llenados de algún modo."

# game/script-a4-rin.rpy:1444
translate es a4_rin_wavelength_965a7f41:

    # n "The only difference is that there I was bound to the ward, guarded by the Cerberus-like nurses."
    n "La única diferencia es que allá estaba atado al ala del hospital, vigilado por enfermeras que parecían Cerberos."

# game/script-a4-rin.rpy:1446
translate es a4_rin_wavelength_12f67585:

    # n "Reading was a good solution back then, but the thought of spending my summer vacation reading books feels… nerdy."
    n "Leer fue una buena solución en ese entonces, pero el pensar en pasar mis vacaciones de verano leyendo se siente… ñoño."

# game/script-a4-rin.rpy:1448
translate es a4_rin_wavelength_a98c4e1c:

    # n "That has nothing to do with the fact that I'm reading even now… I'm just killing time and trying to fight my anxiety."
    n "Eso no tiene nada que ver con el hecho de que estoy leyendo incluso ahora… Solo estoy matando el tiempo y tratando de vencer mi ansiedad."

# game/script-a4-rin.rpy:1450
translate es a4_rin_wavelength_ac10d3df:

    # n "Besides, my mind is on other matters, stretching in too many directions to make sense of any of them."
    n "Además, mi mente está en otros asuntos, estirándose en demasiadas direcciones como para encontrarle sentido a alguno de ellos."

# game/script-a4-rin.rpy:1475
translate es a4_rin_wavelength_d41d8cd9_7:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1457
translate es a4_rin_wavelength_8860c35a:

    # "Thus, the book I've been on since Tuesday is progressing s{w=0.3}l{w=0.3}o{w=0.3}w{w=0.3}l{w=0.3}y{w=0.3}."
    "Por ende, el libro con el que he estado desde el martes va progresando l{w=0.3}e{w=0.3}n{w=0.3}t{w=0.3}o{w=0.3}."

# game/script-a4-rin.rpy:1459
translate es a4_rin_wavelength_4e28a1bf:

    # "It feels like this book is taking me longer to read than it took the author to write."
    "Se siente como si este libro me estuviese tomando más tiempo de leer que el que le tomó al autor escribirlo."

# game/script-a4-rin.rpy:1461
translate es a4_rin_wavelength_a1425505:

    # "I try to put it down for a while, then read some again, start all over from the beginning, read each page twice."
    "Intento dejarlo por un rato, luego volver a leer un poco, empezar todo desde el principio, leer cada página dos veces."

# game/script-a4-rin.rpy:1463
translate es a4_rin_wavelength_779fba11:

    # "Nothing works, I have zero concentration."
    "Nada funciona, tengo cero concentración."

# game/script-a4-rin.rpy:1465
translate es a4_rin_wavelength_3ab31011:

    # "Taking it with me just in case, I head out to get some fresh air and inspiration as to what to do."
    "Llevándolo conmigo solo por si acaso, me dirijo hacia afuera para tomar un poco de aire fresco e inspiración en cuanto a qué hacer."

# game/script-a4-rin.rpy:1470
translate es a4_rin_wavelength_f77d067f:

    # "I make my way to the quad, passing by students heading for the gates."
    "Me dirijo al patio, pasando frente a estudiantes que van al portón."

# game/script-a4-rin.rpy:1472
translate es a4_rin_wavelength_1e743d52:

    # "The hastiest ones are leaving for their homes already, judging from the luggage some are dragging with them."
    "Los más apresurados se van ya a sus casas, juzgando por las maletas que algunos arrastran consigo."

# game/script-a4-rin.rpy:1474
translate es a4_rin_wavelength_feb6c205:

    # "I guess that no matter how hospitable Yamaku is, home is still home. Still, I heard some people will be staying here over the vacation."
    "Supongo que no importa cuán hospitalario sea Yamaku, el hogar sigue siendo el hogar. Aun así, escuché que algunos se quedarán durante las vacaciones."

# game/script-a4-rin.rpy:1476
translate es a4_rin_wavelength_b8936b45:

    # "The quad is big enough for its center to be shadowless no matter how high or low the sun is."
    "El patio es lo suficientemente grande como para que su centro esté sin sombra sin importar qué tan alto o bajo se encuentre el sol."

# game/script-a4-rin.rpy:1478
translate es a4_rin_wavelength_61cf9b68:

    # "I stop in the middle and bask in the warmth."
    "Me detengo en la mitad y disfruto de la calidez."

# game/script-a4-rin.rpy:1480
translate es a4_rin_wavelength_62b8076d:

    # "The brightness makes me squint my eyes when I look towards the main building."
    "El brillo me hace entrecerrar los ojos cuando veo hacia el edificio principal."

# game/script-a4-rin.rpy:1482
translate es a4_rin_wavelength_56243082:

    # "It looks all but abandoned already."
    "Parece ya estar abandonado por completo."

# game/script-a4-rin.rpy:1484
translate es a4_rin_wavelength_ebb9ceb1:

    # "Yuuko wasn't at work today, so the next time I can get books from the school library is after vacation."
    "Yuuko no estaba en el trabajo hoy, así que la siguiente vez que pueda sacar libros de la biblioteca es después de las vacaciones."

# game/script-a4-rin.rpy:1486
translate es a4_rin_wavelength_62d933f8:

    # "There is a public library somewhere, I'm sure, but I'm feeling too lethargic to find out where it is."
    "Hay una biblioteca pública en alguna parte, estoy seguro, pero me siento demasiado aletargado como para averiguar dónde está."

# game/script-a4-rin.rpy:1491
translate es a4_rin_wavelength_b0a8f307:

    # "The hall is equally dead so I have to content myself with returning to the dorms, ending my leisurely walk sooner than I expected."
    "El vestíbulo está igualmente carente de vida así que tengo que conformarme con regresar a los dormitorios, terminando mi paseo antes de lo que esperaba."

# game/script-a4-rin.rpy:1493
translate es a4_rin_wavelength_6b043fbf:

    # "Then again, I wasn't quite sure what I was expecting in the first place."
    "Por otro lado, no estaba muy seguro de qué estaba esperando en primer lugar."

# game/script-a4-rin.rpy:1498
translate es a4_rin_wavelength_460dd3b4:

    # "On a moment's impulse I enter the girls' dorm to see if Rin or Emi are there."
    "En un impulso del momento entro al dormitorio de chicas para ver si Rin o Emi están ahí."

# game/script-a4-rin.rpy:1500
translate es a4_rin_wavelength_d9d89bdb:

    # "Neither is, so I go back to my own room to dwell on my lethargy."
    "Ninguna está, así que regreso a mi propia habitación para estar en mi letargo."

# game/script-a4-rin.rpy:1528
translate es a4_rin_wavelength_d41d8cd9_8:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1510
translate es a4_rin_wavelength_e38a9633:

    # n "{vspace=60}I should talk things through with Rin."
    n "{vspace=60}Tengo que hablar a fondo con Rin."

# game/script-a4-rin.rpy:1512
translate es a4_rin_wavelength_4a5e72e3:

    # n "She really bothers me."
    n "Ella en verdad me molesta."

# game/script-a4-rin.rpy:1514
translate es a4_rin_wavelength_809c916d:

    # n "{vspace=60}Defying the conceptual equivalent of gravity, she balances on the thin line zigzagging between insanity, incomprehensibility and instability."
    n "{vspace=60}Desafiando el equivalente conceptual de la gravedad, ella se balancea sobre la delgada línea que zigzaguea entre la locura, lo incomprensible y la inestabilidad."

# game/script-a4-rin.rpy:1516
translate es a4_rin_wavelength_5aad4976:

    # n "Rin affects me too. She challenges me in ways that I didn't know… or more accurately, didn't hope existed."
    n "Rin me afecta también. Me reta en maneras que yo no sabía… o más correctamente, no esperaba que existieran."

# game/script-a4-rin.rpy:1518
translate es a4_rin_wavelength_5efc5572:

    # n "{vspace=60}I've started to wonder whether these feelings are really love, or I was just fooling myself."
    n "{vspace=60}He comenzado a preguntarme si estos sentimientos son realmente amor, o si estaba engañándome a mí mismo."

# game/script-a4-rin.rpy:1520
translate es a4_rin_wavelength_175d0cbc:

    # n "Surely, it would be insanity to consider that?"
    n "Seguramente, ¿sería una locura considerar eso?"

# game/script-a4-rin.rpy:1522
translate es a4_rin_wavelength_d46739d0:

    # nvl clear
    # n "{vspace=60}For the rest of the day, Rin, the hospital, Yamaku and vacation swirl through my head."
    nvl clear
    n "{vspace=60}Por el resto del día, Rin, el hospital, Yamaku y las vacaciones se arremolinan por mi cabeza."

# game/script-a4-rin.rpy:1526
translate es a4_rin_wavelength_f0f8a830:

    # n "{vspace=30}I can't concentrate even on concentrating."
    n "{vspace=30}No puedo concentrarme ni en concentrarme."

# game/script-a4-rin.rpy:1528
translate es a4_rin_wavelength_71457912:

    # n "{vspace=30}Thoughts seem to come and go haphazardly, fragmented into too-small pieces of cognition."
    n "{vspace=30}Los pensamientos parecen ir y venir sin orden ni concierto, fragmentados en pedazos de cognición demasiado pequeños."

# game/script-a4-rin.rpy:1530
translate es a4_rin_wavelength_58a290e8:

    # n "{vspace=30}I pick up the book and manage to read a hundred pages, but I'm sure by tomorrow I'll have no recollection of what happened in the story."
    n "{vspace=30}Levanto el libro y logro leer cien páginas, pero estoy seguro de que para mañana no tendré memoria de qué sucedió en la historia."

# game/script-a4-rin.rpy:1532
translate es a4_rin_wavelength_3a7fa6e0:

    # n "{vspace=30}I try to clean up my room, but even that proves to be too bothersome, too time-consuming and requiring too much attention to detail."
    n "{vspace=30}Trato de limpiar mi cuarto, pero incluso eso resulta ser demasiado molesto, consumiendo demasiado tiempo y requiriendo demasiada atención al detalle."

# game/script-a4-rin.rpy:1534
translate es a4_rin_wavelength_554e90c4:

    # n "It's usually like this. When you have 'nothing to do,' you do nothing even if you could."
    n "Usualmente es así. Cuando no tienes “nada que hacer”, no haces nada incluso si pudieras."

# game/script-a4-rin.rpy:1560
translate es a4_rin_wavelength_d41d8cd9_9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1545
translate es a4_rin_wavelength_61247b6a:

    # "As expected, mom calls me and I end up promising to see if I can get a train ticket for tomorrow, or failing that, the day after."
    "Como es de esperarse, mi mamá llama y termino prometiéndole que veré si puedo conseguir un boleto de tren para mañana, o de no poder, para el día siguiente."

# game/script-a4-rin.rpy:1569
translate es a4_rin_wavelength_d41d8cd9_10:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1566
translate es a4_rin_wavelength_89cff40f:

    # n "{vspace=60}Maybe I'll go downtown tomorrow anyway. I could do some shopping or something."
    n "{vspace=60}Quizás vaya mañana al centro de todos modos. Podría hacer un poco de compras o algo así."

# game/script-a4-rin.rpy:1568
translate es a4_rin_wavelength_b24c0736:

    # n "It's not that I need anything, but maybe there are summer sales, and I could pick up… something."
    n "No es que necesite algo, pero tal vez haya ventas de verano, y podría conseguir… algo."

# game/script-a4-rin.rpy:1572
translate es a4_rin_wavelength_5e03b606:

    # n "{vspace=60}…Why am I trying to force myself?"
    n "{vspace=60}…¿Por qué estoy tratando de forzarme a mí mismo?"

# game/script-a4-rin.rpy:1574
translate es a4_rin_wavelength_ae2b6e01:

    # n "Before, I was content with having nothing to do, save for kicking the ball every now and then at the field."
    n "Antes, estaba conforme sin tener nada que hacer, aparte de patear la pelota de vez en cuando en el campo."

# game/script-a4-rin.rpy:1576
translate es a4_rin_wavelength_b4d7f7eb:

    # n "Now it seems that I can't settle down at all."
    n "Ahora parece que no puedo calmarme en absoluto."

# game/script-a4-rin.rpy:1578
translate es a4_rin_wavelength_dce4c6f8:

    # n "{vspace=30}Is it because I have changed, or because my world has changed?"
    n "{vspace=30}¿Acaso es porque he cambiado, o porque mi mundo ha cambiado?"

# game/script-a4-rin.rpy:1601
translate es a4_rin_wavelength_d41d8cd9_11:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1586
translate es a4_rin_wavelength_2d1d7ef9:

    # "By eleven, the darkness bids me to sleep."
    "A las once, la oscuridad me manda a dormir."

# game/script-a4-rin.rpy:1597
translate es a4_rin_wavelength_d76e4f6d:

    # "The medication bottles are innocuously arranged on my night table, not at all beckoning, rather pointedly reminding me of the reality instead."
    "Los frascos con medicamentos están acomodados de manera inocua sobre mi mesa de noche, para nada llamativos, más bien recordándome puntualmente la realidad."

# game/script-a4-rin.rpy:1599
translate es a4_rin_wavelength_b9c7e988:

    # "It's evening so I have to open three bottles, extract one large oval-shaped one, two small round ones and one large flat that has to be cut into half, close the bottles and chug down the medications with a chaser of fresh tap water."
    "Es de noche, así que tengo que abrir tres frascos, extraer una grande de forma ovalada, dos pequeñas redondas y una grande y plana que tiene que ser cortada a la mitad, cerrar los frascos y pasar los medicamentos con un trago de fresca agua del grifo."

# game/script-a4-rin.rpy:1608
translate es a4_rin_wavelength_adcc9949:

    # "The water tastes metallic on my tongue."
    "El agua tiene sabor metálico en mi lengua."

# game/script-a4-rin.rpy:1610
translate es a4_rin_wavelength_675cc894:

    # "I swallow it along with the pills anyway and head to the bathroom."
    "La trago junto con las píldoras de todos modos y me dirijo al baño."

# game/script-a4-rin.rpy:1615
translate es a4_rin_wavelength_3e442dfc:

    # "The mindless job of brushing my teeth is fit for trying to sort my thoughts."
    "La tarea mecánica de cepillarme los dientes es apropiada para tratar de organizar mis pensamientos."

# game/script-a4-rin.rpy:1617
translate es a4_rin_wavelength_98aa888b:

    # "One emerges from the mass, clearly rising above the others."
    "Uno emerge de la masa, claramente levantándose sobre los demás."

# game/script-a4-rin.rpy:1652
translate es a4_rin_wavelength_d41d8cd9_12:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1622
translate es a4_rin_wavelength_fe43f2c2:

    # n "{vspace=270}I want to see Rin."
    n "{vspace=270}Quiero ver a Rin."

# game/script-a4-rin.rpy:1624
translate es a4_rin_wavelength_950439c0:

    # n "I can't let my outburst of anger be the last thing between us before the vacation."
    n "No puedo dejar que mi explosión de enojo sea lo último entre nosotros antes de las vacaciones."

# game/script-a4-rin.rpy:1660
translate es a4_rin_wavelength_d41d8cd9_13:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1634
translate es a4_rin_wavelength_4215315d:

    # n "{vspace=240}I have to see her, tomorrow."
    n "{vspace=240}Tengo que verla, mañana."

# game/script-a4-rin.rpy:1636
translate es a4_rin_wavelength_9f609591:

    # n "Sleep overcomes my confused mind with more ease than it should."
    n "El sueño vence mi mente confundida con más facilidad de la que debería."

# game/script-a4-rin.rpy:1672
translate es a4_rin_wavelength_d41d8cd9_14:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:1659
translate es a4_rin_blue_period_8d8a2921:

    # "Rain is falling on my summer vacation like an uncountable number of small bad omens."
    "Lluvia cae sobre mis vacaciones de verano como un incontable número de pequeños malos augurios."

# game/script-a4-rin.rpy:1661
translate es a4_rin_blue_period_d0255cc1:

    # "Luckily I'm not superstitious, but the bad weather makes me downcast too."
    "Por suerte no soy supersticioso, pero el mal tiempo me hace sentir apesadumbrado, también."

# game/script-a4-rin.rpy:1663
translate es a4_rin_blue_period_198a7d03:

    # "It's been like this since morning and there is no end in sight."
    "Ha sido así desde la mañana y no se le ve el fin."

# game/script-a4-rin.rpy:1665
translate es a4_rin_blue_period_86398206:

    # "An impenetrable gray mass of clouds shadows the sky as much as it shadows my mood."
    "Una impenetrable masa gris de nubes ensombrece el cielo tanto como ensombrece mi ánimo."

# game/script-a4-rin.rpy:1667
translate es a4_rin_blue_period_28e7b6ce:

    # "In a bout of defiance, I finished cleaning up this morning, but after that was done I ended up staring out of the window in hope of the weather clearing."
    "En un arranque de despecho, terminé de limpiar esta mañana, pero después de que eso estuviese hecho me quedé mirando fuera de la ventana con esperanzas de que el tiempo clareara."

# game/script-a4-rin.rpy:1669
translate es a4_rin_blue_period_39a3b2a9:

    # "The incessant drumming of rainfall against the roof and the pavement is mesmerizing, a droning background noise to lose your mind into."
    "El tamborileo incesante de las gotas de lluvia contra el tejado y el pavimento es hipnotizante, un zumbido como ruido de fondo en el cual perder tu mente."

# game/script-a4-rin.rpy:1671
translate es a4_rin_blue_period_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:1673
translate es a4_rin_blue_period_604cb789:

    # "… …"
    "… …"

# game/script-a4-rin.rpy:1680
translate es a4_rin_blue_period_96f490b9:

    # "This won't do, I have to get a move on."
    "Esto no servirá, tengo que moverme."

# game/script-a4-rin.rpy:1682
translate es a4_rin_blue_period_efd0c57f:

    # "Should I pack now, or later?"
    "¿Debería empacar ahora o más tarde?"

# game/script-a4-rin.rpy:1689
translate es a4_rin_blue_period_0707f71c:

    # "I decide on the latter and make my way outside, pausing briefly at Kenji's door to listen to the odd clunking sounds from the other side."
    "Me decido por la segunda y me dirijo hacia afuera, haciendo una pausa breve frente a la puerta de Kenji para escuchar al extraño sonido de golpes sordos desde el otro lado de la puerta."

# game/script-a4-rin.rpy:1693
translate es a4_rin_blue_period_74a05ef3:

    # "I don't dare to knock, out of the fear of finding out what he is doing."
    "No me atrevo a tocar, por miedo de descubrir qué es lo que está haciendo."

# game/script-a4-rin.rpy:1701
translate es a4_rin_blue_period_154c5f79:

    # "Braving the rain from under my trusty umbrella, I cross the space to the girls' dorm."
    "Enfrentándome a la lluvia desde debajo de un confiable paraguas, cruzo el espacio hacia el dormitorio de chicas."

# game/script-a4-rin.rpy:1710
translate es a4_rin_blue_period_c939dd20:

    # "Knocking on Rin's door yields no answer, but the door behind me opens instead."
    "Llamar a la puerta de Rin no da respuesta, pero la puerta detrás de mí se abre en su lugar."

# game/script-a4-rin.rpy:1724
translate es a4_rin_blue_period_69ba2711:

    # emi "Hisao? Hi!"
    emi "¿Hisao? ¡Qué tal!"

# game/script-a4-rin.rpy:1729
translate es a4_rin_blue_period_8fe0d634:

    # emi "Terrible weather. I even missed my morning jog."
    emi "Un tiempo terrible. Hasta me perdí mi trote matutino."

# game/script-a4-rin.rpy:1731
translate es a4_rin_blue_period_bec77409:

    # "She frowns, but I would be glad if I was her. Emi's morning jogs are anything but leisurely."
    "Frunce el entrecejo, pero estaría contento si fuese ella. Los trotes matutinos de Emi son cualquier cosa menos relajados."

# game/script-a4-rin.rpy:1733
translate es a4_rin_blue_period_46e17644:

    # hi "Oh, hi, I was—"
    hi "Oh, qué tal, estaba—"

# game/script-a4-rin.rpy:1738
translate es a4_rin_blue_period_6c727612:

    # emi "If you're looking for Rin, I don't think she is there."
    emi "Si estás buscando a Rin, no creo que esté ahí."

# game/script-a4-rin.rpy:1740
translate es a4_rin_blue_period_6c0eb2c4:

    # hi "Have you seen her recently?"
    hi "¿La has visto recientemente?"

# game/script-a4-rin.rpy:1745
translate es a4_rin_blue_period_7028b2c1:

    # emi "Yeah, just this morning when I woke her up."
    emi "Sí, apenas esta mañana cuando la desperté."

# game/script-a4-rin.rpy:1747
translate es a4_rin_blue_period_db78729a:

    # "The mention of waking up makes Emi yawn like a cat, and makes me feel silly."
    "La mención de despertar hace a Emi bostezar como un gato, y me hace sentir tonto."

# game/script-a4-rin.rpy:1749
translate es a4_rin_blue_period_45509f61:

    # "Of course she has seen Rin. Emi wakes her up and helps her get dressed on most mornings, even makes her lunch boxes every now and then."
    "Por supuesto que ha visto a Rin. Emi la despierta y la ayuda a vestirse en la mayoría de las mañanas, incluso hace sus almuerzos de vez en cuando."

# game/script-a4-rin.rpy:1751
translate es a4_rin_blue_period_cb6e2bc7:

    # "They are like sisters, even though they seem to have nothing in common."
    "Ellas son como hermanas, aunque no parecen tener nada en común."

# game/script-a4-rin.rpy:1754
translate es a4_rin_blue_period_78b4f0b5:

    # "I wonder which one is the elder sister? Probably Emi, against all odds."
    "¿Me pregunto quién será la hermana mayor? Probablemente Emi, contra toda posibilidad."

# game/script-a4-rin.rpy:1756
translate es a4_rin_blue_period_7da774f5:

    # "She is really diligent, even though she gives the feeling of someone who would be a total airhead."
    "Ella es bastante diligente, aun si da la impresión de ser alguien que sería toda una cabeza hueca."

# game/script-a4-rin.rpy:1758
translate es a4_rin_blue_period_2f5a5bea:

    # "Why does it feel odd that she is so dutiful under that cheery grin of hers?"
    "¿Por qué se siente tan extraño que ella sea tan solícita bajo esa sonrisa alegre suya?"

# game/script-a4-rin.rpy:1763
translate es a4_rin_blue_period_d064ef9b:

    # emi "She left for the gallery a few hours ago… hey, are you listening?"
    emi "Se fue a la galería hace algunas horas… oye, ¿estás escuchando?"

# game/script-a4-rin.rpy:1765
translate es a4_rin_blue_period_64dbce1d:

    # "Maybe I'm making a funny face or something, since Emi tilts her face quizzically, looking at me with her eyes round and inquisitive."
    "Tal vez estoy haciendo una cara graciosa o algo así, ya que Emi ladea la cara con curiosidad, mirándome con sus ojos redondos e inquisitivos."

# game/script-a4-rin.rpy:1770
translate es a4_rin_blue_period_19972527:

    # emi "Hmm?"
    emi "¿Hmm?"

# game/script-a4-rin.rpy:1772
translate es a4_rin_blue_period_e13a2526:

    # "Her innocent face seems to request my attention."
    "Su inocente rostro parece pedir mi atención."

# game/script-a4-rin.rpy:1774
translate es a4_rin_blue_period_58c0300f:

    # hi "Yeah, I'm listening…"
    hi "Sí, estoy escuchando."

# game/script-a4-rin.rpy:1779
translate es a4_rin_blue_period_4f68b1dd:

    # emi "Can I ask you a question?"
    emi "¿Puedo hacerte una pregunta?"

# game/script-a4-rin.rpy:1781
translate es a4_rin_blue_period_2541be0f:

    # hi "Yeah, of course."
    hi "Sí, claro."

# game/script-a4-rin.rpy:1786
translate es a4_rin_blue_period_0ce6898f:

    # "She furrows her brow, licking her lips as if to prepare for something."
    "Ella frunce el ceño y se lame los labios como si se preparara para algo."

# game/script-a4-rin.rpy:1791
translate es a4_rin_blue_period_9474eb39:

    # emi "Why do you care so much about Rin?"
    emi "¿Por qué te preocupas tanto por Rin?"

# game/script-a4-rin.rpy:1796
translate es a4_rin_blue_period_c748bc35:

    # emi "I mean, you probably hang around her more than I do, and we even slept in the same bed sometimes until, er, lately."
    emi "Digo, tú probablemente te juntas con ella más que yo, y nosotras incluso dormimos en la misma cama algunas veces hasta, bueno, recientemente."

# game/script-a4-rin.rpy:1798
translate es a4_rin_blue_period_1d3b01d3:

    # hi "After she banned you because you ravaged her hair?"
    hi "¿Hasta que te expulsó porque devastaste su cabello?"

# game/script-a4-rin.rpy:1803
translate es a4_rin_blue_period_3cea227a:

    # "A shock of horror widens Emi's eyes at least twofold, making them seem even more saucer-like than usual, while a healthy blush rises on her cheeks and ears."
    "Un choque de horror ensancha los ojos de Emi por lo menos al doble, haciéndolos ver aún más como platillos que lo usual, mientras un saludable rubor se alza en sus mejillas y orejas."

# game/script-a4-rin.rpy:1808
translate es a4_rin_blue_period_4814b0e8:

    # emi "She told?! Ohhh… I'm going to strangle that Rin or something other horrible…"
    emi "¡¿Te dijo?! Ohhh… Voy a estrangular a esa Rin o algo otro horrible…"

# game/script-a4-rin.rpy:1810
translate es a4_rin_blue_period_c0bbe1c9:

    # "I hold back my laughter, lest she direct her disdain at me."
    "Contengo mi risa, no sea que dirija su desdén hacia mí."

# game/script-a4-rin.rpy:1815
translate es a4_rin_blue_period_2f42f19e:

    # "Emi recuperates quickly from the embarrassment and seems to forgive Rin in the same instant, getting her focus back on me."
    "Emi se recupera rápidamente de su vergüenza y parece perdonar a Rin al mismo instante, poniendo su atención de vuelta sobre mí."

# game/script-a4-rin.rpy:1820
translate es a4_rin_blue_period_2d8d6cf8:

    # emi "Anyway, are you in love with her or something?"
    emi "De cualquier modo, ¿estás enamorado de ella o algo?"

# game/script-a4-rin.rpy:1822
translate es a4_rin_blue_period_d887d4a8:

    # "Uh oh. This really feels like an elder sister questioning a suitor. Emi is kinda nosy, and not in a good happy way, if there even is one to begin with."
    "Oh oh. Esto realmente se siente como una hermana mayor interrogando a un pretendiente. Emi es un tanto entrometida, y no de una manera buena y alegre, si es que siquiera haya una para empezar."

# game/script-a4-rin.rpy:1824
translate es a4_rin_blue_period_cc0d7719:

    # "She'd make a good partner for Misha, to be honest. The horror."
    "Ella haría una buena pareja con Misha, para ser honesto. Qué horror."

# game/script-a4-rin.rpy:1826
translate es a4_rin_blue_period_6ce201a3:

    # hi "That's already your second question, so I don't think I have to answer."
    hi "Esa ya es tu segunda pregunta, así que no creo que tenga que responder."

# game/script-a4-rin.rpy:1828
translate es a4_rin_blue_period_1e558655:

    # "I try to conjure up a front made of pure, crystallized cool and uninvolvement."
    "Trato de conjurar una fachada hecha de pura y cristalizada frialdad y carencia de implicación."

# game/script-a4-rin.rpy:1830
translate es a4_rin_blue_period_16e3c11f:

    # "I wonder whether I manage to fool even myself."
    "Me pregunto si siquiera logro engañarme a mí mismo."

# game/script-a4-rin.rpy:1835
translate es a4_rin_blue_period_f8ba3511:

    # "At least Emi is waggling her eyebrows dangerously, with a nasty smirk on her lips."
    "Por lo menos Emi está meneando sus cejas con peligrosidad, con una desagradable sonrisa en sus labios."

# game/script-a4-rin.rpy:1837
translate es a4_rin_blue_period_5e48eea3:

    # emi "Is that a yes?"
    emi "¿Es eso un sí?"

# game/script-a4-rin.rpy:1839
translate es a4_rin_blue_period_d5dfe2a6:

    # hi "No, it's not a yes."
    hi "No, no es un sí."

# game/script-a4-rin.rpy:1844
translate es a4_rin_blue_period_9a9d7040:

    # "Obviously unsatisfied at my refusal to answer her way-too-intimate question, she has enough sense to back off."
    "Obviamente insatisfecha con mi rechazo a responder su pregunta demasiado íntima, ella tiene la suficiente sensatez de retroceder."

# game/script-a4-rin.rpy:1849
translate es a4_rin_blue_period_37c97d73:

    # "Doesn't stop her from sticking out her tongue at me like a kid, and giggling again."
    "No la detiene de sacarme la lengua como un niño, y reír de nuevo."

# game/script-a4-rin.rpy:1854
translate es a4_rin_blue_period_1c1021da:

    # emi "If that's your answer, I don't think I have to talk with you any more."
    emi "Si esa es tu respuesta, no creo que tenga que hablar más contigo."

# game/script-a4-rin.rpy:1856
translate es a4_rin_blue_period_c3144138:

    # "It's easy to see that she's not really angry."
    "Es fácil ver que ella en realidad no está enojada."

# game/script-a4-rin.rpy:1861
translate es a4_rin_blue_period_a32de8d5:

    # emi "Besides, I have to go pack now. Mom will be worried if I miss my bus."
    emi "Además, tengo que ir a empacar. Mi mamá estará preocupada si pierdo el autobús."

# game/script-a4-rin.rpy:1863
translate es a4_rin_blue_period_23b8a84f:

    # emi "Seeya!"
    emi "¡Nos vemos!"

# game/script-a4-rin.rpy:1865
translate es a4_rin_blue_period_4f28c382:

    # hi "Yeah, bye."
    hi "Sí, hasta luego."

# game/script-a4-rin.rpy:1874
translate es a4_rin_blue_period_3fe99f78:

    # "She retreats back into her room, leaving me alone in the hallway."
    "Ella se retira a su habitación, dejándome solo en el pasillo."

# game/script-a4-rin.rpy:1876
translate es a4_rin_blue_period_044d7110:

    # "What's between me and Rin is not her business, right?"
    "Lo que hay entre Rin y yo no es asunto suyo, ¿cierto?"

# game/script-a4-rin.rpy:1878
translate es a4_rin_blue_period_7d0711e0:

    # "That's why I ended up not saying anything about our fight to Emi. Rin must have not said anything either."
    "Es por eso que terminé sin decirle nada a Emi sobre nuestra pelea. Rin tampoco debe de haber dicho algo."

# game/script-a4-rin.rpy:1880
translate es a4_rin_blue_period_23e440a1:

    # "I guess… even though they are friends, there are things they don't talk about."
    "Supongo que… aun siendo amigas, hay cosas de las que no hablan."

# game/script-a4-rin.rpy:1882
translate es a4_rin_blue_period_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:1884
translate es a4_rin_blue_period_5f51b53c:

    # "So, if Rin is at the gallery, I'd have to go all the way there."
    "Y bien, si Rin está en la galería, tendré que ir hasta allá."

# game/script-a4-rin.rpy:1886
translate es a4_rin_blue_period_5796dd87:

    # "Now that I managed to get out of my room, I suppose it's not that much of a bother to go downtown."
    "Ahora que logré salir de mi habitación, supongo que no es tanta molestia ir al centro."

# game/script-a4-rin.rpy:1888
translate es a4_rin_blue_period_35d8f7e3:

    # "I could go get a ticket, but the train back home will have to wait, at least until tomorrow."
    "Podría ir por un boleto, pero el tren de regreso a casa tendrá que esperar, al menos hasta mañana."

# game/script-a4-rin.rpy:1890
translate es a4_rin_blue_period_d5cfa695:

    # "No way I'm going to carry baggage to the train station in this rain, even if there's not that much of it."
    "De ninguna manera voy a cargar equipaje a la estación de tren en esta lluvia, aun si no es mucho."

# game/script-a4-rin.rpy:1898
translate es a4_rin_blue_period_6e23fb6d:

    # "Rain makes all outlines seem very unstable, as if they were fading away."
    "La lluvia hace todos los contornos parecer muy inestables, como si se estuviesen desvaneciendo."

# game/script-a4-rin.rpy:1900
translate es a4_rin_blue_period_6d9c9471:

    # "The townscape turns into a shapeless collection of various fuzzy tones of gray, instead of distinct forms of buildings and cars."
    "El paisaje urbano se torna una colección sin forma de varios tonos borrosos de gris, en lugar de formas distintas de edificios y autos."

# game/script-a4-rin.rpy:1902
translate es a4_rin_blue_period_4cdb6f6a:

    # "Those poor souls who are forced into the downpour try to make as much haste as they possibly can, pitying each other for their shared misery."
    "Esas pobres almas forzadas a estar en el diluvio tratan de apresurarse tanto como les sea posible, compadeciéndose mutuamente por su miseria compartida."

# game/script-a4-rin.rpy:1907
translate es a4_rin_blue_period_5e73aadb:

    # "I turn the final corner, the twenty-second corner so to say, and immediately feel stupid for being amused by my own pun."
    "Doy vuelta en la última esquina, la vigésima segunda por así decirlo, e inmediatamente me siento estúpido por divertirme con mi propio juego de palabras."

# game/script-a4-rin.rpy:1909
translate es a4_rin_blue_period_1e586a06:

    # "The door beckons me with promises of warmth."
    "La puerta me atrae con promesas de calor."

# game/script-a4-rin.rpy:1918
translate es a4_rin_blue_period_b1ffb743:

    # "The rainwater dripping from my umbrella makes interesting, almost artistic patterns on the floor."
    "El agua de lluvia goteando de mi paraguas hace interesantes y casi artísticos patrones sobre el suelo."

# game/script-a4-rin.rpy:1920
translate es a4_rin_blue_period_1a859d51:

    # "I am not wet, apart from my shoes that leave small puddles in my wake, completing the rainwater-artwork."
    "No estoy mojado, aparte de mis zapatos que dejan pequeños charcos detrás de mí, completando el arte en agua de lluvia."

# game/script-a4-rin.rpy:1926
translate es a4_rin_blue_period_976fb099:

    # "Nomiya is here too, chatting with Sae at the back of the gallery. Rin's nowhere to be seen, though."
    "Nomiya también está aquí, charlando con Sae al fondo de la galería. Sin embargo, Rin no se encuentra en ningún lado."

# game/script-a4-rin.rpy:1928
translate es a4_rin_blue_period_e769e9e0:

    # "Maybe she's upstairs."
    "Quizás esté arriba."

# game/script-a4-rin.rpy:1930
translate es a4_rin_blue_period_39a5be74:

    # "There are no customers though, which figures, considering the bucketloads of water dropping on the neck of anyone daring to brave the weather today."
    "Aunque no hay clientes, lo cual es de esperarse, considerando los cubetazos de agua cayendo en el cuello de quienquiera que se atreva a enfrentar al clima en este día."

# game/script-a4-rin.rpy:1935
translate es a4_rin_blue_period_477455a9:

    # sa "Welcome."
    sa "Bienvenido."

# game/script-a4-rin.rpy:1937
translate es a4_rin_blue_period_dbbe0fc0:

    # hi "Hello. Sorry to interrupt…"
    hi "Hola. Perdón por interrumpir…"

# game/script-a4-rin.rpy:1942
translate es a4_rin_blue_period_c387670f:

    # no "Ah, good afternoon Nakai."
    no "Ah, buenas tardes Nakai."

# game/script-a4-rin.rpy:1947
translate es a4_rin_blue_period_a80d47c7:

    # no "Came all the way here for a visit?"
    no "¿Viniste hasta acá para visitar?"

# game/script-a4-rin.rpy:1949
translate es a4_rin_blue_period_f5c395b7:

    # hi "Ah… no, I think it was just an impulse. I was around the neighborhood, shopping, and decided to stop by."
    hi "Ah… no, creo que fue solo un impulso. Estaba por el vecindario, de compras, y decidí pasar por aquí."

# game/script-a4-rin.rpy:1951
translate es a4_rin_blue_period_df253efd:

    # "My reflexive reaction is a white lie, which surprises myself."
    "Mi reacción de reflejo es una mentira blanca, lo cual me sorprende."

# game/script-a4-rin.rpy:1953
translate es a4_rin_blue_period_99490866:

    # "Maybe I just don't want to say that I came specifically to see Rin, even though that much must be obvious."
    "Tal vez solo no quiero decir que vine específicamente para ver a Rin, aun si eso debe de ser obvio."

# game/script-a4-rin.rpy:1958
translate es a4_rin_blue_period_58fbae9f:

    # sa "My, you chose a bad day for shopping. Would you like some tea to warm you up?"
    sa "Vaya, escogiste un mal día para ir de compras. ¿Gustarías una taza de té para calentarte?"

# game/script-a4-rin.rpy:1960
translate es a4_rin_blue_period_5aaebf04:

    # hi "Thank you but I'm fine, really."
    hi "Gracias, estoy bien, en serio."

# game/script-a4-rin.rpy:1962
translate es a4_rin_blue_period_e62c76ae:

    # hi "The weather could be better though. Rain on the first day of vacations is a bit depressing."
    hi "Aunque el tiempo podría ser mejor. Lluvia en el primer día de vacaciones es un poco deprimente."

# game/script-a4-rin.rpy:1968
translate es a4_rin_blue_period_f366b0d0:

    # no "Hahaha! Well, I'm sure it'll get better."
    no "¡Jajaja! Bueno, estoy seguro de que mejorará."

# game/script-a4-rin.rpy:1970
translate es a4_rin_blue_period_f924db9d:

    # "Nomiya offers his hearty laughter, bordering on abrasive."
    "Nomiya ofrece su afable carcajada, lindando en lo abrasivo."

# game/script-a4-rin.rpy:1972
translate es a4_rin_blue_period_f53e863a:

    # hi "Rain doesn't get you down, teacher?"
    hi "La lluvia no lo pone de malas, ¿maestro?"

# game/script-a4-rin.rpy:1977
translate es a4_rin_blue_period_78963b55:

    # no "Well, I do prefer clear weather as well. I was actually leaving just now to meet someone, and I'd prefer not getting my jacket soaked. It's very expensive."
    no "Bueno, yo también prefiero buen tiempo. De hecho estaba a punto de irme para reunirme con alguien, y preferiría no mojar mi chaqueta. Es bastante cara."

# game/script-a4-rin.rpy:1982
translate es a4_rin_blue_period_a20af79a:

    # no "But of course I'm in a good mood!"
    no "¡Pero por supuesto que estoy de buen humor!"

# game/script-a4-rin.rpy:1987
translate es a4_rin_blue_period_60db5139:

    # no "What did you think about the exhibition? It was wonderful, wasn't it?"
    no "¿Qué pensaste de la exhibición? Fue maravillosa, ¿no es así?"

# game/script-a4-rin.rpy:1989
translate es a4_rin_blue_period_d62d902f:

    # hi "Yeah, it was very fancy."
    hi "Sí, fue muy elegante."

# game/script-a4-rin.rpy:1991
translate es a4_rin_blue_period_b9fa781d:

    # "My unenthusiastic answer only spurs him to carry on, walking around the gallery while blabbering about the opening."
    "Mi respuesta sin entusiasmo solo lo incentiva a que continúe, caminando por la galería mientras parlotea sobre la apertura."

# game/script-a4-rin.rpy:1993
translate es a4_rin_blue_period_bc0f5784:

    # "He talks more and louder when he is moving. It's something I noticed at the club meetings too."
    "Habla todavía más y con más fuerza cuando se mueve. Es algo que noté en las reuniones del club, también."

# game/script-a4-rin.rpy:1998
translate es a4_rin_blue_period_9a424440:

    # no "We got to talk with many good people and make valuable contacts."
    no "Logramos hablar con mucha gente buena y hacer contactos valiosos."

# game/script-a4-rin.rpy:2003
translate es a4_rin_blue_period_53acf0d3:

    # no "One of Tezuka's paintings even got sold, to a collector from Osaka."
    no "Incluso una de las pinturas de Tezuka se vendió, a un coleccionista de Osaka."

# game/script-a4-rin.rpy:2010
translate es a4_rin_blue_period_39df7101:

    # "I follow his eyes to an empty space in the wall. I can't even remember which painting was hanging on that spot."
    "Sigo sus ojos hacia un espacio vacío en la pared. Ni siquiera puedo recordar cuál pintura estaba colgando en ese lugar."

# game/script-a4-rin.rpy:2012
translate es a4_rin_blue_period_a3764746:

    # "Well, it's gone now."
    "Bueno, se ha ido."

# game/script-a4-rin.rpy:2020
translate es a4_rin_blue_period_46494bcb:

    # no "It was lucky that she was all right despite that dizzy spell."
    no "Fue una suerte que se encontrara bien a pesar de ese mareo."

# game/script-a4-rin.rpy:2025
translate es a4_rin_blue_period_a500c4ec:

    # no "She got a little quiet though, so I told her to rest well. Then again she's always been pretty shy."
    no "Aunque se puso un poco callada, así que le dije que descansara bien. Por otro lado ella siempre ha sido bastante tímida."

# game/script-a4-rin.rpy:2027
translate es a4_rin_blue_period_356770ba:

    # "Shy? Whatever, I just nod along with the teacher."
    "¿Tímida? Como sea, simplemente asiento junto con el maestro."

# game/script-a4-rin.rpy:2032
translate es a4_rin_blue_period_e5ef1dfc:

    # no "The reception was very positive in general. I might be able to get one of my friends to write a little article on a magazine to—"
    no "La recepción fue bastante positiva en general. Quizás pueda conseguir que uno de mis amigos escriba un articulito en una revista para—"

# game/script-a4-rin.rpy:2034
translate es a4_rin_blue_period_701959b0:

    # sa "Shinichi, your meeting. You're making Mr. Takahashi wait."
    sa "Shinichi, tu reunión. Estás haciendo esperar al señor Takahashi."

# game/script-a4-rin.rpy:2039
translate es a4_rin_blue_period_0cbb9e1a:

    # "Sae's remark makes him stop in his tracks and check his watch."
    "El comentario de Sae lo hace detenerse en seco y revisar su reloj."

# game/script-a4-rin.rpy:2041
translate es a4_rin_blue_period_71e02f13:

    # "Nomiya frowns in displeasure at the interruption to his tirade."
    "Nomiya frunce el entrecejo en disgusto ante la interrupción de su diatriba."

# game/script-a4-rin.rpy:2046
translate es a4_rin_blue_period_3515c2d3:

    # no "Oh, right. Yes, well, I'll be off then. We'll meet in September, Nakai."
    no "Oh, cierto. Sí, bueno, me retiraré entonces. Nos vemos en septiembre, Nakai."

# game/script-a4-rin.rpy:2048
translate es a4_rin_blue_period_361fdc97:

    # hi "Bye."
    hi "Hasta luego."

# game/script-a4-rin.rpy:2056
translate es a4_rin_blue_period_68b1a74e:

    # "Wow. Teacher really doesn't hold back when it comes to Rin's budding artist career."
    "Guau. El maestro en verdad no se contiene cuando se trata de la carrera de artista en ciernes de Rin."

# game/script-a4-rin.rpy:2058
translate es a4_rin_blue_period_7b2cdcc7:

    # "I guess it takes a lot to succeed, but I suppose his job would be easier if Rin was more cooperative."
    "Creo que se necesita mucho para tener éxito, pero supongo que su trabajo sería más fácil si Rin fuera más cooperativa."

# game/script-a4-rin.rpy:2098
translate es a4_rin_blue_period_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:2066
translate es a4_rin_blue_period_b2628fcf:

    # n "{vspace=30}She's too indecisive even though she's doing just fine. Like that 'dizzy spell' from the night before."
    n "{vspace=30}Ella es demasiado indecisa aun si lo está haciendo bien. Como ese “mareo” de la noche anterior."

# game/script-a4-rin.rpy:2068
translate es a4_rin_blue_period_9f131eba:

    # n "She just got freaked out or something, and I didn't do anything to help her."
    n "Simplemente le entró pánico o algo así, y no hice nada para ayudarla."

# game/script-a4-rin.rpy:2070
translate es a4_rin_blue_period_aaa316ef:

    # n "{vspace=30}I sigh."
    n "{vspace=30}Suspiro."

# game/script-a4-rin.rpy:2072
translate es a4_rin_blue_period_cbf6d502:

    # n "It feels like the gap between me and Rin is only widening."
    n "Se siente como si la brecha entre Rin y yo solo se estuviese ensanchando."

# game/script-a4-rin.rpy:2074
translate es a4_rin_blue_period_05ebc2df:

    # n "She's going to become something great while I'm still feeling like I'm bogged down, despite promising myself to try and make something of my life."
    n "Ella se convertirá en algo grandioso mientras yo me sigo sintiendo atascado, a pesar de prometerme a mí mismo tratar de hacer algo de mi vida."

# game/script-a4-rin.rpy:2076
translate es a4_rin_blue_period_899baf52:

    # n "On top of that, we had that fight and the longer we keep not talking, the harder the wounds become to heal."
    n "Además de eso, tuvimos esa pelea y mientras más tiempo pase sin que hablemos, más difíciles serán de curar esas heridas."

# game/script-a4-rin.rpy:2078
translate es a4_rin_blue_period_1c9d389d:

    # n "If that even is what we want. I never found out what Rin felt, and now I'm not sure what I feel myself."
    n "Si es que eso es lo que queremos. Nunca descubrí qué sentía Rin, y ahora no estoy seguro de qué siento yo."

# game/script-a4-rin.rpy:2080
translate es a4_rin_blue_period_038d7c50:

    # n "I wish I could understand her. But Rin is not very open for interpretation."
    n "Desearía poder entenderla. Pero Rin es muy abierta a la interpretación."

# game/script-a4-rin.rpy:2082
translate es a4_rin_blue_period_456caa70:

    # n "Not that she's hiding anything, she just seems to defy my attempts at making sense of what she is talking about on any given day."
    n "No es que ella esté ocultando algo, simplemente parece desafiar mis intentos de comprender de qué está hablando en un día cualquiera."

# game/script-a4-rin.rpy:2123
translate es a4_rin_blue_period_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:2093
translate es a4_rin_blue_period_ef9162e6:

    # sa "Something on your mind?"
    sa "¿Algo en mente?"

# game/script-a4-rin.rpy:2095
translate es a4_rin_blue_period_19a1ba7c:

    # "I realize I've been spacing out in the middle of the gallery for who knows how long."
    "Me percato de que me he quedado ido en medio de la galería por quién sabe cuánto tiempo."

# game/script-a4-rin.rpy:2097
translate es a4_rin_blue_period_fe9dc336:

    # hi "Ahh… nothing special…"
    hi "Ahh… nada en especial…"

# game/script-a4-rin.rpy:2099
translate es a4_rin_blue_period_4b81de86:

    # "I pretend to study the closest paintings to distract her."
    "Pretendo estudiar la pintura más cercana para distraerla."

# game/script-a4-rin.rpy:2109
translate es a4_rin_blue_period_6bbf3831:

    # "I've seen it before."
    "La he visto antes."

# game/script-a4-rin.rpy:2111
translate es a4_rin_blue_period_659d36c8:

    # "The now all-too-familiar strokes of color, twisting and melting into each other seemingly randomly still manage to feel like there is something happening behind the scenes, so to speak."
    "Los ya muy familiares trazos de color, torciéndose y fundiéndose unos con otros aparentemente al azar todavía logran sentirse como si hubiera algo sucediendo detrás de escena, por así decirlo."

# game/script-a4-rin.rpy:2113
translate es a4_rin_blue_period_6dd8e5bc:

    # "Rin's style is so much like her. Abstract, incomprehensible, colorful."
    "El estilo de Rin es tan parecido a ella. Abstracto, incomprensible, colorido."

# game/script-a4-rin.rpy:2115
translate es a4_rin_blue_period_82eaf5c1:

    # "Mysterious."
    "Misterioso."

# game/script-a4-rin.rpy:2117
translate es a4_rin_blue_period_5b5ddb02:

    # "I wonder if to understand an artist, one must understand art?"
    "¿Me pregunto si para entender a un artista, uno debe entender el arte?"

# game/script-a4-rin.rpy:2124
translate es a4_rin_blue_period_57d5369c:

    # hi "Umm… I may have a question."
    hi "Ahh… puede que tenga una pregunta."

# game/script-a4-rin.rpy:2129
translate es a4_rin_blue_period_cacde930:

    # sa "Oh?"
    sa "¿Oh?"

# game/script-a4-rin.rpy:2131
translate es a4_rin_blue_period_6abbb1d0:

    # "She looks up from the magazine she was idly leafing through, seeming delighted at my display of unspecified interest."
    "Ella levanta la vista de la revista que estaba hojeando distraídamente, aparentemente encantada con mi muestra de interés sin especificar."

# game/script-a4-rin.rpy:2133
translate es a4_rin_blue_period_16f66972:

    # hi "How do you interpret art?"
    hi "¿Cómo interpreta el arte?"

# game/script-a4-rin.rpy:2138
translate es a4_rin_blue_period_c5a8be80:

    # sa "What do you mean?"
    sa "¿De qué hablas?"

# game/script-a4-rin.rpy:2140
translate es a4_rin_blue_period_b0c698ee:

    # "Her eyebrows rise high into questioning arcs, as if the question was too complicated to even begin to answer without clarification."
    "Sus cejas se levantan alto formando arcos inquisitivos, como si la pregunta fuese demasiado complicada como para siquiera empezar a responderla sin clarificación."

# game/script-a4-rin.rpy:2142
translate es a4_rin_blue_period_b4f07c87:

    # hi "Sorry if I'm asking something stupid."
    hi "Perdón si estoy preguntando algo estúpido."

# game/script-a4-rin.rpy:2144
translate es a4_rin_blue_period_6547df5c:

    # hi "I don't think I really understand art like the pros do."
    hi "No creo que realmente entienda el arte como los profesionales lo hacen."

# game/script-a4-rin.rpy:2149
translate es a4_rin_blue_period_e0ce30d7:

    # sa "Oh, there's no trick to it."
    sa "Oh, no hay truco para ello."

# game/script-a4-rin.rpy:2151
translate es a4_rin_blue_period_af1350ce:

    # "Sae waves my question away with a simple but efficient flick of her wrist."
    "Sae descarta mi pregunta con un simple pero eficiente giro de muñeca."

# game/script-a4-rin.rpy:2156
translate es a4_rin_blue_period_5ce0787f:

    # sa "Everyone interprets art as they will, and interpretation is as much in the eye of the beholder as in the intentions of the creator."
    sa "Cada uno interpreta el arte como gusta, y la interpretación depende tanto de quien lo mira como de las intenciones del creador."

# game/script-a4-rin.rpy:2158
translate es a4_rin_blue_period_5875ce85:

    # sa "'Pros' have their own way, because there is this thing called art theory."
    sa "Los “profesionales” tienen su propia manera, porque existe esta cosa llamada teoría del arte."

# game/script-a4-rin.rpy:2160
translate es a4_rin_blue_period_453cf356:

    # sa "There are patterns in art, just like in everything, and we assume that it's possible to draw some conclusions from observing those patterns."
    sa "Hay patrones en el arte, al igual que en todo, y asumimos que es posible sacar algunas conclusiones al observar esos patrones."

# game/script-a4-rin.rpy:2162
translate es a4_rin_blue_period_6d031a38:

    # "Her voice is like a teacher's, lecturing and adding emphasis on random words to keep the listeners on their toes."
    "Su voz es como la de una maestra, dando una lección y añadiendo énfasis en palabras al azar para mantener a los oyentes atentos."

# game/script-a4-rin.rpy:2167
translate es a4_rin_blue_period_a6c2cfdd:

    # sa "In the end, I suppose it's pretty meaningless."
    sa "Al final, supongo que no tiene mucho sentido."

# game/script-a4-rin.rpy:2169
translate es a4_rin_blue_period_acf4c476:

    # "She moves to musing seemingly to herself, muttering loud enough for me to clearly hear."
    "Ella cambia a meditar aparentemente para sí misma, murmurando lo suficientemente fuerte como para que yo la escuche claramente."

# game/script-a4-rin.rpy:2171
translate es a4_rin_blue_period_31fa8ff4:

    # sa "A good piece of art will make you feel something and that's all there is to it."
    sa "Una buena pieza de arte te hará sentir algo y eso es todo."

# game/script-a4-rin.rpy:2173
translate es a4_rin_blue_period_7d678c9f:

    # sa "Feelings change and they affect the art we create and the art we see."
    sa "Los sentimientos cambian y ellos afectan el arte que creamos y el arte que vemos."

# game/script-a4-rin.rpy:2175
translate es a4_rin_blue_period_b91d705c:

    # hi "But…"
    hi "Pero…"

# game/script-a4-rin.rpy:2180
translate es a4_rin_blue_period_aff67f9e:

    # sa "I'll tell you a story."
    sa "Te contaré una historia."

# game/script-a4-rin.rpy:2182
translate es a4_rin_blue_period_36500938:

    # hi "Do you have to? The last one was depressing…"
    hi "¿Tiene que hacerlo? La última fue deprimente…"

# game/script-a4-rin.rpy:2184
translate es a4_rin_blue_period_2c333e8c:

    # sa "It's important. Listen…"
    sa "Es importante. Escucha…"

# game/script-a4-rin.rpy:2186
translate es a4_rin_blue_period_67359d64:

    # sa "About a hundred years ago a little known painter got news that his friend, a man called Casagemas, had committed suicide."
    sa "Hace alrededor de cien años a un pintor poco conocido le llegó la noticia de que su amigo, un hombre llamado Casagemas, se había suicidado."

# game/script-a4-rin.rpy:2188
translate es a4_rin_blue_period_5414ed21:

    # sa "This happened while he was away and hadn't seen his friend for a while."
    sa "Esto había sucedido cuando él estaba fuera y no había visto a su amigo por un tiempo."

# game/script-a4-rin.rpy:2190
translate es a4_rin_blue_period_770733bf:

    # sa "So obviously he must have felt even more conflicted than you normally would after hearing of such a thing."
    sa "Así que obviamente se debe de haber sentido aún más conflictuado de lo que normalmente te sentirías después de escuchar tal cosa."

# game/script-a4-rin.rpy:2192
translate es a4_rin_blue_period_a15d1905:

    # sa "For four years after that, our main character did nothing but monochromatic paintings because he was so deeply affected by the news."
    sa "Por cuatro años después de eso, nuestro personaje principal no hizo nada más que pinturas monocromáticas porque había sido afectado tan profundamente por esas noticias."

# game/script-a4-rin.rpy:2194
translate es a4_rin_blue_period_ecef6122:

    # sa "Whatever he did, he always kept returning to that same color until it let him out of its grasp."
    sa "Cualquier cosa que hiciera, siempre regresaba a ese color hasta que este se desaferró de él."

# game/script-a4-rin.rpy:2196
translate es a4_rin_blue_period_a4a66667:

    # "She takes a little pause to check whether I'm still following."
    "Ella hace una ligera pausa para revisar si todavía le sigo la pista."

# game/script-a4-rin.rpy:2198
translate es a4_rin_blue_period_69bf7d08:

    # "I am, to an extent, so I give her the prompt that storytellers seem to live for."
    "Sí lo hago, en cierta medida, así que le doy la señal para la que viven los cuentacuentos."

# game/script-a4-rin.rpy:2200
translate es a4_rin_blue_period_c85221a4:

    # hi "So…"
    hi "Así que…"

# game/script-a4-rin.rpy:2202
translate es a4_rin_blue_period_d0a57d48:

    # "It's hard to continue from that, as I can't seem to come up with the question she wants me to come up with."
    "Es difícil continuar de ahí, ya que parezco no poder descubrir la pregunta que ella quiere que descubra."

# game/script-a4-rin.rpy:2204
translate es a4_rin_blue_period_a646deae:

    # "Like a half-baked Socrates, she thought she laid out all the tools for revelation in front of me."
    "Como un Sócrates sin madurar, ella pensó que había puesto todas las herramientas para la revelación frente a mí."

# game/script-a4-rin.rpy:2209
translate es a4_rin_blue_period_3f4eff04:

    # sa "Don't you see the point yet?"
    sa "¿Todavía no ves el punto?"

# game/script-a4-rin.rpy:2211
translate es a4_rin_blue_period_7da429ed:

    # "Only, her student proved to be too dense to get it."
    "Solo que su estudiante probó ser demasiado bobo para entenderlo."

# game/script-a4-rin.rpy:2216
translate es a4_rin_blue_period_92ae8521:

    # "She looks discontent at my slowness."
    "Ella se ve descontenta con mi torpeza."

# game/script-a4-rin.rpy:2218
translate es a4_rin_blue_period_db9e06d8:

    # sa "Picasso's Blue Period is one of the most lauded in the history of art, but who knows what he felt when he worked on those masterpieces?"
    sa "El periodo azul de Picasso es uno de los más afamados en la historia del arte, pero, ¿quién sabe qué sintió cuando trabajó en esas obras maestras?"

# game/script-a4-rin.rpy:2220
translate es a4_rin_blue_period_b3ef9f66:

    # sa "Sadness? Longing? Regret?"
    sa "¿Tristeza? ¿Anhelo? ¿Arrepentimiento?"

# game/script-a4-rin.rpy:2222
translate es a4_rin_blue_period_51a79c7c:

    # sa "Nobody can tell."
    sa "Nadie puede decirlo."

# game/script-a4-rin.rpy:2224
translate es a4_rin_blue_period_74a940f2:

    # sa "If you now see one of his Blue Period paintings, you'd probably interpret it differently from before you knew about Picasso's friend Casagemas."
    sa "Si ahora ves una de sus pinturas del periodo azul, probablemente la interpretes de manera diferente a antes de que supieras de Casagemas, el amigo de Picasso."

# game/script-a4-rin.rpy:2229
translate es a4_rin_blue_period_3dcfcfb4:

    # sa "Experiencing art is always personal, only interactive by chance or circumstances."
    sa "Experimentar el arte es siempre personal, solo interactivo por casualidad o circunstancia."

# game/script-a4-rin.rpy:2231
translate es a4_rin_blue_period_f4cd323a:

    # sa "There are a million explanations for any given piece of art, but it might be that none of them are what the creator intended."
    sa "Hay millones de explicaciones para cualquier obra de arte, pero puede ser que ninguna de ellas sea lo que el creador quería."

# game/script-a4-rin.rpy:2236
translate es a4_rin_blue_period_2921a328:

    # sa "No man is an island, you know?"
    sa "Ningún hombre es una isla, ¿lo sabes?"

# game/script-a4-rin.rpy:2238
translate es a4_rin_blue_period_0eba98cb:

    # "I nod without understanding what that last remark meant."
    "Asiento sin entender qué significa ese último comentario."

# game/script-a4-rin.rpy:2240
translate es a4_rin_blue_period_3baa71c1:

    # "What she said made sense otherwise, except for one thing."
    "Lo que dijo tenía sentido de otro modo, excepto por una cosa."

# game/script-a4-rin.rpy:2242
translate es a4_rin_blue_period_aaf874d1:

    # "If art is communication like Rin said, but everyone is talking their own secret language like Sae said, what can anyone ever hope to communicate?"
    "Si el arte es comunicación como Rin dijo, pero todos hablan su propio idioma secreto como dijo Sae, ¿cómo puede alguien tener la esperanza de comunicarse?"

# game/script-a4-rin.rpy:2244
translate es a4_rin_blue_period_319d0dbd:

    # "It seems so futile, and pointless."
    "Se siente tan vano e inútil."

# game/script-a4-rin.rpy:2246
translate es a4_rin_blue_period_950ebfcb:

    # "Art really is not a thing for me."
    "El arte en verdad no es algo para mí."

# game/script-a4-rin.rpy:2253
translate es a4_rin_blue_period_71ac2452:

    # "Sae returns to her art magazine, and I make a round in the gallery, trying to see what Rin can see in her own paintings."
    "Sae regresa a su revista, y yo doy una vuelta por la galería, tratando de ver lo que Rin ve en sus propias pinturas."

# game/script-a4-rin.rpy:2255
translate es a4_rin_blue_period_62b50f3a:

    # "A soothing mood takes hold of the gallery surrounded by the rainstorm, the big windows making the transparent isolation feel more comfortable."
    "Un ambiente relajante se apodera de la galería rodeada por la tormenta, las grandes ventanas haciendo al transparente aislamiento sentirse más cómodo."

# game/script-a4-rin.rpy:2260
translate es a4_rin_blue_period_1a3f4747:

    # "A tinkle of the bell disrupts the tranquil mood."
    "Un tintineo de la campana interrumpe el ambiente tranquilo."

# game/script-a4-rin.rpy:2270
translate es a4_rin_blue_period_da04f110:

    # "Rin pushes the door open with her shoulder and steps in."
    "Rin abre la puerta al empujarla con el hombro y da un paso adentro."

# game/script-a4-rin.rpy:2272
translate es a4_rin_blue_period_addf6b79:

    # "I had almost forgotten that she was the reason why I came to the gallery in the first place."
    "Casi había olvidado que ella era la razón por la que vine a la galería en primer lugar."

# game/script-a4-rin.rpy:2277
translate es a4_rin_blue_period_56f08f96:

    # rin "I think I'm ready—{w=0.3}{nw}"
    rin "Creo que estoy lista—{w=0.3}{nw}"

# game/script-a4-rin.rpy:2282
translate es a4_rin_blue_period_476b888f:

    # "She pauses mid-sentence, noticing my presence."
    "Ella se detiene a media palabra, percatándose de mi presencia."

# game/script-a4-rin.rpy:2284
translate es a4_rin_blue_period_a43a6b46:

    # "The needle-dropping silence lasts for exactly one and a half seconds, not enough for either me or Sae to open our mouths, but enough for Rin to react."
    "El silencio absoluto dura exactamente un segundo y medio, no lo suficiente como para que Sae o yo abramos la boca, pero suficiente para que Rin reaccione."

# game/script-a4-rin.rpy:2289
translate es a4_rin_blue_period_f2fc5ea0:

    # rin "I'm going for a walk."
    rin "Iré a dar un paseo."

# game/script-a4-rin.rpy:2296
translate es a4_rin_blue_period_fbb3c86c:

    # "Heading back outside with a reckless pace uncharacteristic of herself, Rin seems to forget that it's still raining."
    "Yendo de vuelta hacia afuera con un paso imprudente inusual para ella, Rin parece olvidar que sigue lloviendo."

# game/script-a4-rin.rpy:2298
translate es a4_rin_blue_period_b53d4867:

    # "Without giving it any real thought, I grab my umbrella and hurry after her."
    "Sin realmente pensarlo demasiado, tomo mi paraguas y me apresuro tras de ella."

# game/script-a4-rin.rpy:2308
translate es a4_rin_blue_period_bfa7b72c:

    # "I catch Rin around the corner, open the umbrella and lift it above the two of us while still having to almost run to keep up with her."
    "Alcanzo a Rin a la vuelta de la esquina, abro el paraguas y lo levanto por sobre nosotros dos sin dejar de tener que casi correr para seguirle el paso."

# game/script-a4-rin.rpy:2310
translate es a4_rin_blue_period_0ea51acf:

    # "She doesn't protest me running after her nor me giving her shelter against the rain, eventually slowing her pace down so I can match it without an immediate danger of overexerting myself."
    "Ella no protesta que corra detrás ni que le dé refugio contra la lluvia, finalmente disminuye el paso para que pueda igualarlo sin un riesgo inmediato de que me esfuerce demasiado."

# game/script-a4-rin.rpy:2312
translate es a4_rin_blue_period_27ae3507:

    # "I relax from the rush, assessing the situation."
    "Me tranquilizo del apuro, evaluando la situación."

# game/script-a4-rin.rpy:2314
translate es a4_rin_blue_period_bfa998d9:

    # "The last time I held my umbrella to guard both of us against the rain, I didn't think too much about it."
    "La última vez que sostuve mi paraguas para protegernos a los dos de la lluvia, no pensé demasiado en ello."

# game/script-a4-rin.rpy:2316
translate es a4_rin_blue_period_d01cb9bb:

    # "But now, all the things that happened since then are gathering into a freezing cold ball around my stomach."
    "Pero ahora, todas las cosas que pasaron desde entonces se han juntado en una bola helada alrededor de mi estómago."

# game/script-a4-rin.rpy:2318
translate es a4_rin_blue_period_6a4cad7e:

    # "Being close to her makes me uncomfortable, and I feel myself flustering slightly."
    "Estar cerca de ella me hace sentir incómodo, y siento cómo me pongo ligeramente nervioso."

# game/script-a4-rin.rpy:2320
translate es a4_rin_blue_period_0252e53d:

    # "It's hard to get words out of my mouth, as it feels suddenly very, very dry."
    "Es difícil sacarme las palabras de la boca, ya que súbitamente se siente muy, muy seca."

# game/script-a4-rin.rpy:2322
translate es a4_rin_blue_period_66523d72:

    # "Still, it's not like I can back off."
    "Aun así, no es como si pudiera echarme para atrás."

# game/script-a4-rin.rpy:2324
translate es a4_rin_blue_period_bf29b048:

    # hi "Why do you keep running away?"
    hi "¿Por qué sigues corriendo?"

# game/script-a4-rin.rpy:2329
translate es a4_rin_blue_period_27f8c077:

    # rin "I don't want to talk to you."
    rin "No quiero hablar contigo."

# game/script-a4-rin.rpy:2331
translate es a4_rin_blue_period_3456971d:

    # hi "I want to talk to you."
    hi "Yo quiero hablar contigo."

# game/script-a4-rin.rpy:2336
translate es a4_rin_blue_period_4210074f:

    # rin "It hurts every time I do."
    rin "Me duele cada vez que lo hago."

# game/script-a4-rin.rpy:2338
translate es a4_rin_blue_period_bc37a1a0:

    # hi "Sometimes it can't be helped."
    hi "Algunas veces no hay más remedio."

# game/script-a4-rin.rpy:2343
translate es a4_rin_blue_period_a992f22b:

    # rin "I don't want to hurt."
    rin "No quiero sentir dolor."

# game/script-a4-rin.rpy:2345
translate es a4_rin_blue_period_434553cb:

    # hi "Fine. We don't have to talk."
    hi "Está bien. No tenemos que hablar."

# game/script-a4-rin.rpy:2350
translate es a4_rin_blue_period_f9ed2b9a:

    # rin "What should we do?"
    rin "¿Qué deberíamos hacer?"

# game/script-a4-rin.rpy:2352
translate es a4_rin_blue_period_f01e297a:

    # hi "Let's just keep walking."
    hi "Solo sigamos caminando."

# game/script-a4-rin.rpy:2357
translate es a4_rin_blue_period_5f7e9809:

    # rin "Just walking?"
    rin "¿Solo caminar?"

# game/script-a4-rin.rpy:2359
translate es a4_rin_blue_period_beb587ba:

    # hi "Just walking."
    hi "Solo caminar."

# game/script-a4-rin.rpy:2364
translate es a4_rin_blue_period_a61bcc4a:

    # rin "Okay."
    rin "Está bien."

# game/script-a4-rin.rpy:2385
translate es a4_rin_the_world_only_can_you_see_332df450:

    # "Our footsteps go 'splish splash' in the shallow puddles forming on the streets as we walk through the rainfall."
    "Nuestras pisadas hacen “splish splash” en los charcos poco profundos formándose en las calles a medida que caminamos por la lluvia."

# game/script-a4-rin.rpy:2387
translate es a4_rin_the_world_only_can_you_see_5e37c119:

    # "Rin, now walking beside me in her unhurried and relaxed manner, doesn't seem to be even a bit bothered by the fact that she is getting wet even though she needn't to."
    "Rin, ahora caminando a mi lado en su manera tranquila y relajada, no parece estar ni un poco molesta por el hecho de que se está mojando aun si no tiene que hacerlo."

# game/script-a4-rin.rpy:2389
translate es a4_rin_the_world_only_can_you_see_55c8f7c2:

    # "She is partially out of the protective shelter of my umbrella, despite it being more than big enough for the two of us."
    "Ella está parcialmente fuera del refugio protector de mi paraguas, a pesar de este ser más grande de lo necesario para los dos."

# game/script-a4-rin.rpy:2391
translate es a4_rin_the_world_only_can_you_see_4d73d9e1:

    # "It's as if she doesn't even notice the rain drenching her shirt."
    "Es como si ni siquiera notara la lluvia mojando su blusa."

# game/script-a4-rin.rpy:2393
translate es a4_rin_the_world_only_can_you_see_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:2395
translate es a4_rin_the_world_only_can_you_see_292de573:

    # "Rin's demeanor always evokes mental images of meditative calm, even when she might be in inner turmoil."
    "El comportamiento de Rin siempre evoca imágenes mentales de meditativa calma, aun cuando ella pudiera tener un conflicto interior."

# game/script-a4-rin.rpy:2397
translate es a4_rin_the_world_only_can_you_see_96895670:

    # "But I don't think that is meditation. That is just getting soaked in rain."
    "Pero no creo que eso sea meditación. Eso es solo estarse empapando en la lluvia."

# game/script-a4-rin.rpy:2399
translate es a4_rin_the_world_only_can_you_see_fdb5b079:

    # "I wish I could be more calm too."
    "También desearía estar más tranquilo."

# game/script-a4-rin.rpy:2401
translate es a4_rin_the_world_only_can_you_see_b646bc5e:

    # "I've become too involved with Rin to retain my usual aloofness."
    "Me he involucrado demasiado con Rin como para mantener mi usual distanciamiento."

# game/script-a4-rin.rpy:2403
translate es a4_rin_the_world_only_can_you_see_67633efc:

    # "It feels like I have become one of those people who fool themselves into thinking they are objective, only to find out they are the worst kind of liars."
    "Se siente como si me hubiese vuelto una de esas personas que se engañan a sí mismas pensando que son objetivas, tan solo para darse cuenta de que son de la peor clase de mentirosos."

# game/script-a4-rin.rpy:2405
translate es a4_rin_the_world_only_can_you_see_ae981667:

    # "Illusions to fool ourselves, what better way to make one feel like a good person?"
    "Ilusiones para engañarnos, ¿qué mejor manera de hacer que uno se sienta como una mejor persona?"

# game/script-a4-rin.rpy:2407
translate es a4_rin_the_world_only_can_you_see_62fa6d03:

    # "It might be better to lose that illusion."
    "Tal vez sea mejor perder esa ilusión."

# game/script-a4-rin.rpy:2413
translate es a4_rin_the_world_only_can_you_see_81068593:

    # hi "I'll be going back home for some time so I thought I'd come to see you before that."
    hi "Iré de regreso a casa por un tiempo así que pensé en venir a verte antes de eso."

# game/script-a4-rin.rpy:2415
translate es a4_rin_the_world_only_can_you_see_5567e945:

    # "I could have thought of a better conversation opener, but Rin actively refusing to talk makes it hard."
    "Podría haber pensado en un mejor inicio de conversación, pero Rin negándose activamente a hablar lo hace más difícil."

# game/script-a4-rin.rpy:2417
translate es a4_rin_the_world_only_can_you_see_2a52ae42:

    # rin "That's good. I might have thought that you'd have been kidnapped otherwise."
    rin "Eso es bueno. Podría haber pensado que habías sido raptado de otro modo."

# game/script-a4-rin.rpy:2419
translate es a4_rin_the_world_only_can_you_see_af1b1a87:

    # hi "You can't keep running away from everything. Not even from me trying to talk seriously."
    hi "No puedes continuar escapando de todo. Ni siquiera de mí tratando de hablar seriamente."

# game/script-a4-rin.rpy:2421
translate es a4_rin_the_world_only_can_you_see_033e6652:

    # rin "I'm always serious. Also I seem to be running very slowly right now."
    rin "Siempre hablo en serio. Además parezco estar corriendo muy lentamente en este momento."

# game/script-a4-rin.rpy:2423
translate es a4_rin_the_world_only_can_you_see_87c81773:

    # rin "Maybe I should take lessons from Emi."
    rin "Tal vez deba tomar lecciones con Emi."

# game/script-a4-rin.rpy:2425
translate es a4_rin_the_world_only_can_you_see_b88e0e80:

    # "It's futile. Like talking to a brick wall that randomly spouts sarcastic nonsense back at you."
    "Es inútil. Como hablar con una pared de ladrillo que al azar te dice tonterías sarcásticas de vuelta."

# game/script-a4-rin.rpy:2427
translate es a4_rin_the_world_only_can_you_see_6b4862b4:

    # hi "Think of your exhibition opening. What if you had run away?"
    hi "Piensa en la apertura de tu exhibición. ¿Qué tal si te hubieses escapado?"

# game/script-a4-rin.rpy:2429
translate es a4_rin_the_world_only_can_you_see_75f5f242:

    # "Rin doesn't answer to that, she just keeps walking. Or running slowly, escaping from me into her silence."
    "Rin no responde a eso, tan solo sigue caminando. O corriendo lentamente, huyendo de mí hacia su silencio."

# game/script-a4-rin.rpy:2431
translate es a4_rin_the_world_only_can_you_see_635773a9:

    # "She has a knack for being alone in company, I've noticed."
    "Ella tiene la habilidad de estar sola en compañía, he notado."

# game/script-a4-rin.rpy:2437
translate es a4_rin_the_world_only_can_you_see_26f42f60:

    # "We head down the street, then turn left, then three times right, then left again."
    "Caminamos por la calle, luego doblamos a la izquierda, y luego tres veces a la derecha, luego a la izquierda de nuevo."

# game/script-a4-rin.rpy:2439
translate es a4_rin_the_world_only_can_you_see_84bcf04d:

    # "It's like that night from some time ago, we keep choosing directions randomly because it doesn't matter where we are going."
    "Es como aquella noche de hace algún tiempo, seguimos escogiendo direcciones al azar porque no importa adónde vamos."

# game/script-a4-rin.rpy:2441
translate es a4_rin_the_world_only_can_you_see_f8daf5f7:

    # "All that matters is walking and the sound of raindrops drumming against the umbrella."
    "Todo lo que importa es caminar y el sonido de las gotas de lluvia tamborileando el paraguas."

# game/script-a4-rin.rpy:2443
translate es a4_rin_the_world_only_can_you_see_1cc7befa:

    # "Water flows down from the roofs of the buildings and into the storm drains in wide rivers."
    "El agua fluye abajo desde los tejados de los edificios y hacia el drenaje pluvial en ríos anchos."

# game/script-a4-rin.rpy:2445
translate es a4_rin_the_world_only_can_you_see_edfcc32c:

    # "Even though I try to step over them, my feet are getting wet through my shoes."
    "Aunque trato de pasar sobre ellos, mis pies están mojándose a través de mis zapatos."

# game/script-a4-rin.rpy:2447
translate es a4_rin_the_world_only_can_you_see_076f37f9:

    # "We keep walking in silence that just begs to be broken again. I'm sure I am the only one feeling like this, though."
    "Seguimos caminando en silencio que ruega por ser roto otra vez. Sin embargo, estoy seguro de que soy el único que se siente así."

# game/script-a4-rin.rpy:2454
translate es a4_rin_the_world_only_can_you_see_e76b0f07:

    # hi "Why did you have the exhibition?"
    hi "¿Por qué tuviste la exhibición?"

# game/script-a4-rin.rpy:2456
translate es a4_rin_the_world_only_can_you_see_172269a2:

    # "Rin just shrugs sullenly and looks in the other direction. I give up at this point."
    "Rin solo se encoge hoscamente de hombros y mira en otra dirección. Me doy por vencido en este punto."

# game/script-a4-rin.rpy:2500
translate es a4_rin_the_world_only_can_you_see_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:2467
translate es a4_rin_the_world_only_can_you_see_0dae1505:

    # n "{vspace=90}It's pointless."
    n "{vspace=90}No tiene punto."

# game/script-a4-rin.rpy:2469
translate es a4_rin_the_world_only_can_you_see_5d37bd05:

    # n "{vspace=30}What did she want to accomplish? What she said at the night of the opening made me feel that there was something… something special she wanted."
    n "{vspace=30}¿Qué es lo que ella quería lograr? Lo que dijo en la noche de la apertura me hizo sentir que había algo… algo especial que ella quería."

# game/script-a4-rin.rpy:2471
translate es a4_rin_the_world_only_can_you_see_9ffa0137:

    # n "It felt to me that Rin hoped for something unattainable."
    n "Para mí se sintió que Rin aspiraba hacia algo inalcanzable."

# game/script-a4-rin.rpy:2473
translate es a4_rin_the_world_only_can_you_see_c78dd15e:

    # n "She set the bar high and inside her own head she failed, no matter how much people liked her works."
    n "Se fijó la meta demasiado alta y dentro de su propia cabeza ella fracasó, sin importar cuánto le haya gustado su trabajo a la gente."

# game/script-a4-rin.rpy:2475
translate es a4_rin_the_world_only_can_you_see_60a233ec:

    # n "It's understandable to lack realism; most people do, even if not quite on the extreme level Rin takes it to."
    n "Es entendible carecer de realismo; a la mayoría de la gente le pasa, aun si no es en el nivel extremo al que Rin lo lleva."

# game/script-a4-rin.rpy:2477
translate es a4_rin_the_world_only_can_you_see_3b02dc03:

    # n "{vspace=30}But it's not a reason to live in your private world that accepts no visitors."
    n "{vspace=30}Pero no es razón para vivir en tu propio mundo privado que no acepta visitantes."

# game/script-a4-rin.rpy:2479
translate es a4_rin_the_world_only_can_you_see_63cf5ae3:

    # nvl clear
    # n "{vspace=90}You can't bend the world to fit your twisted, megalomaniac cosmology where everything works just like you want."
    nvl clear
    n "{vspace=90}No puedes doblar el mundo para que sea acorde a tu cosmología retorcida y megalómana donde todo funciona justo como tú quieres."

# game/script-a4-rin.rpy:2483
translate es a4_rin_the_world_only_can_you_see_bdadc52d:

    # n "{vspace=30}That's what frustrates me the most in Rin."
    n "{vspace=30}Eso es lo que más me frustra de Rin."

# game/script-a4-rin.rpy:2485
translate es a4_rin_the_world_only_can_you_see_fe231760:

    # n "{vspace=30}She wants the world to live by her rules, disregarding everything that conflicts with those as irrelevant or unnecessary."
    n "{vspace=30}Ella quiere que el mundo viva bajo sus reglas, descartando todo lo que entre en conflicto con aquellas como irrelevante o innecesario."

# game/script-a4-rin.rpy:2487
translate es a4_rin_the_world_only_can_you_see_ff42a06b:

    # n "I can't believe how anyone in Yamaku could not have the bare minimum perception to understand that the world can sometimes be very unfair."
    n "No puedo creer que alguien en Yamaku podría no tener la más mínima percepción para entender que el mundo puede algunas veces ser muy injusto."

# game/script-a4-rin.rpy:2489
translate es a4_rin_the_world_only_can_you_see_1581ca86:

    # n "I'm sure she's not the only one who wishes some things were different, but we can at least grasp the facts as they are."
    n "Estoy seguro de que ella no es la única que desea que algunas cosas fuesen diferentes, pero por lo menos podemos comprender los hechos como son."

# game/script-a4-rin.rpy:2531
translate es a4_rin_the_world_only_can_you_see_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:2502
translate es a4_rin_the_world_only_can_you_see_63d1a98c:

    # "I take a sideways glance at Rin, who is looking up at our dome-shaped cover. It is a poor replacement for a real sky in its monochrome bleakness."
    "Le echo una mirada de reojo a Rin, quien está mirando nuestra cubierta en forma de domo. Es un pobre reemplazo para el cielo verdadero con su austeridad monocromática."

# game/script-a4-rin.rpy:2504
translate es a4_rin_the_world_only_can_you_see_971cdcdf:

    # "The rain just keeps falling."
    "La lluvia solo sigue cayendo."

# game/script-a4-rin.rpy:2506
translate es a4_rin_the_world_only_can_you_see_10b54f63:

    # "Just like the clouds today, Rin doesn't really give the feeling of wanting to be watched."
    "Al igual que las nubes hoy, Rin realmente no da la sensación de querer ser observada."

# game/script-a4-rin.rpy:2508
translate es a4_rin_the_world_only_can_you_see_8f57b2d9:

    # "She sulks, in unison with the sky that she loves so."
    "Se enfada al unísono con el cielo que ella ama tanto."

# game/script-a4-rin.rpy:2510
translate es a4_rin_the_world_only_can_you_see_548676a9:

    # "I shouldn't have come. Her presence only reminds me of how angry I got because of these exact same reasons, and how those reasons probably can't ever change."
    "No debería de haber venido. Su presencia únicamente me recuerda cuán enojado me puse a causa de exactamente estas razones, y cómo esas razones probablemente nunca podrán cambiar."

# game/script-a4-rin.rpy:2512
translate es a4_rin_the_world_only_can_you_see_4a4dd5a6:

    # "Even though I want to say I'm sorry, even though I don't want us to break apart, I can't bring myself to say either of these things."
    "Aun si quiero decir que lo siento, a pesar de que no quiero que nos separemos, no puedo obligarme a decir alguna de estas cosas."

# game/script-a4-rin.rpy:2518
translate es a4_rin_the_world_only_can_you_see_f49fa71d:

    # "We keep measuring the rain-drenched streets one step at a time."
    "Seguimos caminando por la calle empapada por la lluvia un paso a la vez."

# game/script-a4-rin.rpy:2520
translate es a4_rin_the_world_only_can_you_see_3a3b6a41:

    # "Often, when you walk with someone else, your steps become synchronized as if through some weird subconscious pact."
    "A menudo, cuando caminas junto con alguien más, tus pasos se sincronizan con los suyos como por algún extraño pacto subconsciente."

# game/script-a4-rin.rpy:2522
translate es a4_rin_the_world_only_can_you_see_6f47e9df:

    # "I noticed that ours never do."
    "He notado que los nuestros nunca lo hacen."

# game/script-a4-rin.rpy:2531
translate es a4_rin_the_world_only_can_you_see_b5c742e3:

    # "Time passes, and the strikes against the drumskin of my umbrella fade as the clouds above slowly disperse to reveal a cerulean blue."
    "El tiempo pasa, y los golpes contra el parche de tambor de mi paraguas se disipan a medida que las nubes sobre nosotros se dispersan poco a poco para revelar un azul cerúleo."

# game/script-a4-rin.rpy:2539
translate es a4_rin_the_world_only_can_you_see_aa6eb418:

    # "Eventually the rain yields enough for me to close the umbrella, shaking the excess water off before I do."
    "Eventualmente la lluvia cede lo suficiente como para que cierre el paraguas, sacudiendo el exceso de agua antes de hacerlo."

# game/script-a4-rin.rpy:2541
translate es a4_rin_the_world_only_can_you_see_c18ccb2a:

    # "While I wrestle with the mechanism, Rin stops so abruptly that I take five steps before realizing that she's not with me any more."
    "Mientras batallo con el mecanismo, Rin se detiene tan abruptamente que doy cinco pasos antes de darme cuenta de que ya no está conmigo."

# game/script-a4-rin.rpy:2543
translate es a4_rin_the_world_only_can_you_see_caaac72a:

    # "Stupid umbrella seems to be jammed."
    "Parece que el estúpido paraguas se atascó."

# game/script-a4-rin.rpy:2553
translate es a4_rin_the_world_only_can_you_see_e1cde282:

    # "When I turn around, I find her staring at me with an impassive face."
    "Cuando me doy vuelta, la encuentro mirándome con un rostro impasible."

# game/script-a4-rin.rpy:2555
translate es a4_rin_the_world_only_can_you_see_1450add9:

    # rin "I wanted someone to say 'I understand how you feel.'"
    rin "Quería que alguien dijera “entiendo cómo te sientes”."

# game/script-a4-rin.rpy:2557
translate es a4_rin_the_world_only_can_you_see_cbf075eb:

    # rin "Wouldn't that be great?"
    rin "¿No sería eso genial?"

# game/script-a4-rin.rpy:2559
translate es a4_rin_the_world_only_can_you_see_59e0e773:

    # "Is that an answer to the question from before? I'm not sure."
    "¿Es esa una respuesta a la pregunta de antes? No estoy seguro."

# game/script-a4-rin.rpy:2561
translate es a4_rin_the_world_only_can_you_see_23d487a5:

    # hi "Yeah… but why is it so important?"
    hi "Sí… pero, ¿por qué es tan importante?"

# game/script-a4-rin.rpy:2566
translate es a4_rin_the_world_only_can_you_see_0217691a:

    # rin "Because otherwise… I don't know if I can bear this."
    rin "Porque de otro modo… no sé si pueda soportar esto."

# game/script-a4-rin.rpy:2568
translate es a4_rin_the_world_only_can_you_see_ed14fd34:

    # "I was still in the middle of folding my umbrella so I just answered something to get the conversation going, but what she says now freezes my blood."
    "Todavía estaba en medio de guardar mi paraguas así que solo respondo cualquier cosa para mantener la conversación andando, pero lo que dice ahora me congela la sangre."

# game/script-a4-rin.rpy:2573
translate es a4_rin_the_world_only_can_you_see_6f762f3c:

    # rin "If someone says a joke and laughs, you laugh with them, right? Because a joy doubled is a joy tripled, right?"
    rin "Si alguien dice una broma y se ríe, te ríes con ellos, ¿verdad? Porque la alegría duplicada es la alegría triplicada, ¿verdad?"

# game/script-a4-rin.rpy:2578
translate es a4_rin_the_world_only_can_you_see_909981a6:

    # rin "If someone is hurt and sad, you comfort and hug them, right? Because that way—"
    rin "Si alguien está lastimado y triste, lo reconfortas y lo abrazas, ¿verdad? Porque de esa manera—"

# game/script-a4-rin.rpy:2580
translate es a4_rin_the_world_only_can_you_see_879b98dc:

    # rin "…"
    rin "…"

# game/script-a4-rin.rpy:2582
translate es a4_rin_the_world_only_can_you_see_83b047f4:

    # "She pauses, her mouth still halfway open, then remembers to close it."
    "Hace una pausa, su boca todavía a medio abrir, entonces recuerda cerrarla."

# game/script-a4-rin.rpy:2587
translate es a4_rin_the_world_only_can_you_see_ca93fe8c:

    # "A gloom sets on her face and simultaneously on my heart."
    "Una penumbra se asienta en su rostro y simultáneamente en mi corazón."

# game/script-a4-rin.rpy:2589
translate es a4_rin_the_world_only_can_you_see_7b710278:

    # rin "I don't know why the right words never come out."
    rin "No sé por qué las palabras correctas nunca salen."

# game/script-a4-rin.rpy:2591
translate es a4_rin_the_world_only_can_you_see_581f7d8b:

    # rin "I don't know why I can laugh only when I make myself."
    rin "No sé por qué puedo reír solo cuando yo me hago reír."

# game/script-a4-rin.rpy:2593
translate es a4_rin_the_world_only_can_you_see_95a2d745:

    # rin "I don't know why everything stays only inside me, even when it feels like I'm going to burst."
    rin "No sé por qué todo se queda solo dentro de mí, aun cuando se siente como si fuese a estallar."

# game/script-a4-rin.rpy:2595
translate es a4_rin_the_world_only_can_you_see_66bf3f0f:

    # "Her flat, expressionless face does not waver even when she says that."
    "Su llano e inexpresivo rostro no vacila siquiera cuando dice eso."

# game/script-a4-rin.rpy:2597
translate es a4_rin_the_world_only_can_you_see_18875c04:

    # "Her usual steady voice becomes only slightly quieter than normal."
    "Su usual voz inalterable tan solo se hace ligeramente más callada de lo normal."

# game/script-a4-rin.rpy:2599
translate es a4_rin_the_world_only_can_you_see_09d03f68:

    # rin "But who… who would ever want to feel like that?"
    rin "Pero, ¿quién… quién quisiera sentirse así?"

# game/script-a4-rin.rpy:2601
translate es a4_rin_the_world_only_can_you_see_cec8cfbb:

    # "Rin looks at me and I imagine the sadness reflecting from her eyes, whether it really is there or not."
    "Rin me mira e imagino la tristeza reflejándose desde sus ojos, sin importar si está realmente ahí o no."

# game/script-a4-rin.rpy:2603
translate es a4_rin_the_world_only_can_you_see_ae6f824c:

    # rin "I don't."
    rin "Yo no."

# game/script-a4-rin.rpy:2605
translate es a4_rin_the_world_only_can_you_see_4998265c:

    # rin "I don't want to feel like that."
    rin "No quiero sentirme así."

# game/script-a4-rin.rpy:2607
translate es a4_rin_the_world_only_can_you_see_56438df9:

    # "We stay silent for a little while after that."
    "Guardamos silencio por un corto momento después de eso."

# game/script-a4-rin.rpy:2609
translate es a4_rin_the_world_only_can_you_see_c2d35d33:

    # "Rin because she said all she has to say at once, I because I have no clue how to process what she just said."
    "Rin porque dijo todo lo que tenía que decir de una vez, yo porque no tengo idea de cómo procesar lo que acaba de decir."

# game/script-a4-rin.rpy:2611
translate es a4_rin_the_world_only_can_you_see_a87c8b78:

    # "I don't understand what Rin is saying. Or I do, but I don't want to."
    "No entiendo lo que Rin está diciendo. O sí lo entiendo, pero no quiero."

# game/script-a4-rin.rpy:2613
translate es a4_rin_the_world_only_can_you_see_464ca93e:

    # "For the first time both of these things happen, and it has to be simultaneously."
    "Por primera vez estas dos cosas suceden, y tiene que ser simultáneo."

# game/script-a4-rin.rpy:2615
translate es a4_rin_the_world_only_can_you_see_4027b135:

    # "The irony is not lost on me."
    "La ironía no se me escapa."

# game/script-a4-rin.rpy:2617
translate es a4_rin_the_world_only_can_you_see_05baf856:

    # hi "I… think everyone wants to be understood. That's universal."
    hi "Yo… creo que todos quieren ser entendidos. Eso es universal."

# game/script-a4-rin.rpy:2619
translate es a4_rin_the_world_only_can_you_see_10eb878c:

    # hi "But… that is impossible. Not only for me, but for anyone."
    hi "Pero… eso es imposible. No solo para mí, pero para todo mundo."

# game/script-a4-rin.rpy:2621
translate es a4_rin_the_world_only_can_you_see_c85f7b29:

    # hi "Sae said so too."
    hi "Sae también lo dijo."

# game/script-a4-rin.rpy:2623
translate es a4_rin_the_world_only_can_you_see_43f6713f:

    # hi "You affect other people and are affected by them, but in the end, you see everything the way only you do."
    hi "Afectas a otras personas y eres afectada por ellas, pero al final, tú ves todo de la manera en la que solo tú puedes."

# game/script-a4-rin.rpy:2625
translate es a4_rin_the_world_only_can_you_see_7a83d220:

    # hi "All people… are alone. We just use each other to alleviate that loneliness."
    hi "Todas las personas… están solas. Tan solo nos utilizamos mutuamente para aliviar esa soledad."

# game/script-a4-rin.rpy:2627
translate es a4_rin_the_world_only_can_you_see_7bfeda32:

    # "I wonder why I put it like that. It just felt that what Sae told me rang true, as if I had always thought like that without knowing it."
    "Me pregunto por qué lo dije de esa manera. Solo se sintió como si lo que dijo Sae fuera la verdad, como si siempre hubiera pensado de ese modo sin saberlo."

# game/script-a4-rin.rpy:2629
translate es a4_rin_the_world_only_can_you_see_e08edf7e:

    # "It feels like she articulated my thoughts in clear, simple words and that stupid story about Picasso."
    "Se siente como si ella hubiese articulado mis pensamientos en claras y simples palabras y en esa estúpida historia sobre Picasso."

# game/script-a4-rin.rpy:2634
translate es a4_rin_the_world_only_can_you_see_32776b4f:

    # "Rin droops her head like a withering flower, letting her bangs fall in front of her eyes so that I can't see them."
    "Rin deja caer la cabeza como una flor marchitándose, dejando que su flequillo caiga frente a sus ojos de modo que yo no pueda verlos."

# game/script-a4-rin.rpy:2636
translate es a4_rin_the_world_only_can_you_see_8462cffb:

    # rin "Why do you say that when you made me feel otherwise?"
    rin "¿Por qué dices eso cuando me hiciste sentir algo más?"

# game/script-a4-rin.rpy:2638
translate es a4_rin_the_world_only_can_you_see_cb976a1a:

    # rin "It's unfair."
    rin "No es justo."

# game/script-a4-rin.rpy:2640
translate es a4_rin_the_world_only_can_you_see_5fdefa48:

    # "The shaky voice that says those words does not belong to Rin."
    "La trémula voz que dice esas palabras no pertenece a Rin."

# game/script-a4-rin.rpy:2645
translate es a4_rin_the_world_only_can_you_see_5f2f6129:

    # rin "I really thought you could be different. That I wouldn't have to be alone."
    rin "Realmente pensé que tú podrías ser diferente. Que no tendría que estar sola."

# game/script-a4-rin.rpy:2647
translate es a4_rin_the_world_only_can_you_see_1da80edf:

    # "It's a bitter voice of disappointment, spoken through clenched teeth and a quivering chest."
    "Es una amarga voz de desilusión, dicha a través de dientes apretados y un pecho tembloroso."

# game/script-a4-rin.rpy:2649
translate es a4_rin_the_world_only_can_you_see_6000cd34:

    # hi "I'm sorry…"
    hi "Lo siento…"

# game/script-a4-rin.rpy:2651
translate es a4_rin_the_world_only_can_you_see_48be6363:

    # rin "If you are, why do you say something unfair like that?"
    rin "Si lo haces, ¿por qué dices algo injusto como eso?"

# game/script-a4-rin.rpy:2653
translate es a4_rin_the_world_only_can_you_see_40e2a175:

    # "Her demanding tone invokes no particular feeling in me, apart from sadness that has been there since yesterday evening."
    "Su tono demandante no invoca ningún sentimiento en particular en mí, además de tristeza que ha estado ahí desde ayer en la noche."

# game/script-a4-rin.rpy:2655
translate es a4_rin_the_world_only_can_you_see_8cfc03b3:

    # "She doesn't intimidate me at all. Not any more."
    "Ella no me intimida en absoluto. Ya no más."

# game/script-a4-rin.rpy:2657
translate es a4_rin_the_world_only_can_you_see_69772e59:

    # "Rin is not a prodigal art genius, nor an unpredictable idiot savant who could tear the logic lobe of my brain into shreds whenever she opened her mouth."
    "Rin no es un genio prodigio, ni una impredecible chica con savantismo que podría desgarrar el lóbulo lógico del cerebro haciéndolo jirones cuando sea que abra su boca."

# game/script-a4-rin.rpy:2659
translate es a4_rin_the_world_only_can_you_see_80eff952:

    # "She is just a girl that I thought I loved, a loved one who wanted to be my friend, a friend whom I let down."
    "Tan solo es una chica que pensé que amaba, una amada que quería ser mi amiga, una amiga que defraudé."

# game/script-a4-rin.rpy:2661
translate es a4_rin_the_world_only_can_you_see_dc671011:

    # hi "I say that, because saying otherwise would feel like lying."
    hi "Digo eso, porque decir otra cosa se sentiría como mentir."

# game/script-a4-rin.rpy:2666
translate es a4_rin_the_world_only_can_you_see_d66e33b2:

    # rin "Why?"
    rin "¿Por qué?"

# game/script-a4-rin.rpy:2668
translate es a4_rin_the_world_only_can_you_see_7b9192bc:

    # "Simple questions are the hardest ones. I have to close my eyes so I can focus my thoughts enough to answer her."
    "Las preguntas simples son las más difíciles. Tengo que cerrar los ojos para así poder concentrar mis pensamientos lo suficiente para responderle."

# game/script-a4-rin.rpy:2670
translate es a4_rin_the_world_only_can_you_see_30c0befa:

    # hi "I'm no artist. I can never be on the same level with you."
    hi "No soy un artista. Nunca podré estar al mismo nivel que tú."

# game/script-a4-rin.rpy:2672
translate es a4_rin_the_world_only_can_you_see_0030f0a3:

    # hi "There is a world only you can see, and to be part of it I would have to become you."
    hi "Hay un mundo que solo tú puedes ver, y para ser parte de él tendría que convertirme en ti."

# game/script-a4-rin.rpy:2674
translate es a4_rin_the_world_only_can_you_see_bdb58370:

    # hi "That's something I can't do, no matter how much you wish me to."
    hi "Eso es algo que no puedo hacer, sin importar qué tanto desees que lo haga."

# game/script-a4-rin.rpy:2676
translate es a4_rin_the_world_only_can_you_see_32e19661:

    # "Rin takes in my explanation without batting an eyelash."
    "Rin toma mi explicación sin mover una pestaña."

# game/script-a4-rin.rpy:2678
translate es a4_rin_the_world_only_can_you_see_70c4d36e:

    # rin "I'm not a real artist either."
    rin "Tampoco soy una artista de verdad."

# game/script-a4-rin.rpy:2680
translate es a4_rin_the_world_only_can_you_see_29cb1b16:

    # rin "I just paint because it makes me feel like I can really feel something."
    rin "Yo solo pinto porque me hace sentir como si en realidad pudiera sentir algo."

# game/script-a4-rin.rpy:2685
translate es a4_rin_the_world_only_can_you_see_a2ea179c:

    # "She holds her breath for a while before releasing it in a long, sigh-like flow."
    "Contiene el aliento por un momento antes de sacarlo en un largo flujo parecido a un suspiro."

# game/script-a4-rin.rpy:2690
translate es a4_rin_the_world_only_can_you_see_a06d90bb:

    # rin "That's why I'll do it."
    rin "Es por eso que lo haré."

# game/script-a4-rin.rpy:2692
translate es a4_rin_the_world_only_can_you_see_9f0de06c:

    # rin "I have decided. I'll do it. If even Hisao says that, then that's what I will do."
    rin "Ya me he decidido. Lo haré. Si incluso Hisao lo dice, entonces eso es lo que haré."

# game/script-a4-rin.rpy:2694
translate es a4_rin_the_world_only_can_you_see_f0418c62:

    # hi "Do what?"
    hi "¿Hacer qué?"

# game/script-a4-rin.rpy:2696
translate es a4_rin_the_world_only_can_you_see_ce5fbd43:

    # "Rin starting a little shows that she had regressed into talking to herself again, but I'm glad I can snap her back even now."
    "Rin, asustándose un poco, muestra que se ha revertido a hablar consigo misma otra vez, pero me alegro de que pueda hacerla volver a la realidad."

# game/script-a4-rin.rpy:2701
translate es a4_rin_the_world_only_can_you_see_cdb6b300:

    # rin "Teacher and Sae have talked with someone who is a very important person. I got a scholarship for a big art school in Tokyo."
    rin "El maestro y Sae han hablado con alguien que es una persona muy importante. Me dieron una beca para una gran escuela de arte en Tokyo."

# game/script-a4-rin.rpy:2703
translate es a4_rin_the_world_only_can_you_see_3af27008:

    # rin "He said I could transfer there and start after the summer is over, if I wanted to."
    rin "Dijo que me podía transferir allá y comenzar después de que termine el verano, si quería."

# game/script-a4-rin.rpy:2705
translate es a4_rin_the_world_only_can_you_see_222b7f04:

    # rin "I don't really get why—"
    rin "En realidad no entiendo por qué—"

# game/script-a4-rin.rpy:2709
translate es a4_rin_the_world_only_can_you_see_5ccc20ae:

    # hi "Hold on, what? Why didn't you tell?"
    hi "Espera, ¿qué? ¿Por qué no lo dijiste?"

# game/script-a4-rin.rpy:2714
translate es a4_rin_the_world_only_can_you_see_89ac8615:

    # rin "I just did. You are the first one I told because I decided it just now."
    rin "Acabo de hacerlo. Eres el primero al que se lo digo porque acabo de decidirlo apenas."

# game/script-a4-rin.rpy:2716
translate es a4_rin_the_world_only_can_you_see_d43185c5:

    # "She keeps her cool, looking only mildly surprised at my shocked interjection."
    "Mantiene su sangre fría, viéndose apenas ligeramente sorprendida de mi exclamación estupefacta."

# game/script-a4-rin.rpy:2718
translate es a4_rin_the_world_only_can_you_see_b75b8aa9:

    # "It's ridiculous how easily she can say something so life-changing."
    "Es ridículo qué tan fácil puede decir algo que te cambia tanto la vida."

# game/script-a4-rin.rpy:2720
translate es a4_rin_the_world_only_can_you_see_1353e44e:

    # "I can't believe it. After what happened in February, I have had enough change for this year."
    "No puedo creerlo. Después de lo que pasó en febrero, ya he tenido suficiente por este año."

# game/script-a4-rin.rpy:2722
translate es a4_rin_the_world_only_can_you_see_bf32c6bd:

    # "Even if things are going badly right now, I don't want everything to change."
    "Aun si las cosas andan mal en este momento, no quiero que todo cambie."

# game/script-a4-rin.rpy:2724
translate es a4_rin_the_world_only_can_you_see_63297b3a:

    # hi "But what about Yamaku? Don't you want to graduate with everyone?"
    hi "Pero, ¿qué hay de Yamaku? ¿No te quieres graduar con todos?"

# game/script-a4-rin.rpy:2726
translate es a4_rin_the_world_only_can_you_see_dd3fd004:

    # "My plea evokes no emotion."
    "Mi súplica no provoca emoción."

# game/script-a4-rin.rpy:2728
translate es a4_rin_the_world_only_can_you_see_6339218e:

    # rin "Everyone who?"
    rin "¿Todos quiénes?"

# game/script-a4-rin.rpy:2730
translate es a4_rin_the_world_only_can_you_see_5b83c79a:

    # hi "Emi, me, everyone!"
    hi "¡Emi, yo, todos!"

# game/script-a4-rin.rpy:2732
translate es a4_rin_the_world_only_can_you_see_fc9d34f8:

    # "I feel my pulse rising unnervingly, and my breathing becomes fast and shallow."
    "Siento mi pulso elevarse alarmantemente, y mi respiración se hace rápida y poco profunda."

# game/script-a4-rin.rpy:2734
translate es a4_rin_the_world_only_can_you_see_6d5053e2:

    # "I don't want this to happen."
    "No quiero que esto ocurra."

# game/script-a4-rin.rpy:2736
translate es a4_rin_the_world_only_can_you_see_9475602e:

    # rin "Their life is not mine."
    rin "Su vida no es la mía."

# game/script-a4-rin.rpy:2738
translate es a4_rin_the_world_only_can_you_see_8fd58498:

    # rin "You just said that everyone is alone."
    rin "Tú acabas de decir que todos están solos."

# game/script-a4-rin.rpy:2740
translate es a4_rin_the_world_only_can_you_see_6111e161:

    # hi "I didn't mean it like that—"
    hi "No quise decirlo de ese modo—"

# game/script-a4-rin.rpy:2742
translate es a4_rin_the_world_only_can_you_see_72a13a46:

    # rin "You always said that you'd have to seize the day and start living your life."
    rin "Tú siempre decías que tendrías que aprovechar el día y comenzar a vivir tu vida."

# game/script-a4-rin.rpy:2744
translate es a4_rin_the_world_only_can_you_see_5dcb8c2f:

    # rin "I have to live my life too."
    rin "También yo tengo que vivir mi vida."

# game/script-a4-rin.rpy:2746
translate es a4_rin_the_world_only_can_you_see_960ce380:

    # "Rin is twisting my words to justify running away again. It makes me angry."
    "Rin está torciendo mis palabras para justificar el huir de nuevo. Me hace enojar."

# game/script-a4-rin.rpy:2748
translate es a4_rin_the_world_only_can_you_see_443f95fc:

    # "Her ease, finality and seriousness in announcing this is unacceptable."
    "Su tranquilidad, firmeza y seriedad al anunciar esto es inaceptable."

# game/script-a4-rin.rpy:2750
translate es a4_rin_the_world_only_can_you_see_c7f719b2:

    # "As if changing your life is something you can do on a moment's whim! No!"
    "¡Como si cambiar tu vida es algo que puedas hacer en un capricho de un momento! ¡No!"

# game/script-a4-rin.rpy:2752
translate es a4_rin_the_world_only_can_you_see_47f0411b:

    # hi "How can you say that? Why don't you even try to belong?"
    hi "¿Cómo puedes decir eso? ¿Por qué ni siquiera tratas de pertenecer?"

# game/script-a4-rin.rpy:2754
translate es a4_rin_the_world_only_can_you_see_db625628:

    # "The desperate accusation has no effect. It feels like I am once again out of weapons, that I can't reach through to her no matter what I try."
    "La acusación desesperada no tiene efecto. Se siente como si una vez más me encontrara sin armas, que no puedo llegar a ella sin importar qué trate."

# game/script-a4-rin.rpy:2756
translate es a4_rin_the_world_only_can_you_see_a0eaae9b:

    # "Rin is so frustratingly absolute in her own judgment that it might make me hate her if I didn't love her, even though I don't know which way I am feeling any more."
    "Rin es tan frustrantemente absoluta en su propio juicio que podría hacerme odiarla si no la amara, aunque ya no sé de qué forma me siento."

# game/script-a4-rin.rpy:2761
translate es a4_rin_the_world_only_can_you_see_d5e62c37:

    # rin "Maybe I am that kind of a person. The kind that belongs only to herself."
    rin "Tal vez soy ese tipo de persona. Del tipo que pertenece solo a sí misma."

# game/script-a4-rin.rpy:2763
translate es a4_rin_the_world_only_can_you_see_53beb146:

    # hi "I won't accept that."
    hi "No aceptaré eso."

# game/script-a4-rin.rpy:2765
translate es a4_rin_the_world_only_can_you_see_8a5624cd:

    # "Her nonchalant eyes do not seem to care whether I accept her decision or not."
    "A sus indiferentes ojos no parece importarles si acepto su decisión o no."

# game/script-a4-rin.rpy:2767
translate es a4_rin_the_world_only_can_you_see_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:2769
translate es a4_rin_the_world_only_can_you_see_40b7767e:

    # "The pause lets me cool down, to find my sensibilities."
    "La pausa me permite tranquilizarme, para encontrar mis sensibilidades."

# game/script-a4-rin.rpy:2771
translate es a4_rin_the_world_only_can_you_see_9a6da514:

    # "While I do, the parting rainclouds reveal a setting sun that still has time to shine its last few warming rays before calling it a day."
    "Mientras lo hago, las nubes de lluvia que se retiran revelan un sol poniéndose que todavía tiene tiempo de brillar sus últimos cálidos rayos antes de terminar el día."

# game/script-a4-rin.rpy:2773
translate es a4_rin_the_world_only_can_you_see_eb76af50:

    # "A mosaic of light and shadow spreads on the walls of the buildings, on the street and the fence circling a park on the other side of the street."
    "Un mosaico de luz y sombra se propaga sobre los muros de los edificios, en la calle y la valla que rodea al parque al otro lado de la calle."

# game/script-a4-rin.rpy:2775
translate es a4_rin_the_world_only_can_you_see_57201433:

    # "Rin's shadow is long enough to reach my feet."
    "La sombra de Rin es lo suficientemente larga para alcanzar mis pies."

# game/script-a4-rin.rpy:2777
translate es a4_rin_the_world_only_can_you_see_254a5629:

    # "It's like one of those western movies, with two cowboys staring each other down, ready to sling their guns at each other."
    "Es como una de esas películas del viejo oeste, con dos vaqueros mirándose fijamente el uno al otro, listos para desenfundar sus pistolas contra el otro."

# game/script-a4-rin.rpy:2779
translate es a4_rin_the_world_only_can_you_see_75bc3df4:

    # "The one who loses his nerve will eat lead."
    "Aquel que se acobarde comerá plomo."

# game/script-a4-rin.rpy:2781
translate es a4_rin_the_world_only_can_you_see_7e45710c:

    # "I realize I would have the disadvantage because the sun is behind Rin, stinging my eyes."
    "Me doy cuenta de que yo tendría la desventaja porque el sol está detrás de Rin, lastimándome los ojos."

# game/script-a4-rin.rpy:2786
translate es a4_rin_the_world_only_can_you_see_956ad423:

    # rin "Do you hate me?"
    rin "¿Me odias?"

# game/script-a4-rin.rpy:2788
translate es a4_rin_the_world_only_can_you_see_11bdbe9f:

    # "She draws first and I have no counter."
    "Ella desenfunda primero y no tengo respuesta."

# game/script-a4-rin.rpy:2790
translate es a4_rin_the_world_only_can_you_see_3b1d8348:

    # hi "I don't know."
    hi "No lo sé."

# game/script-a4-rin.rpy:2792
translate es a4_rin_the_world_only_can_you_see_ad8b8b19:

    # "Did I lose?"
    "¿Acaso perdí?"

# game/script-a4-rin.rpy:2794
translate es a4_rin_the_world_only_can_you_see_5a5c0a67:

    # hi "Even if I did, what would it matter?"
    hi "Aun si lo hiciera, ¿qué importancia tendría?"

# game/script-a4-rin.rpy:2796
translate es a4_rin_the_world_only_can_you_see_4f4e1a70:

    # "I scramble for words, words that could salvage this. I find none."
    "Batallo para encontrar palabras, palabras que podrían rescatar esto. No encuentro ninguna."

# game/script-a4-rin.rpy:2798
translate es a4_rin_the_world_only_can_you_see_fb7e1ba6:

    # hi "You are my friend, I promised you that. I am not the kind of guy who forgets about promises."
    hi "Eres mi amiga, prometiste eso. No soy de la clase de tipos que olvidan las promesas."

# game/script-a4-rin.rpy:2800
translate es a4_rin_the_world_only_can_you_see_707e1298:

    # hi "I think that is the most important thing. We could try to—"
    hi "Creo que eso es lo más importante. Podríamos tratar de—"

# game/script-a4-rin.rpy:2805
translate es a4_rin_the_world_only_can_you_see_2edda4e4:

    # rin "Don't say it."
    rin "No lo digas."

# game/script-a4-rin.rpy:2812
translate es a4_rin_the_world_only_can_you_see_3623627b:

    # "Predicting what I was going to say, Rin throws herself into my arms, pressing her body against mine."
    "Prediciendo lo que iba a decir, Rin se arroja a mis brazos, presionando su cuerpo contra el mío."

# game/script-a4-rin.rpy:2814
translate es a4_rin_the_world_only_can_you_see_2736a4f0:

    # "I feel her rising to her tiptoes to match my height and snuggle closer."
    "La siento pararse de puntillas para igualar mi altura y acurrucarse más cerca."

# game/script-a4-rin.rpy:2816
translate es a4_rin_the_world_only_can_you_see_a4f3b77f:

    # "The scent of her hair is that of rain and paint thinner. Her body feels as cold as always. Her breathing against my neck is as hot as always."
    "El aroma de su cabello es aquel de lluvia y solvente de pintura. Su cuerpo se siente tan frío como siempre. Su aliento contra mi cuello es tan caliente como siempre."

# game/script-a4-rin.rpy:2818
translate es a4_rin_the_world_only_can_you_see_a0af7b9d:

    # "It's funny how all of those feel so familiar even though Rin, as a whole, does not."
    "Es gracioso cómo todos ellos se sienten tan familiares aun si Rin, como un todo, no."

# game/script-a4-rin.rpy:2823
translate es a4_rin_the_world_only_can_you_see_a334318d:

    # rin "Are you sure you can't hate me?"
    rin "¿Estás seguro de que no puedes odiarme?"

# game/script-a4-rin.rpy:2825
translate es a4_rin_the_world_only_can_you_see_e54d1688:

    # "Rin whispers into my ear so close I can feel the movements of her lips against my earlobe."
    "Rin susurra en mi oído tan cerca que puedo sentir el movimiento de sus labios en el lóbulo de mi oreja."

# game/script-a4-rin.rpy:2827
translate es a4_rin_the_world_only_can_you_see_d1a66320:

    # "It's teasing, taunting. If this was some other kind of situation I'm sure it would tickle tantalizingly and I would giggle even though I'm a guy."
    "Es provocativo y burlón. Si esto fuera una situación de algún otro tipo estoy seguro de que me haría cosquillas de manera seductora y me reiría nerviosamente aunque sea un hombre."

# game/script-a4-rin.rpy:2829
translate es a4_rin_the_world_only_can_you_see_7a4aa9a5:

    # rin "It would be easier if you did."
    rin "Sería más fácil si lo hicieras."

# game/script-a4-rin.rpy:2831
translate es a4_rin_the_world_only_can_you_see_fbe64de4:

    # hi "Dunno. It's pretty hard when you are hugging me like that."
    hi "No sé. Es bastante difícil cuando me estás abrazando así."

# game/script-a4-rin.rpy:2836
translate es a4_rin_the_world_only_can_you_see_b9eaea40:

    # "I wonder if it's because of my sullen voice, but she takes a step back, looking wistfully at her short arms."
    "Me pregunto si es por mi voz taciturna, pero ella da un paso hacia atrás, viendo con melancolía a sus cortos brazos."

# game/script-a4-rin.rpy:2838
translate es a4_rin_the_world_only_can_you_see_4b12741f:

    # "I wish she hadn't done that."
    "Desearía que ella no hubiese hecho eso."

# game/script-a4-rin.rpy:2840
translate es a4_rin_the_world_only_can_you_see_658b4d83:

    # rin "I can't hug anyone, Hisao."
    rin "No puedo abrazar a nadie, Hisao."

# game/script-a4-rin.rpy:2842
translate es a4_rin_the_world_only_can_you_see_7d52b351:

    # rin "I'm a bad person like that."
    rin "Soy una persona así de mala."

# game/script-a4-rin.rpy:2847
translate es a4_rin_the_world_only_can_you_see_29e24d1e:

    # rin "That's why I have to go."
    rin "Es por eso que tengo que irme."

# game/script-a4-rin.rpy:2849
translate es a4_rin_the_world_only_can_you_see_f2da7cf9:

    # "She disarms me completely with three simple sentences, rendering me unable to argue any more."
    "Me desarma por completo con tres simples oraciones, dejándome incapaz de discutir más."

# game/script-a4-rin.rpy:2851
translate es a4_rin_the_world_only_can_you_see_d92b597d:

    # "And since I can't, Rin is free to continue as she wills, shifting her weight from one foot to the other before she does."
    "Y ya que no puedo, Rin es libre de continuar como desee, balanceándose de un pie al otro antes de hacerlo."

# game/script-a4-rin.rpy:2856
translate es a4_rin_the_world_only_can_you_see_c14dd8d3:

    # rin "I will learn to hug people in my own way."
    rin "Aprenderé a abrazar a la gente a mi propia manera."

# game/script-a4-rin.rpy:2858
translate es a4_rin_the_world_only_can_you_see_5a5e600a:

    # rin "I'm sure I can become a real artist."
    rin "Estoy segura de que puedo convertirme en una artista verdadera."

# game/script-a4-rin.rpy:2860
translate es a4_rin_the_world_only_can_you_see_f734e332:

    # rin "But if I do… I might not be able to be me any more."
    rin "Pero si lo hago… Puede que ya no sea capaz de ser yo."

# game/script-a4-rin.rpy:2862
translate es a4_rin_the_world_only_can_you_see_56bd51cc:

    # "The hint of a smile on her lips is a betrayal, a false sign of self-confidence in a future that even Rin can't foresee."
    "La insinuación de una sonrisa sobre sus labios es una traición, una falsa señal de confianza en un futuro que incluso Rin no puede prever."

# game/script-a4-rin.rpy:2864
translate es a4_rin_the_world_only_can_you_see_0aa77a3c:

    # "I'd want to interpret it as a sign of hope, but I know better."
    "Quisiera interpretarlo como una señal de esperanza, pero sé que no puedo."

# game/script-a4-rin.rpy:2866
translate es a4_rin_the_world_only_can_you_see_984cd297:

    # "Rin just keeps smiling that awkward, forced smile of hers."
    "Rin solo continúa sonriendo esa extraña y forzada sonrisa suya."

# game/script-a4-rin.rpy:2868
translate es a4_rin_the_world_only_can_you_see_35c122ed:

    # rin "That's why… please forget about me, and I will forget about you too."
    rin "Es por eso que… por favor olvídate de mí, y yo también me olvidaré de ti."

# game/script-a4-rin.rpy:2870
translate es a4_rin_the_world_only_can_you_see_c893b703:

    # rin "I'm sure that—{w=0.5}{nw}"
    rin "Estoy segura de—{w=0.5}{nw}"

# game/script-a4-rin.rpy:2875
translate es a4_rin_the_world_only_can_you_see_766706de:

    # "She chokes in the middle of saying something I would never come to hear."
    "Rin se ahoga a la mitad de decir algo que nunca llegaré a escuchar."

# game/script-a4-rin.rpy:2877
translate es a4_rin_the_world_only_can_you_see_c663fefa:

    # "I don't think I'd wanted to hear it anyway."
    "No creo que hubiera querido oírlo de cualquier modo."

# game/script-a4-rin.rpy:2879
translate es a4_rin_the_world_only_can_you_see_268c61ba:

    # "This is not fair."
    "Esto no es justo."

# game/script-a4-rin.rpy:2881
translate es a4_rin_the_world_only_can_you_see_1054418b:

    # "Rin is not joking. Rin is always serious. But I can't accept it, I can't."
    "Rin no está bromeando. Rin siempre habla en serio. Pero no puedo aceptarlo, no puedo."

# game/script-a4-rin.rpy:2883
translate es a4_rin_the_world_only_can_you_see_b1787844:

    # "Forget about you? How could I ever…?"
    "¿Olvidarme de ti? ¿Cómo podría…?"

# game/script-a4-rin.rpy:2885
translate es a4_rin_the_world_only_can_you_see_af6e097f:

    # "That's what I'd like to say. But I don't know how I would continue. I can't come up with anything good to say, so I have to challenge her."
    "Eso es lo que quisiera decir. Pero no sé cómo continuaría. No se me ocurre nada bueno que decir, así que tengo que retarla."

# game/script-a4-rin.rpy:2887
translate es a4_rin_the_world_only_can_you_see_8ec01e7b:

    # hi "How can you say such a thing?"
    hi "¿Cómo puedes decir tal cosa?"

# game/script-a4-rin.rpy:2892
translate es a4_rin_the_world_only_can_you_see_e068ae10:

    # "Rin raises her eyes to meet mine, they are serious and deep, a perfect image of the uncharted territory I always thought they were."
    "Rin levanta los ojos para encontrarse con los míos, son serios y profundos, una imagen perfecta del territorio sin explorar que siempre pensé que eran."

# game/script-a4-rin.rpy:2894
translate es a4_rin_the_world_only_can_you_see_8dd10a61:

    # "Even now, I can't read her emotions from those unblinking, jade irises that never could reflect what they saw."
    "Incluso ahora, no puedo leer sus emociones a partir de esos iris imperturbables de jade que nunca pudieron reflejar lo que veían."

# game/script-a4-rin.rpy:2896
translate es a4_rin_the_world_only_can_you_see_0f93dbe1:

    # rin "It's easy. After all, I am good at forgetting things."
    rin "Es fácil. Después de todo, soy buena olvidando cosas."

# game/script-a4-rin.rpy:2898
translate es a4_rin_the_world_only_can_you_see_ce617998_2:

    # "…"
    "…"

# game/script-a4-rin.rpy:2900
translate es a4_rin_the_world_only_can_you_see_fc924da8:

    # "Her unfairness is choking my throat, but I manage to utter the question burning my mind."
    "Su falta de justicia me hace un nudo en la garganta, pero logro pronunciar la pregunta ardiendo en mi mente."

# game/script-a4-rin.rpy:2902
translate es a4_rin_the_world_only_can_you_see_edab4dd5:

    # hi "So, is this it? Is this goodbye?"
    hi "Así que, ¿esto es todo? ¿Esto es el adiós?"

# game/script-a4-rin.rpy:2904
translate es a4_rin_the_world_only_can_you_see_ce617998_3:

    # "…"
    "…"

# game/script-a4-rin.rpy:2906
translate es a4_rin_the_world_only_can_you_see_445a5121:

    # "Rin kept looking at me gently, without answering my question."
    "Rin siguió viéndome gentilmente, sin responder mi pregunta."

# game/script-a4-rin.rpy:2908
translate es a4_rin_the_world_only_can_you_see_d9c22d49:

    # "From her eyes I could see that she didn't even need to say anything."
    "De sus ojos pude ver que ella ni siquiera necesitaba decir algo."

# game/script-a4-rin.rpy:2910
translate es a4_rin_the_world_only_can_you_see_c9799485:

    # "There were no more words for us."
    "No había más palabras para nosotros."

# game/script-a4-rin.rpy:2917
translate es a4_rin_the_world_only_can_you_see_9324cd77:

    # "She turned around and walked off without looking back."
    "Se dio la vuelta y caminó sin mirar atrás."

# game/script-a4-rin.rpy:2919
translate es a4_rin_the_world_only_can_you_see_91ab4a5f:

    # "All around me, the world kept changing, little by little, but I was left standing there."
    "Todo a mi alrededor, el mundo siguió cambiando, poco a poco, pero yo me quedé de pie ahí."

# game/script-a4-rin.rpy:2924
translate es a4_rin_the_world_only_can_you_see_6c80a6b7:

    # "The sun dropped below the horizon, casting long and thin shadows across the street."
    "El sol cayó debajo del horizonte, proyectando largas y delgadas sombras por la calle."

# game/script-a4-rin.rpy:2926
translate es a4_rin_the_world_only_can_you_see_55bcdcc7:

    # "In the waning light, Rin's distancing back seemed to be like from a faraway dream."
    "En la menguante luz, la espalda distante de Rin parece haber sido de un sueño lejano."

# game/script-a4-rin.rpy:2928
translate es a4_rin_the_world_only_can_you_see_09b6a8a5:

    # "The gap between us grew slowly."
    "La brecha entre nosotros creció lentamente."

# game/script-a4-rin.rpy:2930
translate es a4_rin_the_world_only_can_you_see_2b6ec8f5:

    # "The ripples on the puddles she stepped on expanded until they met the limits of their tiny existence and disappeared without a trace."
    "Las ondas en los charcos que pisó se expandieron hasta llegar a los límites de su pequeña existencia y desaparecieron sin dejar rastro."

# game/script-a4-rin.rpy:2932
translate es a4_rin_the_world_only_can_you_see_f4d5e348:

    # "Her words stayed frozen deep inside my heart."
    "Sus palabras se quedaron congeladas en lo más profundo de mi corazón."

# game/script-a4-rin.rpy:2980
translate es a4_rin_desperate_glory_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:2945
translate es a4_rin_desperate_glory_e96c8674:

    # n "{vspace=30}Vacation, huh?"
    n "{vspace=30}Vacaciones, ¿eh?"

# game/script-a4-rin.rpy:2947
translate es a4_rin_desperate_glory_329304f8:

    # n "Some people will stay at the school even over the holidays, some will go back to their families."
    n "Algunos se quedarán en la escuela aun durante las vacaciones, otros volverán con sus familias."

# game/script-a4-rin.rpy:2949
translate es a4_rin_desperate_glory_77de5f7d:

    # n "I don't know what to do. I should go buy a train ticket for my trip back home, but I can't bring myself to do it."
    n "Yo no sé qué hacer. Debería ir a comprar un boleto de tren para mi viaje a casa, pero simplemente no me dan ganas de hacerlo."

# game/script-a4-rin.rpy:2951
translate es a4_rin_desperate_glory_64e8f2ca:

    # n "I bet I'm going to get a call from home again. Mom's going to pester me about when I'm coming back, and I'm not going to know what to answer."
    n "Apuesto a que recibiré una llamada de casa otra vez. Mi mamá me molestará con cuándo voy a volver, y no voy a saber qué responder."

# game/script-a4-rin.rpy:2953
translate es a4_rin_desperate_glory_8e72648f:

    # n "{vspace=30}This is really lousy. In the current state of things with Rin, it feels like I can't just bail out of here and pretend we are through."
    n "{vspace=30}Esto realmente apesta. En el estado actual del asunto con Rin, se siente como si simplemente no me pueda largar de aquí y pretender que terminamos."

# game/script-a4-rin.rpy:2955
translate es a4_rin_desperate_glory_cbef346b:

    # n "{vspace=30}And now, she has other problems of her own. I thought that the exhibition opening would give her a breather, but I was sorely mistaken."
    n "{vspace=30}Y ahora, ella tiene otros problemas suyos. Pensé que la apertura de la exhibición le daría un descanso, pero estaba muy equivocado."

# game/script-a4-rin.rpy:2957
translate es a4_rin_desperate_glory_a734eb67:

    # n "{vspace=60}The tangle just seems to thicken."
    n "{vspace=60}El enredo tan solo parece hacerse más grande."

# game/script-a4-rin.rpy:2999
translate es a4_rin_desperate_glory_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:2966
translate es a4_rin_desperate_glory_bbc8465f:

    # "A sharp knock on the door interrupts the quiet but frantic mood of the last 15 minutes of the exam."
    "Un llamado fuerte a la puerta interrumpe el ánimo silencioso pero frenético de los últimos 15 minutos del examen."

# game/script-a4-rin.rpy:2973
translate es a4_rin_desperate_glory_0de9b330:

    # mu "Come in."
    mu "Pase."

# game/script-a4-rin.rpy:2986
translate es a4_rin_desperate_glory_0583d852:

    # "The opening door reveals the art teacher, who steps in with his jacket swirling around him as though in a gust of wind."
    "La puerta que se abre revela al maestro de arte, quien entra con su chaqueta arremolinándose en torno a él como si estuviese en una ráfaga de viento."

# game/script-a4-rin.rpy:2988
translate es a4_rin_desperate_glory_1589bc1c:

    # "He glances at Mutou, who glances back at him."
    "Le echa una mirada a Mutou, quien lo mira de vuelta."

# game/script-a4-rin.rpy:2996
translate es a4_rin_desperate_glory_c1fcfe89:

    # "A frown spreads simultaneously on both of their faces as the men measure each other with their gazes."
    "El ceño se frunce simultáneamente en ambos rostros mientras los hombres se miden el uno al otro con sus miradas."

# game/script-a4-rin.rpy:2998
translate es a4_rin_desperate_glory_4f63ba5a:

    # no "Excuse me, could I borrow Mr. Nakai for a moment?"
    no "Discúlpeme, ¿podría llevarme al señor Nakai por un momento?"

# game/script-a4-rin.rpy:3000
translate es a4_rin_desperate_glory_d060c84f:

    # mu "Excuse {b}me{/b}, Mr. Nomiya, but we are in the middle of an exam here."
    mu "Discúlpeme a {b}mí{/b}, señor Nomiya, pero estamos en la mitad de un examen."

# game/script-a4-rin.rpy:3002
translate es a4_rin_desperate_glory_b0ffeef2:

    # "A chilly atmosphere suddenly spreads in the middle of the summer afternoon as the two men try to stare each other down."
    "Un ambiente frío súbitamente se propaga a media tarde de verano a medida que los dos hombres siguen observándose fijamente."

# game/script-a4-rin.rpy:3007
translate es a4_rin_desperate_glory_909b80ce:

    # no "This is urgent, and it seems that Nakai has already finished."
    no "Esto es urgente, y parece que Nakai ya ha terminado."

# game/script-a4-rin.rpy:3009
translate es a4_rin_desperate_glory_de5f031f:

    # "Both men turn to look at me, staring at me like a pair of basilisks trying to petrify a tasty snack."
    "Los dos voltean a verme, mirándome como un par de basiliscos tratando de petrificar un sabroso bocadillo."

# game/script-a4-rin.rpy:3011
translate es a4_rin_desperate_glory_93f0744a:

    # "It's true that I've been idle for a good while now, so Nomiya is right, but…"
    "Es cierto que no he estado haciendo nada por un buen tiempo, así que Nomiya está en lo correcto, pero…"

# game/script-a4-rin.rpy:3016
translate es a4_rin_desperate_glory_2ce8ee7f:

    # mu "Nakai, would you like to check your answers one more time?"
    mu "Nakai, ¿te gustaría revisar tus respuestas una vez más?"

# game/script-a4-rin.rpy:3018
translate es a4_rin_desperate_glory_df9ba85a:

    # "Mutou speaks with an odd intonation, weighting certain words as if trying to send a message."
    "Mutou habla con una extraña entonación, dándole peso a ciertas palabras como si tratara de mandar un mensaje."

# game/script-a4-rin.rpy:3020
translate es a4_rin_desperate_glory_9f6ddf1d:

    # "The pressure from their stares makes me rapidly shake my head, which is apparently interpreted as an answer of some sort."
    "La presión de sus miradas me hace sacudir rápidamente la cabeza, lo cual es interpretado como una respuesta de algún tipo."

# game/script-a4-rin.rpy:3027
translate es a4_rin_desperate_glory_7b8ebea7:

    # mu "Very well. Nakai, go with Mr. Nomiya, if you please."
    mu "Muy bien. Nakai, ve con el señor Nomiya, por favor."

# game/script-a4-rin.rpy:3029
translate es a4_rin_desperate_glory_b7850a7f:

    # mu "Take your bag with you and bring your test paper to my desk."
    mu "Llévate tu mochila y trae las hojas de tu examen a mi escritorio."

# game/script-a4-rin.rpy:3034
translate es a4_rin_desperate_glory_2ea5b16f:

    # mu "You have a nice vacation."
    mu "Que tengas felices vacaciones."

# game/script-a4-rin.rpy:3036
translate es a4_rin_desperate_glory_3d7834a3:

    # hi "Umm. Er, you too, teacher."
    hi "Ahhh. Eh, igualmente, maestro."

# game/script-a4-rin.rpy:3038
translate es a4_rin_desperate_glory_a6dff9e2:

    # "The entire world… well, at least the classroom seems to hold its breath just for me, putting the exam on hold until I stand up, collect my stuff and walk to the door."
    "El mundo entero… bueno, al menos el grupo parece mantener el aliento solo para mí, suspendiendo el examen hasta que me ponga de pie, recoja mis cosas y camine fuera de la puerta."

# game/script-a4-rin.rpy:3040
translate es a4_rin_desperate_glory_35f58f7c:

    # "I can feel the stares in the back of my neck. My classmates probably think I'm in for some detention or something, on the last day of the school before summer vacation."
    "Puedo sentir las miradas sobre mi nuca. Mis compañeros de clase probablemente piensan que me castigarán o algo así, en el último día de escuela antes de las vacaciones de verano."

# game/script-a4-rin.rpy:3042
translate es a4_rin_desperate_glory_e767c37c:

    # "I don't know what the teacher wants from me, but I can guess it probably is not detention and also that it probably has something to do with Rin again."
    "No sé qué quiere el maestro de mí, pero puedo imaginar que posiblemente no es un castigo y también que posiblemente tiene algo que ver con Rin otra vez."

# game/script-a4-rin.rpy:3050
translate es a4_rin_desperate_glory_9f1852b3:

    # "Nomiya doesn't take me anywhere, contenting himself with the hallway as it's completely abandoned."
    "Nomiya no me lleva a ningún lugar, conformándose con el pasillo ya que está completamente abandonado."

# game/script-a4-rin.rpy:3057
translate es a4_rin_desperate_glory_1e354d89:

    # no "Do you know where Tezuka is?"
    no "¿Sabes dónde está Tezuka?"

# game/script-a4-rin.rpy:3059
translate es a4_rin_desperate_glory_8c4ea6f8:

    # "So she is trying to avoid the teacher… par for the course, probably."
    "Así que ella está tratando de evitar al maestro… como es de costumbre, probablemente."

# game/script-a4-rin.rpy:3061
translate es a4_rin_desperate_glory_566d2aa5:

    # "I wonder if she realizes that she can't avoid dealing with this indefinitely."
    "Me pregunto si se da cuenta de que no puede evitar enfrentarse con esto indefinidamente."

# game/script-a4-rin.rpy:3063
translate es a4_rin_desperate_glory_2d914b5b:

    # hi "I have no idea."
    hi "No tengo idea."

# game/script-a4-rin.rpy:3065
translate es a4_rin_desperate_glory_f3678e70:

    # hi "You have probably asked in her homeroom next door."
    hi "Quizás ya haya preguntado por ella en su grupo en la puerta de enseguida."

# game/script-a4-rin.rpy:3070
translate es a4_rin_desperate_glory_389fb581:

    # no "Of course I have! I have searched every nook and cranny of this blasted school and the girls' dorm."
    no "¡Por supuesto que lo he hecho! He buscado en todos los rincones de esta maldita escuela y en el dormitorio de chicas."

# game/script-a4-rin.rpy:3072
translate es a4_rin_desperate_glory_7f306d19:

    # no "You are the last one to see her since yesterday and you are her friend."
    no "Eres el último que la vio desde ayer y eres su amigo."

# game/script-a4-rin.rpy:3077
translate es a4_rin_desperate_glory_82fbee90:

    # no "Work with me here. Aren't you worried?"
    no "Échame la mano. ¿Que no estás preocupado?"

# game/script-a4-rin.rpy:3079
translate es a4_rin_desperate_glory_cc663513:

    # "I am, but I don't know what I could do."
    "Lo estoy, pero no sé qué podría hacer."

# game/script-a4-rin.rpy:3081
translate es a4_rin_desperate_glory_502ebbdf:

    # "Rin did something incomprehensible yesterday, even for her."
    "Rin hizo algo incomprensible anoche, incluso para ella."

# game/script-a4-rin.rpy:3083
translate es a4_rin_desperate_glory_7a68094c:

    # "She seemed really confused."
    "Se veía muy confundida."

# game/script-a4-rin.rpy:3085
translate es a4_rin_desperate_glory_15b583f7:

    # hi "Maybe she just wants some time to think then. I got the feeling that she had second thoughts about having that exhibition."
    hi "Tal vez solo quiere algo de tiempo para pensar. Tuve la sensación de que ella estaba dudando de tener esa exhibición."

# game/script-a4-rin.rpy:3087
translate es a4_rin_desperate_glory_0a4398c3:

    # "Or something. She really didn't explain what was wrong."
    "O algo así. Realmente no explicó qué era lo que ocurría."

# game/script-a4-rin.rpy:3092
translate es a4_rin_desperate_glory_a6fe6193:

    # no "What second thoughts?"
    no "¿Dudando de qué?"

# game/script-a4-rin.rpy:3094
translate es a4_rin_desperate_glory_6e9b4029:

    # hi "I dunno. Just got that feeling."
    hi "No lo sé. Solo tuve esa sensación."

# game/script-a4-rin.rpy:3096
translate es a4_rin_desperate_glory_7956358e:

    # "I am being a little dishonest with the teacher, but this is not something I should be meddling with."
    "Estoy siendo un poco deshonesto con el maestro, pero esto no es algo en lo que me debería de estar entrometiendo."

# game/script-a4-rin.rpy:3098
translate es a4_rin_desperate_glory_e7e1c948:

    # "He came to me… yes, why? Maybe he thinks I'm some kind of confidant of Rin's, but I don't think I can help with this matter."
    "Él vino hacia mí… sí, ¿por qué? Quizás piense que soy como alguien de confianza para Rin, pero no creo que pueda ayudar en este asunto."

# game/script-a4-rin.rpy:3103
translate es a4_rin_desperate_glory_67914ac7:

    # "The teacher huffs and scratches his head in confusion."
    "El maestro resopla y se rasca la cabeza en confusión."

# game/script-a4-rin.rpy:3105
translate es a4_rin_desperate_glory_bc4932fe:

    # no "What's up with that girl? This is so unlike her, she's always been so goal-driven."
    no "¿Qué pasa con esa niña? Esto no es como ella, siempre ha sido tan determinada."

# game/script-a4-rin.rpy:3107
translate es a4_rin_desperate_glory_a698630c:

    # "'Goal-driven?' Those don't really strike me as words to describe Rin with."
    "¿“Determinada”? Esa realmente no me parece una palabra con la cual describir a Rin."

# game/script-a4-rin.rpy:3109
translate es a4_rin_desperate_glory_aae2da5f:

    # "To me, she always felt obsessive at best."
    "Para mí, ella siempre dio la sensación de ser obsesiva como mínimo."

# game/script-a4-rin.rpy:3111
translate es a4_rin_desperate_glory_b7db4b12:

    # hi "Er, I don't mean to be rude, but wasn't it you who pushed Rin to that direction in the first place?"
    hi "Eh, no quiero ser grosero, ¿pero no fue usted quien empujó a Rin en esa dirección en primer lugar?"

# game/script-a4-rin.rpy:3116
translate es a4_rin_desperate_glory_5a0efb0a:

    # no "Her goal is my goal. That is a mentor's job."
    no "Su meta es mi meta. Ese es el trabajo de un mentor."

# game/script-a4-rin.rpy:3118
translate es a4_rin_desperate_glory_c9e6b5c1:

    # hi "I guess so. I just don't know if painting can make her happy."
    hi "Supongo. Es solo que no sé si pintar puede hacerla feliz."

# game/script-a4-rin.rpy:3123
translate es a4_rin_desperate_glory_88a7d18b:

    # no "That's pretty preposterous of you to say, Nakai."
    no "Tú diciendo eso es bastante absurdo, Nakai."

# game/script-a4-rin.rpy:3125
translate es a4_rin_desperate_glory_d223337e:

    # "He suddenly sounds pretty irate. Did I say something stupid?"
    "Repentinamente suena bastante furioso. ¿Dije algo estúpido?"

# game/script-a4-rin.rpy:3130
translate es a4_rin_desperate_glory_c6fd24d7:

    # no "You don't understand, do you? It is not a question of happiness. For every gain there is a sacrifice to be made."
    no "Tú no entiendes, ¿verdad? No es una cuestión de felicidad. Por cada victoria hay un sacrificio por hacerse."

# game/script-a4-rin.rpy:3135
translate es a4_rin_desperate_glory_458c5978:

    # no "There is no free lunch, but could I… would I let that girl waste away her talent if she has a moment of doubt? Never!"
    no "No puedes tener algo a cambio de nada, pero acaso yo podría… ¿dejaría a esa chica desperdiciar su talento si tiene un momento de incertidumbre? ¡Nunca!"

# game/script-a4-rin.rpy:3137
translate es a4_rin_desperate_glory_cc053d13:

    # no "Painting is work just like any other. Tezuka might make it look like child's play to you, but she works hard every day to make her art."
    no "Pintar es un trabajo como cualquier otro. Tezuka puede hacerlo parecer como un juego de niños para ti, pero ella trabaja duro cada día para hacer su arte."

# game/script-a4-rin.rpy:3139
translate es a4_rin_desperate_glory_de48fc4a:

    # no "To become extraordinary, one has to make an extraordinary effort."
    no "Para ser extraordinario, uno tiene que hacer un esfuerzo extraordinario."

# game/script-a4-rin.rpy:3141
translate es a4_rin_desperate_glory_5600875c:

    # "The more the teacher talks, the more I feel that Rin doesn't think like that, even though I have no idea how she thinks."
    "Mientras más habla el maestro, más pienso que Rin no piensa de ese modo, aun si no tengo idea de cómo piensa ella."

# game/script-a4-rin.rpy:3146
translate es a4_rin_desperate_glory_7a9b4bba:

    # no "I can very well understand why she would sacrifice her summer vacation and make up for the lost classes and exams to get a chance at showing her art."
    no "Puedo entender muy bien por qué sacrificaría sus vacaciones de verano y repondría las clases perdidas y los exámenes para tener una oportunidad de mostrar su arte."

# game/script-a4-rin.rpy:3148
translate es a4_rin_desperate_glory_cbba8f9c:

    # no "This is the path she has taken, and to go all the way, that's not easy."
    no "Este es el camino que ella ha tomado, y seguirlo hasta el final, no es fácil."

# game/script-a4-rin.rpy:3150
translate es a4_rin_desperate_glory_c59dea95:

    # no "I know she is young, and things are hard for her just like for all the kids here in this school, but that's no excuse."
    no "Sé que es joven, y es difícil para ella al igual que lo es para todos los muchachos en esta escuela, pero eso no es excusa."

# game/script-a4-rin.rpy:3152
translate es a4_rin_desperate_glory_80f7a08f:

    # "He is finished."
    "Ya ha terminado."

# game/script-a4-rin.rpy:3154
translate es a4_rin_desperate_glory_f2449dda:

    # hi "But—"
    hi "Pero—"

# game/script-a4-rin.rpy:3159
translate es a4_rin_desperate_glory_24d8a14e:

    # no "Do you have anything like what art is to Tezuka?"
    no "¿Tienes algo tuyo como el arte es para Tezuka?"

# game/script-a4-rin.rpy:3161
translate es a4_rin_desperate_glory_0f269d05:

    # hi "No…"
    hi "No…"

# game/script-a4-rin.rpy:3163
translate es a4_rin_desperate_glory_0191c814:

    # "That's right. I have only vague ideas of my future, no goal to shoot for, no dream to blindly reach for."
    "Es cierto. Tan solo tengo vagas ideas sobre mi futuro, no hay una meta a la cual llegar, ni un sueño por tratar de alcanzar ciegamente."

# game/script-a4-rin.rpy:3165
translate es a4_rin_desperate_glory_68bd3fa5:

    # "I joined the art club in search of something I could be interested in, to get inspired by."
    "Me uní al club de arte en busca de algo en lo que podría estar interesado, para inspirarme."

# game/script-a4-rin.rpy:3167
translate es a4_rin_desperate_glory_c57ea0ce:

    # "Did I find something like that?"
    "¿Encontré algo como eso?"

# game/script-a4-rin.rpy:3169
translate es a4_rin_desperate_glory_c61de6a1:

    # "All I found in the end… was Rin."
    "Al final todo lo que encontré… fue a Rin."

# game/script-a4-rin.rpy:3171
translate es a4_rin_desperate_glory_4f4dbaab:

    # hi "No, I don't have a passion like that."
    hi "No, no tengo una pasión como esa."

# game/script-a4-rin.rpy:3176
translate es a4_rin_desperate_glory_98ac8d70:

    # no "Then you can't understand."
    no "Entonces no puedes entender."

# game/script-a4-rin.rpy:3178
translate es a4_rin_desperate_glory_cd6fc25f:

    # "His flat statement allows no counterargument."
    "Su llana declaración no permite argumento en contra."

# game/script-a4-rin.rpy:3180
translate es a4_rin_desperate_glory_01af5b29:

    # hi "But… she might not understand even herself."
    hi "Pero… puede que ni ella misma entienda."

# game/script-a4-rin.rpy:3182
translate es a4_rin_desperate_glory_69301cc8:

    # "Still, I carry on arguing, out of spite if for nothing else."
    "Aun así, sigo discutiendo, por despecho si no es por algo más."

# game/script-a4-rin.rpy:3187
translate es a4_rin_desperate_glory_3b9a8234:

    # no "How could she not? She's been at it so hard for the past few weeks that she put off even coming to school, not to say anything about attending class. Don't be ridiculous."
    no "¿Cómo podría no entenderlo? Ha estado trabajando tan duro por las últimas semanas que pospuso incluso venir a la escuela, eso sin mencionar asistir a clases. No digas ridiculeces."

# game/script-a4-rin.rpy:3189
translate es a4_rin_desperate_glory_87e84184:

    # "I don't think I'm being ridiculous, but as I have no rebuttal, Nomiya seems to consider this one his win."
    "No pienso que esté diciendo ridiculeces, pero como no tengo modo de rebatir, Nomiya parece considerar esto como su victoria."

# game/script-a4-rin.rpy:3194
translate es a4_rin_desperate_glory_7901202f:

    # no "At any rate, the opening was quite successful despite Tezuka hardly showing up."
    no "Sea como sea, la apertura fue todo un éxito a pesar de que Tezuka apenas se haya mostrado."

# game/script-a4-rin.rpy:3196
translate es a4_rin_desperate_glory_0c222304:

    # no "Many people were interested in her work and one piece was even sold for a reasonable price."
    no "Muchas personas estaban interesadas en su trabajo y una pieza incluso se vendió a un precio razonable."

# game/script-a4-rin.rpy:3198
translate es a4_rin_desperate_glory_cb9cf776:

    # hi "Well, that's nice isn't it?"
    hi "Bueno, eso está bien, ¿no es así?"

# game/script-a4-rin.rpy:3203
translate es a4_rin_desperate_glory_d9a29ef8:

    # no "Yes, it's fantastic news! I hoped that Tezuka would come to her senses when she heard about this, but…"
    no "Sí, ¡son noticias fantásticas! Esperaba que Tezuka recobrara la cordura al escuchar sobre esto, pero…"

# game/script-a4-rin.rpy:3205
translate es a4_rin_desperate_glory_b33e402c:

    # "He sighs and takes off his glasses, cleaning them against his jacket before putting them on his nose again."
    "Suspira y se quita los anteojos, limpiándolos con su chaqueta antes de ponérselos de vuelta sobre su nariz."

# game/script-a4-rin.rpy:3210
translate es a4_rin_desperate_glory_0477d0e4:

    # no "At any rate, I should be going. There is this mess to be settled with Sae and everyone."
    no "De cualquier modo, debo marcharme. Está este desastre por arreglarse con Sae y todos los demás."

# game/script-a4-rin.rpy:3212
translate es a4_rin_desperate_glory_439f3294:

    # no "If you see Tezuka, please ask her to come see me. Otherwise, have a nice vacation."
    no "Si ves a Tezuka, por favor dile que venga a verme. De otro modo, que tengas felices vacaciones."

# game/script-a4-rin.rpy:3214
translate es a4_rin_desperate_glory_e5c8fee6:

    # hi "Thanks…"
    hi "Gracias…"

# game/script-a4-rin.rpy:3223
translate es a4_rin_desperate_glory_d10cffe6:

    # "After he has disappeared around the corner, I ponder where Rin could really be."
    "Después de que ha desaparecido a la vuelta de la esquina, reflexiono en dónde podría estar Rin realmente."

# game/script-a4-rin.rpy:3225
translate es a4_rin_desperate_glory_a985ab63:

    # "It feels like she has not one, but at least half a dozen of these 'secret places.'"
    "Siento como si no tuviera solo uno, sino por lo menos media docena de estos “lugares secretos”."

# game/script-a4-rin.rpy:3227
translate es a4_rin_desperate_glory_a915d233:

    # "I balance between the desire to solve this tangle and to drop it for good."
    "Sopeso entre el deseo de resolver este enredo y de dejarlo por la paz."

# game/script-a4-rin.rpy:3229
translate es a4_rin_desperate_glory_337552a9:

    # "The disused classroom is just a few feet away."
    "El salón desocupado está a solo unos pasos de distancia."

# game/script-a4-rin.rpy:3231
translate es a4_rin_desperate_glory_da38e87f:

    # "What to do?"
    "¿Qué hacer?"

# game/script-a4-rin.rpy:3233
translate es a4_rin_desperate_glory_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:3241
translate es a4_rin_desperate_glory_4a9e5232:

    # "As I push open the door, only the shadows greet me from the inside."
    "Cuando abro la puerta, solo me reciben las sombras del interior."

# game/script-a4-rin.rpy:3243
translate es a4_rin_desperate_glory_f957c2b7:

    # hi "Hey there."
    hi "Qué tal."

# game/script-a4-rin.rpy:3256
translate es a4_rin_problems_of_self_referential_logic_ee3f73f1:

    # "She's standing in the middle of the sunlit room, peering through the gaps of the curtains out into the yard."
    "Ella está de pie en medio del salón iluminado por el sol, asomándose a través de la separación entre las cortinas hacia el patio."

# game/script-a4-rin.rpy:3258
translate es a4_rin_problems_of_self_referential_logic_8c7aaa2e:

    # "Like so often before, she doesn't start or jump, just calmly waits for me to make the first move."
    "Como muchas veces antes, no se asusta ni salta, solo espera tranquilamente a que yo haga el primer movimiento."

# game/script-a4-rin.rpy:3260
translate es a4_rin_problems_of_self_referential_logic_03a649fa:

    # "It's as if she is trying to become a permanent part of the furniture."
    "Es como si ella estuviese tratando de convertirse en una parte permanente de los muebles."

# game/script-a4-rin.rpy:3262
translate es a4_rin_problems_of_self_referential_logic_d59819a9:

    # hi "The teacher is looking for you."
    hi "El maestro te está buscando."

# game/script-a4-rin.rpy:3264
translate es a4_rin_problems_of_self_referential_logic_96f6b750:

    # "A blank look over her shoulder is all I get, accompanied by a cryptic nonexpression on her face."
    "Una mirada en blanco sobre su hombro es todo lo que obtengo, acompañada de una cara críptica e inexpresiva."

# game/script-a4-rin.rpy:3266
translate es a4_rin_problems_of_self_referential_logic_85c360b3:

    # rin "Are you looking for me too?"
    rin "¿Tú también me estás buscando?"

# game/script-a4-rin.rpy:3268
translate es a4_rin_problems_of_self_referential_logic_3642d3bc:

    # hi "Nah, I already found you, didn't I?"
    hi "Nah, ya te encontré, ¿o no?"

# game/script-a4-rin.rpy:3270
translate es a4_rin_problems_of_self_referential_logic_6edd8a73:

    # rin "Did you?"
    rin "¿Lo hiciste?"

# game/script-a4-rin.rpy:3275
translate es a4_rin_problems_of_self_referential_logic_b7f14a11:

    # "She furrows her brow, looking so puzzled that it makes me wonder if the question was asked in all seriousness."
    "Ella frunce el ceño, viéndose tan confundida que me hace dudar de si la pregunta fue hecha en serio."

# game/script-a4-rin.rpy:3277
translate es a4_rin_problems_of_self_referential_logic_fcda1a5e:

    # "Maybe it was."
    "Tal vez lo fue."

# game/script-a4-rin.rpy:3279
translate es a4_rin_problems_of_self_referential_logic_c077d5b0:

    # hi "Are you talking metaphorically now?"
    hi "¿Estás ahora hablando metafóricamente?"

# game/script-a4-rin.rpy:3284
translate es a4_rin_problems_of_self_referential_logic_a3aa92a4:

    # rin "Do you mean like eels, caves and dark, stormy nights?"
    rin "¿Quieres decir como las anguilas, cuevas y oscuras noches tormentosas?"

# game/script-a4-rin.rpy:3289
translate es a4_rin_problems_of_self_referential_logic_8adb383e:

    # rin "I am bad at talking like that."
    rin "Soy mala hablando de ese modo."

# game/script-a4-rin.rpy:3291
translate es a4_rin_problems_of_self_referential_logic_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:3295
translate es a4_rin_problems_of_self_referential_logic_0ec3e92f:

    # "The abruptly-ended greetings give me the chance to close the door behind me and sit down on a dust-covered desktop."
    "El saludo terminado abruptamente me da la oportunidad de cerrar la puerta detrás de mí y sentarme en la mesa de un banco cubierto de polvo."

# game/script-a4-rin.rpy:3300
translate es a4_rin_problems_of_self_referential_logic_e6a49020:

    # "Rin stays standing, but at least she turns around."
    "Rin se queda de pie, pero por lo menos se da la vuelta."

# game/script-a4-rin.rpy:3302
translate es a4_rin_problems_of_self_referential_logic_046e264a:

    # "I soon wish she didn't though, so oppressive is her expectant stare."
    "Pero pronto deseo que no lo hubiera hecho, su mirada anticipativa es tan opresiva."

# game/script-a4-rin.rpy:3304
translate es a4_rin_problems_of_self_referential_logic_755ef64b:

    # "This is her place and I'm an intruder, although a tolerated one. Despite that, she still waits for me to say something."
    "Este es su lugar y yo soy un intruso, aunque uno tolerado. A pesar de ello, ella sigue esperando a que diga algo."

# game/script-a4-rin.rpy:3306
translate es a4_rin_problems_of_self_referential_logic_3b3ea37a:

    # "If I only knew what."
    "Si solo supiera qué."

# game/script-a4-rin.rpy:3308
translate es a4_rin_problems_of_self_referential_logic_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:3310
translate es a4_rin_problems_of_self_referential_logic_04a59135:

    # "The sunlit silence presses me towards decisions."
    "El silencio iluminado por el sol me presiona hacia las decisiones."

# game/script-a4-rin.rpy:3312
translate es a4_rin_problems_of_self_referential_logic_672af16b:

    # "I came here without really thinking what I would do, apart from delivering Nomiya's short message in case Rin was here."
    "Vine aquí sin realmente pensar qué haría, aparte de entregar el corto mensaje de Nomiya en el caso de que Rin estuviera aquí."

# game/script-a4-rin.rpy:3314
translate es a4_rin_problems_of_self_referential_logic_4150f5aa:

    # "She was, and now I don't know what else I want to say… what else I should say?"
    "Sí estaba, y ahora no sé qué más quiero decir… ¿qué más debería decir?"

# game/script-a4-rin.rpy:3316
translate es a4_rin_problems_of_self_referential_logic_5d122f4b:

    # "I hover between my two options for a moment."
    "Considero mis dos opciones por un momento."

# game/script-a4-rin.rpy:3318
translate es a4_rin_problems_of_self_referential_logic_e9d74956:

    # "Rin being troubled troubles me too. It's a surprising revelation, almost as big as realizing that she really is troubled was."
    "Que Rin esté preocupada también me preocupa. Es una revelación sorprendente, casi tan grande como lo fue el percatarse de que ella en verdad está preocupada."

# game/script-a4-rin.rpy:3320
translate es a4_rin_problems_of_self_referential_logic_9ef1978c:

    # "Nothing I can do would probably help, and I might be partially to blame too."
    "Nada que yo pueda hacer podría ayudar, y quizás también sea parcialmente culpable de eso."

# game/script-a4-rin.rpy:3322
translate es a4_rin_problems_of_self_referential_logic_33817fb1:

    # "Does it mean I should just wash my hands of her?"
    "¿Acaso eso significa que debería simplemente lavarme las manos de ella?"

# game/script-a4-rin.rpy:3324
translate es a4_rin_problems_of_self_referential_logic_801d9523:

    # "Didn't think so."
    "No lo creo."

# game/script-a4-rin.rpy:3326
translate es a4_rin_problems_of_self_referential_logic_c7124952:

    # hi "So… what's wrong?"
    hi "Y bien… ¿qué está mal?"

# game/script-a4-rin.rpy:3328
translate es a4_rin_problems_of_self_referential_logic_ce617998_2:

    # "…"
    "…"

# game/script-a4-rin.rpy:3333
translate es a4_rin_problems_of_self_referential_logic_be162ee9:

    # rin "Nothing."
    rin "Nada."

# game/script-a4-rin.rpy:3335
translate es a4_rin_problems_of_self_referential_logic_66076e58:

    # "She starts to turn away again, as if trying to physically exit a conversation she doesn't want to have."
    "Comienza a apartarse de nuevo, como si tratase de salirse físicamente de una conversación que no quiere tener."

# game/script-a4-rin.rpy:3337
translate es a4_rin_problems_of_self_referential_logic_8a4478d7:

    # hi "Rin, stop trying to dodge me or I'll leave."
    hi "Rin, deja de intentar evadirme o me iré."

# game/script-a4-rin.rpy:3342
translate es a4_rin_problems_of_self_referential_logic_a61bcc4a:

    # rin "Okay."
    rin "Está bien."

# game/script-a4-rin.rpy:3344
translate es a4_rin_problems_of_self_referential_logic_b3376472:

    # hi "Do you want me to leave?"
    hi "¿Quieres que me vaya?"

# game/script-a4-rin.rpy:3349
translate es a4_rin_problems_of_self_referential_logic_7a4f3897:

    # rin "Are you still angry?"
    rin "¿Sigues enojado?"

# game/script-a4-rin.rpy:3351
translate es a4_rin_problems_of_self_referential_logic_9f1fab98:

    # "It took us - or was it only me? - ten seconds to swamp the conversation into this."
    "Nos tomó —¿o fue solo a mí?— diez segundos hundir la conversación hasta esto."

# game/script-a4-rin.rpy:3353
translate es a4_rin_problems_of_self_referential_logic_1bf70863:

    # "I wish we could erase the past, or failing that, forget all about it."
    "Desearía que pudiéramos borrar el pasado, o en su defecto, olvidar todo de este."

# game/script-a4-rin.rpy:3355
translate es a4_rin_problems_of_self_referential_logic_a5e21d72:

    # "I've wished for that more than once in the last few months."
    "He deseado eso más de una vez en los últimos meses."

# game/script-a4-rin.rpy:3357
translate es a4_rin_problems_of_self_referential_logic_8e367d4b:

    # hi "Let's put that aside for the time being, all right?"
    hi "Dejemos eso de lado por el momento, ¿está bien?"

# game/script-a4-rin.rpy:3362
translate es a4_rin_problems_of_self_referential_logic_e787f564:

    # rin "If you say so."
    rin "Si tú lo dices."

# game/script-a4-rin.rpy:3364
translate es a4_rin_problems_of_self_referential_logic_1fc2aff8:

    # hi "I do. So… what's wrong?"
    hi "Lo digo. Así que… ¿qué ocurre?"

# game/script-a4-rin.rpy:3366
translate es a4_rin_problems_of_self_referential_logic_f071e6cb:

    # hi "Sae and Nomiya were not too happy that you just ran off yesterday."
    hi "Sae y Nomiya no estaban muy contentos con que simplemente te escaparas ayer."

# game/script-a4-rin.rpy:3368
translate es a4_rin_problems_of_self_referential_logic_b7034cb6:

    # hi "You left them in quite a pinch, and I suppose the teacher wants some kind of an explanation."
    hi "Los dejaste en un gran aprieto, y supongo que el maestro quiere algún tipo de explicación."

# game/script-a4-rin.rpy:3370
translate es a4_rin_problems_of_self_referential_logic_b87a1272:

    # hi "It seemed like you just threw out everything you had worked for. And I don't get why."
    hi "Parecía que básicamente hubieses tirado todo por lo que habías trabajado. Y no entiendo por qué."

# game/script-a4-rin.rpy:3375
translate es a4_rin_problems_of_self_referential_logic_bcc87ff2:

    # rin "Did I make a mistake?"
    rin "¿Cometí un error?"

# game/script-a4-rin.rpy:3377
translate es a4_rin_problems_of_self_referential_logic_b5b2603f:

    # "My reprimanding and her flat answer go so much against the usual expectations and presumed interactions that it might just as well be somebody else talking."
    "Mi reprimenda y su respuesta llana van tan en contra de las expectativas usuales y las interacciones presupuestas que bien podría ser otra persona hablando."

# game/script-a4-rin.rpy:3379
translate es a4_rin_problems_of_self_referential_logic_23734b94:

    # "Neither of us is like we used to be, this stiff, constricting feeling I get every time I look at Rin nowadays seems to be mirrored in her own behavior."
    "Ninguno de los dos es como solía ser, esta sensación rígida y estrecha que tengo cada vez que veo a Rin actualmente parece ser reflejada en su comportamiento."

# game/script-a4-rin.rpy:3381
translate es a4_rin_problems_of_self_referential_logic_db9eab9c:

    # "I hate things that go irreparably wrong. Ever since February, I have hated them."
    "Odio las cosas que van irreparablemente mal. Desde febrero las he odiado."

# game/script-a4-rin.rpy:3383
translate es a4_rin_problems_of_self_referential_logic_edbaad66:

    # "What can I say?"
    "¿Qué puedo decir?"

# game/script-a4-rin.rpy:3385
translate es a4_rin_problems_of_self_referential_logic_1456a0d8:

    # "Her question is trailed by a compelling, quizzical stare that makes me sigh and frown."
    "Su pregunta es seguida por una mirada imperiosa e inquisitiva que me hace suspirar y fruncir el entrecejo."

# game/script-a4-rin.rpy:3387
translate es a4_rin_problems_of_self_referential_logic_57d59b05:

    # "Conversations nobody wants to have are the worst."
    "Las conversaciones que nadie quiere tener son las peores."

# game/script-a4-rin.rpy:3389
translate es a4_rin_problems_of_self_referential_logic_8dc8cb6d:

    # hi "I don't know. I mean, it's not the end of the world but it probably was pretty stupid."
    hi "No lo sé. Quiero decir, no es el fin del mundo pero probablemente fue bastante estúpido."

# game/script-a4-rin.rpy:3394
translate es a4_rin_problems_of_self_referential_logic_23b82067:

    # "She responds with a sigh of her own, although hers is not nearly as heavy as mine was."
    "Ella responde con un suspiro suyo, aunque el de ella no es ni cercanamente tan pesado como el mío."

# game/script-a4-rin.rpy:3399
translate es a4_rin_problems_of_self_referential_logic_41f678a6:

    # rin "I just couldn't do it."
    rin "Es solo que no podía hacerlo."

# game/script-a4-rin.rpy:3401
translate es a4_rin_problems_of_self_referential_logic_d773584d:

    # hi "But… why? What's wrong?"
    hi "Pero… ¿Por qué? ¿Qué ocurre?"

# game/script-a4-rin.rpy:3406
translate es a4_rin_problems_of_self_referential_logic_d5db3a09:

    # "A pause, a furrowed brow, a quiet voice."
    "Una pausa, un ceño fruncido, una voz callada."

# game/script-a4-rin.rpy:3408
translate es a4_rin_problems_of_self_referential_logic_813fbe53:

    # rin "Let it be, Hisao."
    rin "Déjalo ser, Hisao."

# game/script-a4-rin.rpy:3410
translate es a4_rin_problems_of_self_referential_logic_db6d9338:

    # rin "I don't think I can really explain it in a way that would make sense."
    rin "No creo que pueda realmente explicarlo de un modo que tenga sentido."

# game/script-a4-rin.rpy:3412
translate es a4_rin_problems_of_self_referential_logic_03f39121:

    # "Yeah, Rin doesn't want to have this conversation either. That may be for the better."
    "Sí, Rin tampoco quiere tener esta conversación. Eso puede ser lo mejor."

# game/script-a4-rin.rpy:3414
translate es a4_rin_problems_of_self_referential_logic_7e09a6bb:

    # "But how rare of her, to admit that even she has some kind of limits."
    "Pero qué raro de ella, admitir que incluso ella tiene algún tipo de límites."

# game/script-a4-rin.rpy:3416
translate es a4_rin_problems_of_self_referential_logic_6e18c75e:

    # "I always thought Rin was all but ignorant of her tendency to get distracted, so much that she inadvertently obfuscates everything she says."
    "Siempre pensé que Rin era completamente ignorante de su tendencia a distraerse, tanto que sin darse cuenta ofusca todo lo que dice."

# game/script-a4-rin.rpy:3418
translate es a4_rin_problems_of_self_referential_logic_ce617998_3:

    # "…"
    "…"

# game/script-a4-rin.rpy:3420
translate es a4_rin_problems_of_self_referential_logic_646a5472:

    # hi "You never explain {b}anything{/b} in a way that would make sense."
    hi "Nunca explicas {b}nada{/b} de un modo que tenga sentido."

# game/script-a4-rin.rpy:3425
translate es a4_rin_problems_of_self_referential_logic_6e44563b:

    # rin "Nobody else has ever asked me to."
    rin "Nadie más me lo había pedido."

# game/script-a4-rin.rpy:3469
translate es a4_rin_problems_of_self_referential_logic_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:3432
translate es a4_rin_problems_of_self_referential_logic_cafeb753:

    # n "I guess that's how it is."
    n "Supongo que así es como es."

# game/script-a4-rin.rpy:3434
translate es a4_rin_problems_of_self_referential_logic_23323e1b:

    # n "But I always wanted to make sense of you, to find out who you are."
    n "Pero yo siempre quise entenderte, encontrar quién eres."

# game/script-a4-rin.rpy:3436
translate es a4_rin_problems_of_self_referential_logic_c243b672:

    # n "I still want to, can't you see?"
    n "Todavía quiero hacerlo, ¿no lo ves?"

# game/script-a4-rin.rpy:3438
translate es a4_rin_problems_of_self_referential_logic_eabec665:

    # n "…"
    n "…"

# game/script-a4-rin.rpy:3440
translate es a4_rin_problems_of_self_referential_logic_5256755c:

    # n "I know you can't."
    n "Sé que no."

# game/script-a4-rin.rpy:3442
translate es a4_rin_problems_of_self_referential_logic_6625e57c:

    # n "But I do."
    n "Pero yo sí."

# game/script-a4-rin.rpy:3444
translate es a4_rin_problems_of_self_referential_logic_545e50c4:

    # n "Is that why I keep this up? It pains you as much as it pains me. It's unlikely to be of any use to either."
    n "¿Es por eso que continúo con esto? Te duele tanto como me duele a mí. Es poco probable que le sea útil a alguno."

# game/script-a4-rin.rpy:3446
translate es a4_rin_problems_of_self_referential_logic_300fc010:

    # n "We did things and said things that can't be undone."
    n "Hicimos y dijimos cosas que no pueden deshacerse."

# game/script-a4-rin.rpy:3448
translate es a4_rin_problems_of_self_referential_logic_5c30b55f:

    # n "It's as if… you and me being close to each other just hurts us both, but we still deliberately keep doing it."
    n "Es como si… tú y yo estando cerca uno del otro solo nos lastimara a los dos, pero aun así seguimos intentándolo deliberadamente."

# game/script-a4-rin.rpy:3450
translate es a4_rin_problems_of_self_referential_logic_7d209e62:

    # n "Isn't that silly?"
    n "¿No es eso tonto?"

# game/script-a4-rin.rpy:3452
translate es a4_rin_problems_of_self_referential_logic_2c4bc0a6:

    # n "Even now, I can see how you force yourself to respond even though you owe me nothing."
    n "Incluso ahora, puedo ver cómo te fuerzas a responder aun si no me debes nada."

# game/script-a4-rin.rpy:3454
translate es a4_rin_problems_of_self_referential_logic_2e0c4c6a:

    # n "Even if it's hard to talk about things like this."
    n "Aun si es difícil hablar de cosas como esta."

# game/script-a4-rin.rpy:3456
translate es a4_rin_problems_of_self_referential_logic_651e1241:

    # n "Why?"
    n "¿Por qué?"

# game/script-a4-rin.rpy:3461
translate es a4_rin_problems_of_self_referential_logic_28de4337:

    # nvl clear
    # hi "Why is it that you paint?"
    nvl clear
    hi "¿Por qué es que pintas?"

# game/script-a4-rin.rpy:3468
translate es a4_rin_problems_of_self_referential_logic_0bb0fa31:

    # rin "I… because I don't know what else I could do."
    rin "Yo… porque no sé qué más podría hacer."

# game/script-a4-rin.rpy:3470
translate es a4_rin_problems_of_self_referential_logic_800cc3cb:

    # rin "It's like this feeling that there is no choice, that it's the only possibility."
    rin "Es como esta sensación de que no hay opción, que es la única posibilidad."

# game/script-a4-rin.rpy:3475
translate es a4_rin_problems_of_self_referential_logic_0bf234f7:

    # rin "Like when there are only watermelon-flavored popsicles left in the store but you need to eat a popsicle."
    rin "Como cuando solo quedan paletas de sandía en la tienda pero necesitas comer una paleta."

# game/script-a4-rin.rpy:3477
translate es a4_rin_problems_of_self_referential_logic_2ee37a8f:

    # "Her poor metaphor aside, she didn't really answer anything. If possible, this makes even less sense than not knowing."
    "Dejando de lado su mala metáfora, realmente no respondió nada. Si es posible, esto tiene todavía menos sentido que no saber."

# game/script-a4-rin.rpy:3479
translate es a4_rin_problems_of_self_referential_logic_aaa5eb21:

    # hi "But… if you don't want to paint…"
    hi "Pero… si no quieres pintar…"

# game/script-a4-rin.rpy:3484
translate es a4_rin_problems_of_self_referential_logic_1c175781:

    # rin "Not like that. You had to come to this school even though you probably didn't want to have a heart attack."
    rin "No así. Tú tuviste que venir a esta escuela aun si probablemente no querías tener un ataque al corazón."

# game/script-a4-rin.rpy:3489
translate es a4_rin_problems_of_self_referential_logic_611f9eb6:

    # "She pauses, frowning as if something in what she said didn't please her."
    "Hace una pausa, frunce el ceño como si algo en lo que dijo no le hubiera gustado."

# game/script-a4-rin.rpy:3494
translate es a4_rin_problems_of_self_referential_logic_6c42b26d:

    # rin "At least I think you wouldn't."
    rin "Al menos creo que no quisieras."

# game/script-a4-rin.rpy:3496
translate es a4_rin_problems_of_self_referential_logic_8e545a1e:

    # "Her careful follow-up is followed in turn by another, shorter pause with another, smaller frown."
    "Su seguimiento cuidadoso es acompañado por otra pausa más corta y otro gesto parecido más pequeño."

# game/script-a4-rin.rpy:3501
translate es a4_rin_problems_of_self_referential_logic_3973e3e4:

    # rin "Would you like to have a heart attack?"
    rin "¿Te gustaría tener un ataque cardiaco?"

# game/script-a4-rin.rpy:3503
translate es a4_rin_problems_of_self_referential_logic_aa41265d:

    # hi "No, I wouldn't and I didn't want to."
    hi "No, no me gustaría y no quise tenerlo."

# game/script-a4-rin.rpy:3508
translate es a4_rin_problems_of_self_referential_logic_75127094:

    # rin "But you're doing fine, aren't you? Or are you still sad about it?"
    rin "Pero te está yendo bien, ¿o no? ¿O todavía estás triste por eso?"

# game/script-a4-rin.rpy:3510
translate es a4_rin_problems_of_self_referential_logic_291d8cab:

    # "Rin's question makes me realize that I haven't really thought about my illness for weeks."
    "La pregunta de Rin hace que me percate de que realmente no he pensado en mi enfermedad por semanas."

# game/script-a4-rin.rpy:3512
translate es a4_rin_problems_of_self_referential_logic_fab79964:

    # "Aside from chugging down my medication every day there has been no need to concern myself with my broken heart, which I'm only thankful for, really."
    "Aparte de tragarme mis medicinas cada día no ha habido necesidad de preocuparme de mi corazón roto, por lo que simplemente estoy agradecido, realmente."

# game/script-a4-rin.rpy:3514
translate es a4_rin_problems_of_self_referential_logic_f7e0cd3b:

    # "Getting to know new people, a new school, a new town… a new life, it all has caught me and made the past fade away."
    "Conocer gente nueva, una escuela nueva, un pueblo nuevo… una vida nueva, todo ello me ha cautivado y hecho al pasado desvanecerse."

# game/script-a4-rin.rpy:3516
translate es a4_rin_problems_of_self_referential_logic_dfc88b52:

    # hi "No… heh, I guess even I can't dwell on the past indefinitely."
    hi "No… je, supongo que ni siquiera yo puedo vivir en el pasado indefinidamente."

# game/script-a4-rin.rpy:3521
translate es a4_rin_problems_of_self_referential_logic_10596138:

    # rin "See? Even watermelon doesn't really taste bad if you have to eat it."
    rin "¿Ves? Incluso la sandía realmente no sabe mala si tienes que comerla."

# game/script-a4-rin.rpy:3523
translate es a4_rin_problems_of_self_referential_logic_88845d6c:

    # "Her half-nonsensical closure seems to put an end to the subject in Rin's mind, so I just nod in uncertain confirmation."
    "Su cierre semiabsurdo parece ponerle fin al tema en la mente de Rin, así que yo solo asiento en insegura confirmación."

# game/script-a4-rin.rpy:3525
translate es a4_rin_problems_of_self_referential_logic_ce617998_4:

    # "…"
    "…"

# game/script-a4-rin.rpy:3527
translate es a4_rin_problems_of_self_referential_logic_ce617998_5:

    # "…"
    "…"

# game/script-a4-rin.rpy:3529
translate es a4_rin_problems_of_self_referential_logic_7225b402:

    # "There are two kinds of silences: awkward ones that you want to break, and comfortable ones that you don't mind."
    "Hay dos tipos de silencios: los incómodos que quieres romper, y los cómodos que no te molestan."

# game/script-a4-rin.rpy:3531
translate es a4_rin_problems_of_self_referential_logic_1d71e413:

    # "The first kind is bad, because it makes your thoughts go awry. Like mine do, now."
    "Los del primer tipo son malos, porque hacen que tus pensamientos vayan mal. Como los míos en este momento."

# game/script-a4-rin.rpy:3533
translate es a4_rin_problems_of_self_referential_logic_d1bf587e:

    # "Looking at Rin makes me feel bad."
    "Mirar a Rin me hace sentir mal."

# game/script-a4-rin.rpy:3577
translate es a4_rin_problems_of_self_referential_logic_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:3540
translate es a4_rin_problems_of_self_referential_logic_7a3f251e:

    # n "{vspace=30}I don't want to feel like this."
    n "{vspace=30}No quiero sentirme así."

# game/script-a4-rin.rpy:3542
translate es a4_rin_problems_of_self_referential_logic_c0d056e5:

    # n "Looking at Rin makes me feel… exhausted. I really tried my best, she tried to… I have no idea."
    n "Mirar a Rin me hace sentir… exhausto. En verdad hice mi mejor esfuerzo, ella trató de… no tengo idea."

# game/script-a4-rin.rpy:3544
translate es a4_rin_problems_of_self_referential_logic_da93884c:

    # n "But we ended up like this, and she ended up screwing up her exhibition opening."
    n "Pero terminamos así, y ella terminó arruinando la apertura de su exhibición."

# game/script-a4-rin.rpy:3546
translate es a4_rin_problems_of_self_referential_logic_fa1ac8c6:

    # n "It feels like we are at a dead end."
    n "Parece que estamos en un callejón sin salida."

# game/script-a4-rin.rpy:3548
translate es a4_rin_problems_of_self_referential_logic_4d95f641:

    # n "There is no direction to continue to."
    n "No hay dirección para continuar."

# game/script-a4-rin.rpy:3550
translate es a4_rin_problems_of_self_referential_logic_a609c6da:

    # n "I reached out for her yesterday, thinking it would be the last time."
    n "Me acerqué a ella ayer, pensando que sería la última vez."

# game/script-a4-rin.rpy:3552
translate es a4_rin_problems_of_self_referential_logic_24374079:

    # n "She walked away."
    n "Se marchó."

# game/script-a4-rin.rpy:3554
translate es a4_rin_problems_of_self_referential_logic_60bcbc14:

    # n "'I want to be me.'"
    n "“Quiero ser yo”."

# game/script-a4-rin.rpy:3556
translate es a4_rin_problems_of_self_referential_logic_eebff0d0:

    # n "What the heck does that even mean? Rin, if anyone, is most definitely herself."
    n "¿Qué diablos significa eso? Si alguien lo es, Rin en definitiva es ella misma."

# game/script-a4-rin.rpy:3558
translate es a4_rin_problems_of_self_referential_logic_e3a9b01f:

    # n "I feel kinda relieved that I am not the one to blame, but this still grates on my mind."
    n "Me siento un poco aliviado de no ser el culpable, pero en mi mente esto todavía me molesta."

# game/script-a4-rin.rpy:3560
translate es a4_rin_problems_of_self_referential_logic_7455413a:

    # n "Why did she run away? It didn't make sense yesterday. It doesn't make sense today."
    n "¿Por qué escapó? No tenía sentido ayer. No tiene sentido hoy."

# game/script-a4-rin.rpy:3562
translate es a4_rin_problems_of_self_referential_logic_87aefa77:

    # n "The things she said feel like they should make sense but they just don't, to me."
    n "Las cosas que dijo se sienten como si debieran de tenerlo pero simplemente no lo tienen, para mí."

# game/script-a4-rin.rpy:3567
translate es a4_rin_problems_of_self_referential_logic_6dacf797:

    # nvl clear
    # hi "You know, about that thing you just said…"
    nvl clear
    hi "Sabes, sobre esa cosa que acabas de decir…"

# game/script-a4-rin.rpy:3574
translate es a4_rin_problems_of_self_referential_logic_14826801:

    # rin "Which one of them?"
    rin "¿Cuál de ellas?"

# game/script-a4-rin.rpy:3576
translate es a4_rin_problems_of_self_referential_logic_20b7af2e:

    # hi "Umm… painting… Sae said something like that to me before… that a true artist does not paint because she wants to, but because she {b}must{/b}."
    hi "Ehh… pintar… Sae me dijo algo así antes… que un artista verdadero no pinta porque quiere, sino porque {b}debe{/b}."

# game/script-a4-rin.rpy:3578
translate es a4_rin_problems_of_self_referential_logic_a4b67d9f:

    # hi "And I've been wondering about what she said. Why do artists {b}have{/b} to paint?"
    hi "Y he estado pensando en lo que ella dijo. ¿Por qué los artistas {b}deben{/b} pintar?"

# game/script-a4-rin.rpy:3580
translate es a4_rin_problems_of_self_referential_logic_18296262:

    # "My question is probably pretty stupid. At least Rin looks at me in the blank way that seems to say so."
    "Mi pregunta probablemente es bastante estúpida. Por lo menos Rin me mira de la manera inexpresiva que parece decir eso."

# game/script-a4-rin.rpy:3585
translate es a4_rin_problems_of_self_referential_logic_195bf102:

    # rin "I don't know. Am I an artist?"
    rin "No lo sé. ¿Soy una artista?"

# game/script-a4-rin.rpy:3587
translate es a4_rin_problems_of_self_referential_logic_fa95ba56:

    # hi "Well, you paint stuff and you have an exhibition too. I'd say you qualify."
    hi "Bueno, pintas cosas y también tienes una exhibición. Diría que calificas."

# game/script-a4-rin.rpy:3592
translate es a4_rin_problems_of_self_referential_logic_c4aaa318:

    # rin "I think I still don't know, but okay."
    rin "Creo que todavía no lo sé, pero está bien."

# game/script-a4-rin.rpy:3594
translate es a4_rin_problems_of_self_referential_logic_77d41dff:

    # "The thinking pause that follows seems to last for half an eternity."
    "La pausa reflexiva que sigue parece durar media eternidad."

# game/script-a4-rin.rpy:3596
translate es a4_rin_problems_of_self_referential_logic_d21be6f0:

    # "Unlike most people, Rin doesn't flavor her thinking pauses by body language or saying 'like' or 'umm' or anything."
    "A diferencia de la mayoría de las personas, Rin no le da sabor a sus pausas pensativas por medio de lenguaje corporal o al decir “como” o “ehhh” o nada."

# game/script-a4-rin.rpy:3598
translate es a4_rin_problems_of_self_referential_logic_74ece31c:

    # "I've noticed that I might prefer her way. The usual way even annoys me, as if people were so infatuated by the sound of their own voice they just have to keep making some noise even when they are just thinking what they could say next."
    "He notado que puede que prefiera su modo. El modo usual incluso me molesta, como si las personas estuviesen tan infatuadas con el sonido de su voz que simplemente tuvieran que seguir haciendo ruido aun cuando solo están pensando en lo siguiente que podrían decir."

# game/script-a4-rin.rpy:3600
translate es a4_rin_problems_of_self_referential_logic_0715462c:

    # "Rin just… comes to a full stop while she is thinking. It's disconcerting, because reacting to people spacing out is always hard, but she comes off as less obnoxious."
    "Rin simplemente… se detiene por completo mientras piensa. Es desconcertante, porque reaccionar a gente que se queda ida es siempre difícil, pero se siente menos repelente."

# game/script-a4-rin.rpy:3605
translate es a4_rin_problems_of_self_referential_logic_ee0f3b45:

    # rin "I think I want someone to see what's inside me. Not the way doctors and serial killers do."
    rin "Creo que quiero a alguien que vea qué hay dentro de mí. No de la manera que los doctores o asesinos en serie lo hacen."

# game/script-a4-rin.rpy:3610
translate es a4_rin_problems_of_self_referential_logic_4ceaf127:

    # rin "The way that doesn't make me feel lonely."
    rin "Del modo que no me haga sentir sola."

# game/script-a4-rin.rpy:3615
translate es a4_rin_problems_of_self_referential_logic_10d8b645:

    # rin "This is what you call metaphorical, you see."
    rin "Esto es lo que llamas metafórico, sabes."

# game/script-a4-rin.rpy:3617
translate es a4_rin_problems_of_self_referential_logic_083951c2:

    # hi "Please don't lecture me about self-evident things."
    hi "Por favor no me des una lección sobre cosas evidentes."

# game/script-a4-rin.rpy:3622
translate es a4_rin_problems_of_self_referential_logic_14f860e5:

    # rin "It's not self-evident that this is self-evident."
    rin "No es evidente que esto sea evidente."

# game/script-a4-rin.rpy:3624
translate es a4_rin_problems_of_self_referential_logic_a097d013:

    # hi "So, you present a painting to someone and expect him to magically see a glimpse of your soul?"
    hi "Entonces, ¿le presentas una pintura a alguien y esperas que él mágicamente capte un destello de tu alma?"

# game/script-a4-rin.rpy:3629
translate es a4_rin_problems_of_self_referential_logic_dde8910c:

    # rin "It's not like that. It's just a little like that but not really. Don't you understand?"
    rin "No es así. Es solo un poquito como eso pero no realmente. ¿Que no entiendes?"

# game/script-a4-rin.rpy:3631
translate es a4_rin_problems_of_self_referential_logic_5e864c48:

    # hi "I do… and I don't."
    hi "Entiendo… y no entiendo."

# game/script-a4-rin.rpy:3633
translate es a4_rin_problems_of_self_referential_logic_e9f88ef6:

    # hi "You know, I feel a little bit of despair every time you ask that question."
    hi "Sabes, me siento un poco desesperanzado cada vez que haces esa pregunta."

# game/script-a4-rin.rpy:3638
translate es a4_rin_problems_of_self_referential_logic_aba56c2b:

    # rin "What question?"
    rin "¿Qué pregunta?"

# game/script-a4-rin.rpy:3640
translate es a4_rin_problems_of_self_referential_logic_4317b1d6:

    # hi "About whether I understand you or not."
    hi "Sobre si te entiendo o no."

# game/script-a4-rin.rpy:3642
translate es a4_rin_problems_of_self_referential_logic_513f47b2:

    # "She seems almost surprised at my clarification."
    "Ella parece casi sorprendida ante mi clarificación."

# game/script-a4-rin.rpy:3647
translate es a4_rin_problems_of_self_referential_logic_386e0709:

    # rin "Oh, it's not really a question. It's one of those kind that you don't have to answer."
    rin "Oh, en realidad no es una pregunta. Es una de ese tipo que no tienes que responder."

# game/script-a4-rin.rpy:3649
translate es a4_rin_problems_of_self_referential_logic_1096c39f:

    # hi "Rhetorical."
    hi "Retórica."

# game/script-a4-rin.rpy:3654
translate es a4_rin_problems_of_self_referential_logic_bebbe0dc:

    # rin "Yeah, that's the word, a question that is not a question is a rhetorical question. How nice."
    rin "Sí, esa es la palabra, una pregunta que no es una pregunta es una pregunta retórica. Qué lindo."

# game/script-a4-rin.rpy:3656
translate es a4_rin_problems_of_self_referential_logic_4fab62b6:

    # rin "That reminds me, it doesn't really make sense. What kind of a question is one that isn't a question?"
    rin "Eso me recuerda, en realidad no tiene sentido. ¿Qué tipo de pregunta es una que no es pregunta?"

# game/script-a4-rin.rpy:3658
translate es a4_rin_problems_of_self_referential_logic_4490b910:

    # hi "A rhetorical one."
    hi "Una retórica."

# game/script-a4-rin.rpy:3660
translate es a4_rin_problems_of_self_referential_logic_843b5534:

    # rin "What kind of an answer is an answer that doesn't answer anything?"
    rin "¿Qué tipo de respuesta es una respuesta que no responde nada?"

# game/script-a4-rin.rpy:3662
translate es a4_rin_problems_of_self_referential_logic_e023ff8d:

    # hi "Is that a rhetorical question?"
    hi "¿Es esa una pregunta retórica?"

# game/script-a4-rin.rpy:3667
translate es a4_rin_problems_of_self_referential_logic_92a01500:

    # rin "You are not funny."
    rin "No eres gracioso."

# game/script-a4-rin.rpy:3672
translate es a4_rin_problems_of_self_referential_logic_eed4370b:

    # rin "But if you don't like it, would you like me to say something else instead?"
    rin "Pero si no te gusta, ¿quisieras que mejor dijera algo diferente?"

# game/script-a4-rin.rpy:3677
translate es a4_rin_problems_of_self_referential_logic_66c7fa1d:

    # rin "I don't have any good ones though. How about… 'Your pants are on fire?'"
    rin "Aunque no tengo ninguna buena. ¿Qué tal… “Tus pantalones se están quemando”?"

# game/script-a4-rin.rpy:3682
translate es a4_rin_problems_of_self_referential_logic_bda4e8c8:

    # rin "This can be our secret language."
    rin "Esto puede ser nuestro lenguaje secreto."

# game/script-a4-rin.rpy:3684
translate es a4_rin_problems_of_self_referential_logic_7682797e:

    # "Rin's honest-to-goodness silliness, made twice more ridiculous by the fact that I know she is dead serious, derails me like it always does."
    "La verdadera bobería de Rin, hecha el doble de ridículo por el hecho de que sé que ella está hablando completamente en serio, me saca de mi ritmo como siempre lo hace."

# game/script-a4-rin.rpy:3686
translate es a4_rin_problems_of_self_referential_logic_dba20ae4:

    # "It's like some kind of a safety lock to prevent me from becoming too much of a worrywart, dragging even my own thoughts off the ground where they should be."
    "Es como un mecanismo de seguridad para prevenir que me convierta en uno de esos tipos angustiados, impidiéndome mantener los pies en la tierra donde deberían de estar."

# game/script-a4-rin.rpy:3688
translate es a4_rin_problems_of_self_referential_logic_05a783f6:

    # "It makes me smile confusedly, but only on the inside."
    "Me hace sonreír confusamente, pero solo en mi interior."

# game/script-a4-rin.rpy:3690
translate es a4_rin_problems_of_self_referential_logic_4de286af:

    # "Even though the corners of my mouth are not drawing into a grin, I'm still impressed by her ease of wrecking any attempt at being too serious."
    "A pesar de que las comisuras de mi boca no se dibujen en una sonrisa, estoy impresionado por la facilidad con la que rompe cualquier intento de ser demasiado serio."

# game/script-a4-rin.rpy:3692
translate es a4_rin_problems_of_self_referential_logic_7881bdf2:

    # "Could she (should she will so) forget and ignore things that bug her, things that bother her?"
    "¿Podría ella (si se lo propusiera) olvidar e ignorar las cosas que le molestan, cosas que la angustian?"

# game/script-a4-rin.rpy:3694
translate es a4_rin_problems_of_self_referential_logic_0e0e5e33:

    # "Could she (should she will so) be free of whatever burden being her means?"
    "¿Podría ella (si se lo propusiera) ser libre de cualquiera que sea la carga que ser ella significa?"

# game/script-a4-rin.rpy:3696
translate es a4_rin_problems_of_self_referential_logic_37e75b58:

    # "Or am I the only one who feels burdened by being myself?"
    "¿O soy yo el único que se siente agobiado por ser yo mismo?"

# game/script-a4-rin.rpy:3698
translate es a4_rin_problems_of_self_referential_logic_5d7bd46a:

    # hi "No thanks."
    hi "No gracias."

# game/script-a4-rin.rpy:3700
translate es a4_rin_problems_of_self_referential_logic_213b0650:

    # hi "But still, the times when I feel that I am on the same page as you are pretty rare."
    hi "Pero igual, las veces en las que siento que coincido contigo son bastante raras."

# game/script-a4-rin.rpy:3702
translate es a4_rin_problems_of_self_referential_logic_b4eb455a:

    # hi "It feels like… there is this huge gap and sometimes you just go to the other side, and I don't… have any way to reach to you from where I am."
    hi "Se siente como… si hubiera esta brecha y algunas veces tú simplemente vas al otro lado, y yo no… tengo ningún modo de alcanzarte desde donde estoy."

# game/script-a4-rin.rpy:3704
translate es a4_rin_problems_of_self_referential_logic_222dda68:

    # hi "It's like you are in some completely different place at times."
    hi "Es como si estuvieras en lugares completamente diferentes, algunas veces."

# game/script-a4-rin.rpy:3706
translate es a4_rin_problems_of_self_referential_logic_d2a3094c:

    # hi "Even though you are right here."
    hi "Aunque estés justo aquí."

# game/script-a4-rin.rpy:3708
translate es a4_rin_problems_of_self_referential_logic_112ac131:

    # "That's right."
    "Es cierto."

# game/script-a4-rin.rpy:3710
translate es a4_rin_problems_of_self_referential_logic_13d325fb:

    # "There is an insurmountable discontinuity, an imaginary glass wall that blocks comprehension from happening."
    "Hay una discontinuidad insuperable, un muro de cristal imaginario que no permite que haya comprensión."

# game/script-a4-rin.rpy:3712
translate es a4_rin_problems_of_self_referential_logic_18f58ea5:

    # "There might be such a gap between any two people, but with Rin, it feels more tangible."
    "Puede que exista tal brecha entre cualquier par de personas, pero con Rin se siente más tangible."

# game/script-a4-rin.rpy:3714
translate es a4_rin_problems_of_self_referential_logic_6a8ac072:

    # "Rin does not react to my thoughts, not to the ones I uttered aloud nor the ones I did not."
    "Rin no reacciona a mis pensamientos, ni a los que dije en voz alta ni a los que no dije."

# game/script-a4-rin.rpy:3716
translate es a4_rin_problems_of_self_referential_logic_fba2ac6a:

    # hi "It's even worse with art."
    hi "Es aun peor con el arte."

# game/script-a4-rin.rpy:3718
translate es a4_rin_problems_of_self_referential_logic_588ba655:

    # hi "I'm not very good at art, I admit it."
    hi "No soy muy bueno con el arte, lo admito."

# game/script-a4-rin.rpy:3720
translate es a4_rin_problems_of_self_referential_logic_9359a0b9:

    # hi "I joined the art club 'cause I thought it could be interesting."
    hi "Me uní al club de arte porque pensé que podría ser interesante."

# game/script-a4-rin.rpy:3722
translate es a4_rin_problems_of_self_referential_logic_07a3eee5:

    # hi "And I guess it is. I like art, I like your art too, but just like with you, I can't comprehend it."
    hi "Y supongo que lo es. Me gusta el arte, me gusta tu arte también, pero al igual que contigo, no puedo comprenderlo."

# game/script-a4-rin.rpy:3724
translate es a4_rin_problems_of_self_referential_logic_a90459f9:

    # hi "And I'm pretty sure nobody really can."
    hi "Y estoy bastante seguro de que nadie puede realmente."

# game/script-a4-rin.rpy:3729
translate es a4_rin_problems_of_self_referential_logic_63627b63:

    # "This seems to worry her slightly."
    "Esto parece preocuparla ligeramente."

# game/script-a4-rin.rpy:3731
translate es a4_rin_problems_of_self_referential_logic_b1e4b14a:

    # rin "Do you think so?"
    rin "¿Tú crees eso?"

# game/script-a4-rin.rpy:3733
translate es a4_rin_problems_of_self_referential_logic_ec0c608c:

    # hi "Yeah. I guess that art is meant to be interpreted, not understood. That's how I'd put it."
    hi "Sí. Supongo que el arte debe de ser interpretado, no entendido. Así es como lo pondría."

# game/script-a4-rin.rpy:3738
translate es a4_rin_problems_of_self_referential_logic_892c793d:

    # rin "That's a sad thought."
    rin "Ese es un pensamiento triste."

# game/script-a4-rin.rpy:3740
translate es a4_rin_problems_of_self_referential_logic_63321a7e:

    # hi "I guess it might feel like one."
    hi "Supongo que podría sentirse como uno."

# game/script-a4-rin.rpy:3742
translate es a4_rin_problems_of_self_referential_logic_178e1309:

    # hi "Does it make you feel sad for yourself?"
    hi "¿Hace que te sientas triste por ti misma?"

# game/script-a4-rin.rpy:3747
translate es a4_rin_problems_of_self_referential_logic_0db8c15b:

    # "Rin thinks about this for a while, and then shakes her head surprisingly vehemently."
    "Rin piensa en esto por un momento, y luego agita su cabeza con sorprendente vehemencia."

# game/script-a4-rin.rpy:3752
translate es a4_rin_problems_of_self_referential_logic_e3bbb7e3:

    # "The first thing she focuses her eyes on afterward is me."
    "Lo primero en lo que fija su mirada después de ello soy yo."

# game/script-a4-rin.rpy:3754
translate es a4_rin_problems_of_self_referential_logic_931bfb86:

    # "Both of these things make me glad, and relieved."
    "Estas dos cosas me hacen sentir alegre y aliviado."

# game/script-a4-rin.rpy:3756
translate es a4_rin_problems_of_self_referential_logic_bd15c7c3:

    # hi "That's good, isn't it? Anyway, you should go see the teacher and apologize properly."
    hi "Eso es bueno, ¿o no? De cualquier modo, deberías de ir a hablar con el maestro y disculparte apropiadamente."

# game/script-a4-rin.rpy:3758
translate es a4_rin_problems_of_self_referential_logic_1f693936:

    # hi "I think he is worried about you."
    hi "Creo que estaba preocupado por ti."

# game/script-a4-rin.rpy:3760
translate es a4_rin_problems_of_self_referential_logic_61ea2e1f:

    # hi "Can you do that?"
    hi "¿Puedes hacer eso?"

# game/script-a4-rin.rpy:3765
translate es a4_rin_problems_of_self_referential_logic_a07805c3:

    # "This time, she nods her head."
    "Esta vez ella asiente con la cabeza."

# game/script-a4-rin.rpy:3769
translate es a4_rin_problems_of_self_referential_logic_8f5ac796:

    # "Only, it's not as vehement."
    "Solo que no es igual de vehemente."

# game/script-a4-rin.rpy:3778
translate es a4_rin_measuring_shadows_0a716e6b:

    # "The hallway is empty, almost intimidating."
    "El pasillo se encuentra vacío. Es casi intimidante."

# game/script-a4-rin.rpy:3780
translate es a4_rin_measuring_shadows_1bacdced:

    # "Nomiya's 'office' is the art classroom at the other end of the third floor hallway."
    "La “oficina” de Nomiya es el salón de arte al otro extremo del pasillo del tercer piso."

# game/script-a4-rin.rpy:3785
translate es a4_rin_measuring_shadows_125fc35a:

    # "Our steps echo disturbingly. The atmosphere is unlike on a normal afternoon. It feels like the school knows that nobody will be coming back for a month, too."
    "Nuestras pisadas hacen eco perturbadoramente. El ambiente es diferente al de una tarde normal. Se siente como si la escuela supiera que nadie vendrá de vuelta por un mes, también."

# game/script-a4-rin.rpy:3787
translate es a4_rin_measuring_shadows_dbce099b:

    # "The door is open, but not very inviting."
    "La puerta está abierta, pero no es muy atrayente."

# game/script-a4-rin.rpy:3789
translate es a4_rin_measuring_shadows_6aff6df4:

    # hi "I'll… um, wait outside."
    hi "Yo… eh, esperaré afuera."

# game/script-a4-rin.rpy:3803
translate es a4_rin_measuring_shadows_6dc07af5:

    # "Nodding barely noticeably, Rin strides in without stopping, and naturally, without knocking."
    "Asintiendo de manera apenas notable, Rin da pasos sin detenerse y, naturalmente, sin llamar a la puerta."

# game/script-a4-rin.rpy:3805
translate es a4_rin_measuring_shadows_86657f4b:

    # "Maybe that's why it takes a few seconds before I hear the teacher's voice from inside."
    "Tal vez ese es el porqué pasan unos segundos antes de que escuche la voz del maestro desde adentro."

# game/script-a4-rin.rpy:3807
translate es a4_rin_measuring_shadows_fc6734c8:

    # no "There you are!"
    no "¡Ahí estás!"

# game/script-a4-rin.rpy:3809
translate es a4_rin_measuring_shadows_811c5f41:

    # rin "Hello."
    rin "Hola."

# game/script-a4-rin.rpy:3811
translate es a4_rin_measuring_shadows_2e34d48b:

    # "A conflict arises: should I stay here or go somewhere else?"
    "Surge un conflicto: ¿Debería quedarme aquí o irme a algún otro lugar?"

# game/script-a4-rin.rpy:3813
translate es a4_rin_measuring_shadows_5a13c96e:

    # "I'm not sure if I even want to eavesdrop on them."
    "No estoy seguro si siquiera quiero escucharlos a escondidas."

# game/script-a4-rin.rpy:3815
translate es a4_rin_measuring_shadows_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:3820
translate es a4_rin_measuring_shadows_e426c925:

    # "Manners lose to curiosity, and so I stay close enough to listen in."
    "Los modales pierden ante la curiosidad, y entonces me quedo lo suficientemente cerca como para escucharlos."

# game/script-a4-rin.rpy:3822
translate es a4_rin_measuring_shadows_4a190024:

    # "Their voices echo in the hallway, but no matter."
    "Sus voces hacen eco en el pasillo, pero no importa."

# game/script-a4-rin.rpy:3824
translate es a4_rin_measuring_shadows_a48f9e1d:

    # "There is nobody around, save for me."
    "No hay nadie alrededor, salvo por mí."

# game/script-a4-rin.rpy:3828
translate es a4_rin_measuring_shadows_7c75511a:

    # no "Dear girl, what on Earth were you thinking, leaving like that on the big night?"
    no "Querida niña, ¿en qué estabas pensando, marchándote de esa manera en la gran noche?"

# game/script-a4-rin.rpy:3830
translate es a4_rin_measuring_shadows_ba978e60:

    # rin "I couldn't say anything."
    rin "No podía decir nada."

# game/script-a4-rin.rpy:3832
translate es a4_rin_measuring_shadows_de7a30cd:

    # "Compared to Nomiya's scolding tone, Rin sounds awfully quiet and withdrawn."
    "Comparado con el tono de regaño de Nomiya, Rin suena terriblemente callada y retraída."

# game/script-a4-rin.rpy:3834
translate es a4_rin_measuring_shadows_bced366c:

    # "Her words seem to drown under his."
    "Sus palabras parecen ahogarse bajo las de él."

# game/script-a4-rin.rpy:3836
translate es a4_rin_measuring_shadows_c5162c6d:

    # no "I have to say, I am very disappointed in you, Tezuka."
    no "Tengo que decir que estoy muy decepcionado de ti, Tezuka."

# game/script-a4-rin.rpy:3838
translate es a4_rin_measuring_shadows_e3306e7d:

    # rin "It was no good at all."
    rin "No fue para nada bueno."

# game/script-a4-rin.rpy:3840
translate es a4_rin_measuring_shadows_35f4098e:

    # no "Never mind all the things I did for you, but what about Sae? What about all the guests who wanted to meet you?"
    no "Olvida todas las cosas que hice por ti, pero, ¿qué hay de Sae? ¿Qué hay de todos los invitados que querían conocerte?"

# game/script-a4-rin.rpy:3842
translate es a4_rin_measuring_shadows_f107b085:

    # rin "There was nobody. Even Hisao…"
    rin "No había nadie. Incluso Hisao…"

# game/script-a4-rin.rpy:3844
translate es a4_rin_measuring_shadows_04098c4e:

    # no "You have embarrassed us very badly, Tezuka."
    no "Nos has avergonzado tanto, Tezuka."

# game/script-a4-rin.rpy:3846
translate es a4_rin_measuring_shadows_1ccaed51:

    # no "Reputation is what counts, surely you know that?"
    no "La reputación es lo que cuenta, ¿seguramente sabes eso?"

# game/script-a4-rin.rpy:3848
translate es a4_rin_measuring_shadows_1a57d6f6:

    # rin "It's all right. I don't need it."
    rin "Está bien. No la necesito."

# game/script-a4-rin.rpy:3850
translate es a4_rin_measuring_shadows_f52bc1ca:

    # no "'Don't need!' What do you think you know?"
    no "¡“No necesitas”! ¿Tú qué crees que sabes?"

# game/script-a4-rin.rpy:3852
translate es a4_rin_measuring_shadows_94db79fe:

    # "Rin's replies only seem to agitate the teacher more, his voice rising with every sentence."
    "Las respuestas de Rin solo parecen agitar más al maestro, su voz se levanta con cada oración."

# game/script-a4-rin.rpy:3854
translate es a4_rin_measuring_shadows_3a0f219c:

    # no "The path of an artist is a thorny one, I'll tell you that! Thorny!"
    no "El camino de un artista está lleno de espinas, ¡te diré eso! ¡Lleno de espinas!"

# game/script-a4-rin.rpy:3856
translate es a4_rin_measuring_shadows_60478bcc:

    # no "You have to see the big picture! There will be bad times and good times!"
    no "¡Tienes que ver el panorama total! ¡Habrá tiempos malos y tiempos buenos!"

# game/script-a4-rin.rpy:3858
translate es a4_rin_measuring_shadows_e566c382:

    # rin "Things are like they are. It'll be all right even—"
    rin "Las cosas son como son. Estará bien aun—"

# game/script-a4-rin.rpy:3860
translate es a4_rin_measuring_shadows_254659b4:

    # no "You might now think that it's oh so wonderful and easy, but how far would you have gotten without me?"
    no "Puede que ahora pienses que es tan, pero tan maravilloso y fácil, ¿pero qué tan lejos habrías llegado sin mí?"

# game/script-a4-rin.rpy:3862
translate es a4_rin_measuring_shadows_acbc9b1f:

    # no "I won't always be there for you!"
    no "¡No estaré siempre ahí para ti!"

# game/script-a4-rin.rpy:3864
translate es a4_rin_measuring_shadows_5eb00a09:

    # no "When you lie on the floor of your minuscule room, your rent three weeks late, your mind blank for the fourth week straight, then you will wish that you had listened to old Nomiya a bit more."
    no "Cuando estés tendida en el suelo de tu minúsculo departamento, tu renta tres semanas retrasada, tu mente en blanco por la cuarta semana seguida, entonces desearás haber escuchado al viejo Nomiya un poco más."

# game/script-a4-rin.rpy:3866
translate es a4_rin_measuring_shadows_0378543b:

    # no "When you keep measuring how the shadow of your chair becomes longer over the spring because that's all your lethargy allows, maybe that's when you will start caring about your career!"
    no "¡Cuando sigas midiendo cómo la sombra de tu silla se hace más larga durante la primavera porque eso es todo lo que te permite tu apatía, tal vez entonces sea cuando empezará a importarte tu carrera!"

# game/script-a4-rin.rpy:3868
translate es a4_rin_measuring_shadows_f1bae754:

    # rin "That doesn't matter."
    rin "Eso no importa."

# game/script-a4-rin.rpy:3870
translate es a4_rin_measuring_shadows_4de65275:

    # no "Your resolve is not enough."
    no "Tu determinación no es suficiente."

# game/script-a4-rin.rpy:3872
translate es a4_rin_measuring_shadows_42cfea3b:

    # rin "I am not a resolved person."
    rin "No soy una persona determinada."

# game/script-a4-rin.rpy:3874
translate es a4_rin_measuring_shadows_29e17732:

    # no "You are not a resolved person…"
    no "No eres una persona determinada…"

# game/script-a4-rin.rpy:3879
translate es a4_rin_measuring_shadows_7ae46f04:

    # no "Then tell me, why… why… WHY DID WE GO THROUGH ALL THIS TROUBLE IF IT AMOUNTS TO A MOSQUITO'S SHIT?"
    no "Entonces dime, por qué… por qué… ¿POR QUÉ FUE QUE PASAMOS POR TODO ESTE PROBLEMA SI TODO ASCIENDE A LA MIERDA DE UN MOSQUITO?"

# game/script-a4-rin.rpy:3881
translate es a4_rin_measuring_shadows_edfbf6aa:

    # "Oh dear, the teacher blew a fuse."
    "Por Dios, el maestro perdió la cabeza."

# game/script-a4-rin.rpy:3883
translate es a4_rin_measuring_shadows_56a2d361:

    # "Him yelling at Rin makes me feel bystander's guilt. If I had gone with her, maybe he wouldn't have gotten so angry."
    "Que él le grite a Rin me hace sentir culpa por ser un espectador. Si hubiese ido con ella, quizás no se hubiese enojado tanto."

# game/script-a4-rin.rpy:3885
translate es a4_rin_measuring_shadows_a5adb09f:

    # "If I had not let her run away, he wouldn't have gotten angry in the first place."
    "Si no la hubiese dejado ir, él no se hubiese enojado en primer lugar."

# game/script-a4-rin.rpy:3887
translate es a4_rin_measuring_shadows_1e75dc9d:

    # "I still could go and save her… I don't think I can."
    "Todavía podría ir y salvarla… no creo que pueda."

# game/script-a4-rin.rpy:3889
translate es a4_rin_measuring_shadows_713fbfe8:

    # "I was the same. I yelled at Rin too, and I'm feeling all the more embarrassed about it now."
    "Yo fui igual. También le grité a Rin, y me siento todavía más avergonzado de ello."

# game/script-a4-rin.rpy:3891
translate es a4_rin_measuring_shadows_5687b0c3:

    # "I felt justified to vent my anger at her face just because… just because I felt it was her fault that I was so frustrated."
    "Me sentí justificado de descargar mi ira en su cara solo porque… solo porque sentí que era su culpa que me sintiera tan frustrado."

# game/script-a4-rin.rpy:3893
translate es a4_rin_measuring_shadows_35f8da37:

    # "I was no more justified than the teacher is."
    "Yo no tenía más justificación que el maestro."

# game/script-a4-rin.rpy:3895
translate es a4_rin_measuring_shadows_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:3897
translate es a4_rin_measuring_shadows_44dea617:

    # "A terrible silence sets upon the hallway."
    "Un terrible silencio se asienta en el pasillo."

# game/script-a4-rin.rpy:3899
translate es a4_rin_measuring_shadows_2c47ca41:

    # "Rin does not have anything to say to Nomiya."
    "Rin no tiene nada que decirle a Nomiya."

# game/script-a4-rin.rpy:3901
translate es a4_rin_measuring_shadows_a79943d3:

    # "Whether she has run out of answers or she knows that arguing would only make him angrier is anyone's guess."
    "Ya sea que se le han acabado las respuestas o que sepa que discutir solo lo enojaría más, nadie lo sabe."

# game/script-a4-rin.rpy:3903
translate es a4_rin_measuring_shadows_0862d794:

    # "The teacher has nothing more to say either, it seems, or maybe he just ran out of breath."
    "El maestro tampoco tiene nada más que decir, parece ser, o tal vez solo se le acabó el aire."

# game/script-a4-rin.rpy:3905
translate es a4_rin_measuring_shadows_a51bbc4a:

    # "For a moment, I imagine the two of them just staring at each other, one full of red-hot anger, the other full of… yes, what?"
    "Por un momento, me imagino a los dos solo mirándose mutuamente, uno rojo lleno de cólera, la otra llena de… sí, ¿de qué?"

# game/script-a4-rin.rpy:3907
translate es a4_rin_measuring_shadows_54a036f9:

    # "I can't tell how Rin feels, not before, not now."
    "No puedo saber cómo se siente Rin, no antes, ni ahora."

# game/script-a4-rin.rpy:3909
translate es a4_rin_measuring_shadows_4f0fb537:

    # "Teacher seems to expect Rin to say something too, but since she doesn't he finally continues in a quieter, but not less angry voice."
    "El maestro parece esperar que Rin diga algo también, pero ya que no lo hace, él finalmente continúa en una voz más callada pero no menos enojada."

# game/script-a4-rin.rpy:3911
translate es a4_rin_measuring_shadows_3e8f651d:

    # no "What worth is there in doing so much work if the outcome is… nothing?"
    no "¿Qué valor tiene hacer tanto trabajo si el resultado es… nada?"

# game/script-a4-rin.rpy:3913
translate es a4_rin_measuring_shadows_cc6452f4:

    # "Still, Rin will not say anything."
    "Aun así, Rin no dirá nada."

# game/script-a4-rin.rpy:3915
translate es a4_rin_measuring_shadows_fd187c3a:

    # no "I'm sorry. I shouldn't have gotten so excited."
    no "Lo lamento. No debería de haberme alterado tanto."

# game/script-a4-rin.rpy:3917
translate es a4_rin_measuring_shadows_b4229233:

    # "He does not sound sorry at all. Rather, his tone is cold and sharp, like he was spitting the words out of his mouth."
    "No suena apenado en absoluto. Más bien, su tono es frío y severo, como si estuviese escupiendo las palabras fuera de su boca."

# game/script-a4-rin.rpy:3919
translate es a4_rin_measuring_shadows_55965a48:

    # no "It seems that I was expecting too much. You are not an artist after all."
    no "Parece que estaba esperando demasiado de ti. No eres una artista, después de todo."

# game/script-a4-rin.rpy:3921
translate es a4_rin_measuring_shadows_927c5f46:

    # "Yeah, not sorry at all."
    "Sí, no se siente para nada apenado."

# game/script-a4-rin.rpy:3939
translate es a4_rin_measuring_shadows_e79fa0b3:

    # "He storms out of the club room and down the stairs without noticing me."
    "Él sale disparado del salón y baja por las escaleras sin darse cuenta de mí."

# game/script-a4-rin.rpy:3941
translate es a4_rin_measuring_shadows_faf1ce19:

    # "After he is gone, I peek carefully inside the classroom."
    "Después de que se ha ido, me asomo con cuidado dentro del salón."

# game/script-a4-rin.rpy:3947
translate es a4_rin_measuring_shadows_ebabbe35:

    # "Rin is left standing there, in front of the teacher's desk."
    "Rin se queda parada ahí, frente al escritorio del maestro."

# game/script-a4-rin.rpy:3952
translate es a4_rin_measuring_shadows_c880ca0d:

    # rin "I couldn't say I am sorry."
    rin "No podía decir que lo sentía."

# game/script-a4-rin.rpy:3954
translate es a4_rin_measuring_shadows_c43e5a6e:

    # "She says it into the humid air of the classroom, not to me."
    "Lo dice al húmedo aire del salón de clases, no a mí."

# game/script-a4-rin.rpy:3956
translate es a4_rin_measuring_shadows_311ce79f:

    # "But since the room won't answer her, I will have to."
    "Pero ya que el salón no le responde, tendré que hacerlo yo."

# game/script-a4-rin.rpy:3958
translate es a4_rin_measuring_shadows_cf06bdb3:

    # hi "That was unfair of him… He was angry, but still…"
    hi "Eso no fue justo de su parte… Estaba enojado, pero de todos modos…"

# game/script-a4-rin.rpy:3960
translate es a4_rin_measuring_shadows_5f883ced:

    # "I can't decide how to end my sentence. Disdaining the teacher feels like disdaining my own behavior from two days ago."
    "No puedo decidir en cómo terminar la oración. Desdeñar al maestro se siente como desdeñar mi propio comportamiento de hace dos días."

# game/script-a4-rin.rpy:3962
translate es a4_rin_measuring_shadows_b2363998:

    # "Stupid, but correct in hindsight."
    "Estúpido, pero correcto en retrospectiva."

# game/script-a4-rin.rpy:3967
translate es a4_rin_measuring_shadows_e87bad7e:

    # "Rin won't answer, staying petrified where she stands, so I walk up to her."
    "Rin no responderá, quedándose petrificada donde yace, así que camino hacia ella."

# game/script-a4-rin.rpy:3969
translate es a4_rin_measuring_shadows_040a94e2:

    # "She stood up for herself. In a way. I didn't expect that."
    "Ella defendió su postura. De cierto modo. No esperaba eso."

# game/script-a4-rin.rpy:3971
translate es a4_rin_measuring_shadows_01114343:

    # "I can't determine whether it's unbecoming or not, but either way, she did it."
    "No puedo determinar si es impropio o no, pero de cualquier manera, ella lo hizo."

# game/script-a4-rin.rpy:3973
translate es a4_rin_measuring_shadows_c7fc8fb0:

    # "Against me, she never did."
    "Contra mí nunca lo hizo."

# game/script-a4-rin.rpy:3975
translate es a4_rin_measuring_shadows_98782b07:

    # "I sort of wish she had, maybe I would not feel this bad then."
    "Creo que desearía que lo hubiese hecho, tal vez no me sentiría así de mal."

# game/script-a4-rin.rpy:3977
translate es a4_rin_measuring_shadows_ac23e748:

    # "Lately, it really seems that I've been wishing for all kinds of things."
    "Últimamente en verdad parece como si estuviese deseando todo tipo de cosas."

# game/script-a4-rin.rpy:3979
translate es a4_rin_measuring_shadows_79b8b7de:

    # hi "Rin?"
    hi "¿Rin?"

# game/script-a4-rin.rpy:3984
translate es a4_rin_measuring_shadows_1149e5d6:

    # rin "Go away."
    rin "Vete."

# game/script-a4-rin.rpy:3995
translate es a4_rin_raison_detre_a86f8847:

    # hi "Why… what are you saying?"
    hi "¿Por qué… qué estás diciendo?"

# game/script-a4-rin.rpy:4000
translate es a4_rin_raison_detre_328e6f5e:

    # rin "You're angry with me too, right?"
    rin "Tú también estás enojado conmigo, ¿verdad?"

# game/script-a4-rin.rpy:4002
translate es a4_rin_raison_detre_17a09b54:

    # rin "I thought you were my friend. I thought he was, too."
    rin "Pensé que eras mi amigo. Pensé que él lo era, también."

# game/script-a4-rin.rpy:4004
translate es a4_rin_raison_detre_10398a35:

    # "Her voice is unlike I've ever heard it, it's bitter, sharp like needles, and she keeps staring pointedly at her toes."
    "Su voz es diferente a la que he escuchado, es amarga, aguda como agujas y ella sigue mirando fijamente a sus dedos."

# game/script-a4-rin.rpy:4006
translate es a4_rin_raison_detre_33b1994b:

    # hi "I don't think it's about that."
    hi "No creo que sea sobre eso."

# game/script-a4-rin.rpy:4008
translate es a4_rin_raison_detre_a68d17f2:

    # hi "He wanted you to be something you are not. And…"
    hi "Él quería que fueras algo que no eres. Y…"

# game/script-a4-rin.rpy:4013
translate es a4_rin_raison_detre_b76c58e4:

    # "I take a deep breath and finally catch her eyes in my own, locking our gazes."
    "Tomo aire profundamente y finalmente atrapo sus ojos con los míos, fijando nuestras miradas."

# game/script-a4-rin.rpy:4015
translate es a4_rin_raison_detre_8581e44e:

    # hi "…I'm sorry. I wanted us to be something else too… more than friends."
    hi "… Lo siento. También quería que fuéramos algo más… más que amigos."

# game/script-a4-rin.rpy:4017
translate es a4_rin_raison_detre_af8ee4ad:

    # hi "Maybe that's why I couldn't contain myself and became so frustrated, just like the teacher did."
    hi "Tal vez es por eso que no pude contenerme y me sentí tan frustrado, al igual que el maestro."

# game/script-a4-rin.rpy:4022
translate es a4_rin_raison_detre_6e9f3c4c:

    # rin "What more? There is nothing more to me than me, that's all I am. I don't understand that."
    rin "¿Más qué? No hay más de mí que yo, eso es todo lo que soy. No lo entiendo."

# game/script-a4-rin.rpy:4024
translate es a4_rin_raison_detre_2f5449fc:

    # "Well… the answer should be obvious, right?"
    "Bueno… la respuesta debería ser obvia, ¿no?"

# game/script-a4-rin.rpy:4026
translate es a4_rin_raison_detre_eb839590:

    # "I remember myself, thinking of the purpose of friendship. To put up with everything and anything, to be there for your friend."
    "Me recuerdo a mí mismo, pensando en el propósito de la amistad. Aguantar de todo y de nada, estar ahí para tu amigo."

# game/script-a4-rin.rpy:4028
translate es a4_rin_raison_detre_aed08c9f:

    # "Did I fail as a friend, thinking it could be a stepping stone for something else?"
    "¿Fracasé como amigo, pensando que podría ser un peldaño para algo diferente?"

# game/script-a4-rin.rpy:4030
translate es a4_rin_raison_detre_d7849ebe:

    # "Maybe because of those thoughts, I didn't manage to put up with things, to keep it together."
    "Tal vez a causa de esos pensamientos, no logré aguantar esas cosas y mantenerme."

# game/script-a4-rin.rpy:4032
translate es a4_rin_raison_detre_d8455860:

    # "As outrageous as Rin is and was, I shouldn't have let myself get caught into that, especially when I started feeling the way I did towards her."
    "Tan intolerable como es y lo fue Rin, no debería de haberme permitido concentrarme en eso, especialmente cuando comencé a sentirme del modo que me sentí por ella."

# game/script-a4-rin.rpy:4034
translate es a4_rin_raison_detre_451e526f:

    # "So, did I fail?"
    "Así que, ¿fracasé?"

# game/script-a4-rin.rpy:4036
translate es a4_rin_raison_detre_3e4b8c69:

    # "That's what her eyes seem to ask."
    "Es lo que sus ojos parecen preguntar."

# game/script-a4-rin.rpy:4038
translate es a4_rin_raison_detre_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:4040
translate es a4_rin_raison_detre_4edfc2ce:

    # hi "I'm sorry, Rin."
    hi "Lo siento, Rin."

# game/script-a4-rin.rpy:4042
translate es a4_rin_raison_detre_77849431:

    # hi "I might not be able to be your friend."
    hi "Puede que no sea capaz de ser tu amigo."

# game/script-a4-rin.rpy:4044
translate es a4_rin_raison_detre_fb09bfa5:

    # hi "I don't think I could ever be a good friend to you."
    hi "No creo que algún día pueda ser un buen amigo para ti."

# game/script-a4-rin.rpy:4046
translate es a4_rin_raison_detre_45f8c1f5:

    # "I say these things because they are true, not because one of us would like to hear them."
    "Digo estas cosas porque son la verdad, no porque a uno de nosotros le agradaría escucharlas."

# game/script-a4-rin.rpy:4048
translate es a4_rin_raison_detre_08731ab3:

    # "But they are something that must be said."
    "Pero son algo que debe ser dicho."

# game/script-a4-rin.rpy:4050
translate es a4_rin_raison_detre_71028a93:

    # "The finality of my words creates a shaking silence, for what could either of us add to that?"
    "La finalidad de mis palabras crea un silencio retumbante, ¿pues qué podría alguno de nosotros añadir a ello?"

# game/script-a4-rin.rpy:4052
translate es a4_rin_raison_detre_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:4057
translate es a4_rin_raison_detre_898bc629:

    # rin "Why? Why does all this happen?"
    rin "¿Por qué? ¿Por qué pasa todo esto?"

# game/script-a4-rin.rpy:4062
translate es a4_rin_raison_detre_0015c967:

    # rin "People are doing things I don't ask for and don't want and everyone keeps getting angry at me, I have no idea what is going on any more and can't stop feeling like I want to run away from everything…"
    rin "La gente hace cosas que yo no pido y no quiero y todos siguen enojándose conmigo, ya no tengo idea de qué es lo que está sucediendo y no puedo dejar de sentir que quiero escaparme de todo…"

# game/script-a4-rin.rpy:4067
translate es a4_rin_raison_detre_8a8d11ae:

    # "She shuts her eyes tight and breathes out deeply, calmly."
    "Cierra los ojos, apretándolos, y deja salir su aliento desde lo más profundo con calma."

# game/script-a4-rin.rpy:4072
translate es a4_rin_raison_detre_9e0b5f21:

    # "When the eyelids open, all I can see is dark green desperation."
    "Cuando los párpados abren, todo lo que puedo ver es desesperación verde oscuro."

# game/script-a4-rin.rpy:4074
translate es a4_rin_raison_detre_f56118e1:

    # rin "{b}I have no idea what's wrong with me!{/b}"
    rin "{b}¡No tengo idea de qué hay de malo en mí!{/b}"

# game/script-a4-rin.rpy:4076
translate es a4_rin_raison_detre_a9fa7df7:

    # "Her frenetic outburst stupefies me for a moment, and for a heartbeat we just gaze into each other's face."
    "Su arrebato frenético me deja estupefacto por un momento, y por un instante simplemente nos miramos el rostro el uno al otro."

# game/script-a4-rin.rpy:4078
translate es a4_rin_raison_detre_6dd64e05:

    # "Seeing her confused eyes desperately looking for answers from mine only makes me sad, because I know I have none."
    "Ver sus ojos confundidos desesperadamente tratando de buscar respuestas en los míos solo me hace sentir triste, porque sé que no tengo ninguna."

# game/script-a4-rin.rpy:4080
translate es a4_rin_raison_detre_6bb4f611:

    # hi "I don't know either."
    hi "Yo tampoco lo sé."

# game/script-a4-rin.rpy:4082
translate es a4_rin_raison_detre_d7caf8fa:

    # hi "But you know, you yourself said that things are not right nor wrong."
    hi "Pero, sabes, tú misma dijiste que las cosas no están bien o mal."

# game/script-a4-rin.rpy:4084
translate es a4_rin_raison_detre_400d49b1:

    # hi "They just are."
    hi "Simplemente son."

# game/script-a4-rin.rpy:4086
translate es a4_rin_raison_detre_02a828c3:

    # hi "You either accept them, work to change them or give up."
    hi "O las aceptas, trabajas en cambiarlas o te rindes."

# game/script-a4-rin.rpy:4088
translate es a4_rin_raison_detre_c0932a9f:

    # hi "It's not that I hate you, or that teacher Nomiya does."
    hi "No es que yo te odie, o que el maestro Nomiya lo haga."

# game/script-a4-rin.rpy:4090
translate es a4_rin_raison_detre_8aaa5d46:

    # hi "I just… think that I am the kind of person who gives up when he feels he can't go on."
    hi "Es solo que… creo que soy del tipo de persona que se rinde cuando siente que ya no puede más."

# game/script-a4-rin.rpy:4092
translate es a4_rin_raison_detre_3c2a5195:

    # hi "And even if you hate it, this… this is… how things are."
    hi "Y aun si lo odias, así… así es… como son las cosas."

# game/script-a4-rin.rpy:4094
translate es a4_rin_raison_detre_29d2ca31:

    # "I'm saying pretty cruel things but I can't stop myself, the words keep rolling off my tongue with slow, hard certainty."
    "Estoy diciendo cosas muy crueles pero no puedo detenerme, las palabras siguen rodando de mi lengua con lenta y dura certeza."

# game/script-a4-rin.rpy:4099
translate es a4_rin_raison_detre_9cdb683d:

    # "I can see them hitting Rin almost like physical blows."
    "Puedo verlas golpear a Rin casi como golpes físicos."

# game/script-a4-rin.rpy:4101
translate es a4_rin_raison_detre_f8ae645e:

    # "As the wetness gathers into the corners of her eyes, they are still wide with the shock of rejection."
    "A medida que la humedad se junta en las orillas de sus ojos, estos siguen bien abiertos con la impresión del rechazo."

# game/script-a4-rin.rpy:4106
translate es a4_rin_raison_detre_ee8600d6:

    # "As the tears start rolling down her pale cheeks, she does nothing to stop them."
    "A medida que las lágrimas comienzan a deslizarse por sus pálidas mejillas, ella no las detiene."

# game/script-a4-rin.rpy:4108
translate es a4_rin_raison_detre_11c5de7b:

    # "As they fall down on the floor one by one, she stands still, staring at me with a gaze full of hollow disbelief."
    "A medida que caen en el piso una a una, ella se queda inmóvil, mirándome con ojos llenos de vacía incredulidad."

# game/script-a4-rin.rpy:4110
translate es a4_rin_raison_detre_879b98dc:

    # rin "…"
    rin "…"

# game/script-a4-rin.rpy:4112
translate es a4_rin_raison_detre_1e44f061:

    # "But reality catches up."
    "Pero la realidad nos alcanza."

# game/script-a4-rin.rpy:4117
translate es a4_rin_raison_detre_d303f825:

    # "Rin slumps forward as if she was deflating, and buries her face as deep in my shirt as she can."
    "Rin se desploma hacia adelante como si se estuviese desinflando, y hunde su cara en mi camisa lo más profundo que puede."

# game/script-a4-rin.rpy:4119
translate es a4_rin_raison_detre_05d6787e:

    # "Rin is heavy and featherlight when I support her weight."
    "Rin es pesada y ligera como una pluma cuando sostengo su peso."

# game/script-a4-rin.rpy:4121
translate es a4_rin_raison_detre_4eaad8de:

    # "She doesn't really sob or bawl, just leans against me, letting her tears burn through my shirt into the skin underneath."
    "Ella realmente no solloza o grita, solo se inclina en mí, dejando que su lágrimas quemen a través de mi camisa hacia la piel debajo."

# game/script-a4-rin.rpy:4123
translate es a4_rin_raison_detre_20566d5f:

    # "And I let her, bringing my hand around her shoulders in a clumsy hug that does no good to comfort her."
    "Y se lo permito, llevando mi mano alrededor de su hombro en un torpe abrazo que no hace nada para consolarla."

# game/script-a4-rin.rpy:4125
translate es a4_rin_raison_detre_71466c97:

    # "I can feel Rin's vertebrae against my fingertips, like hard and jagged reminders of how messed up things are."
    "Puedo sentir las vertebras de Rin en las yemas de mis dedos, como duros y dentados recordatorios de cuán mal se encuentran las cosas."

# game/script-a4-rin.rpy:4127
translate es a4_rin_raison_detre_e97643b3:

    # "Her slim shoulder quivering against my palm is a pitiable sight, and the hopelessness of being part of the cause for Rin's sadness keeps shredding my heart."
    "Sus delgados hombros temblando en mi palma son un espectáculo digno de dar pena, y la angustia de ser parte de la causa de la tristeza de Rin sigue desgarrando mi corazón."

# game/script-a4-rin.rpy:4177
translate es a4_rin_raison_detre_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4134
translate es a4_rin_raison_detre_d368b86c:

    # n "{vspace=120}To make a girl cry is the most despicable thing to do."
    n "{vspace=120}Hacer llorar a una chica es lo más despreciable que puede hacerse."

# game/script-a4-rin.rpy:4136
translate es a4_rin_raison_detre_aba60d25:

    # n "{vspace=30}Even Rin. Especially Rin."
    n "{vspace=30}Incluso a Rin. Especialmente a Rin."

# game/script-a4-rin.rpy:4138
translate es a4_rin_raison_detre_4a1d6033:

    # n "{vspace=30}Behind that veil of aloofness, Rin is just a human being too."
    n "{vspace=30}Detrás de ese velo de indiferencia, Rin es solo un ser humano, también."

# game/script-a4-rin.rpy:4140
translate es a4_rin_raison_detre_cad5f2ab:

    # n "Just as confused, scared and lost as any of us is."
    n "Igual de confundido, asustado y perdido como el resto de nosotros."

# game/script-a4-rin.rpy:4142
translate es a4_rin_raison_detre_687c3d91:

    # n "Most of the time it seems that there is no rhyme or reason for what Rin does and says, but for once, I think I really understand how she feels."
    n "La mayor parte del tiempo parece que no hay razón ni propósito de lo que Rin hace y dice, pero por única vez, creo que realmente entiendo cómo se siente ella."

# game/script-a4-rin.rpy:4144
translate es a4_rin_raison_detre_b4ed80bb:

    # n "{vspace=60}But no words can express it, and no words can make it better."
    n "{vspace=60}Pero ninguna palabra puede expresarlo, y ninguna palabra puede mejorarlo."

# game/script-a4-rin.rpy:4194
translate es a4_rin_raison_detre_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4157
translate es a4_rin_raison_detre_d0080779:

    # n "{vspace=150}So wordless we stay, quietly waiting for her tears to run out."
    n "{vspace=150}Y así nos quedamos sin palabras, esperando en silencio a que sus lágrimas se acaben."

# game/script-a4-rin.rpy:4159
translate es a4_rin_raison_detre_024094a7:

    # n "Time passes agonizingly slowly, even the lazy specks of dust floating in the air seem to pause into a standstill."
    n "El tiempo pasa agonizantemente lento, incluso las letárgicas motas de polvo flotando en el aire parecen detenerse por completo."

# game/script-a4-rin.rpy:4161
translate es a4_rin_raison_detre_2254b4a9:

    # n "The obligatory wall clock is ticking distractingly from above the door."
    n "El obligatorio reloj de pared hace tic-tac distractoramente sobre la puerta."

# game/script-a4-rin.rpy:4163
translate es a4_rin_raison_detre_e7e72875:

    # n "I decide against counting the seconds, because it would make them feel longer."
    n "Decido no contar los segundos, porque los haría sentir más largos."

# game/script-a4-rin.rpy:4165
translate es a4_rin_raison_detre_16d51601:

    # n "{vspace=60}…"
    n "{vspace=60}…"

# game/script-a4-rin.rpy:4216
translate es a4_rin_raison_detre_d41d8cd9_2:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4175
translate es a4_rin_raison_detre_b60b0732:

    # "Eventually Rin stirs a little and still smothering herself against my chest, mutters into my shirt."
    "Finalmente Rin se mueve un poco y, aún ahogándose a sí misma en mi pecho, murmura sobre mi camisa."

# game/script-a4-rin.rpy:4177
translate es a4_rin_raison_detre_ed876ef2:

    # rin "Let me be here for a while."
    rin "Déjame estar aquí por un tiempo."

# game/script-a4-rin.rpy:4182
translate es a4_rin_raison_detre_351c6e6c:

    # rin "Please, Hisao."
    rin "Por favor, Hisao."

# game/script-a4-rin.rpy:4184
translate es a4_rin_raison_detre_a31911cf:

    # rin "Just give me a little while."
    rin "Solo dame un poco de tiempo."

# game/script-a4-rin.rpy:4186
translate es a4_rin_raison_detre_6fef3343:

    # "A soothing deluge spreads into my consciousness, the knowledge that while being here for Rin is all I can do for her, that's all she wants right now, even after all we've gone through."
    "Un tranquilizador diluvio se propaga por mi consciencia, el conocimiento de que aunque estar aquí para Rin es todo lo que puedo hacer por ella, eso es todo lo que ella quiere en este momento, aun después de todo lo que hemos pasado."

# game/script-a4-rin.rpy:4188
translate es a4_rin_raison_detre_21ce94c5:

    # hi "Sure."
    hi "Claro."

# game/script-a4-rin.rpy:4190
translate es a4_rin_raison_detre_3af3c03c:

    # "So she stays there."
    "Y así se queda allí."

# game/script-a4-rin.rpy:4192
translate es a4_rin_raison_detre_f04f3643:

    # "But I still can't bring myself to draw her closer so I could embrace her properly."
    "Pero aun así no puedo obligarme a acercarla más hacia mí para poder abrazarla apropiadamente."

# game/script-a4-rin.rpy:4194
translate es a4_rin_raison_detre_93c1815a:

    # "It's because doing it would just make me so sad that I don't know if I could bear it."
    "Es porque hacerlo solo me haría sentir tan triste que no sé si podría soportarlo."

# game/script-a4-rin.rpy:4244
translate es a4_rin_raison_detre_d41d8cd9_3:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4201
translate es a4_rin_raison_detre_2b137976:

    # n "{vspace=60}The realization that we might never really be able to become what we want to be for the other crystallizes into my mind as a diamond-hard enlightenment."
    n "{vspace=60}La idea de que probablemente nunca seamos realmente capaces de convertirnos en lo que queremos ser para el otro se cristaliza en mi mente como una iluminación tan dura como diamante."

# game/script-a4-rin.rpy:4203
translate es a4_rin_raison_detre_dc129c07:

    # n "A pang surges through my heart like an electric shock."
    n "Una punzada atraviesa mi corazón como un choque eléctrico."

# game/script-a4-rin.rpy:4205
translate es a4_rin_raison_detre_1ea11879:

    # n "It's painful."
    n "Es doloroso."

# game/script-a4-rin.rpy:4207
translate es a4_rin_raison_detre_0804e70b:

    # n "This clarity… hurts."
    n "Esta claridad… lastima."

# game/script-a4-rin.rpy:4209
translate es a4_rin_raison_detre_ecb7b8b8:

    # n "What can we be for each other? What meaning is there for us to desperately cling to each other even though it seems so futile?"
    n "¿Qué podemos ser para el otro? ¿Qué significado tiene el que nos aferremos desesperadamente el uno al otro aunque parezca ser inútil?"

# game/script-a4-rin.rpy:4211
translate es a4_rin_raison_detre_81c3655e:

    # n "What should I say to Rin? How can I make her feel better?"
    n "¿Qué debería decirle a Rin? ¿Cómo hacerla sentir mejor?"

# game/script-a4-rin.rpy:4213
translate es a4_rin_raison_detre_0493a82f:

    # n "I do not know any of those things, and I fear knowing them would only hurt more."
    n "No sé ninguna de esas cosas, y tengo miedo de que saberlas únicamente lastimaría más."

# game/script-a4-rin.rpy:4215
translate es a4_rin_raison_detre_f8b35533:

    # n "Forcefully, I push all of that out of my mind because I don't want to be thinking of hurtful truths."
    n "A la fuerza, empujo todo ello fuera de mi mente porque no quiero estar pensando en verdades dolorosas."

# game/script-a4-rin.rpy:4217
translate es a4_rin_raison_detre_3a4b7b00:

    # n "My thoughts calm down soon enough, the sadness disperses until all that is left is me and Rin and the tender feeling of her warmth and softness against my chest."
    n "Mis pensamientos se tranquilizan pronto, la tristeza se dispersa hasta que todo lo que queda somos Rin y yo y la tierna sensación de su calidez y suavidad en mi pecho."

# game/script-a4-rin.rpy:4219
translate es a4_rin_raison_detre_d938d1c3:

    # nvl clear
    # n "{vspace=60}When did I fall in love with her?"
    nvl clear
    n "{vspace=60}¿Cuándo fue que me enamoré de ella?"

# game/script-a4-rin.rpy:4223
translate es a4_rin_raison_detre_64f7d26f:

    # n "I can't remember, but I'm certain it was way before the warm touch of her lips on my own, on that orange-colored afternoon when she was sick with cold and I went to see her because of unclear reasons."
    n "No puedo recordarlo, pero estoy seguro de que fue mucho antes de sentir el cálido toque de sus labios sobre los míos, en la tarde color naranja cuando ella estaba enferma de gripe y yo fui a verla por razones poco claras."

# game/script-a4-rin.rpy:4225
translate es a4_rin_raison_detre_6f5c9eeb:

    # n "Her carefree attitude, the air of otherness around her, all the things that make Rin herself… those things captured me with irresistible force."
    n "Su actitud despreocupada, el aire de alteridad a su alrededor, todas las cosas que hacen de Rin ella misma… aquellas cosas me capturaron con fuerza irresistible."

# game/script-a4-rin.rpy:4227
translate es a4_rin_raison_detre_3238220f:

    # n "The way she could take in anything and everything giving it only the value she herself placed, weighing all things fairly and without prejudice, seeing the world as she wanted."
    n "La manera en la que ella podía tomar todas y cada una de las cosas dándole solo el valor que ella misma le otorgaba, valorando todas las cosas justamente y sin prejuicio, viendo el mundo como ella quería."

# game/script-a4-rin.rpy:4229
translate es a4_rin_raison_detre_31a9f0ec:

    # n "This is something I could never do, and Rin was probably more of a muse to me than anything ever was to her."
    n "Esto es algo que yo nunca podría hacer, y Rin probablemente fue más una musa para mí que cualquier otra cosa jamás lo fue para ella."

# game/script-a4-rin.rpy:4231
translate es a4_rin_raison_detre_cd935f1e:

    # n "She seemed so free to me, truly a free spirit. While I, constantly worrying about everything, seemed so inhibited that it was almost embarrassing."
    n "Ella me parecía tan libre, verdaderamente un espíritu libre. Mientras que yo, preocupándome constantemente de todo, parecía tan inhibido que era casi vergonzoso."

# game/script-a4-rin.rpy:4233
translate es a4_rin_raison_detre_0e32f4d7:

    # n "Maybe that's why I latched so tightly on to Rin, trying to get inside her world that was so different from my own bleak life."
    n "Tal vez es por eso que me aferré con tanta fuerza de Rin, tratando de entrar en su mundo que era tan diferente de mi propia vida sombría."

# game/script-a4-rin.rpy:4235
translate es a4_rin_raison_detre_e5657d2b:

    # nvl clear
    # n "{vspace=60}Before I noticed it, that irresistible force had pulled me dangerously close to her, but it turned out to be way too alien for me."
    nvl clear
    n "{vspace=60}Antes de que me diera cuenta de ello, esa fuerza irresistible me había jalado peligrosamente cerca de ella, pero resultó ser demasiado ajeno a mí."

# game/script-a4-rin.rpy:4239
translate es a4_rin_raison_detre_792d39a9:

    # n "And I had forgotten Newton, of all things."
    n "Y me había olvidado de Newton, de entre todas las cosas."

# game/script-a4-rin.rpy:4241
translate es a4_rin_raison_detre_23d2f7d9:

    # n "The gravitational force is inversely proportional to the square of the distance between the objects…"
    n "La fuerza gravitacional es inversamente proporcional al cuadrado de la distancia entre los objetos."

# game/script-a4-rin.rpy:4243
translate es a4_rin_raison_detre_e04bb72e:

    # n "So if two people feel something for each other…"
    n "Así que si dos personas sienten algo por la otra…"

# game/script-a4-rin.rpy:4245
translate es a4_rin_raison_detre_ceccb846:

    # n "Heh."
    n "Je."

# game/script-a4-rin.rpy:4247
translate es a4_rin_raison_detre_770c0e91:

    # n "Even though feelings are not governed by the constants of the universe, I can't help thinking that for some time now I've been a satellite to Rin's brightly shining planet."
    n "Aunque los sentimientos no son gobernados por las constantes universales, no puedo evitar pensar que por algún tiempo ya he sido un satélite del planeta intensamente brillante de Rin."

# game/script-a4-rin.rpy:4249
translate es a4_rin_raison_detre_6e1b8abb:

    # n "{vspace=30}Planet Rin."
    n "{vspace=30}Planeta Rin."

# game/script-a4-rin.rpy:4251
translate es a4_rin_raison_detre_c32b9b24:

    # n "{vspace=30}The thought makes me almost laugh, she really does seem to be from another planet at times, minus green skin and possibly some tentacles."
    n "{vspace=30}El pensamiento casi me hace reír, ella en verdad parece ser de otro planeta algunas veces, menos la piel verde y posiblemente algunos tentáculos."

# game/script-a4-rin.rpy:4302
translate es a4_rin_raison_detre_d41d8cd9_4:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4261
translate es a4_rin_raison_detre_5955a6a7:

    # "Perhaps because of my stifled laughter, Rin pulls away and I let her go, feeling the cold when her warmth goes away, and slight embarrassment for letting my thoughts run wild like that."
    "Quizás a causa de mi risa contenida, Rin se aparta y la dejo ir, sintiendo el frío cuando su calor se ha ido, y un poco de vergüenza por dejar a mis pensamientos correr sin control de esa manera."

# game/script-a4-rin.rpy:4263
translate es a4_rin_raison_detre_9a30057f:

    # "I credit that as Rin being a bad influence on me, while being glad at the same time that she can't read thoughts for real."
    "Le doy crédito de ello a Rin siendo una mala influencia para mí, a la vez que me siento contento de que ella no puede leer los pensamientos en realidad."

# game/script-a4-rin.rpy:4265
translate es a4_rin_raison_detre_19219675:

    # "Rin's bitter tears have dried up, and she looks a little more like herself again."
    "Las lágrimas amargas de Rin se han secado, y ella se ve un poco más como sí misma de nuevo."

# game/script-a4-rin.rpy:4270
translate es a4_rin_raison_detre_12e51051:

    # "The lost look in her eyes is still there though. Her gaze wanders around restlessly before stopping at me."
    "Aunque el aspecto perdido en sus ojos sigue ahí. Su mirada se pasea alrededor sin descansar antes de detenerse en mí."

# game/script-a4-rin.rpy:4272
translate es a4_rin_raison_detre_adacdb45:

    # rin "What happened just now?"
    rin "¿Qué acaba de pasar?"

# game/script-a4-rin.rpy:4274
translate es a4_rin_raison_detre_0223cbca:

    # rin "Can you tell me?"
    rin "¿Puedes decirme?"

# game/script-a4-rin.rpy:4276
translate es a4_rin_raison_detre_87c4a6de:

    # hi "What? What do you mean?"
    hi "¿Qué? ¿De qué hablas?"

# game/script-a4-rin.rpy:4281
translate es a4_rin_raison_detre_a8947010:

    # rin "I cried."
    rin "Lloré."

# game/script-a4-rin.rpy:4283
translate es a4_rin_raison_detre_ad8f6622:

    # "She says that hesitantly, as if not believing it herself."
    "Dice eso dudándolo, como si no creyera en sí misma."

# game/script-a4-rin.rpy:4285
translate es a4_rin_raison_detre_8a2b55c1:

    # hi "Yes…"
    hi "Sí…"

# game/script-a4-rin.rpy:4287
translate es a4_rin_raison_detre_ce617998_2:

    # "…"
    "…"

# game/script-a4-rin.rpy:4289
translate es a4_rin_raison_detre_cfc1f971:

    # "She keeps staring at me, as if pleading guidance so that she wouldn't have to feel so lost."
    "Sigue mirándome, como si suplicara por orientación para que no tuviera que sentirse tan perdida."

# game/script-a4-rin.rpy:4291
translate es a4_rin_raison_detre_ce617998_3:

    # "…"
    "…"

# game/script-a4-rin.rpy:4296
translate es a4_rin_raison_detre_d66e33b2:

    # rin "Why?"
    rin "¿Por qué?"

# game/script-a4-rin.rpy:4298
translate es a4_rin_raison_detre_349e3329:

    # hi "You were sad."
    hi "Estabas triste."

# game/script-a4-rin.rpy:4300
translate es a4_rin_raison_detre_3f88c33e:

    # hi "Is that what you want me to say? But isn't that obvious?"
    hi "¿Es eso lo que querías que dijera? Pero, ¿no es eso obvio?"

# game/script-a4-rin.rpy:4305
translate es a4_rin_raison_detre_be7c1a33:

    # rin "I don't know. It feels weird to cry."
    rin "No lo sé. Se siente raro llorar."

# game/script-a4-rin.rpy:4307
translate es a4_rin_raison_detre_bf4cb129:

    # hi "What? I don't believe it. I mean, everyone does that. It's nor—"
    hi "¿Qué? No lo creo. Quiero decir, todo mundo lo hace. Es nor—"

# game/script-a4-rin.rpy:4309
translate es a4_rin_raison_detre_52d15ee0:

    # "I bite my tongue before I finish my argument about normality."
    "Me muerdo la lengua antes de terminar mi argumento sobre la normalidad."

# game/script-a4-rin.rpy:4311
translate es a4_rin_raison_detre_bf6a1b2a:

    # "Norms do not apply to the person I'm talking to."
    "Las normas no aplican a la persona con la que estoy hablando."

# game/script-a4-rin.rpy:4316
translate es a4_rin_raison_detre_db33f92d:

    # rin "It always felt so wrong, different from what is in me. Like I couldn't really tell what I felt."
    rin "Siempre se sintió tan mal, diferente de lo que hay dentro de mí. Como si no pudiera saber qué es lo que siento."

# game/script-a4-rin.rpy:4318
translate es a4_rin_raison_detre_8bf039d1:

    # rin "So I started thinking that maybe I don't know what I'm feeling. Maybe it's me who is wrong—"
    rin "Así que empecé a pensar que tal vez no sé qué es lo que siento. Tal vez soy yo quien está mal—"

# game/script-a4-rin.rpy:4320
translate es a4_rin_raison_detre_f27b9343:

    # rin "I thought those kinds of things."
    rin "Pensé ese tipo de cosas."

# game/script-a4-rin.rpy:4325
translate es a4_rin_raison_detre_c406ac59:

    # rin "I thought… that painting was enough because it felt that I did at least that right."
    rin "Pensé… que pintar era suficiente porque se sentía como si al menos hiciera eso bien."

# game/script-a4-rin.rpy:4327
translate es a4_rin_raison_detre_4b28f38d:

    # rin "That all that is inside me could become a picture if I tried really hard. And it could."
    rin "Que todo lo que hay dentro de mí pudiera convertirse en una imagen si realmente me esforzaba. Y sí podía."

# game/script-a4-rin.rpy:4329
translate es a4_rin_raison_detre_8c7a520e:

    # rin "But it doesn't feel like it's enough any more. Because if nobody else can see that, I will still be alone."
    rin "Pero ya no se siente como si fuera suficiente. Porque si nadie más puede verlo, seguiré estando sola."

# game/script-a4-rin.rpy:4334
translate es a4_rin_raison_detre_b6c80e2a:

    # rin "Was it wrong to try? Everyone got really angry at me for that."
    rin "¿Estuvo mal intentar? Todos se enojaron mucho conmigo por eso."

# game/script-a4-rin.rpy:4338
translate es a4_rin_raison_detre_8095eb63:

    # "I've rarely heard Rin say this much at once before."
    "Rara vez he escuchado antes a Rin decir tanto de una sola vez."

# game/script-a4-rin.rpy:4340
translate es a4_rin_raison_detre_29fdde40:

    # "Once she finishes, she simply shuts up, looking so neutral that it's hard to believe she just said what she did."
    "Una vez que termina, simplemente se calla, viéndose tan neutral que es difícil creer que acabara de decir lo que dijo."

# game/script-a4-rin.rpy:4342
translate es a4_rin_raison_detre_a444f39f:

    # "I don't know what to think."
    "No sé qué pensar."

# game/script-a4-rin.rpy:4344
translate es a4_rin_raison_detre_ce617998_4:

    # "…"
    "…"

# game/script-a4-rin.rpy:4346
translate es a4_rin_raison_detre_d23c6534:

    # "Rin was desperate for someone to look at her paintings, and somehow see right through them into her soul, to understand her feelings…"
    "Rin estaba desesperada por que alguien viera sus pinturas, y de algún modo viera a través de ellas dentro de su alma, que entendiera sus sentimientos…"

# game/script-a4-rin.rpy:4348
translate es a4_rin_raison_detre_953b2b5c:

    # "Because… she felt she could not express them in any other way?"
    "¿Porque… ella sintió que no podía expresarlos de alguna otra manera?"

# game/script-a4-rin.rpy:4350
translate es a4_rin_raison_detre_c0ba05f9:

    # "How can one say whether that is right or wrong?"
    "¿Cómo puede alguien decir si eso está bien o mal?"

# game/script-a4-rin.rpy:4352
translate es a4_rin_raison_detre_dd6ad7cf:

    # "Could it be that all this time she's been trying to reach out to me like I've tried to reach out to her?"
    "¿Podría ser que durante todo este tiempo ella ha estado tratando de alcanzarme como yo he estado tratando de alcanzarla a ella?"

# game/script-a4-rin.rpy:4354
translate es a4_rin_raison_detre_ce617998_5:

    # "…"
    "…"

# game/script-a4-rin.rpy:4356
translate es a4_rin_raison_detre_04c6ce66:

    # "I sit down on a desk to think, and to rest my legs that kept us both standing for a long while."
    "Me siento en un banco para pensar, y para descansar mis piernas que nos mantuvieron a los dos de pie por un largo rato."

# game/script-a4-rin.rpy:4360
translate es a4_rin_raison_detre_2c896449:

    # hi "You know, when I read a good book or look at a starry sky or whatever, sometimes I too feel something… profound, like a… shoot, I don't know how to describe it."
    hi "Sabes, cuando leo un libro o veo un cielo estrellado o lo que sea, algunas veces también yo siento algo… profundo, como un… rayos, no sé cómo describirlo."

# game/script-a4-rin.rpy:4362
translate es a4_rin_raison_detre_6a9a2fd2:

    # hi "But the instant I try to put it into words I feel that I lose something, it doesn't feel as real, as true as it did inside my head."
    hi "Pero el instante que trato de ponerlo en palabras siento que pierdo algo, no se siente igual de real, igual de verdadero que como se sentía dentro de mi cabeza."

# game/script-a4-rin.rpy:4364
translate es a4_rin_raison_detre_48a460c2:

    # hi "It feels a bit phony. Damn, even what I just said felt phony."
    hi "Se siente un poco falso. Maldición, incluso lo que acabo de decir se sintió falso."

# game/script-a4-rin.rpy:4366
translate es a4_rin_raison_detre_e682e6a2:

    # "I offer a smile that is meant to be between funny and self-deprecating, but Rin doesn't react."
    "Le ofrezco una sonrisa que se supone ser entre graciosa y autocrítica, pero Rin no reacciona."

# game/script-a4-rin.rpy:4368
translate es a4_rin_raison_detre_b054bfeb:

    # hi "Anyway…"
    hi "De cualquier modo…"

# game/script-a4-rin.rpy:4370
translate es a4_rin_raison_detre_107c4e75:

    # hi "It might be that nobody can ever express their true feelings so that others understand."
    hi "Puede ser que nadie puede jamás expresar sus verdaderos sentimientos de manera que otros entiendan."

# game/script-a4-rin.rpy:4372
translate es a4_rin_raison_detre_9b1b6d9c:

    # hi "Reality has no chance of living up to what someone has inside their head."
    hi "La realidad no tiene oportunidad de compararse con lo que alguien tiene dentro de su cabeza."

# game/script-a4-rin.rpy:4374
translate es a4_rin_raison_detre_6a9bf8c7:

    # hi "Nothing can match that. Not even your paintings, except maybe for you."
    hi "Nada puede igualarlo. Ni siquiera tus pinturas, excepto tal vez para ti."

# game/script-a4-rin.rpy:4376
translate es a4_rin_raison_detre_34fe76fc:

    # hi "But I suppose you can't keep everything inside, you'd explode for real then."
    hi "Pero supongo que tú no puedes mantener todo dentro, entonces explotarías realmente."

# game/script-a4-rin.rpy:4378
translate es a4_rin_raison_detre_ee6be804:

    # hi "What I'm trying to say is… I don't think it's wrong to express your feelings, even if you use painting as your conduit."
    hi "Lo que estoy tratando de decir es… no creo que esté mal expresar tus sentimientos, aun si usas la pintura como tu conducto."

# game/script-a4-rin.rpy:4380
translate es a4_rin_raison_detre_ef735d30:

    # hi "You just can't expect people to understand you any better than they would if you did it any other way."
    hi "Tú simplemente no puedes esperar que la gente te comprenda mejor que si lo hubieras hecho de cualquier otra forma."

# game/script-a4-rin.rpy:4382
translate es a4_rin_raison_detre_8a82649e:

    # hi "In fact, you can't expect people to understand you at all."
    hi "De hecho, no puedes esperar que la gente te comprenda en absoluto."

# game/script-a4-rin.rpy:4384
translate es a4_rin_raison_detre_02f41c2d:

    # hi "It's because everything is so subjective. You see the world the way you do, but it's different from everyone else."
    hi "Es porque todo es tan subjetivo. Tú ves el mundo de tu manera, pero es diferente del de todos los demás."

# game/script-a4-rin.rpy:4389
translate es a4_rin_raison_detre_6448d5be:

    # rin "But isn't that terrible?"
    rin "¿Pero no es eso terrible?"

# game/script-a4-rin.rpy:4391
translate es a4_rin_raison_detre_d7aeaf20:

    # hi "I guess it is, in a way."
    hi "Supongo que lo es, de cierto modo."

# game/script-a4-rin.rpy:4393
translate es a4_rin_raison_detre_ce617998_6:

    # "…"
    "…"

# game/script-a4-rin.rpy:4398
translate es a4_rin_raison_detre_3e9cf840:

    # "She frowns, looking probably as stricken as she can. Which is not much, but it's enough for me to understand that Rin is not particularly happy."
    "Ella frunce el ceño, viéndose probablemente tan afligida como puede. Que no es mucho, pero suficiente para que yo entienda que Rin no se encuentra especialmente contenta."

# game/script-a4-rin.rpy:4400
translate es a4_rin_raison_detre_e87bd7d2:

    # rin "I think it might make me sad after all."
    rin "Creo que me puede hacer sentir triste, después de todo."

# game/script-a4-rin.rpy:4402
translate es a4_rin_raison_detre_6845161a:

    # hi "Yeah. I know."
    hi "Sí. Lo sé."

# game/script-a4-rin.rpy:4404
translate es a4_rin_raison_detre_6c4db884:

    # hi "I wish I could do something to help it."
    hi "Desearía poder hacer algo para remediarlo."

# game/script-a4-rin.rpy:4406
translate es a4_rin_raison_detre_413175ba:

    # "I don't think I sound bitter, even though I am, a little."
    "No creo que suene amargado, aunque lo esté, un poco."

# game/script-a4-rin.rpy:4408
translate es a4_rin_raison_detre_8bee9134:

    # "This is my problem. I cannot be what Rin wants for her. And for the same reason, she can't do the same for me either."
    "Este es mi problema. No puedo ser lo que Rin quiere para sí. Y por la misma razón, ella tampoco puede hacer lo mismo por mí."

# game/script-a4-rin.rpy:4410
translate es a4_rin_raison_detre_ce617998_7:

    # "…"
    "…"

# game/script-a4-rin.rpy:4415
translate es a4_rin_raison_detre_3b349d31:

    # "She makes a difficult face, carefully trying to pick the words she wants to say."
    "Ella hace una cara complicada, cuidadosamente tratando de escoger las palabras que quiere decir."

# game/script-a4-rin.rpy:4417
translate es a4_rin_raison_detre_d42dc0de:

    # "So Rin has times when it's hard to say anything, too."
    "Así que Rin tiene momentos en los que le es difícil decir algo, también."

# game/script-a4-rin.rpy:4422
translate es a4_rin_raison_detre_87c9fe2f:

    # rin "It can't be helped, I think."
    rin "No hay más remedio, creo."

# game/script-a4-rin.rpy:4427
translate es a4_rin_raison_detre_99177fda:

    # rin "…but… if you say that…"
    rin "… Pero… si tú dices eso…"

# game/script-a4-rin.rpy:4432
translate es a4_rin_raison_detre_dd4a67b1:

    # rin "It makes me feel a little better."
    rin "Me hace sentir un poco mejor."

# game/script-a4-rin.rpy:4434
translate es a4_rin_raison_detre_ce617998_8:

    # "…"
    "…"

# game/script-a4-rin.rpy:4436
translate es a4_rin_raison_detre_019de7b4:

    # "It's funny how some seemingly irrelevant things are the most significant ones at times like this."
    "Es gracioso cómo cosas aparentemente irrelevantes son las más significativas en momentos como este."

# game/script-a4-rin.rpy:4438
translate es a4_rin_raison_detre_a64f84b7:

    # "Like how Rin's voice is very very small, barely audible when she says that."
    "Como la voz de Rin que suena tan tan baja, apenas audible cuando dice algo."

# game/script-a4-rin.rpy:4440
translate es a4_rin_raison_detre_a2c417aa:

    # "And how even her short bangs can cover her eyes when she looks downwards."
    "Y cómo incluso su corto flequillo puede cubrir sus ojos cuando mira hacia abajo."

# game/script-a4-rin.rpy:4445
translate es a4_rin_raison_detre_5751c244:

    # "And how they can't cover the deep red color rising on her cheeks and all the way to the tips of her ears."
    "Y cómo no puede cubrir el color rojo creciendo en sus mejillas hasta la punta de sus orejas."

# game/script-a4-rin.rpy:4447
translate es a4_rin_raison_detre_59a25684:

    # "They turn into a very interesting shade of red."
    "Se convierten en un tono de rojo muy interesante."

# game/script-a4-rin.rpy:4449
translate es a4_rin_raison_detre_549bdf94:

    # "A deafening silence follows."
    "Un silencio ensordecedor le sigue."

# game/script-a4-rin.rpy:4451
translate es a4_rin_raison_detre_3d3c20fc:

    # "It's very awkward, as if I saw something that wasn't meant to be seen, even if it wasn't on purpose."
    "Es muy incómodo, como si hubiese visto algo que no debería de haber visto, aun si no fue a propósito."

# game/script-a4-rin.rpy:4453
translate es a4_rin_raison_detre_c6c92ca9:

    # "I don't know what to say to that, but I keep feeling like I should know."
    "No sé qué decir, pero sigo sintiéndome como si debería saberlo."

# game/script-a4-rin.rpy:4455
translate es a4_rin_raison_detre_c0e8f418:

    # "She doesn't either."
    "Ella tampoco sabe."

# game/script-a4-rin.rpy:4457
translate es a4_rin_raison_detre_1038150c:

    # "Still, it feels like there is no momentum to lose even if we keep silent."
    "De cualquier modo, se siente como si no hubiera un impulso que perder aun si nos quedáramos en silencio."

# game/script-a4-rin.rpy:4459
translate es a4_rin_raison_detre_52d28473:

    # "Like we have some weird, wordless connection that would hold even so."
    "Como si tuviésemos alguna conexión rara sin palabras que seguiría de igual manera."

# game/script-a4-rin.rpy:4464
translate es a4_rin_raison_detre_557cc958:

    # "Rin keeps shifting her weight from one foot to the other restlessly, looking everywhere around the room except at me."
    "Rin sigue balanceándose de una pierna a otra sin parar, viendo a todos lados del salón excepto a mí."

# game/script-a4-rin.rpy:4466
translate es a4_rin_raison_detre_0a7e78aa:

    # "She is the one who finally breaks the spell."
    "Ella es quien finalmente acaba con el momento."

# game/script-a4-rin.rpy:4471
translate es a4_rin_raison_detre_0b2a8ec0:

    # rin "Can we go? I don't want to stay here."
    rin "¿Podemos irnos? No quiero estar aquí."

# game/script-a4-rin.rpy:4473
translate es a4_rin_raison_detre_1f756dac:

    # hi "Oh, yeah, of course. Where?"
    hi "Oh, sí, por supuesto. ¿Adónde?"

# game/script-a4-rin.rpy:4475
translate es a4_rin_raison_detre_53ce0290:

    # "My reply is covering my nervousness as badly as her question is covering hers."
    "Mi respuesta cubre mi nerviosismo igual de mal como su pregunta cubre el de ella."

# game/script-a4-rin.rpy:4480
translate es a4_rin_raison_detre_0218adc5:

    # rin "You can go wherever you like. I want to sleep. I haven't really slept for a few weeks."
    rin "Tú puedes ir adonde gustes. Yo quiero dormir. No he dormido en realidad por algunos días."

# game/script-a4-rin.rpy:4485
translate es a4_rin_raison_detre_73997863:

    # rin "It feels like there is a flock of light blue butterflies inside my head. It makes it hard to think properly."
    rin "Se siente como si hubiera una bandada de mariposas azul claro dentro de mi cabeza. Hace difícil pensar apropiadamente."

# game/script-a4-rin.rpy:4490
translate es a4_rin_raison_detre_b2ffe712:

    # rin "The kind that you think is too blue to really exist, like Emi's panties this morning."
    rin "Del tipo que piensas que es demasiado azul para realmente existir, como las pantaletas de Emi esta mañana."

# game/script-a4-rin.rpy:4516
translate es a4_rin_raison_detre_476897b1:

    # "She shakes her head, and I almost expect a couple of ultramarine-colored Morphos to pop out of her ears."
    "Ella sacude la cabeza, y casi espero que un par de morfos color azul ultramar salten de sus orejas."

# game/script-a4-rin.rpy:4521
translate es a4_rin_raison_detre_1ed49f2d:

    # "A small smile tugs upwards the corners of her mouth."
    "Una pequeña sonrisa tira hacia arriba las comisuras de sus labios."

# game/script-a4-rin.rpy:4523
translate es a4_rin_raison_detre_2350ff9c:

    # rin "That reminds me. The blue, not the panties."
    rin "Eso me recuerda. El azul, no las pantaletas."

# game/script-a4-rin.rpy:4528
translate es a4_rin_raison_detre_38a3db9d:

    # rin "The word for a flock of butterflies is a swarm. I looked it up."
    rin "La palabra para una bandada de mariposas es enjambre. La busqué."

# game/script-a4-rin.rpy:4530
translate es a4_rin_raison_detre_4ed50446:

    # "That makes my eyebrow rise into a questioning arch."
    "Eso hace que mis cejas se alcen formando un arco inquisitivo."

# game/script-a4-rin.rpy:4532
translate es a4_rin_raison_detre_e18b1573:

    # hi "Why don't you use it then?"
    hi "¿Por qué no la usas, entonces?"

# game/script-a4-rin.rpy:4537
translate es a4_rin_raison_detre_dd415c81:

    # rin "I like the other word better."
    rin "Me gusta más la otra palabra."

# game/script-a4-rin.rpy:4539
translate es a4_rin_raison_detre_28e0f0ee:

    # "Why look it up in the first place, then?"
    "¿Entonces para qué buscarla en primer lugar?"

# game/script-a4-rin.rpy:4541
translate es a4_rin_raison_detre_8fa37f5c:

    # hi "Then you should use it, right?"
    hi "Entonces deberías de usarla, ¿no?"

# game/script-a4-rin.rpy:4546
translate es a4_rin_raison_detre_0a07eb38:

    # "She nods and falls silent, her gaze escaping mine to the side, attracted by the dark orange sunlight refracting from the windows."
    "Ella asiente y guarda silencio, su mirada escapando de la mía hacia un lado, atraída por el naranja oscuro de la luz refractándose por las ventanas."

# game/script-a4-rin.rpy:4548
translate es a4_rin_raison_detre_4ff98111:

    # "We stay like that for a little while: me silently looking at her silently looking out of the window."
    "Nos quedamos así por un momento: yo silenciosamente viéndola a ella viendo silenciosamente por la ventana."

# game/script-a4-rin.rpy:4550
translate es a4_rin_raison_detre_a3c26168:

    # hi "Hey… you all right now?"
    hi "Oye… ¿ya te encuentras bien?"

# game/script-a4-rin.rpy:4555
translate es a4_rin_raison_detre_cfdd5168:

    # "She glances at me from the corner of her eye, looking wistful again. The sunlight's reflection doesn't betray any more of her inner feelings."
    "Me echa un vistazo desde el rabillo del ojo, viéndose melancólica de nuevo. El reflejo de la luz del sol no delata más de sus sentimientos internos."

# game/script-a4-rin.rpy:4557
translate es a4_rin_raison_detre_5e2df80d:

    # rin "I'll need to think about that."
    rin "Necesitaré pensar en eso."

# game/script-a4-rin.rpy:4559
translate es a4_rin_raison_detre_e5b3dad1:

    # "I want to continue this conversation, grasping at those straws that she finally revealed to even exist."
    "Quiero continuar con esta conversación, aferrándome a ese pequeño indicio de esperanza que finalmente ha revelado que existe."

# game/script-a4-rin.rpy:4564
translate es a4_rin_raison_detre_4b4035a8:

    # "But Rin is looking out of the window so absentmindedly that I know she won't be responsive in any way that would make sense."
    "Pero Rin está viendo fuera de la ventana tan distraídamente que sé que no me responderá de ningún modo que tenga sentido."

# game/script-a4-rin.rpy:4566
translate es a4_rin_raison_detre_029ac659:

    # "It's like some kind of defense mechanism of hers, to avoid being sensible."
    "Es como algún tipo de mecanismo de defensa suyo, para evitar ser sensata."

# game/script-a4-rin.rpy:4568
translate es a4_rin_raison_detre_d2c707f8:

    # "Her mind is like a butterfly in itself, always fluttering somewhere away whenever it's stirred."
    "Su mente es como una mariposa en sí misma, siempre revoloteando por algún lugar cuando sea que es provocada."

# game/script-a4-rin.rpy:4570
translate es a4_rin_raison_detre_8e04dc58:

    # "Just when I thought I could see behind her veil, she jumps out of my reach again."
    "Justo cuando pensé que podía ver detrás de su velo, ella salta fuera de mi alcance de nuevo."

# game/script-a4-rin.rpy:4572
translate es a4_rin_raison_detre_f7df2706:

    # "Maybe that's just how Rin is."
    "Tal vez simplemente es así como es Rin."

# game/script-a4-rin.rpy:4574
translate es a4_rin_raison_detre_84b3b28c:

    # "Maybe that's something I should just accept to get some peace of mind."
    "Tal vez eso es algo que debería solo de aceptar para obtener un poco de paz interior."

# game/script-a4-rin.rpy:4576
translate es a4_rin_raison_detre_16a7dec2:

    # hi "Okay."
    hi "Está bien."

# game/script-a4-rin.rpy:4578
translate es a4_rin_raison_detre_5ff982ec:

    # hi "I'll walk you back to the dorms then."
    hi "Te acompañaré a los dormitorios, entonces."

# game/script-a4-rin.rpy:4583
translate es a4_rin_raison_detre_a0d0eacd:

    # rin "Thanks."
    rin "Gracias."

# game/script-a4-rin.rpy:4588
translate es a4_rin_raison_detre_d88922f3:

    # rin "Really."
    rin "En verdad."

# game/script-a4-rin.rpy:4595
translate es a4_rin_raison_detre_6c3b0809:

    # "The empty hallways of the school devoid of its students feel very lonely."
    "Los pasillos vacíos de la escuela, carentes de sus estudiantes se sienten muy solitarios."

# game/script-a4-rin.rpy:4597
translate es a4_rin_raison_detre_6000513c:

    # "Less than one hour after the summer vacation began, the building seems to be deserted, and all that intrudes on the stillness of the hallways are our footsteps."
    "Menos de una hora después de que comenzaran las vacaciones de verano, el edificio parecía desierto, y todo lo que se entromete con la quietud son nuestros pasos."

# game/script-a4-rin.rpy:4599
translate es a4_rin_raison_detre_6905c3de:

    # "The change is sudden, but it shows how the building is just an empty shell, dead without its students and teachers."
    "El cambio es súbito, pero muestra cómo el edificio es tan solo un cascarón vacío, muerto sin sus estudiantes y maestros."

# game/script-a4-rin.rpy:4601
translate es a4_rin_raison_detre_adbfa89c:

    # "It's as though the school has become a private world for only the two of us, a desolate place filled with silence and chalk dust."
    "Es como si la escuela se hubiera convertido en un mundo privado solo para nosotros dos, un lugar desolado lleno de silencio y polvo de tiza."

# game/script-a4-rin.rpy:4607
translate es a4_rin_raison_detre_28b47659:

    # rin "I think I have to change."
    rin "Creo que tengo que cambiar."

# game/script-a4-rin.rpy:4609
translate es a4_rin_raison_detre_c8a91802:

    # "She says it out of the blue while we walk down the staircase from the third floor, still managing to feel like she is mirroring what I was thinking just before."
    "Ella dice eso de la nada mientras bajamos por las escaleras desde el tercer piso, todavía logrando sentirse como si estuviese reflejando lo que estaba pensando apenas antes."

# game/script-a4-rin.rpy:4611
translate es a4_rin_raison_detre_72471f0d:

    # hi "That's what people must do, sometimes."
    hi "Eso es lo que la gente debe hacer, algunas veces."

# game/script-a4-rin.rpy:4659
translate es a4_rin_raison_detre_d41d8cd9_5:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4616
translate es a4_rin_raison_detre_ae118e0e:

    # n "{vspace=240}That's the last thing we say to each other that day, even though there would be so much to talk about."
    n "{vspace=240}Eso es lo último que nos decimos el uno al otro ese día, aun si habría tanto de qué hablar."

# game/script-a4-rin.rpy:4618
translate es a4_rin_raison_detre_dfb8e2b0:

    # n "And even those words drown in the all-encompassing silence, disappearing into the stagnant air as if they were never said."
    n "E incluso esas palabras se ahogan en el silencio que lo abarca todo, desapareciendo en el aire enrarecido como si nunca hubiesen sido dichas."

# game/script-a4-rin.rpy:4666
translate es a4_rin_raison_detre_d41d8cd9_6:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4640
translate es a4_rin_wb_and_ws_c1dc1f90:

    # "The first day of summer vacation is a disappointment."
    "El primer día de las vacaciones de verano es una decepción."

# game/script-a4-rin.rpy:4642
translate es a4_rin_wb_and_ws_83c1e36f:

    # "I woke up. Water came down from the leaden sky in Biblical proportions."
    "Me desperté. El agua bajó desde el cielo plomizo en proporciones bíblicas."

# game/script-a4-rin.rpy:4644
translate es a4_rin_wb_and_ws_02317591:

    # "I was optimistic at the time."
    "Era optimista en ese momento."

# game/script-a4-rin.rpy:4646
translate es a4_rin_wb_and_ws_b9437469:

    # "A quick summer shower, I thought. Torrents of rain for a few minutes, then it's gone."
    "Una llovizna rápida, pensé. Torrentes de lluvia por unos minutos, después se va."

# game/script-a4-rin.rpy:4648
translate es a4_rin_wb_and_ws_ed6bb294:

    # "No such luck."
    "No hay tal suerte."

# game/script-a4-rin.rpy:4657
translate es a4_rin_wb_and_ws_be4e215c:

    # "Rainwater is relentlessly pouring down from the blue-gray sky outside, streaming down the glass of my window in small brooks and rivers and gathering together to form miniature ponds on the walkways."
    "El agua de lluvia cae a raudales sin cansancio desde el cielo azul grisáceo fuera, corriendo por el vidrio de mi ventana en pequeños ríos y riachuelos, juntándose para formar estanques en miniatura en las aceras."

# game/script-a4-rin.rpy:4659
translate es a4_rin_wb_and_ws_0167ac62:

    # "Just like it has done for the past two and a half hours."
    "Igual que lo ha estado haciendo por las últimas dos horas y media."

# game/script-a4-rin.rpy:4666
translate es a4_rin_wb_and_ws_d88ef36c:

    # "So I've been half-assedly cleaning up in between half-assedly reading a book, packing my stuff on the side when I get bored of the first two."
    "Así que he estado entre limpiar sin ganas y leer sin ganas un libro, empacando mis cosas a ratos cuando me aburro de las primeras dos."

# game/script-a4-rin.rpy:4668
translate es a4_rin_wb_and_ws_df831c42:

    # "The weather drags my spirits pretty down too, making it harder to do anything properly."
    "El clima arrastra mis ánimos por los suelos, haciendo más difícil el hacer algo apropiadamente."

# game/script-a4-rin.rpy:4672
translate es a4_rin_wb_and_ws_22b52622:

    # "Something bumping quite loudly against my door rouses me from my apathy."
    "Algo chocando contra mi puerta me saca de mi apatía."

# game/script-a4-rin.rpy:4674
translate es a4_rin_wb_and_ws_c552a4ad:

    # "I hope it's not Kenji and his crazy indoors bowling alley."
    "Espero que no sea Kenji y su loca línea de bolos bajo techo."

# game/script-a4-rin.rpy:4676
translate es a4_rin_wb_and_ws_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:4678
translate es a4_rin_wb_and_ws_b844a4f7:

    # "I hear no more sounds from the corridor until I walk to the door and open it."
    "No escucho más sonidos del corredor hasta que camino hacia la puerta y la abro."

# game/script-a4-rin.rpy:4688
translate es a4_rin_wb_and_ws_e1d2edbf:

    # "Rin."
    "Rin."

# game/script-a4-rin.rpy:4690
translate es a4_rin_wb_and_ws_ab3de4e6:

    # "I wish seeing her would evoke some more emotion in me, but for one, I'm too surprised that she came to see me and for two, she is soaking wet."
    "Desearía que verla evocara algo más de emoción en mí, pero para empezar, estoy demasiado sorprendido de que ella haya venido a verme y para continuar, está empapada."

# game/script-a4-rin.rpy:4692
translate es a4_rin_wb_and_ws_1c628974:

    # "Her uniform shirt is drenched and she is standing in a self-created puddle."
    "La blusa de su uniforme está mojada y ella está de pie sobre un charco hecho por ella misma."

# game/script-a4-rin.rpy:4694
translate es a4_rin_wb_and_ws_a4ac716a:

    # "Droplets of rainwater are dripping from her short bangs and sliding down her nose until they fall down from the tip."
    "Gotas de agua resbalan por su corto flequillo y se deslizan por su nariz hasta caer desde la punta."

# game/script-a4-rin.rpy:4696
translate es a4_rin_wb_and_ws_d3ae6300:

    # "One.{w=0.7} By.{w=0.7} One."
    "Una.{w=0.7} Por.{w=0.7} Una."

# game/script-a4-rin.rpy:4698
translate es a4_rin_wb_and_ws_9abb5df8:

    # hi "Umm… hi."
    hi "Ahh… hola."

# game/script-a4-rin.rpy:4700
translate es a4_rin_wb_and_ws_c65cf440:

    # hi "How are you feeling?"
    hi "¿Cómo te sientes?"

# game/script-a4-rin.rpy:4705
translate es a4_rin_wb_and_ws_79478467:

    # rin "Medium normal."
    rin "Medio normal."

# game/script-a4-rin.rpy:4709
translate es a4_rin_wb_and_ws_bc7af2e4:

    # "The relative questionability of her statement aside, she sure doesn't look too good."
    "Dejando de lado la relativa cuestionabilidad de su expresión, ella sí que no se ve muy bien."

# game/script-a4-rin.rpy:4711
translate es a4_rin_wb_and_ws_4451b91a:

    # hi "You're all wet."
    hi "Estás toda mojada."

# game/script-a4-rin.rpy:4716
translate es a4_rin_wb_and_ws_8de3af21:

    # rin "It's because I come from the outside. Do you know it?"
    rin "Es porque vengo de afuera. ¿Sabes eso?"

# game/script-a4-rin.rpy:4718
translate es a4_rin_wb_and_ws_251d293f:

    # hi "Why'd you be outside? It's raining buckets out there, if you haven't noticed."
    hi "¿Por qué estarías afuera? Está lloviendo a cántaros, por si no lo has notado."

# game/script-a4-rin.rpy:4723
translate es a4_rin_wb_and_ws_bcc78a0a:

    # rin "I haven't. It's raining pretty hard though. I was on a walk."
    rin "No lo he notado. Aunque está lloviendo bastante fuerte. Estaba dando un paseo."

# game/script-a4-rin.rpy:4725
translate es a4_rin_wb_and_ws_4e2d34b6:

    # hi "Is this what you call 'wallowing in self-pity?'"
    hi "¿Es esto a lo que llamas “regodearse en la autocompasión”?"

# game/script-a4-rin.rpy:4730
translate es a4_rin_wb_and_ws_9567d639:

    # rin "Do you think I'm pitiful?"
    rin "¿Crees que doy pena?"

# game/script-a4-rin.rpy:4732
translate es a4_rin_wb_and_ws_ddb58c5d:

    # hi "No, I implied that you think you are."
    hi "No, quise decir que tú piensas eso."

# game/script-a4-rin.rpy:4737
translate es a4_rin_wb_and_ws_d04f0b0b:

    # rin "I'm not, and rain is not a sad thing."
    rin "No lo hago, y la lluvia no es algo triste."

# game/script-a4-rin.rpy:4742
translate es a4_rin_wb_and_ws_02779218:

    # rin "Don't you ever walk in the rain?"
    rin "¿Acaso nunca caminas en la lluvia?"

# game/script-a4-rin.rpy:4744
translate es a4_rin_wb_and_ws_a13a35de:

    # hi "I do, but only with proper equipment, like an umbrella."
    hi "Sí, pero solo con el equipo apropiado, como un paraguas."

# game/script-a4-rin.rpy:4749
translate es a4_rin_wb_and_ws_ef730b2d:

    # rin "You just need to imagine you have a blue umbrella with white stripes."
    rin "Solo necesitas imaginar que tienes un paraguas azul con rayas blancas."

# game/script-a4-rin.rpy:4751
translate es a4_rin_wb_and_ws_43056c15:

    # hi "It might be tough when rain is falling on my head."
    hi "Puede que sea difícil cuando hay lluvia cayendo sobre mi cabeza."

# game/script-a4-rin.rpy:4756
translate es a4_rin_wb_and_ws_e5c65b99:

    # rin "Just imagine harder."
    rin "Solo imagina con más fuerza."

# game/script-a4-rin.rpy:4758
translate es a4_rin_wb_and_ws_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:4760
translate es a4_rin_wb_and_ws_25700211:

    # "Yeah, she definitely is back to normal."
    "Sí, definitivamente ha vuelto a la normalidad."

# game/script-a4-rin.rpy:4762
translate es a4_rin_wb_and_ws_73ba8a99:

    # "Those half-sarcastic, inconsiderate remarks that really push my buttons even though she doesn't mean it, that vacant, spaced-out stare that always expects more than it gives."
    "Esos comentarios semisarcásticos y desconsiderados que realmente me hacen enojar aunque ella no lo haya querido, esa mirada ida y vacante que siempre espera recibir más de lo que da."

# game/script-a4-rin.rpy:4764
translate es a4_rin_wb_and_ws_9b78fc7d:

    # "It's so… very much like her."
    "Es tan… como ella."

# game/script-a4-rin.rpy:4769
translate es a4_rin_wb_and_ws_9cbab9a8:

    # rin "I may need to come in. I need some help with this water and clothes I'm wearing."
    rin "Puede que necesite entrar. Necesito un poco de ayuda con esta agua y prendas que llevo."

# game/script-a4-rin.rpy:4771
translate es a4_rin_wb_and_ws_932b2747:

    # "My brain quickly solves this equation, and I stumble with my words, a stark display of contrast against Rin's easygoing self-invitation."
    "Mi cerebro pronto resuelve la ecuación, y tropiezo con mis palabras, una clara muestra de contraste a la invitación despreocupada de Rin."

# game/script-a4-rin.rpy:4773
translate es a4_rin_wb_and_ws_bbcd449a:

    # hi "But, Emi…"
    hi "Pero, Emi…"

# game/script-a4-rin.rpy:4778
translate es a4_rin_wb_and_ws_409c511a:

    # "Rin shakes her head vehemently, causing water to sprinkle everywhere."
    "Rin sacude la cabeza con vehemencia, haciendo que gotas de agua se esparzan por todas partes."

# game/script-a4-rin.rpy:4780
translate es a4_rin_wb_and_ws_c8c9c35a:

    # rin "She left."
    rin "Se fue."

# game/script-a4-rin.rpy:4785
translate es a4_rin_wb_and_ws_b7457483:

    # rin "Besides she would just worry and fuss until she could not worry or fuss any more, which always takes a troublesomely long time."
    rin "Además ella solo se preocuparía y armaría un alboroto hasta que ya no pudiera preocuparse o alborotarse más, lo que siempre toma un tiempo fastidiosamente largo."

# game/script-a4-rin.rpy:4790
translate es a4_rin_wb_and_ws_3894ffc0:

    # rin "It's in fact longer than I want to hear her fussing, and I thought you probably are not the fussing kind."
    rin "Es de hecho más largo de lo que quiero escucharla quejarse, y pensé que probablemente tú no eres del tipo que hace un escándalo."

# game/script-a4-rin.rpy:4806
translate es a4_rin_wb_and_ws_8d352ed3:

    # "She slumps down on my desk with a squishy sound."
    "Se deja caer sobre mi escritorio con un sonido húmedo."

# game/script-a4-rin.rpy:4808
translate es a4_rin_wb_and_ws_6f478650:

    # "Her soaked clothes are making the desk and everything on it wet but she doesn't care."
    "Su ropa empapada moja mi escritorio y todo lo demás pero a ella no le importa."

# game/script-a4-rin.rpy:4810
translate es a4_rin_wb_and_ws_ce617998_2:

    # "…"
    "…"

# game/script-a4-rin.rpy:4812
translate es a4_rin_wb_and_ws_86c397ea:

    # hi "Okay. Fine. I'll help you out."
    hi "Está bien, está bien. Te ayudaré."

# game/script-a4-rin.rpy:4814
translate es a4_rin_wb_and_ws_f2442de3:

    # hi "I have a towel somewhere. Do you want dry clothes? Is a uniform fine? I'm taller than you, but…"
    hi "Tengo una toalla en algún lugar. ¿Quieres ropa seca? ¿Está bien un uniforme? Soy más alto que tú, pero…"

# game/script-a4-rin.rpy:4819
translate es a4_rin_wb_and_ws_ef494bfa:

    # rin "Everything is fine."
    rin "Todo está bien."

# game/script-a4-rin.rpy:4824
translate es a4_rin_wb_and_ws_e5074b89:

    # "With a little searching I find a fresh uniform and a fluffy towel from the depths of my closet."
    "Buscando un poco encuentro un uniforme fresco y una toalla suave en las profundidades de mi armario."

# game/script-a4-rin.rpy:4829
translate es a4_rin_wb_and_ws_507a5528:

    # "The towel in one hand and the uniform in the other, I turn to face Rin again, uncertain of the next step."
    "Con la toalla en una mano y el uniforme en la otra, me doy vuelta hacia Rin otra vez, inseguro del siguiente paso."

# game/script-a4-rin.rpy:4831
translate es a4_rin_wb_and_ws_2e80a3cc:

    # "There is something wrong with me, a normal guy would just—{w=1.0}{nw}"
    "Hay algo mal conmigo, un tipo normal simplemente—{w=1.0}{nw}"

# game/script-a4-rin.rpy:4836
translate es a4_rin_wb_and_ws_b1d5d5de:

    # rin "Stop worrying. It is not a problem."
    rin "Deja de preocuparte. No es problema."

# game/script-a4-rin.rpy:4838
translate es a4_rin_wb_and_ws_07c04b46:

    # "She probably could see right through my hesitant demeanor."
    "Probablemente pudo ver justo a través de mi rostro titubeante."

# game/script-a4-rin.rpy:4840
translate es a4_rin_wb_and_ws_7d48a506:

    # "As if I was completely transparent to her."
    "Como si fuese completamente transparente para ella."

# game/script-a4-rin.rpy:4842
translate es a4_rin_wb_and_ws_cfdf9dbf:

    # "I push my anxiety away and concentrate on the eight buttons lined on her shirt, just like mine has."
    "Hago a mi ansiedad a un lado y me concentro en los ocho botones alineados en su blusa, justo como en mi camisa."

# game/script-a4-rin.rpy:4844
translate es a4_rin_wb_and_ws_611ba244:

    # "Only the first button is an obstacle, and after getting it over I undo the others with slightly less shaking hands."
    "Solo el primer botón es un obstáculo, y después de terminar con él desabotono los demás con manos un poco menos temblorosas."

# game/script-a4-rin.rpy:4855
translate es a4_rin_wb_and_ws_ebf3ed8f:

    # "Throwing the soaked shirt aside, I reveal Rin's pale upper body, shrouded only in her light blue brassiere which instantly reminds me of her saying it's her favorite color."
    "Arrojando a un lado la blusa empapada, revelo el pálido cuerpo superior de Rin, envuelto únicamente en un sostén azul claro lo cual me recuerda al instante que ella dijo era su color favorito."

# game/script-a4-rin.rpy:4857
translate es a4_rin_wb_and_ws_1229aacd:

    # "I try not to think too much about… stuff, but it's hard not to look at her body with what I can only think of as mixed feelings."
    "Trato de no pensar demasiado en… cosas, pero es difícil no ver su cuerpo con lo que solo puedo considerar como sentimientos encontrados."

# game/script-a4-rin.rpy:4859
translate es a4_rin_wb_and_ws_3564b3a0:

    # "I don't know what to think of this, so I just watch her. Rin looks… brittle."
    "No sé qué pensar de esto, así que solo la miro. Rin se ve… quebradiza."

# game/script-a4-rin.rpy:4861
translate es a4_rin_wb_and_ws_a79d2366:

    # "She is like a shell, a fragile thing just barely holding together."
    "Es como un cascarón, algo frágil manteniéndose apenas unido."

# game/script-a4-rin.rpy:4863
translate es a4_rin_wb_and_ws_24eacbe1:

    # "Her ribs, each of them visible under her pale skin, are moving up and down in the rhythm of her breaths."
    "Sus costillas, cada una de ellas visible bajo su pálida piel, se mueven de arriba a abajo al ritmo de su respiración."

# game/script-a4-rin.rpy:4865
translate es a4_rin_wb_and_ws_b912e64d:

    # "Rin always struck me as quite thin, but I realize now that the manic creative period before the exhibition opening might've caused her to lose weight."
    "Rin siempre me pareció como alguien muy delgada, pero ahora me doy cuenta de que el maníaco periodo creativo antes de la apertura de la exhibición pudo haber causado que perdiera peso."

# game/script-a4-rin.rpy:4867
translate es a4_rin_wb_and_ws_4c0f9d7f:

    # "Did she eat properly and enough? Definitely not and probably not."
    "¿Comió apropiadamente y suficiente? Definitivamente no y probablemente no."

# game/script-a4-rin.rpy:4869
translate es a4_rin_wb_and_ws_512d4945:

    # "This ugly, yet beautiful bare minimum of a human body that belongs to someone I care about is a contradiction of aesthetics in itself, oddly becoming of her."
    "Este feo y sin embargo hermoso mínimo de un cuerpo humano que le pertenece a alguien que me importa es una contradicción de la estética misma, extrañamente propio de ella."

# game/script-a4-rin.rpy:4871
translate es a4_rin_wb_and_ws_fa911beb:

    # "My eyes follow her collarbone to her shoulder and down her arm until the abrupt end."
    "Mis ojos siguen su clavícula hacia su hombro y abajo por el brazo hasta su abrupto final."

# game/script-a4-rin.rpy:4873
translate es a4_rin_wb_and_ws_f62cfa24:

    # "No, it's less than the bare minimum, I think with a passing pang of sadness and some guilt for thinking like that."
    "No, es menos que el mínimo, pienso con una pasajera sensación de tristeza y un poco de culpa por pensar de esa manera."

# game/script-a4-rin.rpy:4881
translate es a4_rin_wb_and_ws_2e2886ad:

    # "Her arms, degenerated into almost nothing but bone and skin due to lack of use, look very short now that the long sleeves of her uniform are not covering them:"
    "Sus brazos, degenerados hasta casi nada más que hueso y piel debido a la falta de uso, se ven muy cortos ahora que las largas mangas de su uniforme no los cubren:"

# game/script-a4-rin.rpy:4883
translate es a4_rin_wb_and_ws_b6b1f8af:

    # "My lack of any negative reaction makes me think that I've actually grown pretty accustomed of the various physical abnormalities of my schoolmates."
    "La falta de cualquier reacción negativa de mi parte me hace pensar que, de hecho, me he acostumbrado bastante a las varias anomalías físicas de mis compañeros."

# game/script-a4-rin.rpy:4885
translate es a4_rin_wb_and_ws_3f77f8ae:

    # "I always wondered why Rin keeps her shirt sleeves long, only tying them in a simple knot at where the elbow would be."
    "Siempre me pregunté por qué Rin mantiene las mangas de sus blusas largas, solo atándolas en un nudo simple donde estaría el codo."

# game/script-a4-rin.rpy:4887
translate es a4_rin_wb_and_ws_a2392e3a:

    # "It seems a bit impractical, but then again she is not exactly the pinnacle of practicality."
    "Parece poco práctico, pero por otro lado ella no es exactamente el pináculo de la practicidad."

# game/script-a4-rin.rpy:4889
translate es a4_rin_wb_and_ws_ae1daa83:

    # "Maybe she likes it, maybe it is somehow important to her. Maybe there is no deeper meaning to it."
    "Tal vez le guste, tal vez es de algún modo importante para ella. Tal vez no hay ningún significado profundo en ello."

# game/script-a4-rin.rpy:4891
translate es a4_rin_wb_and_ws_c45528be:

    # "I feel like asking, and almost do, but Rin's miserable state requires a higher priority of my attention."
    "Siento ganas de preguntar, y casi lo hago, pero el estado miserable de Rin requiere una atención de más alta prioridad."

# game/script-a4-rin.rpy:4898
translate es a4_rin_wb_and_ws_1fb4ce3e:

    # "She's stopped talking too, after we ran out of spiky greetings."
    "También ha dejado de hablar, después de que se nos acabaran los ásperos saludos."

# game/script-a4-rin.rpy:4900
translate es a4_rin_wb_and_ws_cd0c5c06:

    # "I guess there is no need for chitchat then."
    "Supongo que no hay necesidad de charlar, entonces."

# game/script-a4-rin.rpy:4905
translate es a4_rin_wb_and_ws_7a3f1c40:

    # "I pick up the towel from the bed and wrap it around her head, rumpling it all over her hair until most of the rainwater is hopefully soaked into the fabric."
    "Recojo la toalla de la cama y la envuelvo alrededor de su cabeza, arrugándola toda alrededor de su cabello hasta que la mayoría del agua, con esperanza, sea absorbida por la tela."

# game/script-a4-rin.rpy:4910
translate es a4_rin_wb_and_ws_322e26f2:

    # "She peeks from below the towel at me, looking up with impassive eyes."
    "Se asoma para verme por debajo de la toalla, mirándome con ojos impasibles."

# game/script-a4-rin.rpy:4912
translate es a4_rin_wb_and_ws_8dd0858a:

    # "It looks like she wants to say something without saying it."
    "Parece que quiere decir algo sin decirlo."

# game/script-a4-rin.rpy:4914
translate es a4_rin_wb_and_ws_f5174080:

    # "It's that kind of a look."
    "Es ese tipo de mirada."

# game/script-a4-rin.rpy:4916
translate es a4_rin_wb_and_ws_71051f20:

    # "But I can't read what she is thinking about from her face, so I just keep on fussing with the towel around her shoulders and hair."
    "Pero no puedo leer qué es en lo que está pensando en su cara, así que simplemente sigo batallando con la toalla alrededor de sus hombros y cabello."

# game/script-a4-rin.rpy:4918
translate es a4_rin_wb_and_ws_d7dbf791:

    # "The silence is oppressive, terrifying."
    "El silencio es opresivo, aterrorizante."

# game/script-a4-rin.rpy:4920
translate es a4_rin_wb_and_ws_7263a715:

    # "Communication between us has suddenly been reduced to the movements of my hands and the towel, and Rin swaying her body to and fro."
    "La comunicación entre nosotros súbitamente ha sido reducida a los movimientos de mis manos y de la toalla, y de Rin meciendo su cuerpo hacia enfrente y atrás."

# game/script-a4-rin.rpy:4922
translate es a4_rin_wb_and_ws_bf17d8b0:

    # "My jagged breathing and her quiet breaths, trying to find a common rhythm that just is not there."
    "Mi respiración entrecortada y la suya silenciosa, tratando de encontrar algún ritmo en común que simplemente no hay."

# game/script-a4-rin.rpy:4924
translate es a4_rin_wb_and_ws_b1731e57:

    # "I think I can hear her heartbeats, or maybe they are just mine redoubled."
    "Creo que puedo escuchar sus latidos, o tal vez son solo los míos duplicados."

# game/script-a4-rin.rpy:4926
translate es a4_rin_wb_and_ws_04263afc:

    # "As I brush a rogue strand of hair aside from her ear, Rin suddenly presses her cheek against the back of my hand."
    "Cuando hago a un lado un solitario mechón de cabello de su oreja, Rin súbitamente presiona su mejilla contra el dorso de mi mano."

# game/script-a4-rin.rpy:4928
translate es a4_rin_wb_and_ws_fad31190:

    # "The contact is electric, a jolt of current surging through me."
    "El contacto es eléctrico, una descarga de corriente se dispara dentro de mí."

# game/script-a4-rin.rpy:4933
translate es a4_rin_wb_and_ws_ae2fee98:

    # "Whether she seeks comfort, warmth or just my touch I wouldn't know, but I can't help touching her back, caressing her soft cheek with my hand."
    "Ya sea que busca consuelo, calor o simplemente mi tacto, no podría saberlo, pero no puedo evitar tocarla de vuelta, acariciar su suave mejilla con mi mano."

# game/script-a4-rin.rpy:4935
translate es a4_rin_wb_and_ws_1651c9cf:

    # "And with closed eyes, she kisses me, on the fingers, counting the joints with her lips…"
    "Y con ojos cerrados, ella me besa, en los dedos, contando los nudillos con sus labios…"

# game/script-a4-rin.rpy:4937
translate es a4_rin_wb_and_ws_671ca505:

    # "I am saddened beyond my expressive capability."
    "Me siento triste más allá de mis capacidades expresivas."

# game/script-a4-rin.rpy:4939
translate es a4_rin_wb_and_ws_2dd8ebac:

    # "Here we are, a boy and girl, both in love or something like that with each other, or maybe not… and yet…"
    "Aquí estamos, un chico y una chica, ambos enamorados o algo similar a ello el uno del otro, o tal vez no… sin embargo…"

# game/script-a4-rin.rpy:4990
translate es a4_rin_wb_and_ws_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4947
translate es a4_rin_wb_and_ws_b89577d1:

    # n "{vspace=30}Something is broken, I can feel it in me and in Rin; in the way our gazes merely brush against each other, shying away from contact; in her closed, timid posture and in my way of touching her like a china doll, afraid of shattering her delicate form."
    n "{vspace=30}Algo está roto, puedo sentirlo dentro de mí y dentro de Rin; en el modo en que nuestras miradas apenas se rozan entre ellas, rehuyendo del contacto; en su postura cerrada y tímida y en mi modo de tocarla como si fuese una muñeca de porcelana, asustado de romper su delicada forma."

# game/script-a4-rin.rpy:4949
translate es a4_rin_wb_and_ws_a0ffa0d1:

    # n "In how we are closer than we have ever been, yet I'm not feeling happy. It's like yesterday."
    n "En cómo estamos más cerca de lo que hemos estado, y aun así no me siento feliz. Es como ayer."

# game/script-a4-rin.rpy:4951
translate es a4_rin_wb_and_ws_711737af:

    # n "When did tenderness and forlornness become one and the same word, acts of affection start invoking only longing? …How, why did we end up like this?"
    n "¿Cuándo fue que la ternura y el desamparo se convirtieron en una misma palabra, y las muestras de afecto comenzaron a invocar únicamente anhelo…? ¿Cómo… por qué fue que terminamos así?"

# game/script-a4-rin.rpy:4953
translate es a4_rin_wb_and_ws_52c54526:

    # n "'No, don't answer that,' I'd like to say to myself, but fighting against the omniscience of self-awareness is a lost cause."
    n "“No, no respondas eso”, me gustaría decirme a mí mismo, pero pelear con la omnisciencia de la consciencia de uno mismo es un caso perdido."

# game/script-a4-rin.rpy:4955
translate es a4_rin_wb_and_ws_82d61142:

    # n "Still, I am here, and Rin is here, and it feels like she might be able to solve whatever problems she has."
    n "Igualmente, estoy aquí, y Rin está aquí, y se siente como si ella podría ser capaz de resolver cualesquiera que sean sus problemas."

# game/script-a4-rin.rpy:4957
translate es a4_rin_wb_and_ws_eab49758:

    # n "And if she can, why couldn't I? Why couldn't we?"
    n "Y si ella puede, ¿por qué no podría yo? ¿Por qué no podríamos nosotros?"

# game/script-a4-rin.rpy:4959
translate es a4_rin_wb_and_ws_39d92670:

    # n "It feels like taking that step is too much, too difficult, too uncertain."
    n "Se siente como si dar ese paso sea demasiado, demasiado difícil, demasiado inseguro."

# game/script-a4-rin.rpy:4961
translate es a4_rin_wb_and_ws_3bb38949:

    # n "So for now, all I can do is dry her up so she won't get a cold again."
    n "Así que, por ahora, todo lo que puedo hacer es secarla para que no pesque un resfriado de nuevo."

# game/script-a4-rin.rpy:5013
translate es a4_rin_wb_and_ws_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:4972
translate es a4_rin_wb_and_ws_0c18ed33:

    # "I pet her head, trying to sort out the hair that refuses to be sorted out even when wet."
    "Le cepillo la cabeza con las manos, tratando de acomodar el cabello que se rehúsa a ser acomodado aun estando mojado."

# game/script-a4-rin.rpy:4974
translate es a4_rin_wb_and_ws_259d0e28:

    # "A pair of dark, glazed eyes follows my every movement."
    "Un par de oscuros ojos vidriosos siguen todos mis movimientos."

# game/script-a4-rin.rpy:4976
translate es a4_rin_wb_and_ws_6c9a16f1:

    # hi "Pants too?"
    hi "¿Los pantalones también?"

# game/script-a4-rin.rpy:4981
translate es a4_rin_wb_and_ws_0ce96b76:

    # "She nods an answer, leans back and spreads her legs, with a grotesquely inviting gesture that makes a nasty feeling crawl up and down my spine like a bad premonition."
    "Ella asiente como respuesta, recargándose hacia atrás y abriendo las piernas, con un gesto grotescamente seductor que provoca a un desagradable sentimiento trepar por mi espina de arriba a abajo como una mala premonición."

# game/script-a4-rin.rpy:4983
translate es a4_rin_wb_and_ws_a5c6a2f9:

    # "It's not enough to sober me though, as the silence is starting to make me feel detached from myself."
    "Sin embargo, no es suficiente para contenerme, mientras el silencio comienza a hacerme sentir separado de mí mismo."

# game/script-a4-rin.rpy:4985
translate es a4_rin_wb_and_ws_059d91d3:

    # "I move automatically, without thinking even though I should; I should talk to her about this, or at least about something."
    "Me muevo automáticamente, sin siquiera pensarlo aun si debería; debería hablar con ella de esto, o por lo menos de algo."

# game/script-a4-rin.rpy:4987
translate es a4_rin_wb_and_ws_4b041f6e:

    # "The silence is a spell, a pact that has bound us to this private world made of the dull sound of rainfall and the soft feel of her skin against my fingers."
    "El silencio es un encanto, un pacto que nos ha atado a este mundo privado hecho del sordo sonido de la lluvia y el suave sentir de su piel en mis dedos."

# game/script-a4-rin.rpy:4989
translate es a4_rin_wb_and_ws_c8165762:

    # "The button of her trousers is fastened tight, but it opens surprisingly easily."
    "El botón de sus pantalones está abrochado firmemente, pero se abre con sorprendente facilidad."

# game/script-a4-rin.rpy:4991
translate es a4_rin_wb_and_ws_ceddcab5:

    # "Slipping them off is hard, mostly because she is sitting on them, with no intention of standing up to ease my task."
    "Quitárselos es difícil, principalmente porque se está sentando en ellos, sin intención de levantarse para facilitar mi tarea."

# game/script-a4-rin.rpy:4997
translate es a4_rin_wb_and_ws_868abb29:

    # "I kneel down uncomfortably and titillatingly between her legs so I can quickly dry her bare feet, remembering that they are as important to her as hands are to me."
    "Me arrodillo incómoda y estimuladoramente entre sus piernas para poder secar rápido sus pies, recordando que son tan importantes para ella como las manos son para mí."

# game/script-a4-rin.rpy:4999
translate es a4_rin_wb_and_ws_5e74e2f0:

    # "As I work the towel up from her ankles, Rin brushes her thigh against my cheek and nudges the small of my back with her heel to make me come closer."
    "A medida que la voy secando con la toalla de sus tobillos hacia arriba, Rin roza mi mejilla con su muslo y me da un golpecito en la espalda baja con su talón para hacer que me acerque más."

# game/script-a4-rin.rpy:5001
translate es a4_rin_wb_and_ws_f9faefce:

    # "I look up to meet her silent stare that was waiting for me to look up."
    "Volteo hacia arriba para encontrarme con su mirada silenciosa que estaba esperando a que volteara."

# game/script-a4-rin.rpy:5003
translate es a4_rin_wb_and_ws_a63b73fb:

    # "That unassuming, expectant stare seems to say that the ball is in my court."
    "Esa mirada modesta y a la espera parece decir que el siguiente paso depende de mí."

# game/script-a4-rin.rpy:5005
translate es a4_rin_wb_and_ws_ce617998_3:

    # "…"
    "…"

# game/script-a4-rin.rpy:5007
translate es a4_rin_wb_and_ws_bc99e8fe:

    # "I fleetingly brush my hand against her inner thigh."
    "Brevemente rozo el interior de su muslo con mi mano."

# game/script-a4-rin.rpy:5012
translate es a4_rin_wb_and_ws_ab2b35d4:

    # "The touch makes her gasp sharply, as if she was trying to hold back breathing."
    "El toque la hace dar un grito ahogado, como si estuviese tratando de contener su aliento."

# game/script-a4-rin.rpy:5014
translate es a4_rin_wb_and_ws_cf62e651:

    # "What if I do this, then?"
    "¿Qué tal si hago esto, entonces?"

# game/script-a4-rin.rpy:5019
translate es a4_rin_wb_and_ws_948e6cf8:

    # "The small kiss I place on her thigh is enough to make Rin lose her composure, to shut her eyes, to squeal almost inaudibly."
    "El pequeño beso que coloco en su muslo es suficiente para hacer que Rin pierda su compostura, que cierre los ojos, que chille de manera casi inaudible."

# game/script-a4-rin.rpy:5021
translate es a4_rin_wb_and_ws_7ff3e211:

    # "…Is that what you want too? Would it be all right now? To take this step?"
    "… ¿Es eso lo que tú también quieres? ¿Estaría bien? ¿El dar ese paso?"

# game/script-a4-rin.rpy:5026
translate es a4_rin_wb_and_ws_a28d4e58:

    # "…What if? Maybe if…"
    "… ¿Qué tal si…? ¿Tal vez si…?"

# game/script-a4-rin.rpy:5028
translate es a4_rin_wb_and_ws_90e71c6b:

    # "Hazy thoughts float somewhere in the back of my unfocused mind."
    "Pensamientos nebulosos flotan en algún lugar en el interior de mi mente dispersa."

# game/script-a4-rin.rpy:5030
translate es a4_rin_wb_and_ws_fb1d0adc:

    # "Somehow, this whole situation is making it hard to think, as if my head was full of cotton fluff."
    "De algún modo, toda esta situación hace que pensar sea muy difícil, como si mi cabeza estuviese colmada de relleno de algodón."

# game/script-a4-rin.rpy:5032
translate es a4_rin_wb_and_ws_0013710b:

    # "But that's all right. It seems thinking is not something we need right now."
    "Pero así está bien. Parece que pensar no es algo que necesitemos en este momento."

# game/script-a4-rin.rpy:5040
translate es a4_rin_wb_and_ws_d29c8be8:

    # "By the grace of vastly smaller amount of fabric, slipping off Rin's panties is considerably easier than her trousers."
    "Por la gracia de una cantidad mucho menor de tela, quitarle las pantaletas a Rin es considerablemente más fácil que sus pantalones."

# game/script-a4-rin.rpy:5042
translate es a4_rin_wb_and_ws_9edd4e36:

    # "They disappear past my field of vision, sliding somewhere away down her legs."
    "Desaparecen más allá de mi campo visual, deslizándose hacia algún lugar bajo sus piernas."

# game/script-a4-rin.rpy:5044
translate es a4_rin_wb_and_ws_245af5e3:

    # "It seems I did a poor job with the towel, since Rin's legs are still wet from the rain."
    "Parece que hice un trabajo deficiente con la toalla, ya que las piernas de Rin siguen húmedas a causa del agua."

# game/script-a4-rin.rpy:5046
translate es a4_rin_wb_and_ws_d97e8d0c:

    # "Well, whatever."
    "Bueno, como sea."

# game/script-a4-rin.rpy:5051
translate es a4_rin_wb_and_ws_19b72ad5:

    # "Guided by instinct more than rationality, I move closer and taste the different kind of wetness."
    "Guiado por el instinto más que por el raciocinio, me acerco más y saboreo el distinto tipo de humedad."

# game/script-a4-rin.rpy:5053
translate es a4_rin_wb_and_ws_7a789a82:

    # "She responds to me, to the slow movements of my tongue on her skin, to my kisses on her flesh."
    "Ella responde a mí, a los lentos movimientos de mi lengua sobre su piel, a mis besos sobre su carne."

# game/script-a4-rin.rpy:5055
translate es a4_rin_wb_and_ws_03445b50:

    # "Her muscles tense and relax in the rhythm, as if what I am doing was uncomfortable."
    "Sus músculos se tensan y relajan al ritmo, como si lo que estuviese haciendo fuese incómodo."

# game/script-a4-rin.rpy:5057
translate es a4_rin_wb_and_ws_9e3c3af0:

    # "To hear Rin trying not to make a sound when I suck on her is… unreal."
    "Escuchar a Rin tratar de no hacer ruido cuando la chupo es… irreal."

# game/script-a4-rin.rpy:5059
translate es a4_rin_wb_and_ws_c203ecde:

    # "This whole morning has been so unreal, like the surreal intangibility of an awakening dream."
    "Toda esta mañana ha sido tan irreal, como la intangibilidad surrealista de un sueño en el despertar."

# game/script-a4-rin.rpy:5061
translate es a4_rin_wb_and_ws_5ca7824d:

    # "I can't believe I am doing this, to her, now. But I am going with the flow."
    "No puedo creer que esté haciendo esto, a ella, ahora. Pero me dejo llevar por la corriente."

# game/script-a4-rin.rpy:5063
translate es a4_rin_wb_and_ws_37217f85:

    # "Besides, the point of no return was a thousand miles ago."
    "Además, el punto de no regreso fue hace miles de kilómetros."

# game/script-a4-rin.rpy:5065
translate es a4_rin_wb_and_ws_6b81f8b3:

    # "I move around, try to do things to her, to find the places where her weakness lies, to tease her, to drive her mad with pleasure because I want to, I want to do this to her."
    "Me muevo alrededor, tratando de hacerle cosas, de encontrar los lugares donde yacen sus debilidades, provocarla, volverla loca de placer porque quiero, quiero hacerle esto a ella."

# game/script-a4-rin.rpy:5067
translate es a4_rin_wb_and_ws_8b9ba5d1:

    # "But she doesn't squeal, she doesn't squirm, for maybe I can't make Rin any madder than she already is, whatever I do."
    "Pero no chilla, no se retuerce, tal vez porque no puedo hacer enloquecer a Rin más de lo que ya está, sin importar qué haga."

# game/script-a4-rin.rpy:5069
translate es a4_rin_wb_and_ws_2ec49f72:

    # "Her ragged, heavy breathing mixed with unintelligible moans is that of a lunatic, but I do not cause it."
    "Su aliento pesado e irregular mezclado con gemidos ininteligibles son los de una lunática, pero no los causo yo."

# game/script-a4-rin.rpy:5071
translate es a4_rin_wb_and_ws_ce5dd7ea:

    # "I only release that from her."
    "Yo únicamente los hago salir de ella."

# game/script-a4-rin.rpy:5073
translate es a4_rin_wb_and_ws_ac55072a:

    # "She becomes more and more moist, and I drink from her, feeling a heat growing inside myself."
    "Ella se humedece más y más, y yo bebo de ella, sintiendo el calor creciendo dentro de mí."

# game/script-a4-rin.rpy:5075
translate es a4_rin_wb_and_ws_771e8784:

    # "I try to reach her deepest places, to feel all of her I can this way."
    "Trato de alcanzar sus lugares más profundos, de sentir todo lo que pueda de ella de esta manera."

# game/script-a4-rin.rpy:5077
translate es a4_rin_wb_and_ws_2fcdd301:

    # "My every action is met with a different reaction, but all of those are out of pure lust."
    "Cada acción mía se encuentra con una reacción diferente, pero todas ellas son a causa de lujuria pura."

# game/script-a4-rin.rpy:5082
translate es a4_rin_wb_and_ws_c1eb7555:

    # "Rin is lost in desire, willing to let anything happen to her if I do it right now."
    "Rin está perdida en el deseo, dispuesta a permitir que cualquier cosa le suceda si lo hago en este momento."

# game/script-a4-rin.rpy:5084
translate es a4_rin_wb_and_ws_655194e7:

    # "She becomes closer and closer to the moment of release, but the way to that is an uphill slope of madness."
    "Ella llega más y más cerca del momento de descarga, pero el camino a ello es una pendiente ascendente de demencia."

# game/script-a4-rin.rpy:5086
translate es a4_rin_wb_and_ws_2197b35b:

    # "Still, she is going that way."
    "Aun así, ella va hacia allá."

# game/script-a4-rin.rpy:5088
translate es a4_rin_wb_and_ws_f4c46b9c:

    # "The muscles don't relax any more between the waves of ecstatic spasms."
    "Los músculos no se relajan entre las olas de espasmos extasiados."

# game/script-a4-rin.rpy:5090
translate es a4_rin_wb_and_ws_4f0d3e48:

    # "Rin just becomes tenser and tenser, contracting so much that it must be physically painful, but I do not let go."
    "Rin solo se tensa cada vez más, contrayéndose tanto que debe ser físicamente doloroso, pero no me detengo."

# game/script-a4-rin.rpy:5092
translate es a4_rin_wb_and_ws_835d7633:

    # "I keep going, and I know she wants it too, she desperately wants me to do this to her."
    "Sigo haciéndolo, y sé que ella también lo quiere, ella quiere desesperadamente que le haga esto."

# game/script-a4-rin.rpy:5094
translate es a4_rin_wb_and_ws_d98c2bb1:

    # "A leg curls around my shoulders and draws me closer, so close that I think that I'm going to choke."
    "Una pierna me rodea el hombro y me lleva más cerca, tan cerca que pienso que voy a ahogarme."

# game/script-a4-rin.rpy:5096
translate es a4_rin_wb_and_ws_aa67e94a:

    # "I keep going because it's the only possibility."
    "Continúo porque es la única posibilidad."

# game/script-a4-rin.rpy:5101
translate es a4_rin_wb_and_ws_6c2ad772:

    # "As I push the button that drives her into gasping for breath, locking her leg into a cramp against my back, losing her mind in the sensation, at that precise moment I seem to forget all that was meant to be, all that should be."
    "Al momento en que presiono ese botón que la hace jadear tratando de respirar, tensando su pierna de un tirón sobre mi espalda, perdiendo la cabeza en la sensación, en ese preciso momento parezco olvidar todo lo que debía ser, todo lo que debería de ser."

# game/script-a4-rin.rpy:5103
translate es a4_rin_wb_and_ws_211c2fad:

    # "All I know is that she came here and… I think there was a towel at some point, too."
    "Todo lo que sé es que ella vino aquí y… creo que había una toalla en algún momento, también."

# game/script-a4-rin.rpy:5105
translate es a4_rin_wb_and_ws_5f33fb7a:

    # "None of it matters, all that matters is this, what we have now."
    "Nada de eso importa, todo lo que importa es esto, lo que tenemos ahora."

# game/script-a4-rin.rpy:5107
translate es a4_rin_wb_and_ws_b83a7f06:

    # "Her orgasm surges through me too, exciting me in a completely new way."
    "Su orgasmo se dispara también a través de mí, excitándome de un modo completamente nuevo."

# game/script-a4-rin.rpy:5109
translate es a4_rin_wb_and_ws_a054b289:

    # "It makes me feel anxious, nervous. Bothered."
    "Me hace sentir ansioso, nervioso. Molesto."

# game/script-a4-rin.rpy:5115
translate es a4_rin_wb_and_ws_db0d05ee:

    # "As her body relaxes, I try to kiss her down there again, but it startles her, causing her to jump."
    "A medida que relaja su cuerpo, trato de besarla ahí abajo otra vez, pero la asusta, haciéndola brincar."

# game/script-a4-rin.rpy:5120
translate es a4_rin_wb_and_ws_f563d778:

    # rin "No… Hisao… Enough."
    rin "No… Hisao… Suficiente."

# game/script-a4-rin.rpy:5122
translate es a4_rin_wb_and_ws_03987e9d:

    # rin "Come here."
    rin "Ven aquí."

# game/script-a4-rin.rpy:5127
translate es a4_rin_wb_and_ws_1e2eef89:

    # "I stand up to remove the last piece of clothing Rin has."
    "Me pongo de pie para remover la última prenda que lleva Rin."

# game/script-a4-rin.rpy:5129
translate es a4_rin_wb_and_ws_1539ab33:

    # "She leans against me to catch her breath, tickling me with warm air exhaled into my shirt."
    "Ella se recarga sobre mí para recuperar su aliento, haciéndome cosquillas con el aire caliente exhalado dentro de mi camisa."

# game/script-a4-rin.rpy:5131
translate es a4_rin_wb_and_ws_428c5dfb:

    # "Blindly, I reach behind her back to feel my way below her shoulder blades, to find the contraption that fastens her bra."
    "Ciegamente estiro el brazo atrás de su espalda para tratar de buscar a tientas debajo de sus omóplatos, para encontrar el mecanismo que abrocha el sostén."

# game/script-a4-rin.rpy:5133
translate es a4_rin_wb_and_ws_2d295887:

    # "It opens more easily than I thought, falling to the floor somewhere."
    "Se abre más fácil de lo que pensé, cayendo en alguna parte del suelo."

# game/script-a4-rin.rpy:5142
translate es a4_rin_wb_and_ws_4232dc2a:

    # "Her bare skin against me is a sensation so wonderful that I want to have more of it, and I do, embracing her."
    "Su piel desnuda sobre mí es una sensación tan maravillosa que quiero tener más de ella, y lo hago, abrazándola."

# game/script-a4-rin.rpy:5144
translate es a4_rin_wb_and_ws_dcc1063d:

    # "Rin's hair smells of rain, and I realize that I'm not hearing the sound of rainfall any more."
    "El cabello de Rin huele a lluvia, y me doy cuenta de que ya no oigo el sonido del agua cayendo."

# game/script-a4-rin.rpy:5146
translate es a4_rin_wb_and_ws_fc569c5d:

    # "It's a sobering thing. The cushion that enveloped us into a reality of our own is now gone, and I realize more clearly what is happening."
    "Es algo que me hace volver en mí. El cojín que nos envolvía en nuestra propia realidad se ha ido, y me percato más claramente de qué es lo que está sucediendo."

# game/script-a4-rin.rpy:5151
translate es a4_rin_wb_and_ws_4bd05e8c:

    # hi "You know, this really is not what friends should be doing."
    hi "Sabes, esto realmente no es algo que los amigos deberían de estar haciendo."

# game/script-a4-rin.rpy:5153
translate es a4_rin_wb_and_ws_d6494819:

    # "I whisper, once again noticing how such a simple matter as talking can be overbearingly difficult at times."
    "Le susurro, una vez más notando cómo algunas veces una cosa simple como hablar puede ser tan difícil."

# game/script-a4-rin.rpy:5158
translate es a4_rin_wb_and_ws_d816abff:

    # rin "Will you stop being my friend?"
    rin "¿Dejarás de ser mi amigo?"

# game/script-a4-rin.rpy:5160
translate es a4_rin_wb_and_ws_deb37c77:

    # "That wasn't what I meant, but her serious tone and the layers of connotations behind Rin's question give me pause."
    "Eso no fue lo que quería decir, pero su tono serio y las capas de connotaciones detrás de la pregunta de Rin me hacen dar una pausa."

# game/script-a4-rin.rpy:5165
translate es a4_rin_wb_and_ws_974c322b:

    # hi "Nah."
    hi "Nah."

# game/script-a4-rin.rpy:5170
translate es a4_rin_wb_and_ws_2559cfc1:

    # rin "I… think it might be all right. Even if you did."
    rin "Yo… creo que podría estar bien. Aun si lo hicieras."

# game/script-a4-rin.rpy:5172
translate es a4_rin_wb_and_ws_008c6c91:

    # "I hug her and smile into her hair, understanding Rin perfectly for once."
    "La abrazo y le sonrío a su cabello, entendiendo perfectamente a Rin por primera vez."

# game/script-a4-rin.rpy:5177
translate es a4_rin_wb_and_ws_666c25bb:

    # rin "You are wet."
    rin "Estás mojado."

# game/script-a4-rin.rpy:5179
translate es a4_rin_wb_and_ws_0571641c:

    # "The remnants of water on her skin have drained into my shirt."
    "Los remanentes de agua sobre su piel se han absorbido en mi camisa."

# game/script-a4-rin.rpy:5181
translate es a4_rin_wb_and_ws_6683f306:

    # "Somehow, even her statements of the obvious make me glad right now."
    "De algún modo, aun sus observaciones de lo obvio me hacen sonreír en este momento."

# game/script-a4-rin.rpy:5186
translate es a4_rin_wb_and_ws_7027cd75:

    # hi "You're right. I am. But that's your fault."
    hi "Tienes razón. Lo estoy. Pero eso es tu culpa."

# game/script-a4-rin.rpy:5191
translate es a4_rin_wb_and_ws_350e4c5d:

    # rin "I want to see you."
    rin "Quiero verte."

# game/script-a4-rin.rpy:5198
translate es a4_rin_wb_and_ws_21881e75:

    # "I comply, standing back to open the buttons of my shirt, much more quickly than when I undid Rin's buttons."
    "Obedezco, dando un paso atrás para desabotonar mi camisa, mucho más rápido que cuando lo hice con los botones de Rin."

# game/script-a4-rin.rpy:5200
translate es a4_rin_wb_and_ws_2d91b7ea:

    # "A sudden sense of haste strikes me, spurring me to rush forward."
    "Una repentina sensación de prisa me arremete, incentivándome a que me apure."

# game/script-a4-rin.rpy:5202
translate es a4_rin_wb_and_ws_435c8b50:

    # "Every second I'm not touching Rin is a second wasted, a chance lost."
    "Cada segundo que no estoy tocando a Rin es un desperdicio, una oportunidad perdida."

# game/script-a4-rin.rpy:5204
translate es a4_rin_wb_and_ws_733ce239:

    # "My belt buckle proves an obstacle despite my ability to open it in an eyeblink under normal circumstances."
    "La hebilla de mi cinto prueba ser un obstáculo a pesar de mi habilidad de abrirlo en un abrir y cerrar de ojos en circunstancias normales."

# game/script-a4-rin.rpy:5209
translate es a4_rin_wb_and_ws_c415f634:

    # "While I fumble with it, I don't notice Rin bringing her foot up between us until she starts tracing my chest with her toe."
    "Mientras titubeo con este, no me doy cuenta de que Rin lleva su pie entre nosotros hasta que comienza a trazar mi pecho con su dedo."

# game/script-a4-rin.rpy:5214
translate es a4_rin_wb_and_ws_ee0d6ee6:

    # "I look down to see what she's looking at…"
    "Miro abajo para ver qué es a lo que está viendo…"

# game/script-a4-rin.rpy:5216
translate es a4_rin_wb_and_ws_f8375736:

    # hi "My heart…"
    hi "Mi corazón…"

# game/script-a4-rin.rpy:5218
translate es a4_rin_wb_and_ws_e3509c53:

    # "I reflexively flinch back, covering the scar tissue in the middle of my chest."
    "Retrocedo por reflejo, cubriendo el tejido cicatricial en medio de mi pecho."

# game/script-a4-rin.rpy:5220
translate es a4_rin_wb_and_ws_875587e7:

    # "The shallow mark that the surgery following my heart attack left on my body has healed already but… well, it's not a particularly pretty sight if not overly repulsive either."
    "Las marcas superficiales que la cirugía que le siguió a mi ataque cardiaco dejó sobre mi cuerpo ya han sanado pero… bueno, no es un espectáculo especialmente bonito aunque no es que sea excesivamente repulsivo."

# game/script-a4-rin.rpy:5222
translate es a4_rin_wb_and_ws_af71731a:

    # "It's barely noticeable, but she does have an eye for detail. Is this why she said she wanted to see me?"
    "Es apenas notable, pero ella definitivamente tiene un ojo para el detalle. ¿Es por esto que dijo que quería verme?"

# game/script-a4-rin.rpy:5224
translate es a4_rin_wb_and_ws_718f6e3e:

    # "I had sorta forgotten about this because of all this mess with Rin, but now all the unpleasant things connected to my condition surface at once, rushing through my mind like a flash flood."
    "Me había medio olvidado de esto a causa de todo este desastre con Rin, pero ahora todo lo desagradable conectado a mi condición surge a la vez, corriendo por mi mente como una inundación repentina."

# game/script-a4-rin.rpy:5226
translate es a4_rin_wb_and_ws_fba91042:

    # "And oh God all the stories about old guys getting heart attacks when having sex, what if…"
    "Y por Dios todas las historias sobre tipos viejos teniendo ataques al corazón cuando tienen sexo, que tal si…"

# game/script-a4-rin.rpy:5231
translate es a4_rin_wb_and_ws_aa95e2ab:

    # rin "Hisao."
    rin "Hisao."

# game/script-a4-rin.rpy:5233
translate es a4_rin_wb_and_ws_ce617998_4:

    # "…"
    "…"

# game/script-a4-rin.rpy:5235
translate es a4_rin_wb_and_ws_8c1f9dd0:

    # "Realizing that I might just have spoiled the mood, I stumble to explain myself."
    "Percatándome de que podría haber arruinado el ambiente, tropiezo para explicarme."

# game/script-a4-rin.rpy:5240
translate es a4_rin_wb_and_ws_cd4cb466:

    # hi "Ah… sorry, it's just that…"
    hi "Ah… disculpa, es solo que…"

# game/script-a4-rin.rpy:5245
translate es a4_rin_wb_and_ws_2fda4de3:

    # rin "Let me touch you."
    rin "Déjame tocarte."

# game/script-a4-rin.rpy:5247
translate es a4_rin_wb_and_ws_3c0633da:

    # "Her eyes are sultry, inviting as she sits there bare naked without an inkling of shame. I never thought Rin could look like that."
    "Sus ojos son sensuales, seduciéndome al sentarse ahí desnuda sin un rastro de vergüenza. Nunca pensé que Rin podría verse así."

# game/script-a4-rin.rpy:5299
translate es a4_rin_wb_and_ws_d41d8cd9_2:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:5255
translate es a4_rin_wb_and_ws_37e3fa9f:

    # n "{vspace=60}Yeah, I know this is not how it should go."
    n "{vspace=60}Sí, sé que esto no es como debería de ser."

# game/script-a4-rin.rpy:5257
translate es a4_rin_wb_and_ws_c340a88e:

    # n "Even though Rin is right here, even though there should be no more questions, no obstacles, not this maddening feeling that something is constantly wrong…"
    n "Aun si Rin está justo aquí, aun si no debería de haber más preguntas, ni obstáculos, ni esta exasperante sensación de que algo está constantemente mal…"

# game/script-a4-rin.rpy:5259
translate es a4_rin_wb_and_ws_9d5c454f:

    # n "The same feeling that clutched my heart yesterday makes its appearance."
    n "El mismo sentimiento que aprisionó a mi corazón ayer hace su aparición."

# game/script-a4-rin.rpy:5261
translate es a4_rin_wb_and_ws_5df195c9:

    # n "We are together. In a way that is difficult to define, it eludes description as stubbornly as it evades change."
    n "Estamos juntos. De una manera que es difícil de definir, elude descripción tan obstinadamente como evade el cambio."

# game/script-a4-rin.rpy:5263
translate es a4_rin_wb_and_ws_aae9885e:

    # n "{vspace=30}Would a relationship like this be all right? Could we ever change to become closer?"
    n "{vspace=30}¿Podría estar bien una relación como esta? ¿Podríamos algún día cambiar para acercarnos más?"

# game/script-a4-rin.rpy:5265
translate es a4_rin_wb_and_ws_c736497f:

    # n "Even though we would stay together for all of eternity, we might never find our mutual understanding."
    n "Aunque permaneciéramos juntos durante toda la eternidad, podríamos nunca encontrar nuestro entendimiento mutuo."

# game/script-a4-rin.rpy:5267
translate es a4_rin_wb_and_ws_1bfb78bc:

    # n "But there is no such thing as eternity. This may mean that we will not be together forever."
    n "Pero no hay tal cosa como la eternidad. Esto puede significar que no estaremos juntos por siempre."

# game/script-a4-rin.rpy:5269
translate es a4_rin_wb_and_ws_42f296c7:

    # n "If not our differences, then the flow of time will pull us apart with irresistible force."
    n "Si no son nuestras diferencias, entonces el pasar del tiempo nos apartará con fuerza irresistible."

# game/script-a4-rin.rpy:5271
translate es a4_rin_wb_and_ws_4a6f1880:

    # nvl clear
    # n "{vspace=60}Rin is a creature of the moment, of whim and of impulse."
    nvl clear
    n "{vspace=60}Rin es una criatura del momento, de caprichos e impulsos."

# game/script-a4-rin.rpy:5275
translate es a4_rin_wb_and_ws_a34c7f98:

    # n "{vspace=30}I am nothing of the sort."
    n "{vspace=30}Yo no soy así para nada."

# game/script-a4-rin.rpy:5277
translate es a4_rin_wb_and_ws_f86d998b:

    # n "{vspace=30}This is a fact that I can understand very clearly."
    n "{vspace=30}Este es un hecho que puedo entender muy claramente."

# game/script-a4-rin.rpy:5279
translate es a4_rin_wb_and_ws_c60b0d90:

    # n "If for no other reason, for this reason I should grasp this moment. Even if it's the only moment we will ever have, I should not let myself spoil it."
    n "Si no es por otra razón, por esta misma debería de aferrarme a este momento. Aun si es el único momento que tendremos, no debería permitirme arruinarlo."

# game/script-a4-rin.rpy:5281
translate es a4_rin_wb_and_ws_d14b3d69:

    # n "Even if I can't escape myself. Rin can't either, I know it now."
    n "Aunque no pueda escapar de mí mismo. Rin tampoco puede, ahora lo sé."

# game/script-a4-rin.rpy:5283
translate es a4_rin_wb_and_ws_7acd07de:

    # n "{vspace=30}We both have things we can't let go, things we can't not think."
    n "{vspace=30}Ambos tenemos cosas que no podemos dejar ir, cosas en las que no podemos no pensar."

# game/script-a4-rin.rpy:5285
translate es a4_rin_wb_and_ws_7a5647e5:

    # n "Feelings we can't not feel."
    n "Sentimientos que no podemos no sentir."

# game/script-a4-rin.rpy:5287
translate es a4_rin_wb_and_ws_bfe9e23d:

    # n "But she allows herself to want me without any restraint. Here and now."
    n "Pero ella se permite desearme sin ningún impedimento. Aquí y ahora."

# game/script-a4-rin.rpy:5339
translate es a4_rin_wb_and_ws_d41d8cd9_3:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:5298
translate es a4_rin_wb_and_ws_958f974a:

    # hi "I'm sorry, you know…"
    hi "Lo siento, sabes…"

# game/script-a4-rin.rpy:5303
translate es a4_rin_wb_and_ws_80257f6b:

    # rin "Hisao, you really have to stop worrying."
    rin "Hisao, realmente tienes que dejar de preocuparte."

# game/script-a4-rin.rpy:5305
translate es a4_rin_wb_and_ws_66c913b3:

    # "Rin interrupts me before I get further, which is good because I don't know what I could have said."
    "Rin me interrumpe antes de que pueda decir más, lo que es bueno porque no sé qué podría haber dicho."

# game/script-a4-rin.rpy:5307
translate es a4_rin_wb_and_ws_b5e01e11:

    # "Her voice, void of its usual spaciness, scolds me softly, without an edge."
    "Su voz, carente de su usual lejanía, me regaña con suavidad, sin filo."

# game/script-a4-rin.rpy:5312
translate es a4_rin_wb_and_ws_0d8874a2:

    # rin "You really have to learn to let go."
    rin "Realmente tienes que aprender a dejar las cosas ir."

# game/script-a4-rin.rpy:5314
translate es a4_rin_wb_and_ws_2f3ea9c1:

    # "She scans me calmly, almost calculatingly."
    "Ella me analiza con tranquilidad, casi de manera calculadora."

# game/script-a4-rin.rpy:5316
translate es a4_rin_wb_and_ws_72a0073f:

    # "I wonder what I look like through her eyes."
    "Me pregunto cómo me veo a través de sus ojos."

# game/script-a4-rin.rpy:5318
translate es a4_rin_wb_and_ws_03b55d8f:

    # "Damn. They are so green it almost hurts."
    "Maldición. Son tan verdes que casi lastiman."

# game/script-a4-rin.rpy:5320
translate es a4_rin_wb_and_ws_a02c9829:

    # "I always was so enchanted by her eyes, those mysterious, captivating eyes that always were too restless for their own good."
    "Siempre me han encantado tanto sus ojos, esos misteriosos y cautivadores ojos que siempre se encontraban demasiado inquietos para su propio bien."

# game/script-a4-rin.rpy:5322
translate es a4_rin_wb_and_ws_6bce045d:

    # "But I was also always intimidated by them."
    "Pero también siempre me sentí intimidado por ellos."

# game/script-a4-rin.rpy:5324
translate es a4_rin_wb_and_ws_11809855:

    # "Yeah. Rin is intimidating, on more than one level and especially right now."
    "Sí. Rin es intimidante, en más de una manera y especialmente ahora."

# game/script-a4-rin.rpy:5326
translate es a4_rin_wb_and_ws_3dd90fee:

    # "She is frighteningly lucid, the goosebumps on her skin giving away that she is cold, or scared too."
    "Ella se encuentra terriblemente lúcida, la piel de gallina delata que tiene frío, o está asustada también."

# game/script-a4-rin.rpy:5328
translate es a4_rin_wb_and_ws_662e69fb:

    # "Either way, I steel myself and step back to Rin, embracing her to feel her in my arms again and to banish my doubts."
    "Sea como sea, me armo de valor y doy un paso de vuelta hacia Rin, abrazándola para sentirla en mis brazos y para disipar mis dudas."

# game/script-a4-rin.rpy:5330
translate es a4_rin_wb_and_ws_2da63880:

    # "The sight of her gentle, loving eyes seems to melt those doubts away like the last snow of winter."
    "Mirar sus gentiles y amorosos ojos parece derretir esas dudas como la última nevada de invierno."

# game/script-a4-rin.rpy:5335
translate es a4_rin_wb_and_ws_da91b700:

    # "She presses her head against my shoulder, seeking a place to rest herself in, leaning against me like I lean against her."
    "Ella presiona su cabeza contra mi hombro, buscando un lugar donde descansar, recargándose sobre mí como yo me recargo en ella."

# game/script-a4-rin.rpy:5337
translate es a4_rin_wb_and_ws_729935a4:

    # rin "Let go."
    rin "Déjalas ir."

# game/script-a4-rin.rpy:5339
translate es a4_rin_wb_and_ws_ce147a6b:

    # "Yes."
    "Sí."

# game/script-a4-rin.rpy:5344
translate es a4_rin_wb_and_ws_00f01785:

    # rin "You should forget about stuff like future and past, it's not like you can change those kinds of things."
    rin "Deberías de olvidarte de cosas como el futuro y el pasado, no es como si pudieras cambiar ese tipo de cosas."

# game/script-a4-rin.rpy:5346
translate es a4_rin_wb_and_ws_7fef17d3:

    # "I wanted to say something to her, but I have lost my voice so I just mumble something unintelligible at her."
    "Quería decirle algo, pero he perdido mi voz así que tan solo le murmuro algo ininteligible."

# game/script-a4-rin.rpy:5348
translate es a4_rin_wb_and_ws_37f57e6c:

    # rin "You should just be with me now."
    rin "Deberías tan solo estar conmigo ahora."

# game/script-a4-rin.rpy:5350
translate es a4_rin_wb_and_ws_d0ac6a2d:

    # "Maybe she understood what I wanted to say even if I didn't."
    "Quizás entendió lo que quería decir aunque no lo dijera."

# game/script-a4-rin.rpy:5352
translate es a4_rin_wb_and_ws_03987e9d_1:

    # rin "Come here."
    rin "Ven aquí."

# game/script-a4-rin.rpy:5354
translate es a4_rin_wb_and_ws_26125840:

    # hi "I am here."
    hi "Estoy aquí."

# game/script-a4-rin.rpy:5359
translate es a4_rin_wb_and_ws_5f1303fe:

    # rin "Come closer."
    rin "Acércate más."

# game/script-a4-rin.rpy:5361
translate es a4_rin_wb_and_ws_1ff0ae96:

    # "My entire body is thinking only in positives now so I do, hugging her more tightly."
    "Mi cuerpo entero solo piensa en cosas positivas ahora así que lo hago, abrazándola más fuertemente."

# game/script-a4-rin.rpy:5366
translate es a4_rin_wb_and_ws_c6716819:

    # rin "Closer."
    rin "Más cerca."

# game/script-a4-rin.rpy:5368
translate es a4_rin_wb_and_ws_b4d2a415:

    # "I press my lower body against hers."
    "Presiono la parte inferior de mi cuerpo sobre ella."

# game/script-a4-rin.rpy:5370
translate es a4_rin_wb_and_ws_8cf7b162:

    # "She tenses a little. Just a little."
    "Ella se tensa un poco. Solo un poco."

# game/script-a4-rin.rpy:5375
translate es a4_rin_wb_and_ws_c6716819_1:

    # rin "Closer."
    rin "Más cerca."

# game/script-a4-rin.rpy:5377
translate es a4_rin_wb_and_ws_c298c05f:

    # "Her final plead is more like a prayer."
    "Su última súplica es más como una plegaria."

# game/script-a4-rin.rpy:5379
translate es a4_rin_wb_and_ws_e7c44892:

    # "There is only one way to be any closer than this."
    "Solo hay una manera de estar más cerca que esto."

# game/script-a4-rin.rpy:5381
translate es a4_rin_wb_and_ws_3e55b3e3:

    # "I reach down between us and guide myself, sinking myself into her."
    "Con mi mano alcanzo entre nosotros y me guío a mí mismo, hundiéndome dentro de ella."

# game/script-a4-rin.rpy:5386
translate es a4_rin_wb_and_ws_4f772e29:

    # "Every muscle in Rin's body stiffens at the same time."
    "Cada músculo en el cuerpo de Rin se tensa al mismo tiempo."

# game/script-a4-rin.rpy:5391
translate es a4_rin_wb_and_ws_504839b3:

    # "She doesn't say anything, or wince, so I push deeper, eventually moving out."
    "Ella no dice nada, ni hace un gesto de dolor, así que empujo más profundamente, finalmente moviéndome hacia afuera."

# game/script-a4-rin.rpy:5393
translate es a4_rin_wb_and_ws_49748624:

    # "And again. And she moves with me."
    "Y otra vez. Y ella se mueve conmigo."

# game/script-a4-rin.rpy:5395
translate es a4_rin_wb_and_ws_2b0cca68:

    # "Our movements melt together into one continuous string of back and forth, in and out."
    "Nuestros movimientos se funden en una hilera continua hacia enfrente y atrás, adentro y afuera."

# game/script-a4-rin.rpy:5397
translate es a4_rin_wb_and_ws_4dc35e48:

    # "All sensations become sharper, amplified tenfold."
    "Todas las sensaciones se hacen más agudas, amplificadas diez veces."

# game/script-a4-rin.rpy:5399
translate es a4_rin_wb_and_ws_601c4c52:

    # "My brain gave up interpreting all this stimulation ages ago, and now I am left with no choice but to feel all of this with my entire body."
    "Mi cerebro desistió de interpretar toda esta estimulación hace una eternidad, y ahora me quedo sin más opción que sentir todo esto con mi cuerpo entero."

# game/script-a4-rin.rpy:5401
translate es a4_rin_wb_and_ws_c5f9a3ed:

    # "It's like that for Rin too, I know it. I can see it. I can feel it."
    "Es también lo mismo para Rin, lo sé. Puedo verlo. Puedo sentirlo."

# game/script-a4-rin.rpy:5403
translate es a4_rin_wb_and_ws_63cfdfe6:

    # "She breathes sharply in and out, losing all composure and grace, breathes warmly against my shoulder."
    "Ella respira con intensidad de adentro hacia afuera, perdiendo toda compostura y gracia, respira cálidamente sobre mi hombro."

# game/script-a4-rin.rpy:5405
translate es a4_rin_wb_and_ws_11411736:

    # "Between those fragile breaths, she sometimes kisses me tenderly, gently, as if she was unsure how to do it properly."
    "En medio de aquellas frágiles aspiraciones, ella algunas veces me besa con ternura, como si no estuviese segura de cómo hacerlo apropiadamente."

# game/script-a4-rin.rpy:5407
translate es a4_rin_wb_and_ws_1a5de89a:

    # "But there is no hesitation."
    "Pero no hay vacilación."

# game/script-a4-rin.rpy:5409
translate es a4_rin_wb_and_ws_aa486ee6:

    # "Desperately clinging to me, drawing me closer so that I can fill all of her, she moves against me, around me so that it's hard to say where I stop and she begins."
    "Desesperadamente aferrándose a mí, llevándome más cerca para que yo pueda llenarla por completo, se mueve sobre y alrededor de mí, por lo que es difícil decir dónde me detengo yo y ella empieza."

# game/script-a4-rin.rpy:5411
translate es a4_rin_wb_and_ws_94393f9d:

    # "We take it slowly, excruciatingly slowly, as if we had all the time in the world even though we have only this moment and nothing beyond that."
    "Lo llevamos con calma, terriblemente despacio, como si tuviésemos todo el tiempo del mundo aunque tengamos solo este momento y nada más allá de este."

# game/script-a4-rin.rpy:5413
translate es a4_rin_wb_and_ws_dc3ca1ab:

    # "That feeling is—{w=0.7}{nw}"
    "Esa sensación es—{w=0.7}{nw}"

# game/script-a4-rin.rpy:5418
translate es a4_rin_wb_and_ws_886e7c0a:

    # rin "Wait…"
    rin "Espera…"

# game/script-a4-rin.rpy:5420
translate es a4_rin_wb_and_ws_3ac2e6dd:

    # "I stop moving, slightly alarmed."
    "Dejo de moverme, ligeramente alarmado."

# game/script-a4-rin.rpy:5422
translate es a4_rin_wb_and_ws_4cc669aa:

    # "Maybe it hurts, or…"
    "Tal vez duela, o…"

# game/script-a4-rin.rpy:5427
translate es a4_rin_wb_and_ws_615746d3:

    # "She looks at me in a way that I can't really begin to interpret."
    "Me mira de un modo en el que realmente no puedo ni empezar a interpretar."

# game/script-a4-rin.rpy:5429
translate es a4_rin_wb_and_ws_24603c9e:

    # rin "Is this it?"
    rin "¿Es esto?"

# game/script-a4-rin.rpy:5431
translate es a4_rin_wb_and_ws_5d79aa7b:

    # hi "…Huh?"
    hi "… ¿Eh?"

# game/script-a4-rin.rpy:5433
translate es a4_rin_wb_and_ws_30d988dc:

    # rin "You said I don't have to be alone."
    rin "Dijiste que no tenía que estar sola."

# game/script-a4-rin.rpy:5438
translate es a4_rin_wb_and_ws_50cbf646:

    # "Her eyes are full of an innocent, fuzzyheaded confusion that makes me chuckle a little and pet the back of her head."
    "Sus ojos están llenos de una inocente y aturdida confusión que me hace reír un poco y acariciarle la nuca."

# game/script-a4-rin.rpy:5440
translate es a4_rin_wb_and_ws_3f8ce92c:

    # hi "Yeah. This is what I meant."
    hi "Sí. Esto es a lo que me refería."

# game/script-a4-rin.rpy:5442
translate es a4_rin_wb_and_ws_075a6638:

    # hi "That you have someone you can come to when you get soaked in a rain."
    hi "Que tengas a alguien al que puedas acudir cuando te empapes bajo la lluvia."

# game/script-a4-rin.rpy:5444
translate es a4_rin_wb_and_ws_f5c30628:

    # hi "It means you are not alone."
    hi "Significa que no estás sola."

# game/script-a4-rin.rpy:5446
translate es a4_rin_wb_and_ws_ac625991:

    # hi "If there is such a person for you."
    hi "Si es que hay una persona así para ti."

# game/script-a4-rin.rpy:5451
translate es a4_rin_wb_and_ws_99614c14:

    # "She answers with a kiss, reminding me that we have stopped moving for no real reason."
    "Ella responde con un beso, recordándome que hemos dejado de movernos sin razón alguna."

# game/script-a4-rin.rpy:5453
translate es a4_rin_wb_and_ws_cd557e60:

    # "So we start from the top, almost at the same time, each mirroring the rhythm of the other."
    "Así que comenzamos nuevamente, casi al mismo tiempo, cada quien reflejando el ritmo del otro."

# game/script-a4-rin.rpy:5455
translate es a4_rin_wb_and_ws_e896411a:

    # "I move faster, faster in and out of her, my sweat mixing with hers, glistening on our shared skin like diamonds and pearls."
    "Me muevo más rápido, más rápido hacia afuera y hacia adentro, mi sudor se mezcla con el suyo, reluciendo en nuestra piel compartida como diamantes y perlas."

# game/script-a4-rin.rpy:5463
translate es a4_rin_wb_and_ws_972ccab8:

    # "She moves faster, grinding herself against me in the throes of our desire."
    "Ella se mueve más rápido, restregándose sobre mí en las convulsiones de nuestro deseo."

# game/script-a4-rin.rpy:5465
translate es a4_rin_wb_and_ws_5493bdc5:

    # "The intoxicating scent of her lust, the mind-blanking feeling that connects our bodies, the sense of all rational thought draining from my mind."
    "El olor intoxicante de su lujuria, el sentimiento que conecta nuestros cuerpos que hace a la mente desvanecerse, la sensación de todo pensamiento racional siendo drenado de mi mente."

# game/script-a4-rin.rpy:5467
translate es a4_rin_wb_and_ws_7e959875:

    # "All those burn my consciousness just like the compelling feeling in my body burns my instincts."
    "Todos ellos queman mi consciencia justo como la irresistible sensación en mi cuerpo quema mis instintos."

# game/script-a4-rin.rpy:5469
translate es a4_rin_wb_and_ws_cd8ed96a:

    # "As those feelings grow, Rin makes no signs of stopping."
    "A medida que esos sentimientos crecen, Rin no da señales de detenerse."

# game/script-a4-rin.rpy:5471
translate es a4_rin_wb_and_ws_5beb3699:

    # "She curls her feet behind my lower back, forcing me to drive myself inside her as deep as physically possible, each millimeter sending waves through my spine."
    "Ella curva sus pies detrás de mi espalda baja, forzándome a conducirme más adentro de ella tan profundo como sea físicamente posible, cada milímetro enviando olas a través de mi espina."

# game/script-a4-rin.rpy:5473
translate es a4_rin_wb_and_ws_d6045af8:

    # "My mind blacks out as the world erupts into a flash of bright white blindness."
    "Mi mente se pone en blanco cuando el mundo hace erupción en un destello blanco de brillo enceguecedor."

# game/script-a4-rin.rpy:5492
translate es a4_rin_proof_of_existence_c55baf0d:

    # centered_b "Present{fast}" with Dissolve(4.0)
    centered_b "Presente{fast}" with Dissolve(4.0)

# game/script-a4-rin.rpy:5494
translate es a4_rin_proof_of_existence_dae2c000:

    # nvl clear
    # nb "'Present' is a fleeting and vague concept at best."
    nvl clear
    nb "El “presente” es un concepto breve e impreciso en el mejor de los casos."

# game/script-a4-rin.rpy:5498
translate es a4_rin_proof_of_existence_00f851bb:

    # nb "The moment between the past and the future?"
    nb "¿El momento entre el pasado y el futuro?"

# game/script-a4-rin.rpy:5500
translate es a4_rin_proof_of_existence_8c328169:

    # nb "That doesn't really mean anything."
    nb "Eso en verdad no significa nada."

# game/script-a4-rin.rpy:5502
translate es a4_rin_proof_of_existence_261964ab:

    # nb "Thinking too much about things that don't make sense is a waste of time."
    nb "Pensar demasiado en algo que no tiene sentido es una pérdida de tiempo."

# game/script-a4-rin.rpy:5504
translate es a4_rin_proof_of_existence_7f3c8d99:

    # nb "That's why living through the present is always the best option."
    nb "Es por eso que vivir a través del presente es la mejor opción."

# game/script-a4-rin.rpy:5506
translate es a4_rin_proof_of_existence_9eca2067:

    # nb "Besides, for us who can't foresee the future and who forget the past too easily, present is really the only proof of our existence."
    nb "Además, para nosotros que no podemos prever el futuro y quienes olvidamos el pasado con demasiada facilidad, el presente es en verdad la única prueba de nuestra existencia."

# game/script-a4-rin.rpy:5508
translate es a4_rin_proof_of_existence_a1f16ab2:

    # nb "Even though existence will go on even if you forget about it for a while, it's good to seize the day at least every once in a while."
    nb "Aunque la existencia seguirá adelante aun si te olvidas de ella por un tiempo, es bueno aprovechar el día por lo menos una vez cada tanto."

# game/script-a4-rin.rpy:5510
# translate es a4_rin_proof_of_existence_0799c3cd:

#     # nvl clear
#     # centered_alive "That way… you can confirm that you are, in fact…"
#     nvl clear
#     centered_alive "De ese modo… puedes confirmar que estás, de hecho…"

# game/script-a4-rin.rpy:5555
translate es a4_rin_proof_of_existence_115223a8:

    # nvl clear
    # centered_b "That way… you can confirm that you are, in fact… {color=#fff}alive.{/color}"
    nvl clear
    centered_b "De ese modo… puedes confirmar que estás, de hecho… {color=#fff}vivo.{/color}"

# game/script-a4-rin.rpy:5532
translate es a4_rin_proof_of_existence_7460cbe0:

    # "I am pretty sure that the girl who is standing there half-naked, staring out of the window of my room, has a much better grasp of 'present' than I do."
    "Estoy bastante seguro de que la chica que está de pie ahí medio desnuda, mirando por fuera de la ventana de mi habitación, tiene un mejor entendimiento del “presente” que yo."

# game/script-a4-rin.rpy:5534
translate es a4_rin_proof_of_existence_0964a88a:

    # "As for me… well, right now I'm somewhat confused by my present state, since I should try to locate my shirt and not stare at Rin's butt."
    "En cuanto a mí… bueno, en este momento estoy un poco confundido a causa de mi estado actual, ya que debería de tratar de encontrar mi camisa y no mirarle el trasero a Rin."

# game/script-a4-rin.rpy:5536
translate es a4_rin_proof_of_existence_bc77f0a9:

    # "But I just can't stop looking at her."
    "Pero simplemente no puedo dejar de verla."

# game/script-a4-rin.rpy:5543
translate es a4_rin_proof_of_existence_67970a7c:

    # "She is so close to the glass that her nose is probably going to leave a mark."
    "Ella se encuentra tan cerca del cristal que su nariz probablemente va a dejar una marca."

# game/script-a4-rin.rpy:5545
translate es a4_rin_proof_of_existence_66ab547f:

    # "At least her breathing does, when it condenses on the rain-cooled window glass before quickly disappearing again."
    "Al menos su aliento lo hace, cuando se condensa en el cristal de la ventana enfriado por la lluvia antes de rápidamente desaparecer de nuevo."

# game/script-a4-rin.rpy:5547
translate es a4_rin_proof_of_existence_b9343c60:

    # "My shuffling around to get dressed doesn't rouse Rin from her contemplation, which is fine, really. I don't mind the silences as much as I used to."
    "El que me mueva por la habitación para vestirme no saca a Rin de su contemplación, lo cual está bien, en serio. No me molesta el silencio tanto como solía hacerlo."

# game/script-a4-rin.rpy:5549
translate es a4_rin_proof_of_existence_11af94eb:

    # "Only after I'm almost finished with buttoning up my shirt does Rin say something, still without turning to look at me."
    "Tan solo después de que casi termino de abotonarme mi camisa Rin dice algo, aun sin darse vuelta para verme."

# game/script-a4-rin.rpy:5554
translate es a4_rin_proof_of_existence_8f8ebc5c:

    # rin "Let's go somewhere."
    rin "Vamos a alguna parte."

# game/script-a4-rin.rpy:5556
translate es a4_rin_proof_of_existence_804bb8e8:

    # hi "Where?"
    hi "¿Adónde?"

# game/script-a4-rin.rpy:5558
translate es a4_rin_proof_of_existence_599351a4:

    # "I can only assume she is inviting me and not the windowsill, but it's a fair guess."
    "Únicamente puedo asumir que me está invitando a mí y no al alféizar, pero es una suposición justa."

# game/script-a4-rin.rpy:5563
translate es a4_rin_proof_of_existence_b1f1c9df:

    # rin "I know."
    rin "Yo sé."

# game/script-a4-rin.rpy:5565
translate es a4_rin_proof_of_existence_81d99227:

    # hi "What?"
    hi "¿Qué?"

# game/script-a4-rin.rpy:5570
translate es a4_rin_proof_of_existence_2e4fe64a:

    # rin "Help me get dressed."
    rin "Ayúdame a vestirme."

# game/script-a4-rin.rpy:5575
translate es a4_rin_proof_of_existence_956b0ce8:

    # rin "I think today is the day."
    rin "Creo que hoy es el día."

# game/script-a4-rin.rpy:5580
translate es a4_rin_proof_of_existence_d2aa1f25:

    # rin "Come on, clothes."
    rin "Vamos, ropa."

# game/script-a4-rin.rpy:5582
translate es a4_rin_proof_of_existence_eb4226ad:

    # "Clothes, clothes… what an impatient tone."
    "Ropa, ropa… qué tono tan impaciente."

# game/script-a4-rin.rpy:5584
translate es a4_rin_proof_of_existence_dfece63c:

    # "I crouch down to pick up her bra from the floor where it had fallen, discarded in the haste of undressing and forgotten there."
    "Me agacho para recoger su sostén del suelo donde había caído, desechado en la prisa de desvestirse y olvidado ahí."

# game/script-a4-rin.rpy:5586
translate es a4_rin_proof_of_existence_f0f01aba:

    # "Hanging it from between my fingers like a dead fish, the same hesitation that grasped me when I was undressing Rin is creeping inside my head again."
    "Colgándolo entre mis dedos como un pescado muerto, la misma vacilación que me sujetó cuando estaba desvistiendo a Rin vuelve a treparse dentro de mi cabeza."

# game/script-a4-rin.rpy:5588
translate es a4_rin_proof_of_existence_d1ebecd8:

    # "Is intimacy really something this difficult for me to handle?"
    "¿Es la intimidad realmente algo tan difícil de manejar para mí?"

# game/script-a4-rin.rpy:5593
translate es a4_rin_proof_of_existence_3876b379:

    # rin "Come on, you got it off just fine. This is the same but the other way around. It's like talking backwards."
    rin "Vamos, me lo quitaste sin mayor problema. Esto es lo mismo pero al revés. Es como hablar al revés."

# game/script-a4-rin.rpy:5598
translate es a4_rin_proof_of_existence_170923fa:

    # rin "Ysae s'ti tub, drah smees."
    rin "Licáf se orep, licífid ecerap."

# game/script-a4-rin.rpy:5600
translate es a4_rin_proof_of_existence_7bd112e1:

    # "Perplexed by her sudden and prodigious display of mental processing capacity, I forget to attempt reversing her gibberish back."
    "Perplejo por su repentina y prodigiosa muestra de capacidad de procesamiento mental, olvido tratar de revertirle sus galimatías."

# game/script-a4-rin.rpy:5602
translate es a4_rin_proof_of_existence_f983896b:

    # "I'm pretty sure I couldn't switch to talking backwards that fluidly even with some practice."
    "Estoy bastante seguro de que no podría cambiarme a hablar al revés con esa fluidez aun con algo de práctica."

# game/script-a4-rin.rpy:5604
translate es a4_rin_proof_of_existence_881c13a4:

    # hi "Umm, could you repeat that?"
    hi "Ahh, ¿podrías repetir eso?"

# game/script-a4-rin.rpy:5609
translate es a4_rin_proof_of_existence_170923fa_1:

    # rin "Ysae s'ti tub, drah smees."
    rin "Licáf se orep, licífid ecerap."

# game/script-a4-rin.rpy:5611
translate es a4_rin_proof_of_existence_ce617998:

    # "…"
    "…"

# game/script-a4-rin.rpy:5613
translate es a4_rin_proof_of_existence_008979ce:

    # hi "Got it. Fine, I'll give it a try."
    hi "Entendido. Bien, lo intentaré."

# game/script-a4-rin.rpy:5615
translate es a4_rin_proof_of_existence_c1e3d7c4:

    # "Rin was right, the locking mechanism is simple enough, and I get the little plastic hooks right on the third attempt."
    "Rin tenía razón, el mecanismo de cerrado es bastante simple, y logro meter correctamente los ganchitos de plástico en el tercer intento."

# game/script-a4-rin.rpy:5617
translate es a4_rin_proof_of_existence_bd6fd538:

    # hi "There."
    hi "Ya está."

# game/script-a4-rin.rpy:5622
translate es a4_rin_proof_of_existence_91a2f43e:

    # rin "Ti tsujda ot evah uoy won."
    rin "Olratsuja euq seneit aroha."

# game/script-a4-rin.rpy:5624
translate es a4_rin_proof_of_existence_8dfe0930:

    # hi "What? Please stop that, I don't speak backwardese."
    hi "¿Qué? Por favor deja eso, no hablo enrevesado."

# game/script-a4-rin.rpy:5629
translate es a4_rin_proof_of_existence_1e9fa4f1:

    # "She shakes her head as if needing to banish the backwards way of thinking with a physical gesture."
    "Ella sacude su cabeza como si necesitara alejar la manera de pensar inversa con un gesto físico."

# game/script-a4-rin.rpy:5631
translate es a4_rin_proof_of_existence_dc719f51:

    # "I know a few people who could benefit from that kind of ability."
    "Conozco algunas personas que podrían beneficiarse de ese tipo de habilidad."

# game/script-a4-rin.rpy:5636
translate es a4_rin_proof_of_existence_7bc86513:

    # rin "I got stuck. Now you have to adjust it."
    rin "Me atoré. Ahora tienes que ajustarlo."

# game/script-a4-rin.rpy:5638
translate es a4_rin_proof_of_existence_a4f872d1:

    # hi "Adjust?"
    hi "¿Ajustar?"

# game/script-a4-rin.rpy:5643
translate es a4_rin_proof_of_existence_d40b3e97:

    # rin "That's what I said."
    rin "Eso es lo que dije."

# game/script-a4-rin.rpy:5645
translate es a4_rin_proof_of_existence_0ec38f07:

    # hi "No, I asked what you meant."
    hi "No, pregunté qué es lo que quisiste decir."

# game/script-a4-rin.rpy:5650
translate es a4_rin_proof_of_existence_0f74b2a9:

    # rin "You know, so that they are… fine."
    rin "Tú sabes, para que ellos estén… bien."

# game/script-a4-rin.rpy:5652
translate es a4_rin_proof_of_existence_0245740c:

    # "Oh. Fine, you say?"
    "Oh. ¿Bien, dices?"

# game/script-a4-rin.rpy:5654
translate es a4_rin_proof_of_existence_ce617998_1:

    # "…"
    "…"

# game/script-a4-rin.rpy:5656
translate es a4_rin_proof_of_existence_398274d5:

    # "As I have no idea when her breasts are supposed to be 'fine,' I end up fumbling around her chest for a good while without really getting anywhere."
    "Como no tengo idea de cuándo sus senos suponen estar “bien”, termino moviéndole el pecho por un buen rato sin realmente lograr algo."

# game/script-a4-rin.rpy:5658
translate es a4_rin_proof_of_existence_380c10ce:

    # "Not that I would complain, but Rin does."
    "No es que me queje, pero Rin lo hace."

# game/script-a4-rin.rpy:5663
translate es a4_rin_proof_of_existence_c461da6b:

    # rin "Emi is better than you at this."
    rin "Emi es mejor que tú en esto."

# game/script-a4-rin.rpy:5665
translate es a4_rin_proof_of_existence_69c2d11a:

    # "Her impatient tone ticks me off, even though I can't really disagree. Rin suddenly seems to be in an awful hurry."
    "Su tono impaciente me hace enojar, aunque en verdad no puedo discutir. Rin de pronto parece tener mucha prisa."

# game/script-a4-rin.rpy:5667
translate es a4_rin_proof_of_existence_ecfade39:

    # hi "Yeah well excuse me, could that be because she is a {b}girl{/b} and can actually relate?"
    hi "Sí bueno discúlpame, ¿podría eso ser porque ella es una {b}chica{/b} y pueden entenderse, de hecho?"

# game/script-a4-rin.rpy:5672
translate es a4_rin_proof_of_existence_178b830b:

    # rin "I don't think so, she has just about as much chest as you do."
    rin "No lo creo, ella tiene casi tanto busto como tú."

# game/script-a4-rin.rpy:5674
translate es a4_rin_proof_of_existence_ce617998_2:

    # "…"
    "…"

# game/script-a4-rin.rpy:5682
translate es a4_rin_proof_of_existence_f0f95faf:

    # "With her bra and breasts eventually 'fine' as they should, the rest of her clothes are considerably easier to put on."
    "Con su sostén y senos finalmente “bien” como deberían de estar, el resto de su ropa es considerablemente más fácil de poner."

# game/script-a4-rin.rpy:5687
translate es a4_rin_proof_of_existence_dda9cce0:

    # "Rin launches towards the door even though her shirt is not even buttoned up all the way yet."
    "Rin se lanza hacia la puerta aunque su blusa ni siquiera esté abotonada por completo todavía."

# game/script-a4-rin.rpy:5689
translate es a4_rin_proof_of_existence_dd1bee78:

    # "Left with little choice, I run after her."
    "Quedándome con pocas opciones, corro tras ella."

# game/script-a4-rin.rpy:5699
translate es a4_rin_proof_of_existence_343a7653:

    # "As soon as I realize that we are heading for the side entrance leading to the forest, I think I know where Rin wanted to go, although I couldn't say why she'd want to go there."
    "Tan pronto como me percato de que nos dirigimos a la entrada lateral que lleva al bosque, creo saber adónde es que Rin quería ir, aunque no podría decir el porqué querría ella ir allá."

# game/script-a4-rin.rpy:5701
translate es a4_rin_proof_of_existence_812c6f75:

    # "Then again, I can't really assume my guesses to be anywhere near correct when Rin is concerned, not even for a quite generous definition of 'correct.'"
    "Por otro lado, realmente no puedo asumir que mis suposiciones podrían estar cerca cuando se trata de Rin, ni siquiera para una definición bastante generosa de “correcto”."

# game/script-a4-rin.rpy:5709
translate es a4_rin_proof_of_existence_14cbe916:

    # "The forest behind the walls smells of rain, the last raindrops are still dripping from the wet undergrowth into the earth despite the rain being gone for a while already."
    "El bosque detrás de los muros huele a lluvia, las últimas gotas todavía están cayendo de los arbustos húmedos hacia la tierra a pesar de que la lluvia se haya ido por un rato ya."

# game/script-a4-rin.rpy:5711
translate es a4_rin_proof_of_existence_0e8c5c6a:

    # "We stroll along with an unhurried pace that Rin sets, giving me time to take in the calming atmosphere."
    "Paseamos con un paso desapresurado que Rin marca, dándome tiempo de apreciar el ambiente tranquilizador."

# game/script-a4-rin.rpy:5713
translate es a4_rin_proof_of_existence_0f3b07ab:

    # "I think I can hear Rin saying hello to at least three different trees while she walks past them, but I ignore it, just like the trees do."
    "Creo que puedo escuchar a Rin decirle hola a por lo menos tres diferentes árboles al pasar caminando frente a ellos, pero lo ignoro, al igual que lo hacen los árboles."

# game/script-a4-rin.rpy:5715
translate es a4_rin_proof_of_existence_d33bda7a:

    # "She leads me to the narrow side path leading up to the hilltops, as I guessed."
    "Me lleva a un camino lateral estrecho que lleva a las colinas, como había supuesto."

# game/script-a4-rin.rpy:5720
translate es a4_rin_proof_of_existence_6c2a3595:

    # "I peek through the canopy trying to find a rainbow, but there doesn't seem to be one."
    "Me asomo por las copas de los árboles tratando de encontrar un arcoíris, pero parece no haber uno."

# game/script-a4-rin.rpy:5722
translate es a4_rin_proof_of_existence_8d37b191:

    # "It's perfect weather for rainbows. The sun is shining low, and rain has passed not too long ago."
    "Es el clima perfecto para los arcoíris. El sol brilla bajo, y la lluvia acabó hace poco."

# game/script-a4-rin.rpy:5724
translate es a4_rin_proof_of_existence_d97e8d0c:

    # "Well, whatever."
    "Bueno, como sea."

# game/script-a4-rin.rpy:5732
translate es a4_rin_proof_of_existence_336dbacc:

    # "I lower my eyes from the treetops to see the gaunt back of the girl who is climbing up the hill slowly, without losing her balance."
    "Bajo los ojos de las copas de los árboles para ver la delgada espalda de la chica que está trepando la colina lentamente, sin perder su equilibrio."

# game/script-a4-rin.rpy:5734
translate es a4_rin_proof_of_existence_2df4b3f5:

    # "A few steps ahead of me on the path, but still within my reach."
    "Algunos pasos del camino frente a mí, pero aún dentro de mi alcance."

# game/script-a4-rin.rpy:5736
translate es a4_rin_proof_of_existence_5a71e44a:

    # "I don't think I ever could reach a rainbow, but reaching Rin… it seems less impossible than it used to seem."
    "No creo que algún día pueda alcanzar a un arcoíris, pero alcanzar a Rin… parece menos imposible de lo que solía parecer."

# game/script-a4-rin.rpy:5744
translate es a4_rin_proof_of_existence_ddb24115:

    # "The clear sky greeting us from above the meadow clearing seems vast and beautiful."
    "El cielo despejado que nos recibe desde lo alto del prado se ve vasto y hermoso."

# game/script-a4-rin.rpy:5746
translate es a4_rin_proof_of_existence_022ea94b:

    # "A strong wind is herding the rainclouds away from the town, to the other side of the mountains in the distance."
    "Un fuerte viento se lleva el rebaño de nubes de lluvia lejos de la ciudad, al otro lado de las montañas a la distancia."

# game/script-a4-rin.rpy:5748
translate es a4_rin_proof_of_existence_bc5d79fd:

    # "The sight is pretty, but…"
    "La vista es bonita, pero…"

# game/script-a4-rin.rpy:5750
translate es a4_rin_proof_of_existence_ce617998_3:

    # "…"
    "…"

# game/script-a4-rin.rpy:5757
translate es a4_rin_proof_of_existence_7390363a:

    # "A speck of white flies past the edge my peripheral vision, but when I turn to look, it's already gone."
    "Una mota de blanco vuela por la orilla de mi visión periférica, pero cuando me giro para ver, ya se ha ido."

# game/script-a4-rin.rpy:5759
translate es a4_rin_proof_of_existence_1a86b3a2:

    # "Another follows, then a third."
    "Otra le sigue, luego una tercera."

# game/script-a4-rin.rpy:5761
translate es a4_rin_proof_of_existence_85e92d9b:

    # "Before I realize it, dozens of almost invisible small tufts of white are flying all around us."
    "Antes de darme cuenta, docenas de casi invisibles motitas peludas blancas vuelan alrededor de nosotros."

# game/script-a4-rin.rpy:5766
translate es a4_rin_proof_of_existence_a19e641e:

    # rin "Look, the flowers."
    rin "Mira, las flores."

# game/script-a4-rin.rpy:5768
translate es a4_rin_proof_of_existence_2017ab36:

    # "Ah. I see it now."
    "Ah. Ya lo veo."

# game/script-a4-rin.rpy:5780
translate es a4_rin_proof_of_existence_3f3a9d27:

    # "The sea of dandelions that covered the hilltop on our last visit has changed over the days."
    "El mar de dientes de león que cubría la cima de la colina en nuestra última visita ha cambiado con el paso de los días."

# game/script-a4-rin.rpy:5782
translate es a4_rin_proof_of_existence_ce939667:

    # "Where there was bright yellow before, there is now fluffy white."
    "Donde antes había amarillo brillante, ahora hay blanco esponjoso."

# game/script-a4-rin.rpy:5784
translate es a4_rin_proof_of_existence_ac7a43ce:

    # "Some of the flowers have already shed their seeds, but many are still waiting for a suitable gust of wind."
    "Algunas de las flores ya han perdido sus semillas, pero muchas todavía siguen esperando una ráfaga de viento adecuada."

# game/script-a4-rin.rpy:5786
translate es a4_rin_proof_of_existence_1fe3e1f3:

    # "Today those gusts are not in short demand, every now and then they shake the grass thoroughly, and suddenly the air is thick with dandelion seeds."
    "Hoy esas ráfagas no escasean, de vez en cuando sacuden el pasto por completo, y de pronto el aire está repleto de semillas de dientes de león."

# game/script-a4-rin.rpy:5788
translate es a4_rin_proof_of_existence_14b98c4c:

    # "One by one, the seeds separate from the flower heads and are lifted away."
    "Una a una, las semillas se separan de las flores y son llevadas lejos."

# game/script-a4-rin.rpy:5790
translate es a4_rin_proof_of_existence_79507d1d:

    # "A commonplace event, but one that seems to fascinate Rin for some reason."
    "Un evento común, pero uno que parece fascinar a Rin por alguna razón."

# game/script-a4-rin.rpy:5797
translate es a4_rin_proof_of_existence_61b3d011:

    # "She's turning her head from side to side, marveling at the change happening all around her as the seeds fly away."
    "Ella gira la cabeza de lado a lado, maravillándose ante el cambio sucediendo a todo su alrededor a medida que las semillas vuelan lejos."

# game/script-a4-rin.rpy:5799
translate es a4_rin_proof_of_existence_e23673ed:

    # "I watch them too, following the white tufts floating with the wind towards the horizon, and imagine that I can see them even after they disappear from my sight."
    "También las miro, siguiendo las motas blancas flotando con el viento hacia el horizonte, e imagino que puedo verlas aun después de que desaparecen de mi vista."

# game/script-a4-rin.rpy:5801
translate es a4_rin_proof_of_existence_ce617998_4:

    # "…"
    "…"

# game/script-a4-rin.rpy:5806
translate es a4_rin_proof_of_existence_aa95e2ab:

    # rin "Hisao."
    rin "Hisao."

# game/script-a4-rin.rpy:5808
translate es a4_rin_proof_of_existence_46b9decc:

    # hi "What is it?"
    hi "¿Qué pasa?"

# game/script-a4-rin.rpy:5813
translate es a4_rin_proof_of_existence_3232d6b6:

    # rin "Do you love me?"
    rin "¿Me amas?"

# game/script-a4-rin.rpy:5815
translate es a4_rin_proof_of_existence_397c2b11:

    # "I snap to attention, to meet her suddenly very serious face that is not looking only at the flowers any more."
    "Me pongo alerta de inmediato, para encontrarme con su repentinamente muy serio rostro que ya no está mirando solo a las flores."

# game/script-a4-rin.rpy:5817
translate es a4_rin_proof_of_existence_7755029f:

    # "What a tough question, asked just like that, out of the blue."
    "Qué pregunta tan difícil, hecha así como así, de la nada."

# game/script-a4-rin.rpy:5819
translate es a4_rin_proof_of_existence_fe94a466:

    # "Still, her bluntness compels me to answer rapidly."
    "Aun así, su brusquedad me fuerza a dar una respuesta rápido."

# game/script-a4-rin.rpy:5821
translate es a4_rin_proof_of_existence_a6e4b6ef:

    # hi "I don't know. Maybe I do."
    hi "No lo sé. Tal vez sí."

# game/script-a4-rin.rpy:5823
translate es a4_rin_proof_of_existence_4febc649:

    # "Maybe too rapidly."
    "Tal vez demasiado rápido."

# game/script-a4-rin.rpy:5828
translate es a4_rin_proof_of_existence_82e60995:

    # rin "What does that mean?"
    rin "¿Qué significa eso?"

# game/script-a4-rin.rpy:5830
translate es a4_rin_proof_of_existence_7319aaeb:

    # hi "…I don't know."
    hi "… No lo sé."

# game/script-a4-rin.rpy:5835
translate es a4_rin_proof_of_existence_4a5175df:

    # "Rin sighs, perhaps unhappy with my wishy-washy answer. I would be too."
    "Rin suspira, tal vez triste con mi respuesta vaga. Yo también lo estaría."

# game/script-a4-rin.rpy:5840
translate es a4_rin_proof_of_existence_6b57485c:

    # rin "Me neither."
    rin "Yo tampoco."

# game/script-a4-rin.rpy:5845
translate es a4_rin_proof_of_existence_6d90b9f0:

    # rin "I don't think I know much about love."
    rin "No creo que sepa mucho sobre el amor."

# game/script-a4-rin.rpy:5847
translate es a4_rin_proof_of_existence_35c4f685:

    # hi "…"
    hi "…"

# game/script-a4-rin.rpy:5849
translate es a4_rin_proof_of_existence_4e3f2a6a:

    # hi "…It's fine, isn't it?"
    hi "… Está bien, ¿no?"

# game/script-a4-rin.rpy:5854
translate es a4_rin_proof_of_existence_e7985cdf:

    # "'How should I know?', the shrug of her shoulders seems to say, hesitating to give a firmer answer."
    "“¿Cómo podría saberlo?”, el cómo se encoge de hombros parece decir eso, titubeando en dar una respuesta más firme."

# game/script-a4-rin.rpy:5856
translate es a4_rin_proof_of_existence_fc27b384:

    # "She stays silent for only a second too long, but even that second isn't long enough for me to think ahead…"
    "Ella guarda silencio por solo un segundo de más, pero aun así ese segundo no es lo suficientemente largo para que yo piense más allá…"

# game/script-a4-rin.rpy:5861
translate es a4_rin_proof_of_existence_ac24e6ca:

    # rin "I love you."
    rin "Te amo."

# game/script-a4-rin.rpy:5863
translate es a4_rin_proof_of_existence_7c8ab19b:

    # "Those three words freeze me in place like a rabbit staring into headlights, but I'm not a rabbit and I'm just staring into Rin's eyes that seem far, far too impassive for what she just let out of her mouth."
    "Esas dos palabras me vuelven piedra como un conejo mirando dentro de los faros de un auto, pero no soy un conejo y tan solo estoy mirando dentro de los ojos de Rin que parecen muy, muy impasibles, demasiado para lo que acaba de dejar salir de su boca."

# game/script-a4-rin.rpy:5868
translate es a4_rin_proof_of_existence_ecd49d5b:

    # "Rin looks pretty serious though, until she sticks out her tongue, frowns a little and confuses me even more than her words did."
    "Sin embargo Rin se ve bastante seria, hasta que saca la lengua, frunce el ceño un poco y me confunde todavía más de lo que lo hicieron sus palabras."

# game/script-a4-rin.rpy:5870
translate es a4_rin_proof_of_existence_5ef4ca24:

    # "Why does she look mildly unhappy?"
    "¿Por qué se ve ligeramente triste?"

# game/script-a4-rin.rpy:5872
translate es a4_rin_proof_of_existence_30173abe:

    # "Was it a confession of her deepest feelings, a test to see how I would react, a test to see how she would react?"
    "Fue una confesión de sus sentimientos más profundos. ¿Una prueba para ver cómo reaccionaría yo? ¿Una prueba de cómo reaccionaría ella?"

# game/script-a4-rin.rpy:5877
translate es a4_rin_proof_of_existence_d7151127:

    # rin "It tastes weird."
    rin "Sabe raro."

# game/script-a4-rin.rpy:5879
translate es a4_rin_proof_of_existence_62452a4c:

    # hi "…Tastes?"
    hi "… ¿Sabe?"

# game/script-a4-rin.rpy:5884
translate es a4_rin_proof_of_existence_b53fc5c7:

    # rin "Yeah. So weird."
    rin "Sí. Muy raro."

# game/script-a4-rin.rpy:5886
translate es a4_rin_proof_of_existence_96ad6c5a:

    # "She laughs, maybe nervously or so I want to think, but stops midway when she notices how strange it sounds."
    "Ella se ríe, tal vez nerviosamente o así quiero pensarlo, pero se detiene a la mitad cuando se da cuenta de cuán raro suena."

# game/script-a4-rin.rpy:5891
translate es a4_rin_proof_of_existence_bdae6aa5:

    # rin "Like… I don't know what, I… don't think there is a word for this."
    rin "Como si… No sé qué, yo… no creo que haya una palabra para esto."

# game/script-a4-rin.rpy:5893
translate es a4_rin_proof_of_existence_cf3aa041:

    # "Rin keeps on talking as though there was no meaning behind her words, steady and careless words dropping from the same tongue that formed the more important ones."
    "Rin sigue hablando como si no hubiera significado detrás de sus palabras, palabras constantes y descuidadas caen de la misma lengua que formó las más importantes."

# game/script-a4-rin.rpy:5898
translate es a4_rin_proof_of_existence_ae6ba128:

    # rin "A word for… ummm…"
    rin "Una palabra para… ahhh…"

# game/script-a4-rin.rpy:5900
translate es a4_rin_proof_of_existence_72fd2351:

    # "Except."
    "Excepto."

# game/script-a4-rin.rpy:5905
translate es a4_rin_proof_of_existence_a4c61d16:

    # rin "…it's like…"
    rin "… es como…"

# game/script-a4-rin.rpy:5907
translate es a4_rin_proof_of_existence_ad4895f3:

    # "She can't."
    "Que ella no puede."

# game/script-a4-rin.rpy:5912
translate es a4_rin_proof_of_existence_879b98dc:

    # rin "…"
    rin "…"

# game/script-a4-rin.rpy:5914
translate es a4_rin_proof_of_existence_61902367:

    # "Find the words."
    "Encontrar las palabras."

# game/script-a4-rin.rpy:5919
translate es a4_rin_proof_of_existence_879b98dc_1:

    # rin "…"
    rin "…"

# game/script-a4-rin.rpy:5921
translate es a4_rin_proof_of_existence_d07d9781:

    # "Rin just keeps staring at me, stumbling with her words as if her brain suddenly ground to a halt."
    "Rin tan solo sigue mirándome, tropezándose con sus palabras como si su cerebro se hubiera detenido súbitamente por completo."

# game/script-a4-rin.rpy:5923
translate es a4_rin_proof_of_existence_b859d842:

    # "She looks awfully confused, much like how I feel right now as I wait for her to explain."
    "Se ve terriblemente confundida, justo como me siento en este momento mientras espero a que ella se explique."

# game/script-a4-rin.rpy:5925
translate es a4_rin_proof_of_existence_8956fa1e:

    # "But she doesn't, she just blinks a few times, the flutter of her long lashes catching my fancy because she looks like she is petrified otherwise."
    "Pero no lo hace, solo parpadea unas cuantas veces, el aleteo de sus largas pestañas captura mi atención porque de otro modo ella parece estar petrificada."

# game/script-a4-rin.rpy:5927
translate es a4_rin_proof_of_existence_f1caef8d:

    # "Until I realize what they were fighting against."
    "Hasta que me doy cuenta de que ellas estaban peleando contra ello."

# game/script-a4-rin.rpy:5932
translate es a4_rin_proof_of_existence_6d4a8e6f:

    # "It's those weird tears again, not associated with sadness or happiness, not pitiable sobbing nor laughter of joy."
    "Son esas lágrimas extrañas otra vez, no asociadas con la tristeza o la felicidad, ni patéticos sollozos ni risa de alegría."

# game/script-a4-rin.rpy:5934
translate es a4_rin_proof_of_existence_d4bc0c2f:

    # "Just tears, spontaneously and without a warning, like that one time in her classroom."
    "Solo lágrimas, espontáneas y sin advertencia, como en aquella ocasión en su salón de clase."

# game/script-a4-rin.rpy:5936
translate es a4_rin_proof_of_existence_b38d4b20:

    # rin "Ah."
    rin "Ah."

# game/script-a4-rin.rpy:5938
translate es a4_rin_proof_of_existence_f74cc040:

    # "Just a few of them, not enough to make a fuss about, so Rin doesn't make a move to hide them even after noticing."
    "Solo unas cuantas de ellas, no lo suficiente para armar un alboroto, así que Rin no se mueve para ocultarlas aun después de darse cuenta de ellas."

# game/script-a4-rin.rpy:5940
translate es a4_rin_proof_of_existence_27e8cb1f:

    # "Rin cries, looking like she has no idea why, and somehow a great uneasiness grows in my chest when I look into her watery eyes that stare right back at me."
    "Rin llora, viéndose como si no tuviera idea de por qué, y de algún modo una gran ansiedad crece dentro de mi pecho cuando miro en sus ojos llorosos que me miran de vuelta."

# game/script-a4-rin.rpy:5942
translate es a4_rin_proof_of_existence_15bb8b45:

    # "It petrifies me too, the shock of the incomprehensibility of this situation."
    "También me petrifica, la impresión de la incomprensibilidad de esta situación."

# game/script-a4-rin.rpy:5944
translate es a4_rin_proof_of_existence_992f1cf2:

    # "I just don't know what is happening any more."
    "Simplemente yo ya no sé qué es lo que está sucediendo."

# game/script-a4-rin.rpy:5946
translate es a4_rin_proof_of_existence_66375617:

    # hi "Rin? What's wrong?"
    hi "¿Rin? ¿Qué ocurre?"

# game/script-a4-rin.rpy:5948
translate es a4_rin_proof_of_existence_6d1989e6:

    # rin "I…"
    rin "Yo…"

# game/script-a4-rin.rpy:5953
translate es a4_rin_proof_of_existence_e8ee8c0b:

    # "She shakes her head in confusion, stumbling to get the words out of her mouth."
    "Ella agita la cabeza en su confusión, batallando para sacar las palabras de su boca."

# game/script-a4-rin.rpy:5958
translate es a4_rin_proof_of_existence_458ca1c2:

    # rin "Sorry…"
    rin "Lo siento…"

# game/script-a4-rin.rpy:5960
translate es a4_rin_proof_of_existence_b36ee50c:

    # rin "I might be a little afraid of you."
    rin "Puede que te tenga un poco de miedo."

# game/script-a4-rin.rpy:5962
translate es a4_rin_proof_of_existence_688baf88:

    # "The words are muttered slowly, with a small voice that is as disbelieving of what it's saying as I am."
    "Las palabras son masculladas lentamente, con una vocecita que es tan incrédula de lo que está diciendo como lo soy yo."

# game/script-a4-rin.rpy:5964
translate es a4_rin_proof_of_existence_d650e2c8:

    # hi "What? Why?"
    hi "¿Qué? ¿Por qué?"

# game/script-a4-rin.rpy:5969
translate es a4_rin_proof_of_existence_bdeaba1d:

    # rin "I don't know. Saying that just made me feel like that."
    rin "No lo sé. Solo decir eso me hizo sentir así."

# game/script-a4-rin.rpy:5974
translate es a4_rin_proof_of_existence_782f7f55:

    # rin "People cry when they are afraid, right?"
    rin "La gente llora cuando está asustada, ¿verdad?"

# game/script-a4-rin.rpy:5979
translate es a4_rin_proof_of_existence_01ca1509:

    # rin "See? I can do it too."
    rin "¿Ves? Yo lo puedo hacer también."

# game/script-a4-rin.rpy:5981
translate es a4_rin_proof_of_existence_4421aa89:

    # "She's averting her gaze now, deliberately not looking at me. It bewilders me, at least as much as what she is saying."
    "Desvía su mirada ahora, deliberadamente para no verme. Me desconcierta, al menos tanto como lo que está diciendo."

# game/script-a4-rin.rpy:5986
translate es a4_rin_proof_of_existence_03e51ec3:

    # rin "I… I sometimes, with you, want to run away so badly but I can't move it's like my legs turn into lemon panna cotta pudding and my heart feels like it's going to explode and…"
    rin "Yo… algunas veces, contigo, quiero escapar tanto pero no puedo moverme, es como si mis piernas se convirtieran en panna cotta de limón y mi corazón se siente como si fuera a estallar y…"

# game/script-a4-rin.rpy:5991
translate es a4_rin_proof_of_existence_db15cc53:

    # "She slumps her shoulders melancholically."
    "Deja caer sus hombros melancólicamente."

# game/script-a4-rin.rpy:5993
translate es a4_rin_proof_of_existence_db2b73fb:

    # rin "Has a thing like this ever happened to you?"
    rin "¿Alguna vez te pasó algo como esto?"

# game/script-a4-rin.rpy:5995
translate es a4_rin_proof_of_existence_7481a787:

    # "…I remember the leaden sky above the frozen forest and the sound of the leafless branches clacking against each other."
    "… Recuerdo el cielo plomizo sobre el bosque congelado y el sonido de las ramas desnudas golpeándose unas con otras."

# game/script-a4-rin.rpy:5997
translate es a4_rin_proof_of_existence_004dc9b0:

    # "It's like a memory from another life."
    "Es como una memoria de otra vida."

# game/script-a4-rin.rpy:5999
translate es a4_rin_proof_of_existence_30b5d4ca:

    # hi "Yeah. Once."
    hi "Sí. Una vez."

# game/script-a4-rin.rpy:6001
translate es a4_rin_proof_of_existence_661cd1ac:

    # hi "My heart hurt a lot back then, too."
    hi "Mi corazón también me dolía mucho entonces."

# game/script-a4-rin.rpy:6006
translate es a4_rin_proof_of_existence_ba43c420:

    # rin "But I thought your thing was not contagious."
    rin "Pero pensé que tu cosa no era contagiosa."

# game/script-a4-rin.rpy:6008
translate es a4_rin_proof_of_existence_510b69cb:

    # "I shake my head and a tiny, slightly forced smile rises on my lips."
    "Sacudo mi cabeza y una diminuta, ligeramente forzada sonrisa surge en mis labios."

# game/script-a4-rin.rpy:6010
translate es a4_rin_proof_of_existence_60be4f85:

    # "The other ailment of my heart could very well be contagious and I wouldn't care a bit."
    "El otro mal de mi corazón bien podría ser contagioso y no me importaría ni un poco."

# game/script-a4-rin.rpy:6012
translate es a4_rin_proof_of_existence_b391119b:

    # hi "What are you afraid of? I never thought I was scary."
    hi "¿De qué tienes miedo? Nunca pensé que daba miedo."

# game/script-a4-rin.rpy:6017
translate es a4_rin_proof_of_existence_ca3241ba:

    # "Rin shakes her head desperately, as if knowing that the tangle inside her mind won't be undone with just that."
    "Rin sacude la cabeza con desesperación, como si supiera que la maraña dentro de su mente no podría desenredarse con solo eso."

# game/script-a4-rin.rpy:6019
translate es a4_rin_proof_of_existence_8ea99569:

    # rin "You make me feel that I should be someone else than me."
    rin "Tú me haces sentir que debería ser alguien más que yo."

# game/script-a4-rin.rpy:6024
translate es a4_rin_proof_of_existence_b53e326b:

    # rin "It's a scary thing."
    rin "Es algo que da miedo."

# game/script-a4-rin.rpy:6029
translate es a4_rin_proof_of_existence_0cf72bce:

    # rin "It happens when you are being nice to me. Like yesterday."
    rin "Pasa cuando eres bueno conmigo. Como ayer."

# game/script-a4-rin.rpy:6031
translate es a4_rin_proof_of_existence_9dd65f28:

    # rin "I never know what to do at times like that. It's hard."
    rin "Nunca sé qué hacer en momentos como ese. Es difícil."

# game/script-a4-rin.rpy:6033
translate es a4_rin_proof_of_existence_68a783da:

    # "Her voice is barely audible, a whispered admission of something that is too embarrassing to even think, not to mention to say aloud."
    "Su voz, apenas audible, susurra una admisión de algo que es demasiado vergonzoso de siquiera pensar, eso sin mencionar decirla en voz alta."

# game/script-a4-rin.rpy:6035
translate es a4_rin_proof_of_existence_29fded1b:

    # "Rin has never been one to be embarrassed so she does utter it aloud, only timidly as if by instinct."
    "Rin nunca ha sido del tipo que se avergüenza así que sí lo pronuncia en voz alta, aunque con timidez como por instinto."

# game/script-a4-rin.rpy:6040
translate es a4_rin_proof_of_existence_4f3fe0a2:

    # rin "But I want to do something. But I don't know if this me can."
    rin "Pero quiero hacer algo. Pero no sé si este yo pueda."

# game/script-a4-rin.rpy:6042
translate es a4_rin_proof_of_existence_93e843ab:

    # "For a moment, we just stare at each other as if waiting for the other to say something."
    "Por un momento, simplemente nos miramos el uno al otro como si esperáramos a que el otro diga algo."

# game/script-a4-rin.rpy:6044
translate es a4_rin_proof_of_existence_ce617998_5:

    # "…"
    "…"

# game/script-a4-rin.rpy:6049
translate es a4_rin_proof_of_existence_e9c67197:

    # hi "You are so stupid."
    hi "Eres tan estúpida."

# game/script-a4-rin.rpy:6054
translate es a4_rin_proof_of_existence_8543f31f:

    # "Rin's lips taste salty and scared against mine."
    "Los labios de Rin saben salados y asustados contra los míos."

# game/script-a4-rin.rpy:6063
translate es a4_rin_proof_of_existence_30bfda0f:

    # "As I grasp her into an embrace, I feel my heart thumping in my chest painfully."
    "Al tomarla en un abrazo, siento mi corazón golpeando en mi pecho dolorosamente."

# game/script-a4-rin.rpy:6123
translate es a4_rin_proof_of_existence_d41d8cd9:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:6078
translate es a4_rin_proof_of_existence_fcc6a40d:

    # n "{vspace=60}Even though I am glad that she can say things like that, they make me sad after all."
    n "{vspace=60}Aunque estoy feliz de que ella pueda decir cosas como esa, me hacen sentir triste después de todo."

# game/script-a4-rin.rpy:6080
translate es a4_rin_proof_of_existence_b396a056:

    # n "Rin's spirit, her passion, her strength. All those things that I hold dear are the ones I don't want to change."
    n "El entusiasmo de Rin, su pasión, su fuerza. Todas esas cosas tan queridas para mí son las que no quiero cambiar."

# game/script-a4-rin.rpy:6082
translate es a4_rin_proof_of_existence_df8f61f5:

    # n "How should I treat them? Where are they headed to? Is that future irrevocably different from mine?"
    n "¿Cómo debería tratarlas? ¿Adónde es que se dirigen? ¿Es ese futuro irrevocablemente diferente al mío?"

# game/script-a4-rin.rpy:6084
translate es a4_rin_proof_of_existence_b8565158:

    # n "That anxiety will never loose its grip on my heart, but I think I could learn to live with it."
    n "Esa ansiedad nunca disminuirá la presión en mi corazón, pero creo que podría aprender a vivir con ella."

# game/script-a4-rin.rpy:6086
translate es a4_rin_proof_of_existence_29a6b7fb:

    # n "Slowly, the pain in my heart dies out, and it settles into the same rhythm as Rin's."
    n "Lentamente, el dolor dentro de mi corazón se apaga, y se asienta en el mismo ritmo que el de Rin."

# game/script-a4-rin.rpy:6088
translate es a4_rin_proof_of_existence_6b3de1e1:

    # n "{vspace=60}We listen to that for some time."
    n "{vspace=60}Lo escuchamos por un rato."

# game/script-a4-rin.rpy:6090
translate es a4_rin_proof_of_existence_eabec665:

    # n "…"
    n "…"

# game/script-a4-rin.rpy:6145
translate es a4_rin_proof_of_existence_d41d8cd9_1:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:6102
translate es a4_rin_proof_of_existence_e9d6a185:

    # "After our lips break apart, it takes a while for either of us to realize that we can say something now."
    "Después de que nuestros labios se separan, toma un tiempo para que alguno de los dos se percate de que ya podemos decir algo."

# game/script-a4-rin.rpy:6104
translate es a4_rin_proof_of_existence_ce617998_6:

    # "…"
    "…"

# game/script-a4-rin.rpy:6109
translate es a4_rin_proof_of_existence_8df36035:

    # rin "See?"
    rin "¿Ves?"

# game/script-a4-rin.rpy:6114
translate es a4_rin_proof_of_existence_ceccb903:

    # rin "You are a really kind person, even when you are not."
    rin "Eres una persona muy amable, aun cuando no lo eres."

# game/script-a4-rin.rpy:6116
translate es a4_rin_proof_of_existence_269538d0:

    # rin "It's the most scariest thing ever."
    rin "Es lo más aterrador de todo."

# game/script-a4-rin.rpy:6121
translate es a4_rin_proof_of_existence_d500678a:

    # rin "I think… that all I was ever afraid of is your kindness."
    rin "Creo… que todo a lo que le tenía miedo era a tu amabilidad."

# game/script-a4-rin.rpy:6123
translate es a4_rin_proof_of_existence_ce617998_7:

    # "…"
    "…"

# game/script-a4-rin.rpy:6125
translate es a4_rin_proof_of_existence_ae4b5795:

    # hi "Is it bad? Even if you are afraid?"
    hi "¿Eso es malo? ¿Aun si tienes miedo?"

# game/script-a4-rin.rpy:6130
translate es a4_rin_proof_of_existence_cfbf1adf:

    # "She thinks about this for a while, furrowing her brow as though this was some kind of hard math problem."
    "Ella piensa en ello por un rato, frunciendo su ceño como si esto fuese algún tipo de problema difícil de Matemáticas."

# game/script-a4-rin.rpy:6135
translate es a4_rin_proof_of_existence_3861dc5c:

    # rin "No. I'm all right with it. It's fine, if it's you."
    rin "No. No hay problema con ello. Está bien, si es contigo."

# game/script-a4-rin.rpy:6137
translate es a4_rin_proof_of_existence_ac87dbcf:

    # "Like a weight lifted from my chest, her words elate my heart, filling it with… I don't know, happiness?"
    "Como un peso que se levanta de mi pecho, sus palabras me alegran el corazón, llenándolo con… no lo sé, ¿felicidad?"

# game/script-a4-rin.rpy:6139
translate es a4_rin_proof_of_existence_654ce13e:

    # "What else could it be?"
    "¿Qué más podría ser?"

# game/script-a4-rin.rpy:6141
translate es a4_rin_proof_of_existence_f251778b:

    # "This time my smile is genuine."
    "Esta vez mi sonrisa es verdadera."

# game/script-a4-rin.rpy:6146
translate es a4_rin_proof_of_existence_deb62200:

    # "Rin steps back, still smiling gently at me like I do at her."
    "Rin da un paso atrás, todavía sonriéndome con gentileza como yo a ella."

# game/script-a4-rin.rpy:6162
translate es a4_rin_proof_of_existence_ab2df5c7:

    # "While she wipes her face on her shoulder, I pick up a round, plump dandelion clock and bring it to my pursed lips."
    "Mientras limpia su rostro en su hombro, recojo un diente de león entero, redondo y esponjoso y lo llevo a mis labios apretados."

# game/script-a4-rin.rpy:6167
translate es a4_rin_proof_of_existence_3a146420:

    # "Pfff…"
    "Pfff…"

# game/script-a4-rin.rpy:6169
translate es a4_rin_proof_of_existence_b8a93731:

    # "They spread out into the wind that picks them up to carry them to a new home."
    "Se esparcen por el viento que los levanta y los lleva a un nuevo hogar."

# game/script-a4-rin.rpy:6171
translate es a4_rin_proof_of_existence_55a80952:

    # "To think, only a few short weeks ago they were so different."
    "Y pensar que, tan solo hace unas cuantas semanas, eran tan diferentes."

# game/script-a4-rin.rpy:6173
translate es a4_rin_proof_of_existence_cb439a12:

    # "This is change."
    "Esto es cambio."

# game/script-a4-rin.rpy:6175
translate es a4_rin_proof_of_existence_ce617998_8:

    # "…"
    "…"

# game/script-a4-rin.rpy:6190
translate es a4_rin_proof_of_existence_fafb6d5d:

    # hi "Hey, so the flowers became what they were meant to become, like you said the last time."
    hi "Oye, entonces las flores se transformaron en lo que debían convertirse, como lo dijiste la última vez."

# game/script-a4-rin.rpy:6192
translate es a4_rin_proof_of_existence_7d645e63:

    # hi "What about you? Did you become a true artist? Or did you not, because you ran away?"
    hi "¿Qué hay de ti? ¿Te convertiste en una artista verdadera? ¿O no, porque te escapaste?"

# game/script-a4-rin.rpy:6197
translate es a4_rin_proof_of_existence_a80c04ab:

    # "She pauses for a while to ponder my question…"
    "Hace una pausa por un momento para reflexionar sobre mi pregunta."

# game/script-a4-rin.rpy:6202
translate es a4_rin_proof_of_existence_257994b8:

    # "…and shrugs her shoulders."
    "… y se encoge de hombros."

# game/script-a4-rin.rpy:6204
translate es a4_rin_proof_of_existence_701fa751:

    # "It almost makes me laugh."
    "Casi me hace reír."

# game/script-a4-rin.rpy:6206
translate es a4_rin_proof_of_existence_36306071:

    # "The carefree easiness of her gesture is a lovely thing, a sign of how Rin can, truly and really, without any restraints whatsoever, shed the entire weight of the world from her shoulders, should she will so."
    "La despreocupada soltura de su gesto es algo adorable, una señal de cómo Rin puede, real y verdaderamente, sin restricciones de algún tipo, deshacerse del peso entero del mundo sobre sus hombros, si ella así lo quiere."

# game/script-a4-rin.rpy:6208
translate es a4_rin_proof_of_existence_f1df82a8:

    # "She is, in every possible and probably a few impossible ways… free."
    "Ella es, de toda manera posible y probablemente algunas imposibles… libre."

# game/script-a4-rin.rpy:6210
translate es a4_rin_proof_of_existence_a91e845d:

    # "And I think I might love her for that."
    "Y creo que podría amarla por ello."

# game/script-a4-rin.rpy:6215
translate es a4_rin_proof_of_existence_6ec9972e:

    # rin "I don't think it matters."
    rin "No creo que importe."

# game/script-a4-rin.rpy:6220
translate es a4_rin_proof_of_existence_ab94e78e:

    # rin "Let's just watch the clouds for today."
    rin "Solo veamos las nubes este día."

# game/script-a4-rin.rpy:6240
translate es a4_rin_proof_of_existence_5a9bcd03:

    # "She takes five steps to climb on a large rock so she can rise as high as it's possible here, and stands on tiptoes."
    "Ella toma dos pasos para escalar una gran roca para poder levantarse tan alto como es posible aquí, y se para de puntillas."

# game/script-a4-rin.rpy:6242
translate es a4_rin_proof_of_existence_e3cea480:

    # "When you reach for the clouds, every inch counts."
    "Cuando tratas de alcanzar las nubes, cada centímetro cuenta."

# game/script-a4-rin.rpy:6244
translate es a4_rin_proof_of_existence_fa1d5ae8:

    # hi "Sure, let's watch the clouds. It's good to do something you really want to do, every now and then."
    hi "Seguro, veamos las nubes. Es bueno hacer algo que realmente quieres, de vez en cuando."

# game/script-a4-rin.rpy:6246
translate es a4_rin_proof_of_existence_185ec3c3:

    # rin "Yeah. You are probably right."
    rin "Sí. Puede que tengas la razón."

# game/script-a4-rin.rpy:6248
translate es a4_rin_proof_of_existence_d5c4a5ec:

    # "I glance upwards at the blue sky opening high above us."
    "Miro hacia arriba al cielo azul abriéndose a lo alto sobre nosotros."

# game/script-a4-rin.rpy:6250
translate es a4_rin_proof_of_existence_4631b68d:

    # "It's a deep, cerulean vastness that spreads to fill my entire field of vision and beyond."
    "Es un vasto y profundo cerúleo que se extiende hasta llenar mi campo visual entero y más allá."

# game/script-a4-rin.rpy:6252
translate es a4_rin_proof_of_existence_262911d4:

    # "Yet Rin stays on her rock, peering at the distant horizon where the rain clouds are drifting farther away from us."
    "Aun así Rin se queda sobre su roca, asomándose por el horizonte distante donde las nubes de lluvia flotan alejándose de nosotros."

# game/script-a4-rin.rpy:6254
translate es a4_rin_proof_of_existence_86bc64cd:

    # rin "I have decided something."
    rin "He decidido algo."

# game/script-a4-rin.rpy:6256
translate es a4_rin_proof_of_existence_28a01829:

    # "That dreaming voice of hers, spoken to the wind that carries it to my ears, is lacking resolve in tone but is full of it in meaning."
    "Esa voz de ensueño suya, hablada al viento que la lleva a mis oídos, le hace falta determinación en el tono pero está llena de sentido."

# game/script-a4-rin.rpy:6258
translate es a4_rin_proof_of_existence_e02da6ea:

    # rin "It's all right to be me after all."
    rin "Está bien ser yo después de todo."

# game/script-a4-rin.rpy:6311
translate es a4_rin_proof_of_existence_d41d8cd9_2:

    # nvl clear
    nvl clear

# game/script-a4-rin.rpy:6266
translate es a4_rin_proof_of_existence_711725db:

    # n "{vspace=90}It's all right? Her decisions always seem to be pretty… far out."
    n "{vspace=90}¿Está bien? Sus decisiones siempre parecen ser bastante… únicas."

# game/script-a4-rin.rpy:6268
translate es a4_rin_proof_of_existence_b15198a8:

    # n "Well, I suppose that is an important realization."
    n "Bueno, supongo que es una epifanía importante."

# game/script-a4-rin.rpy:6270
translate es a4_rin_proof_of_existence_35f1365e:

    # n "Coming to terms with oneself, accepting yourself, being fine with what you are."
    n "Llegar a un acuerdo con uno mismo, aceptarte a ti mismo, estar bien con quién eres."

# game/script-a4-rin.rpy:6272
translate es a4_rin_proof_of_existence_4e0a0991:

    # n "A simple resolution of heart that for some people is overbearingly hard to do, if not impossible."
    n "Una simple resolución del corazón que para algunas personas es excesivamente difícil, si no es que imposible."

# game/script-a4-rin.rpy:6274
translate es a4_rin_proof_of_existence_524d52d2:

    # n "I do realize well enough that I might also be one of those people."
    n "Me doy cuenta bien de que podría yo también ser de ese tipo de personas."

# game/script-a4-rin.rpy:6276
translate es a4_rin_proof_of_existence_f26147d8:

    # n "Rin too… "
    n "También Rin…"

# game/script-a4-rin.rpy:6278
translate es a4_rin_proof_of_existence_fcf64c52:

    # n "Maybe we are not that different after all."
    n "Tal vez no seamos tan diferentes después de todo."

# game/script-a4-rin.rpy:6280
translate es a4_rin_proof_of_existence_939c1acd:

    # n "Maybe to accept someone else, you must first accept yourself."
    n "Quizás para aceptar a alguien más, primero debes aceptarte a ti mismo."

# game/script-a4-rin.rpy:6282
translate es a4_rin_proof_of_existence_34f1076f:

    # n "Maybe that is a necessary step, which we didn't take until now."
    n "Tal vez ese sea un paso necesario, el cual no tomamos hasta ahora."

# game/script-a4-rin.rpy:6288
translate es a4_rin_proof_of_existence_04d21395:

    # nvl clear
    # "Looking at her standing on that rock, I believe that she can find whatever she is looking for."
    nvl clear
    "Viéndola de pie sobre esa roca, yo creo que ella puede encontrar lo que sea que esté buscando."

# game/script-a4-rin.rpy:6292
translate es a4_rin_proof_of_existence_9f0b1364:

    # "And so can I."
    "Y yo también."

# game/script-a4-rin.rpy:6299
translate es a4_rin_proof_of_existence_82835758:

    # "The wind catches her hair and clothes, and Rin spreads her short arms into an embrace that is so very very tiny, but as wide as she can ever do."
    "El viento atrapa su cabello y ropa, y Rin abre sus cortos brazos en un abrazo que es tan pero tan diminuto, pero tan amplio como ella puede hacerlo."

# game/script-a4-rin.rpy:6301
translate es a4_rin_proof_of_existence_db7f0eed:

    # "For a moment it looks like she herself might take flight, and I have to hold myself back to not reach for her shoulder, to not drag her back to me."
    "Por un momento parece que ella misma podría levantar el vuelo, y tengo que detenerme para no sujetar su hombro, para no arrastrarla de vuelta hacia mí."

# game/script-a4-rin.rpy:6303
translate es a4_rin_proof_of_existence_b8499f43:

    # "But this picture is something I can only watch, it is something for me to remember."
    "Pero esta imagen es algo que tan solo puedo mirar, es algo para que yo recuerde."

# game/script-a4-rin.rpy:6305
translate es a4_rin_proof_of_existence_9b19a246:

    # "Rin's sleeves are flapping freely in the wind, her hair wildly tousled by it, her skin touched by the setting sun."
    "Las mangas de Rin aletean libremente en el viento, su cabello alborotado incontrolablemente por este, su piel tocada por el sol poniente."

# game/script-a4-rin.rpy:6307
translate es a4_rin_proof_of_existence_493bb41b:

    # "Her sleek form that I've come to adore is quivering in the cool wind that carries the small white specks past her, each a beginning of a new flower."
    "Su forma elegante que he llegado a adorar se estremece en el frío viento que lleva las pequeñas motas blancas frente a ella, cada una el inicio de una nueva flor."

# game/script-a4-rin.rpy:6309
translate es a4_rin_proof_of_existence_f688e61c:

    # "All that is engraved inside my heart."
    "Todo ello está grabado dentro de mi corazón."

# game/script-a4-rin.rpy:6311
translate es a4_rin_proof_of_existence_91a6076d:

    # "Like those tiny seeds scattered into the wind, I'm sure that Rin too can take her place in this world without the need to create her own inside of it."
    "Como aquellas diminutas semillas esparcidas por el viento, estoy seguro de que Rin también puede tomar su lugar en este mundo sin la necesidad de crear el suyo propio dentro de este."

# game/script-a4-rin.rpy:6313
translate es a4_rin_proof_of_existence_92de47e3:

    # "Maybe she believes it too, and standing as close to heaven as possible, she is giving the world a big hug."
    "Tal vez ella lo crea también, y estando de pie tan cerca del cielo como es posible, ella le da al mundo un gran abrazo."

# game/script-a4-rin.rpy:6315
translate es a4_rin_proof_of_existence_2e70a0c7:

    # "To me it seems like the entire world really could fit there, between those small arms of hers, inside of her all-encompassing embrace."
    "Para mí parece como si el mundo entero realmente pudiera caber ahí, entre esos pequeños brazos suyos, dentro de su abrazo que lo cubre todo."

# game/script-a4-rin.rpy:6320
translate es a4_rin_proof_of_existence_7ea48cf2:

    # rin "Hisao?"
    rin "¿Hisao?"

# game/script-a4-rin.rpy:6322
translate es a4_rin_proof_of_existence_0d6a7662:

    # "She looks at me in the same way she calls my name, carelessly over her shoulder with a strange happiness in her voice and in her eyes."
    "Ella me mira del mismo modo con el que llama mi nombre, con descuido por encima de su hombro con un extraño tono de felicidad en su voz y en sus ojos."

# game/script-a4-rin.rpy:6333
translate es a4_rin_proof_of_existence_6c8c180f:

    # "I gaze into those mysterious, dark eyes that are curiously twinkling from below her auburn hair."
    "Miro dentro de aquellos misteriosos y oscuros ojos que centellean desde debajo de su cabello cobrizo."

# game/script-a4-rin.rpy:6335
translate es a4_rin_proof_of_existence_eb1b7eba:

    # "Although I'm too far from her to see it, I'm sure they are reflecting my image."
    "Aunque estoy demasiado lejos de ella para verlo, estoy seguro de que reflejan mi imagen."

# game/script-a4-rin.rpy:6337
translate es a4_rin_proof_of_existence_46b9decc_1:

    # hi "What is it?"
    hi "¿Qué sucede?"

# game/script-a4-rin.rpy:6339
translate es a4_rin_proof_of_existence_c7aa4277:

    # rin "What's the word for when it feels inside your heart that everything in the world is all right?"
    rin "¿Cuál es la palabra para cuando sientes dentro de tu corazón que todo en el mundo está bien?"

# game/script-a4-rin.rpy:6355
translate es separate_of_dg_and_wl_d5203e2e:

    # "The last day before summer vacations is waning slowly."
    "El último día antes de las vacaciones de verano está terminando poco a poco."

# game/script-a4-rin.rpy:6357
translate es separate_of_dg_and_wl_defa6eb6:

    # "Science is the final exam of the trimester and then we are free."
    "El examen final del trimestre es el de Ciencias y entonces estamos libres."

# game/script-a4-rin.rpy:6359
translate es separate_of_dg_and_wl_67353f28:

    # "The collective yearning for liberty is almost palpable in the classroom, even though the weather seems a tad cloudy."
    "El anhelo colectivo de la libertad es casi palpable en este salón de clases, aun si el tiempo es un poco nublado."

# game/script-a4-rin.rpy:6361
translate es separate_of_dg_and_wl_d75f9f63:

    # "It might rain today, who knows."
    "Puede que llueva hoy, quién sabe."

# game/script-a4-rin.rpy:6368
translate es separate_of_dg_and_wl_90719bae:

    # "I've already finished the test because it was pretty easy, so I'm doodling lazily on the flip side of the paper, waiting for Mutou to call time."
    "Ya he terminado el examen porque era bastante fácil, así que estoy garabateando con flojera en la parte de atrás de la hoja, esperando a que Mutou diga que se terminó el tiempo."

# game/script-a4-rin.rpy:6370
translate es separate_of_dg_and_wl_7e3bb01a:

    # "It also prevents Misha from trying to covertly look at my answers over my shoulder."
    "También previene que Misha trate de mirar disimuladamente mis respuestas por sobre mi hombro."

# game/script-a4-rin.rpy:6372
translate es separate_of_dg_and_wl_8bc66d8f:

    # "She might fool the inattentive teacher, but I can tell that she is trying to look."
    "Puede que engañe al maestro distraído, pero puedo darme cuenta de que está tratando de ver."

# game/script-a4-rin.rpy:6374
translate es separate_of_dg_and_wl_af1ad692:

    # "I guess it's her best bet at passing the test. Doesn't make me feel any mercy though, so I just ignore her and look around me."
    "Supongo que es su mejor apuesta para pasar la prueba. Sin embargo no me hace sentir nada de piedad, así que solo la ignoro y miro a mi alrededor."

# game/script-a4-rin.rpy:6381
translate es separate_of_dg_and_wl_eff9b4f7:

    # "It's quiet."
    "Es silencioso."

# game/script-a4-rin.rpy:6383
translate es separate_of_dg_and_wl_116de5c0:

    # "The only sounds in the classroom are the quiet shuffling of papers and Mutou's constant coughing."
    "Los únicos sonidos en el salón de clases son el callado barajar de hojas y la constante tos de Mutou."

# game/script-a4-rin.rpy:6385
translate es separate_of_dg_and_wl_6453686b:

    # "It makes my awareness of the surroundings slowly drift to the backstage of consciousness, giving room to other things."
    "Hace que la percepción de mi entorno lentamente se desplace al fondo de mi consciencia, dándole espacio a otras cosas."

# game/script-a4-rin.rpy:6392
translate es a4rc1o1_18720e4d:

    # hi "But aren't you happy people are interested in your paintings?"
    hi "¿Pero que no estás feliz de que la gente esté interesada en tus pinturas?"

# game/script-a4-rin.rpy:6394
translate es a4rc1o1_87c0a71d:

    # hi "I mean, isn't that why you went ahead with having the exhibition and all?"
    hi "Quiero decir, ¿no es esa la razón de que hayas seguido adelante con la exhibición y todo eso?"

# game/script-a4-rin.rpy:6396
translate es a4rc1o1_dc7080cb:

    # hi "Of course they would ask you questions, if they think it's interesting."
    hi "Por supuesto que te harían preguntas, si pensaran que es interesante."

# game/script-a4-rin.rpy:6401
translate es a4rc1o1_4871a9fb:

    # rin "It's like having sunrise twice in a row when you want to bathe naked in moonlight."
    rin "Es como si tuvieras dos amaneceres uno después del otro cuando quieres bañarte desnuda bajo la luz de la luna."

# game/script-a4-rin.rpy:6406
translate es a4rc1o1_051dcce2:

    # rin "Nice, but…"
    rin "Agradable, pero…"

# game/script-a4-rin.rpy:6408
translate es a4rc1o1_fab0bf10:

    # "…it's not good enough, I complete the sentence for her even though I don't understand her inappropriate metaphor."
    "… no es lo suficientemente bueno, completo la oración por ella aun si no entiendo su metáfora inapropiada."

# game/script-a4-rin.rpy:6410
translate es a4rc1o1_8ad29a9a:

    # hi "I don't get it."
    hi "No lo comprendo."

# game/script-a4-rin.rpy:6412
translate es a4rc1o1_f64309d0:

    # hi "You should try to be happier. It's your big night, after all."
    hi "Deberías de tratar de estar más alegre. Es tu gran noche, después de todo."

# game/script-a4-rin.rpy:6414
translate es a4rc1o1_eea5f11c:

    # hi "All these people are here to see your paintings. I think it's awesome."
    hi "Todas estas personas están aquí para ver tus pinturas. Yo creo que es estupendo."

# game/script-a4-rin.rpy:6416
translate es a4rc1o1_91bccc4c:

    # "I wait for her to say something, either for or against, but Rin keeps brooding."
    "Espero a que diga algo, ya sea a favor o en contra, pero Rin se sigue lamentando."

# game/script-a4-rin.rpy:6418
translate es a4rc1o1_8780b264:

    # "She doesn't want to answer questions, or explain to me what's wrong."
    "No quiere responder preguntas, o explicarme qué es lo que está mal."

# game/script-a4-rin.rpy:6420
translate es a4rc1o1_3a247e77:

    # "If she had something to say, the words are left unspoken."
    "Si ella tenía algo que decir, las palabras se quedan sin ser dichas."

# game/script-a4-rin.rpy:6422
translate es a4rc1o1_21534cbb:

    # "The words that she cannot say."
    "Las palabras que ella no puede decir."

# game/script-a4-rin.rpy:6424
translate es a4rc1o1_4148f075:

    # "I shudder against the chill wind that blows in the streets, and its howling fills the silence."
    "Me da un escalofrío el viento helado que sopla en las calles, y su aullido llena el silencio."

# game/script-a4-rin.rpy:6426
translate es a4rc1o1_28aa5a0b:

    # hi "We should go back in."
    hi "Deberíamos volver adentro."

# game/script-a4-rin.rpy:6428
translate es a4rc1o1_54f14009:

    # hi "You've got everyone worried."
    hi "Tienes a todos preocupados."

# game/script-a4-rin.rpy:6442
translate es a4rc1o1_fe38fc84:

    # no "Ah, there you are! Feeling better? It can get pretty hot in here, a dizzy spell can catch you off guard."
    no "¡Ah, ahí estás! ¿Te sientes mejor? Puede ponerse muy acalorado aquí adentro, un mareo puede tomarte desprevenido."

# game/script-a4-rin.rpy:6447
translate es a4rc1o1_240a236e:

    # "He laughs brashly, almost obnoxiously."
    "Se ríe con descaro, casi es desagradable."

# game/script-a4-rin.rpy:6452
translate es a4rc1o1_c849accb:

    # no "You should drink something if you're feeling weak, Tezuka."
    no "Deberías de beber algo si te sientes débil, Tezuka."

# game/script-a4-rin.rpy:6467
translate es a4rc1o1_e86f6490:

    # "Rin nods weakly, but it seems to be enough to convince Nomiya that she is fine."
    "Rin asiente débilmente, pero parece ser suficiente para convencer a Nomiya de que está bien."

# game/script-a4-rin.rpy:6469
translate es a4rc1o1_760f0ae4:

    # "He pushes Rin a bit forward to introduce her to the person he was conversing with before."
    "Empuja a Rin un poco hacia adelante para presentarla a la persona con la que él estaba conversando anteriormente."

# game/script-a4-rin.rpy:6474
translate es a4rc1o1_2fe1c94a:

    # no "So, about what we were talking about before…"
    no "Y bien, de lo que estábamos hablando antes…"

# game/script-a4-rin.rpy:6476
translate es a4rc1o1_e2636d67:

    # "Man" "Ah yes, I'm very excited to meet…"
    "Hombre" "Ah sí, estoy muy emocionado por conocer…"

# game/script-a4-rin.rpy:6480
translate es a4rc1o1_e9c3779b:

    # "I am shut out of the conversation, and the background noise of dozens of other discussions fills my ears with indistinct buzz."
    "Soy retirado de la conversación, y el ruido de fondo de docenas de otras discusiones llena mis oídos con un zumbido indistinto."

# game/script-a4-rin.rpy:6482
translate es a4rc1o1_2ce4371a:

    # "Even Emi has disappeared somewhere."
    "Incluso Emi ha desaparecido en algún lugar."

# game/script-a4-rin.rpy:6484
translate es a4rc1o1_770334b6:

    # "Standing in the middle of a crowd is a surprisingly lonely feeling."
    "Estar de pie en medio de una multitud es una sensación sorprendentemente solitaria."

# game/script-a4-rin.rpy:6486
translate es a4rc1o1_0da00f08:

    # "Not only Rin, but everyone else here seems to be a part of something I am not a part of."
    "No solo Rin, sino todos los demás parecen ser parte de algo de lo que yo no soy parte."

# game/script-a4-rin.rpy:6488
translate es a4rc1o1_e389ab9e:

    # "I am happy for her, I really am, but it makes me feel that I haven't accomplished anything yet."
    "Estoy feliz por ella, en verdad lo estoy, pero me hace sentir como si yo no hubiera logrado nada todavía."

# game/script-a4-rin.rpy:6490
translate es a4rc1o1_26eb7d36:

    # "Rin is living proof of the potential of a human being. She overcame her disability, even made it a strength."
    "Rin es prueba viviente del potencial de un ser humano. Ella derrotó su discapacidad, incluso la hizo una fuerza."

# game/script-a4-rin.rpy:6494
translate es a4rc1o1_2e1e0b92:

    # "She should be happy."
    "Ella debería de estar feliz."

# game/script-a4-rin.rpy:6496
translate es a4rc1o1_5b8d757f:

    # "What is my potential?"
    "¿Cuál es mi potencial?"

# game/script-a4-rin.rpy:6498
translate es a4rc1o1_727d0478:

    # "Rin made it this far, but how far can I go?"
    "Rin logró llegar hasta acá, pero, ¿qué tan lejos puedo llegar yo?"

# game/script-a4-rin.rpy:6508
translate es a4rc1o2_12952237:

    # hi "But if you found someone like that, then what?"
    hi "Pero si encuentras a alguien así, ¿entonces qué?"

# game/script-a4-rin.rpy:6510
translate es a4rc1o2_bb78d683:

    # hi "Do you really think that it would be some kind of be-all, end-all thing, star-crossed lovers and happily ever after?"
    hi "¿En verdad piensas que sería alguna especie de cosa definitiva, amantes destinados y todos felices para siempre?"

# game/script-a4-rin.rpy:6515
translate es a4rc1o2_391938f6:

    # "My question is met with a blank stare, the darkness in her eyes unfazed by the thinly veiled bitterness."
    "Mi pregunta se encuentra con una mirada en blanco, la oscuridad en sus ojos impávida ante la amargura mal disimulada."

# game/script-a4-rin.rpy:6520
translate es a4rc1o2_11f58aa4:

    # rin "No, I don't think that."
    rin "No, no pienso eso."

# game/script-a4-rin.rpy:6525
translate es a4rc1o2_24d42360:

    # rin "But at least then I wouldn't have to be alone."
    rin "Pero por lo menos entonces no tendría que estar sola."

# game/script-a4-rin.rpy:6527
translate es a4rc1o2_0187605a:

    # "She whispers the words to the lights of the town but I hear them anyway."
    "Le susurra las palabras a las luces de la ciudad pero las escucho de igual modo."

# game/script-a4-rin.rpy:6532
translate es a4rc1o2_80268023:

    # rin "I shouldn't have done this. Not yet."
    rin "No debería de haber hecho esto. No todavía."

# game/script-a4-rin.rpy:6534
translate es a4rc1o2_b6bcf26d:

    # hi "The exhibition?"
    hi "¿La exhibición?"

# game/script-a4-rin.rpy:6539
translate es a4rc1o2_c0434140:

    # "She nods and closes her eyes, breathing calmly out as if to prove she can, and then continues talking to herself."
    "Ella asiente y cierra los ojos, dejando salir el aire con tranquilidad como si probara que puede, y entonces continúa hablando consigo misma."

# game/script-a4-rin.rpy:6541
translate es a4rc1o2_e477ae1d:

    # hi "Why? Wrong conjunction of the planets?"
    hi "¿Por qué? ¿Mala conjunción de los planetas?"

# game/script-a4-rin.rpy:6546
translate es a4rc1o2_b5bcea0b:

    # rin "No, not that. I double-checked, and I got up with the right, I mean left, foot and did everything else left, I mean right."
    rin "No, eso no. Revisé dos veces, y me levanté con el pie derecho, quiero decir izquierdo, e hice todo a la izquierda, quiero decir correcto."

# game/script-a4-rin.rpy:6551
translate es a4rc1o2_4418e933:

    # rin "It's me."
    rin "Soy yo."

# game/script-a4-rin.rpy:6556
translate es a4rc1o2_716d492a:

    # rin "I was wrong."
    rin "Yo estaba mal."

# game/script-a4-rin.rpy:6561
translate es a4rc1o2_34bc704c:

    # "She stands straight and stretches before stepping past me out into the street."
    "Ella se para derecha y se estira antes de pasar frente a mí y hacia la calle."

# game/script-a4-rin.rpy:6563
translate es a4rc1o2_3d95fc07:

    # hi "Wait, where are you going?"
    hi "Espera, ¿adónde vas?"

# game/script-a4-rin.rpy:6568
translate es a4rc1o2_0a8a9421:

    # "She stops on her tracks and turns around, looking at me quizzically."
    "Ella se detiene en seco y se da vuelta, mirándome con curiosidad."

# game/script-a4-rin.rpy:6573
translate es a4rc1o2_5b5a364e:

    # rin "School. I'm leaving."
    rin "Escuela. Me voy."

# game/script-a4-rin.rpy:6575
translate es a4rc1o2_29a331e5:

    # hi "What… why?"
    hi "Qué… ¿Por qué?"

# game/script-a4-rin.rpy:6580
translate es a4rc1o2_552c9d52:

    # rin "Because I want to be me."
    rin "Porque quiero ser yo."

# game/script-a4-rin.rpy:6587
translate es a4rc1o2_e86075f3:

    # "Rin walks off, leaving me behind utterly confused."
    "Rin se marcha, dejándome detrás completamente confundido."

# game/script-a4-rin.rpy:6589
translate es a4rc1o2_8ed7ccf1:

    # hi "Rin!"
    hi "¡Rin!"

# game/script-a4-rin.rpy:6591
translate es a4rc1o2_96f2635f:

    # "But… something she said really touched me, or maybe it was the way she said it."
    "Pero… algo que ella dijo en verdad me tocó, o quizás fue cómo lo dijo."

# game/script-a4-rin.rpy:6593
translate es a4rc1o2_7faf4463:

    # "Maybe it was the fact that {b}she{/b} said it."
    "Tal vez fue el hecho de que {b}ella{/b} lo dijera."

# game/script-a4-rin.rpy:6595
translate es a4rc1o2_a245dd68:

    # "I want to say something back to her, before I forget this feeling again."
    "Quiero decirle algo de vuelta, antes de que olvide este sentimiento de nuevo."

# game/script-a4-rin.rpy:6597
translate es a4rc1o2_367c704e:

    # "As if granting me a wish, Rin stops in her tracks. She doesn't turn around, just keeps waiting for me to say what I want to even though I didn't have time to think what…"
    "Como si me cumpliera un deseo, Rin se detiene en seco. Ella no se da vuelta, solo sigue esperando a que yo diga lo que quiero decir aun si no tuve tiempo de pensar en qué…"

# game/script-a4-rin.rpy:6599
translate es a4rc1o2_11c487bd:

    # hi "Rin… listen. I… I don't believe you have to be alone, even if you never meet anyone like that."
    hi "Rin… escucha. Yo… Yo no creo que tengas que estar sola, aun si nunca conoces a alguien así."

# game/script-a4-rin.rpy:6601
translate es a4rc1o2_07e66ab3:

    # "I don't know if she heard my words, but either way, she doesn't react in any way."
    "No sé si escuchó mis palabras, pero de cualquier modo, no reacciona de ninguna manera."

# game/script-a4-rin.rpy:6603
translate es a4rc1o2_1f3c09f6:

    # "For the final time, she starts walking away from the gallery."
    "Por última vez, ella comienza a caminar alejándose de la galería."

# game/script-a4-rin.rpy:6617
translate es a4rc1o2_f4b98541:

    # no "So? Where's Tezuka?"
    no "¿Y bien? ¿Dónde está Tezuka?"

# game/script-a4-rin.rpy:6619
translate es a4rc1o2_d8f9258c:

    # "I can only shake my head, but as it doesn't seem to be a sufficient answer I have to say something."
    "Tan solo puedo sacudir la cabeza, pero ya que no parece ser una respuesta suficiente tengo que decir algo."

# game/script-a4-rin.rpy:6621
translate es a4rc1o2_59b1f273:

    # hi "She ran away."
    hi "Se escapó."

# game/script-a4-rin.rpy:6626
translate es a4rc1o2_4d546cdb:

    # no "What?"
    no "¿Qué?"

# game/script-a4-rin.rpy:6628
translate es a4rc1o2_4dcd46e7:

    # "The horrific realization spreads on his face like wildfire."
    "La espantosa comprensión se extiende por su rostro como un incendio."

# game/script-a4-rin.rpy:6633
translate es a4rc1o2_d211f350:

    # no "This is a fiasco! Catastrophe!"
    no "¡Esto es un fiasco! ¡Una catástrofe!"

# game/script-a4-rin.rpy:6635
translate es a4rc1o2_f576e242:

    # no "What is that girl thinking, the most important event of her life, and she just runs off?"
    no "¿En qué está pensando esa niña?, el evento más importante de su vida, ¿y simplemente corre?"

# game/script-a4-rin.rpy:6640
translate es a4rc1o2_aab0e52b:

    # no "And you! Why didn't you stop her? I'm going to hold you personally responsible…"
    no "¡Y tú! ¿Por qué no la detuviste? Voy a hacerte personalmente responsable…"

# game/script-a4-rin.rpy:6645
translate es a4rc1o2_4dd132a6:

    # "Sae interrupts him, holding her hands up calmingly."
    "Sae lo interrumpe, manteniendo sus manos en alto tranquilamente."

# game/script-a4-rin.rpy:6647
translate es a4rc1o2_ccf72d37:

    # "It's good she intervened; the teacher was starting to get a few weird looks from the nearby guests."
    "Es bueno que ella interviniera; el maestro empezaba a recibir algunas miradas raras de los invitados cercanos."

# game/script-a4-rin.rpy:6652
translate es a4rc1o2_6186cb54:

    # sa "Now, now, Shinichi. She probably just had stage fright. I don't know her as well as you people do, but I did get the image that she is somewhat peculiar."
    sa "Ya, ya, Shinichi. Ella probablemente solo tuvo pánico escénico. No la conozco tan bien como ustedes, pero sí tuve la impresión de que ella es un tanto peculiar."

# game/script-a4-rin.rpy:6654
translate es a4rc1o2_699e0211:

    # sa "This kind of thing can happen."
    sa "Este tipo de cosas suceden."

# game/script-a4-rin.rpy:6659
translate es a4rc1o2_e9541e90:

    # sa "It'll be fine. I'll explain that she suddenly became ill. The guests will surely understand."
    sa "Estará bien. Les explicaré que de pronto enfermó. Los invitados seguramente entenderán."

# game/script-a4-rin.rpy:6664
translate es a4rc1o2_bff5c3a2:

    # no "But…"
    no "Pero…"

# game/script-a4-rin.rpy:6669
translate es a4rc1o2_bcc4f1ea:

    # sa "Look around you, everyone seems to be rather happy with their free wine and chitchat."
    sa "Mira a tu alrededor, todos parecen bastante contentos con su vino gratis y charlas."

# game/script-a4-rin.rpy:6674
translate es a4rc1o2_26c512d1:

    # no "The guests will be fine, but we are missing on opportunities here! Networking, making contacts and acquaintances!"
    no "¡Los invitados estarán bien, pero estamos perdiendo una oportunidad aquí! ¡Interconexiones, hacer contactos y conocidos!"

# game/script-a4-rin.rpy:6689
translate es a4rc1o2_b72a99cb:

    # "As the adults keep arguing about something that can't be helped, Emi tugs my sleeve to get my attention."
    "Mientras los adultos siguen discutiendo sobre algo que no tiene remedio, Emi tira de mi manga para llamar mi atención."

# game/script-a4-rin.rpy:6691
translate es a4rc1o2_be724e19:

    # "She doesn't look very happy either."
    "Ella tampoco se ve muy contenta."

# game/script-a4-rin.rpy:6696
translate es a4rc1o2_5b319ca3:

    # emi "Come on."
    emi "Vamos."

# game/script-a4-rin.rpy:6698
translate es a4rc1o2_804bb8e8:

    # hi "Where?"
    hi "¿Adónde?"

# game/script-a4-rin.rpy:6703
translate es a4rc1o2_5ac06225:

    # emi "We are going to find Rin and kick her ass."
    emi "Vamos a encontrar a Rin y patearle el trasero."

# game/script-a4-rin.rpy:6705
translate es a4rc1o2_81d99227:

    # hi "What?"
    hi "¿Qué?"

# game/script-a4-rin.rpy:6710
translate es a4rc1o2_4746b5f5:

    # emi "I can't believe it, she is so stupid!"
    emi "No puedo creerlo, ¡es tan estúpida!"

# game/script-a4-rin.rpy:6712
translate es a4rc1o2_a0a7c883:

    # emi "That Rin, how can she do this? I'm telling you, she doesn't have a bit of common sense in her head!"
    emi "Esa Rin, ¿cómo puede hacer esto? Déjame decirte, ¡ella no tiene un cachito de sentido común en la cabeza!"

# game/script-a4-rin.rpy:6714
translate es a4rc1o2_fc406d47:

    # "Emi is seriously angry, only missing steam rising from her ears."
    "Emi en serio está furiosa, solo le falta vapor alzándose de sus orejas."

# game/script-a4-rin.rpy:6716
translate es a4rc1o2_469a5e22:

    # "I guess I understand Emi, she is {b}that{/b} kind of a person."
    "Supongo que entiendo a Emi, ella es de {b}ese{/b} tipo de personas."

# game/script-a4-rin.rpy:6718
translate es a4rc1o2_57d437cc:

    # "'Give up' has never felt like a part of her vocabulary, and maybe she feels it shouldn't be a part of anyone's vocabulary."
    "“Rendirse” nunca ha sido parte de su vocabulario, y tal vez siente como si no debiera de ser parte del vocabulario de nadie."

# game/script-a4-rin.rpy:6720
translate es a4rc1o2_c5c7b1c4:

    # hi "It's probably best to leave her alone for tonight."
    hi "Probablemente lo mejor es dejarla sola esta noche."

# game/script-a4-rin.rpy:6725
translate es a4rc1o2_c92c0faf:

    # emi "What? Are you a Rin expert now?"
    emi "¿Qué? ¿Eres un experto en Rin ahora?"

# game/script-a4-rin.rpy:6727
translate es a4rc1o2_102f9059:

    # "She takes a firm stance and puts her hands on her hips confrontationally."
    "Ella toma una posición firme y pone sus manos sobre sus caderas de modo confrontacional."

# game/script-a4-rin.rpy:6729
translate es a4rc1o2_b4c8dd33:

    # "It's like she wants to pick a fight with me too."
    "Es como si quisiera pelearse conmigo también."

# game/script-a4-rin.rpy:6731
translate es a4rc1o2_b360fb5b:

    # hi "No, I don't think that's even possible in the first place."
    hi "No, no creo que eso siquiera sea posible en primer lugar."

# game/script-a4-rin.rpy:6733
translate es a4rc1o2_7415e9df:

    # hi "I just don't think kicking her ass would do her any good."
    hi "Es solo que no creo que patearle el trasero le haga algún bien."

# game/script-a4-rin.rpy:6738
translate es a4rc1o2_0d60f350:

    # "My melancholic remark surprisingly works, as Emi slumps her shoulders a little and sighs."
    "Mi melancólica observación sorpresivamente funciona, y Emi deja caer sus hombros un poco y suspira."

# game/script-a4-rin.rpy:6740
translate es a4rc1o2_33b33736:

    # emi "I know that."
    emi "Lo sé."

# game/script-a4-rin.rpy:6742
translate es a4rc1o2_9f1e5e09:

    # hi "You do?"
    hi "¿En serio?"

# game/script-a4-rin.rpy:6749
translate es a4rc1o2_ea4fef41:

    # emi "The last time I did that, it changed nothing."
    emi "La última vez que hice eso no cambió nada."

# game/script-a4-rin.rpy:6759
translate es a4rc1o2_141290f6:

    # "The ride back to school in an empty late-night bus is silent."
    "El viaje de regreso a la escuela en un autobús vacío en medianoche es callado."

# game/script-a4-rin.rpy:6761
translate es a4rc1o2_10a2ec94:

    # "Both of us keep staring at the lights flashing past the windows without saying a word."
    "Ambos nos quedamos viendo las luces brillantes pasar frente a la ventana sin decir una palabra."

# game/script-a4-rin.rpy:6770
translate es a4rc1o2_9a67d91c:

    # "The nightly grounds are quiet, lit only by the wan moon and yellow lamp posts."
    "El campus nocturno está en silencio, iluminado solo por la pálida luna y postes de luz amarillos."

# game/script-a4-rin.rpy:6772
translate es a4rc1o2_6d116af1:

    # "We say our goodnights in front of my dormitory."
    "Nos decimos buenas noches frente a mi dormitorio."

# game/script-a4-rin.rpy:6777
translate es a4rc1o2_2d7f296b:

    # "Emi reflexively clenching her fists compels me to ensure that she won't assault Rin the moment I let her out of my sight."
    "Emi aprieta sus puños por reflejo y me obliga a asegurarme de que no atacará a Rin en el momento en que la pierda de vista."

# game/script-a4-rin.rpy:6779
translate es a4rc1o2_a98dd700:

    # hi "Promise me to not go scold her?"
    hi "¿Me prometes no ir a regañarla?"

# game/script-a4-rin.rpy:6784
translate es a4rc1o2_457735d4:

    # "She looks up at me, her eyes again flaring with anger that I match with as calming a stare as I can."
    "Ella me ve, sus ojos ardiendo con ira que igualo con una mirada tan tranquilizadora como pueda."

# game/script-a4-rin.rpy:6786
translate es a4rc1o2_48ea0262:

    # "It's only easy to face an angry woman if you are not the target of her ire."
    "Solo es fácil enfrentar a una mujer enojada cuando no eres tú el blanco de su ira."

# game/script-a4-rin.rpy:6788
translate es a4rc1o2_0f65ea78:

    # "After a minute of the mismatched staring contest, she sighs and shakes her head in defeat."
    "Después de un minuto de la dispareja competencia de miradas, ella suspira y agita la cabeza, derrotada."

# game/script-a4-rin.rpy:6793
translate es a4rc1o2_13518a67:

    # emi "You are too nice, Hisao."
    emi "Eres demasiado amable, Hisao."

# game/script-a4-rin.rpy:6798
translate es a4rc1o2_afaddf81:

    # emi "Did you know that?"
    emi "¿Sabías eso?"

# game/script-a4-rin.rpy:6800
translate es a4rc1o2_68488c56:

    # "Hints of a smile are tugging the corners of her mouth as she says that, and she seems a lot more relaxed."
    "Los indicios de una sonrisa tiran de las comisuras de su boca mientras dice eso, y se ve mucho más relajada."

# game/script-a4-rin.rpy:6802
translate es a4rc1o2_8e0778e9:

    # "What a sudden change of mood."
    "Qué cambio de humor tan repentino."

# game/script-a4-rin.rpy:6804
translate es a4rc1o2_113949e1:

    # "Maybe she wasn't as angry as it seemed to begin with."
    "Tal vez no estaba tan enojada como parecía en un principio."

# game/script-a4-rin.rpy:6806
translate es a4rc1o2_a3433504:

    # "Maybe her moods change easily."
    "Tal vez su humor cambia con facilidad."

# game/script-a4-rin.rpy:6808
translate es a4rc1o2_c5ad6a42:

    # hi "If I was, I would've let you have your way."
    hi "Si lo fuera, te hubiera dejado hacer lo que quisieras."

# game/script-a4-rin.rpy:6813
translate es a4rc1o2_81baeec0:

    # emi "Does that mean you are only nice to Rin?"
    emi "¿Eso significa que solo eres amable con Rin?"

# game/script-a4-rin.rpy:6815
translate es a4rc1o2_64604aac:

    # "Both of us are hiding our concern behind empty jokes, but at least it puts me in a good mood."
    "Ambos escondemos nuestra preocupación detrás de bromas vacías, pero por lo menos me pone de buen humor."

# game/script-a4-rin.rpy:6817
translate es a4rc1o2_52c406c1:

    # "Emi waggles her eyebrows with a half-amused smirk, trying to push my buttons. Not gonna work."
    "Emi menea sus cejas con una sonrisa ligeramente burlona, tratando de hacerme reaccionar. No funcionará."

# game/script-a4-rin.rpy:6819
translate es a4rc1o2_384eaea7:

    # hi "No, it just means I'm not nice only to you."
    hi "No, solo significa que no soy amable solo contigo."

# game/script-a4-rin.rpy:6824
translate es a4rc1o2_00433e07:

    # emi "HEY!"
    emi "¡OYE!"

# game/script-a4-rin.rpy:6828
translate es a4rc1o2_6f926c60:

    # hi "Good night, Emi."
    hi "Buenas noches, Emi."

translate es strings:

    # game/script-a4-rin.rpy:5559
    old "That way… you can confirm that you are, in fact… {color=#fff}alive.{/color}"
    new "De ese modo… puedes confirmar que estás, de hecho… {color=#fff}vivo.{/color}"

# TODO: Translation updated at 2026-02-24 23:15

translate es strings:

    # game/script-a4-rin.rpy:1233
    old "But aren't you happy people are interested in your paintings?"
    new "¿Pero que no estás feliz de que la gente esté interesada en tus pinturas?"

    # game/script-a4-rin.rpy:1238
    old "But if you found someone like that, then what?"
    new "Pero si encuentras a alguien así, ¿entonces qué?"

    # game/script-a4-rin.rpy:5565
    old "That way… you can confirm that you are, in fact… alive."
    new "De ese modo… puedes confirmar que estás, de hecho… vivo."

